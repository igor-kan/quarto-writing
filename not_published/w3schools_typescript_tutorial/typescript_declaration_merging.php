...
<head>
<title>TypeScript Declaration Merging</title>
<meta property="og:description" content="TypeScript declaration merging allows you to combine multiple declarations with the same name into a single definition.">  
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>TypeScript Declaration Merging</h1>

<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="typescript_index_signatures.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="typescript_async.php">Next &#10095;</a>
</div>

<hr>

<h2>Understanding Declaration Merging</h2>

<p>Declaration merging is a powerful TypeScript feature that allows you to combine multiple declarations with the same name into a single definition.</p>
<p>This enables you to build up complex types incrementally and extend existing types in a type-safe manner.</p>
<hr>
<div class="w3-panel">
  <h3>Key Benefits</h3>
  <ul>
    <li><b>Progressive Enhancement</b>: Build types incrementally across multiple declarations</li>
    <li><b>Extensibility</b>: Add new members to existing types without modifying original definitions</li>
    <li><b>Organization</b>: Split large type definitions into logical groupings</li>
    <li><b>Compatibility</b>: Extend third-party type definitions when needed</li>
  </ul>
</div>
...
<div class="w3-panel">
  ...
  <ul>
    <li>Extending built-in types and third-party library types</li>
    <li>Adding type information for JavaScript libraries</li>
    <li>Organizing large interfaces across multiple files</li>
    <li>Creating fluent APIs with method chaining</li>
    <li>Implementing the module augmentation pattern</li>
  </ul>
</div>

<hr>

<h2>Interface Merging</h2>

<p>Interfaces with the same name are automatically merged:</p>
...
<br>
// Second declaration with the same name<br>
interface Person {<br>
&nbsp;&nbsp;address: string;<br>
&nbsp;&nbsp;email: string;<br>
}<br>
<br>
// TypeScript merges them into:<br>
// interface Person {<br>
//   name: string;<br>
//   age: number;<br>
//   address: string;<br>
...
<a class="w3-btn w3-margin-bottom" href="trytypescript.php?filename=demo_declaration_merging_interface" target="_blank">Try it Yourself &raquo;</a>
</div>
...
<h2>Function Overloads with Merging</h2>

<p>You can define multiple function declarations that later merge when implemented:</p>

<div class="w3-example">
  <h3>Example</h3>
<div class="w3-code notranslate jsHigh">
// Function overloads<br>
function processValue(value: string): string;<br>
function processValue(value: number): number;<br>
function processValue(value: boolean): boolean;<br>
<br>
// Implementation that handles all overloads<br>
function processValue(value: string | number | boolean): string | number | boolean {<br>
...
<br>
// Using the function with different types<br>
console.log(processValue("hello"));  // "HELLO"<br>
console.log(processValue(10));       // 20<br>
console.log(processValue(true));     // false
</div>
<a class="w3-btn w3-margin-bottom" href="trytypescript.php?filename=demo_declaration_merging_functions" target="_blank">Try it Yourself &raquo;</a>
</div>

<hr>

<h2>Namespace Merging</h2>

<p>Namespaces with the same name are merged:</p>
...
<br>
namespace Validation {<br>
&nbsp;&nbsp;export interface NumberValidator {<br>
&nbsp;&nbsp;&nbsp;&nbsp;isValid(n: number): boolean;<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;export class ZipCodeValidator implements StringValidator {<br>
&nbsp;&nbsp;&nbsp;&nbsp;isValid(s: string): boolean {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return s.length === 5 && /^\d+$/.test(s);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;}<br>
}<br>
<br>
// After merging:<br>
// namespace Validation {<br>
//   export interface StringValidator { isValid(s: string): boolean; }<br>
//   export interface NumberValidator { isValid(n: number): boolean; }<br>
//   export class ZipCodeValidator implements StringValidator { ... }<br>
// }<br>
<br>
// Using the merged namespace<br>
const zipValidator = new Validation.ZipCodeValidator();<br>
<br>
console.log(zipValidator.isValid("12345"));  // true<br>
console.log(zipValidator.isValid("1234"));   // false<br>
console.log(zipValidator.isValid("abcde"));  // false
</div>
<a class="w3-btn w3-margin-bottom" href="trytypescript.php?filename=demo_declaration_merging_namespace" target="_blank">Try it Yourself &raquo;</a>
</div>

<hr>

<h2>Class and Interface Merging</h2>

<p>A class declaration can merge with an interface of the same name:</p>
...
interface Cart {<br>
&nbsp;&nbsp;calculateTotal(): number;<br>
}<br>
<br>
// Class declaration with same name<br>
class Cart {<br>
&nbsp;&nbsp;items: { name: string; price: number }[] = [];<br>
<br>
&nbsp;&nbsp;addItem(name: string, price: number): void {<br>
&nbsp;&nbsp;&nbsp;&nbsp;this.items.push({ name, price });<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;  // Must implement the interface method<br>
&nbsp;&nbsp;  calculateTotal(): number {<br>
&nbsp;&nbsp;&nbsp;&nbsp;return this.items.reduce((sum, item) => sum + item.price, 0);<br>
&nbsp;&nbsp;}<br>
}<br>
<br>
// Using the merged class and interface<br>
const cart = new Cart();<br>
cart.addItem("Book", 15.99);<br>
cart.addItem("Coffee Mug", 8.99);<br>
<br>
...
<p>Enum declarations with the same name are merged:</p>

<div class="w3-example">
  <h3>Example</h3>
<div class="w3-code notranslate jsHigh">
// First part of the enum<br>
enum Direction {<br>
&nbsp;&nbsp;North,<br>
&nbsp;&nbsp;South<br>
}<br>
<br>
// Second part of the enum<br>
enum Direction {<br>
&nbsp;&nbsp;East = 2,<br>
&nbsp;&nbsp;West = 3<br>
}<br>
...
// }<br>
<br>
console.log(Direction.North);  // 0<br>
console.log(Direction.South);  // 1<br>
console.log(Direction.East);   // 2<br>
console.log(Direction.West);   // 3<br>
<br>
// Can also access by value<br>
console.log(Direction[0]);     // "North"<br>
console.log(Direction[2]);     // "East"
</div>
<a class="w3-btn w3-margin-bottom" href="trytypescript.php?filename=demo_declaration_merging_enum" target="_blank">Try it Yourself &raquo;</a>
</div>
...
<p>You can extend existing modules or libraries by declaring additional types and functionality:</p>

<div class="w3-example">
  <h3>Example</h3>
<div class="w3-code notranslate jsHigh">
// Original library definition<br>
// Imagine this comes from a third-party library<br>
declare namespace LibraryModule {<br>
&nbsp;&nbsp;export interface User {<br>
&nbsp;&nbsp;&nbsp;&nbsp;id: number;<br>
&nbsp;&nbsp;&nbsp;&nbsp;name: string;<br>
&nbsp;&nbsp;}<br>
&nbsp;&nbsp;export function getUser(id: number): User;<br>
}<br>
<br>
// Augmenting with additional functionality (your code)<br>
declare namespace LibraryModule {<br>
&nbsp;&nbsp;// Add new interface<br>
&nbsp;&nbsp;export interface UserPreferences {<br>
&nbsp;&nbsp;&nbsp;&nbsp;theme: string;<br>
&nbsp;&nbsp;&nbsp;&nbsp;notifications: boolean;<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Add new property to existing interface<br>
&nbsp;&nbsp;export interface User {<br>
&nbsp;&nbsp;&nbsp;&nbsp;preferences?: UserPreferences;<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Add new function<br>
&nbsp;&nbsp;export function getUserPreferences(userId: number): UserPreferences;<br>
}<br>
<br>
// Using the augmented module<br>
const user = LibraryModule.getUser(123);<br>
console.log(user.preferences?.theme);<br>
<br>
const prefs = LibraryModule.getUserPreferences(123);<br>
console.log(prefs.notifications);
</div>
<a class="w3-btn w3-margin-bottom" href="trytypescript.php?filename=demo_declaration_merging_module" target="_blank">Try it Yourself &raquo;</a>
</div>

<hr>
<h2>Best Practices</h2>

<p>There are some rules to consider when using declaration merging:</p>

<ul>
  <li><strong>Order matters for function overloads</strong>: The implementation signature should be the most general</li>
  <li><strong>Non-function members must be compatible</strong>: If two interfaces declare a property with the same name, the types must be identical or compatible</li>
  <li><strong>Later interfaces take precedence</strong>: If conflicts exist in merged interfaces, the last declaration wins</li>
  <li><strong>Private and protected members</strong>: Classes can't merge if they have private or protected members with the same name but different types</li>
  <li><strong>Namespace exports</strong>: Only exported declarations are visible outside the namespace after merging</li>
</ul>
...
<div class="w3-panel">
  <ul>
    <li><strong>Compilation Time</strong>: Excessive declaration merging can increase compilation time</li>
    <li><strong>Type Checking</strong>: Complex merged types may impact IDE performance</li>
    <li><strong>Bundle Size</strong>: Declaration merging doesn't affect runtime performance or bundle size</li>
  </ul>
  
  <h4>Optimization Tips:</h4>
  <ul>
    <li>Keep merged interfaces focused and cohesive</li>
    <li>Avoid deep nesting in merged types</li>
    <li>Use <code class="w3-codespan">type</code> aliases for simple type combinations instead of merging</li>
  </ul>
</div>

<hr>
<div id="exercisecontainer" src="xrcise_declaration_merging.js" data-extensions="php"></div>
<hr>
<br>
