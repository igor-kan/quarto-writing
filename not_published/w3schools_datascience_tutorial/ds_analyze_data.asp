...
<html lang="en-US">
<head>
<title>Data Science Data Preparation and Analyze Data</title>
<meta charset="utf-8">
...
<h1>Data Science <span class="color_h1">- Data Preparation</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="ds_functions.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="ds_linear_functions.asp">Next &#10095;</a>
</div>
<hr>
<p class="intro">Before analyzing data, a Data Scientist must extract the data, 
and make it clean and valuable.</p>
<hr>

<h2>Extract and Read Data With Pandas</h2>
<p>Before data can be analyzed, it must be imported/extracted.</p>
<p>In the example below, we show you how to import data using Pandas in Python.</p>
<p>We  use the <code class="w3-codespan">read_csv()</code> function to import a CSV file with the health data:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  import pandas as pd<br><br>health_data = pd.read_csv(&quot;data.csv&quot;, header=0, sep=",")<br>
  <br>print(health_data)</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_analyze_data&multi=demo_data_raw_multi">Try it Yourself &raquo;</a>
</div>
<h3>Example Explained</h3>
<ul>
  <li>Import the Pandas library</li>
  <li>Name the data frame as <code>health_data</code>.</li>
  <li><code>header=0</code> means that the headers for the variable names are to be found in the first row (note that 
  0 means the first row in Python)</li>
  <li><code>sep=",&quot;</code> means that "," is used as the separator between the 
  values. This is because we are using the file type .csv (comma separated 
  values)</li>
</ul>
<p><strong>Tip:</strong> If you have a large CSV file, you can use the <code class="w3-codespan">
head()</code> function to only show the top 5rows:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  import pandas as pd<br><br>health_data = pd.read_csv(&quot;data.csv&quot;, header=0, sep=",")<br>
  <br>print(health_data.head())</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_analyze_data_head&multi=demo_data_raw_multi">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Data Cleaning</h2>
<p>Look at the imported data. As you can see, the data are &quot;dirty&quot; with wrongly or unregistered values:</p>
<img src="img_data_raw.png" style="max-width:100%;height:auto" alt="Dirty data">
<ul>
  <li>There are some blank fields</li>
  <li>Average pulse of 9 000 is not possible</li>
  <li>9 000 will be treated as non-numeric, because of the space separator</li>
  <li>One observation of max pulse is denoted as &quot;AF&quot;, which does not make sense</li>
</ul>
<p>So, we must clean the data in order to perform the analysis.</p>
<hr>

<h2>Remove Blank Rows</h2>
<p>We see that the non-numeric values (9 000 and AF) are in the same rows with missing values.</p>
<p>Solution: We can remove the rows with missing observations to fix this problem.</p>
<p>When we load a data set using Pandas, all blank cells are automatically converted into &quot;NaN&quot; values. </p>
<p>So, removing the NaN cells gives us a clean data set that can be analyzed.</p>
<p>We can 
use the <code class="w3-codespan">dropna()</code> function to remove the NaNs. axis=0 means that we want to remove all rows that have a NaN value:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  health_data.dropna(axis=0,inplace=True)<br><br>print(health_data)</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_clean_data&multi=demo_data_raw_multi">Try it Yourself &raquo;</a>
</div>

<p>The result is a data set without NaN rows:</p>
<img src="img_data_cleaned.png" style="max-width:100%;height:auto" alt="Cleaned data">
<hr>
...
<p>To analyze data, we also need to know the types of data we are dealing with.</p>
<p>Data can be split into two main categories:</p>
<ol>
  <li><strong>Quantitative Data</strong> - Can be expressed as a number or can 
  be quantified. Can be divided into two sub-categories:
    <ul>
      <li><strong>Discrete data</strong>: Numbers are counted as "whole", e.g. 
      number of students in a class, number of goals in a soccer game</li>
      <li><strong>Continuous data</strong>: Numbers can be of infinite precision. 
      e.g. weight of a person, shoe size, temperature</li>
    </ul>
  </li>
  <li><strong>Qualitative Data</strong> - Cannot be expressed as a number and 
  cannot be quantified. Can be divided into two sub-categories:
  <ul>
    <li><strong>Nominal data</strong>: Example: gender, hair color, ethnicity</li>
    <li><strong>Ordinal data</strong>: Example: school grades (A, B, C), 
    economic status (low, middle, high)</li>
  </ul>
  </li>
</ol>

<p>By knowing the type of your data, you will be able to know what technique to use when analyzing them.</p>
<hr>

<h2>Data Types</h2>
<p>We can use the <code class="w3-codespan">info()</code> function to list the data types 
within our data set:&nbsp;</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  print(health_data.info())</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_datatypes_data&multi=demo_data_raw_multi">Try it Yourself &raquo;</a>
</div>
<p>Result:</p>
<img src="img_data_float_object.png" style="max-width:100%;height:auto" alt="Datatype float and object">
<br>

<p>We see that this data set has two different types of data:</p>
<ul>
  <li>Float64</li>
  <li>Object</li>
</ul>
<p>We cannot use objects to calculate and perform analysis here. We must convert 
the type object to float64 (float64 is a number with a decimal in Python).</p>
<p>We can use the <code class="w3-codespan">astype()</code> function to convert the data into float64.</p>
<p>The following example converts &quot;Average_Pulse&quot; and &quot;Max_Pulse&quot; into data 
type float64 (the other variables are already of data type float64): </p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  health_data[&quot;Average_Pulse&quot;] 
  = health_data['Average_Pulse'].astype(float)<br>health_data[&quot;Max_Pulse&quot;] = 
  health_data[&quot;Max_Pulse&quot;].astype(float)<br><br>print 
  (health_data.info())</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_float_data&multi=demo_data_raw_multi">Try it Yourself &raquo;</a>
</div>
<p>Result:</p>
<img src="img_data_float.png" style="max-width:100%;height:auto" alt="Datatype float">
<p>Now, the data set has only float64 data types.</p>
<hr>

<h2>Analyze the Data</h2>
<p>When we have cleaned the data set, we can start analyzing the data. </p>
<p>We can use the <code class="w3-codespan">describe()</code> function in Python 
to summarize data: </p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate pythonHigh">
  print(health_data.describe())</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython_multi_csv.asp?filename=demo_describe_data&multi=demo_data_multi">Try it Yourself &raquo;</a>
</div>
...
<ul>
  <li><strong>Count</strong> - Counts the number of observations</li>
  <li><strong>Mean</strong> - The average value</li>
  <li><strong>Std</strong> - Standard deviation (explained in the statistics chapter)</li>
  <li><strong>Min</strong> - The lowest value</li>
  <li><strong>25%</strong>, <strong>50%</strong> and <strong>75%</strong> are percentiles 
  (explained in the statistics chapter)</li>
  <li><strong>Max</strong> - The highest value</li>
</ul>
