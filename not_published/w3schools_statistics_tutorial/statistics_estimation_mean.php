<!DOCTYPE html>
<html lang="en-US">
<head><title>Statistics - Estimating Population Means</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Statistics - Estimating Population Means</h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="statistics_estimation_proportion.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="statistics_z_table.php">Next &#10095;</a>
</div>
<hr>
<p class="intro">A population <a href="statistics_mean.php">mean</a> is an average of a <a href="statistics_data_types.php">numerical</a> population variable.</p>
<p class="intro">Confidence intervals are used to <a href="statistics_estimation.php">estimate</a> population means.</p>
<hr>
<h2>Estimating Population Mean</h2>
<p>A statistic from a <a href="statistics_populations_and_samples.php">sample</a> is used to estimate a parameter of the population.</p>
<p>The most likely value for a parameter is the <strong>point estimate</strong>.</p>
<p>Additionally, we can calculate a <strong>lower bound</strong> and an <strong>upper bound</strong> for the estimated parameter.</p>
<p>The <strong>margin of error</strong> is the difference between the lower and upper bounds from the point estimate.</p>
<p>Together, the lower and upper bounds define a <strong>confidence interval</strong>.</p>
<hr>
<h2>Calculating a Confidence Interval</h2>
<p>The following steps are used to calculate a confidence interval:</p>
<ol>
  <li>Check the conditions</li>
  <li>Find the point estimate</li>
  <li>Decide the confidence level</li>
  <li>Calculate the margin of error</li>
  <li>Calculate the confidence interval</li>
</ol>
<p>For example:</p>
<ul>
  <li><strong>Population</strong>: Nobel Prize winners</li>
  <li><strong>Variable</strong>: Age when they received the Nobel Prize</li>
</ul>
<p>We can take a sample and calculate the mean and the <a href="statistics_standard_deviation.php">standard deviation</a> of that sample.</p>
<p>The sample data is used to make an estimation of the average age of <strong>all</strong> the Nobel Prize winners.</p>
<p>By randomly selecting 30 Nobel Prize winners we could find that:</p>
<div class="w3-code notranslate">
  <p>The mean age in the sample is 62.1</p>
  <p>The standard deviation of age in the sample is 13.46</p>
</div>
<p>From this data we can calculate a confidence interval with the steps below.</p>
<hr>
<h2>1. Checking the Conditions</h2>
<p>The conditions for calculating a confidence interval for a mean are:</p>
<ul>
...
<h2>2. Finding the Point Estimate</h2>
<p>The point estimate is the <a href="statistics_mean.php">sample mean</a> (\(\bar{x}\)).</p>
<p>The formula for calculating the sample mean is the sum of all the values \(\sum x_{i}\) divided by the sample size (\(n\)):</p>
<p>\(\displaystyle \bar{x} = \frac{\sum x_{i}}{n}\)</p>
<p>In our example, the mean age was 62.1 in the sample.</p>
<hr>
<h2>3. Deciding the Confidence Level</h2>
<p>The confidence level is expressed with a percentage or a decimal number.</p>
<p>For example, if the confidence level is 95% or 0.95:</p>
<p>The remaining probability (\(\alpha\)) is then: 5%, or 1 - 0.95 = 0.05.</p>
<p>Commonly used confidence levels are:</p>
<ul>
  <li>90&#37; with \(\alpha\) = 0.1</li>
  <li>95&#37; with \(\alpha\) = 0.05</li>
  <li>99&#37; with \(\alpha\) = 0.01</li>
</ul>
<div class="w3-panel ws-note">
  <p><strong>Note:</strong> A 95&#37; confidence level means that if we take 100 different samples and make confidence intervals for each:</p>
  <p>The true parameter will be inside the confidence interval 95 out of those 100 times.</p>
</div>
<p>We use the <a href="statistics_students_t_distribution.php">student's t-distribution</a> to find the <strong>margin of error</strong> for the confidence interval.</p>
<p>The t-distribution is adjusted for the sample size with 'degrees of freedom' (df).</p>
<p>The degrees of freedom is the sample size (n) - 1, so in this example it is 30 - 1 = 29</p>
<p>The remaining probabilities (\(\alpha\)) are divided in two so that half is in each tail area of the distribution.</p>
<p>The values on the t-value axis that separate the tails area from the middle are called <strong>critical t-values</strong>.</p>
<p>Below are graphs of the standard normal distribution showing the tail areas (\(\alpha\)) for different confidence levels at 29 degrees of freedom (df).</p>
<p><img src="img_t_ci.svg" style="width:100%; max-width:1200px; height:auto" alt="Student's t-distributions with two tail areas, with different sizes."></p>

<hr>
<h2>4. Calculating the Margin of Error</h2>
<p>The margin of error is the difference between the point estimate and the lower and upper bounds.</p>
<p>The margin of error (\(E\)) for a proportion is calculated with a <a href="statistics_t_table.php">critical t-value</a> and the <strong>standard error</strong>:</p>
<p>\(\displaystyle E = t_{\alpha/2}(df) \cdot \frac{s}{\sqrt{n}} \)</p>
<p>The critical t-value \(t_{\alpha/2}(df) \) is calculated from the standard normal distribution and the confidence level.</p>
<p>The standard error \(\frac{s}{\sqrt{n}} \) is calculated from the sample standard deviation (\(s\)) and the sample size (\(n\)).</p>
<p>In our example with a sample standard deviation (\(s\)) of 13.46 and sample size of 30 the standard error is:</p>
<p>\(\displaystyle \frac{s}{\sqrt{n}} = \frac{13.46}{\sqrt{30}} \approx \frac{13.46}{5.477} = \underline{2.458}\)</p>
<p>If we choose 95&#37; as the confidence level, the \(\alpha\) is 0.05.</p>
<p>So we need to find the critical t-value \(t_{0.05/2}(29) = t_{0.025}(29)\)</p>
<p>The critical t-value can be found using a <a href="statistics_t_table.php">t-table</a> or with a programming language function:</p>
<div class="w3-example">
  <h3>Example</h3>
  <div>
    <p>With Python use the Scipy Stats library <code class="w3-codespan">t.ppf()</code> function find the t-value for an \(\alpha\)/2 = 0.025 and 29 degrees of freedom.</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(stats.t.ppf(1-0.025, 29))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_t_score">Try it Yourself &raquo;</a>
</div>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <div>
    <p>With R use the built-in <code class="w3-codespan">qt()</code> function to find the t-value for an \(\alpha\)/2 = 0.025 and 29 degrees of freedom.</p>
  <div class="w3-code notranslate pythonHigh">
    qt(1-0.025, 29)
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_t_score">Try it Yourself &raquo;</a>
</div>
</div>
<p>Using either method we can find that the critical t-value \(t_{\alpha/2}(df)\) is \(\approx \underline{2.05} \)</p>
<p>The standard error \(\frac{s}{\sqrt{n}}\) was \( \approx \underline{2.458}\)</p>
<p>So the margin of error (\(E\)) is:</p>
<p>\(\displaystyle E = t_{\alpha/2}(df) \cdot \frac{s}{\sqrt{n}} \approx 2.05 \cdot 2.458  = \underline{5.0389}\)</p>
<hr>
<h2>5. Calculate the Confidence Interval</h2>
<p>The lower and upper bounds of the confidence interval are found by subtracting and adding the margin of error (\(E\)) from the point estimate (\(\bar{x}\)).</p>
<p>In our example the point estimate was 0.2 and the margin of error was 0.143, then:</p>
...
<p>\([57.06, 67.14]\)</p>
<p>And we can summarize the confidence interval by stating:</p>
<div class="w3-code notranslate">
  <p>The <strong>95&#37;</strong> confidence interval for the mean age of Nobel Prize winners is between <strong>57.06 and 67.14 years</strong></p>
</div>
<hr>
<h2>Calculating a Confidence Interval with Programming</h2>
<p>A confidence interval can be calculated with many programming languages.</p>
...
<div class="w3-panel ws-note">
  <p><strong>Note:</strong> The results from using the programming code will be more accurate  because of rounding of values when calculating by hand.</p>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <div>
    <p>With Python use the scipy and math libraries to calculate the confidence interval for an estimated proportion.</p>
    <p>Here, the sample size is 30, sample mean is 62.1 and sample standard deviation is 13.46.</p>
    <div class="w3-code notranslate pythonHigh">
      import scipy.stats as stats
      <br>
      import math
    ...
    <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_mean_ci">Try it Yourself &raquo;</a>
  </div>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <div>
    <p>R can use built-in math and statistics functions to calculate the confidence interval for an estimated proportion.</p>
    <p>Here, the sample size is 30, sample mean is 62.1 and sample standard deviation is 13.46.</p>
    <div class="w3-code notranslate pythonHigh">
      # Specify sample mean (x_bar), sample standard deviation (s), sample size (n) and confidence level
      <br>
    ...
    <a target="_blank" class="w3-btn w3-margin-top w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_mean_ci">Try it Yourself &raquo;</a>
  </div>
</div>
<div class="w3-panel ws-info">
  <p><strong>Note:</strong> R also has a built in function for calculating a confidence interval for a population mean.</p>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <div>
    <p>R can use the built-in <code class="w3-codespan">t.test()</code> function to calculate the confidence interval for an estimated mean.</p>
    <p>Here, the sample is 30 randomly generated values with a mean of 60 and standard deviation is 12.5 using the <code class="w3-codespan">rnorm()</code> function to generate the sample.</p>
    <div class="w3-code notranslate pythonHigh">
    ...
    <a target="_blank" class="w3-btn w3-margin-top w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_mean_ci2">Try it Yourself &raquo;</a>
  </div>
</div>
<br>
<div class="w3-clear nextprev">
