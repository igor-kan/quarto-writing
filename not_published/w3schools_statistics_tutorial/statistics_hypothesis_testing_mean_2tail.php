<!DOCTYPE html>
<html lang="en-US">
<head><title>Statistics - Hypothesis Testing a Mean (Two Tailed)</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Statistics - Hypothesis Testing a Mean (Two Tailed)</h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="statistics_hypothesis_testing_mean_leftphp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="statistics_exam.php">Next &#10095;</a>
</div>
<hr>
<p class="intro">A population <a href="statistics_mean.php">mean</a> is an average of value a population.</p>
<p class="intro">Hypothesis tests are used to check a claim about the size of that population mean.</p>
<hr>
<h2>Hypothesis Testing a Mean</h2>
...
<ul>
  ...
  <li><strong>Category</strong>: Age when they received the prize.</li>
</ul>
<p>And we want to check the claim:</p>
<div class="w3-code notranslate">
  <p>"The average age of Nobel Prize winners when they received the prize is <strong>not</strong> 60"</p>
</div>
<p>By taking a sample of 30 randomly selected Nobel Prize winners we could find that:</p>
<div class="w3-code notranslate">
  <ul>
    <li>The mean age in the sample (\(\bar{x}\)) is 62.1</li>
    <li>The standard deviation of age in the sample (\(s\)) is 13.46</li>
  </ul>
</div>
...
<ul>
  ...
  <li>And either:
  <ul>
    <li>The population data is normally distributed</li>
    <li>Sample size is large enough</li>
  </ul>
  </li>
</ul>
<p>A moderately large sample size, like 30, is typically large enough.</p>
<p>In the example, the sample size was 30 and it was randomly selected, so the conditions are fulfilled.</p>
<div class="w3-panel ws-note">
  <p><strong>Note:</strong> Checking if the data is normally distributed can be done with specialized statistical tests.</p>
</div>
<hr>
...
<p>In this case, the <strong>parameter</strong> is the mean age of Nobel Prize winners when they received the prize (\(\mu\)).</p>
<p>The null and alternative hypothesis are then:</p>
<div class="w3-code notranslate">
  <p><strong>Null hypothesis</strong>: The average age was 60.</p>
...
<p>This is a '<strong>two-tailed</strong>' test, because the alternative hypothesis claims that the proportion is <strong>different</strong> from the null hypothesis.</p>
...
<p>The formula for the test statistic (TS) of a population mean is:</p>
<p>\(\displaystyle \frac{\bar{x} - \mu}{s} \cdot \sqrt{n} \)</p>
<p>\(\bar{x}-\mu\) is the <strong>difference</strong> between the <strong>sample</strong> mean (\(\bar{x}\)) and the claimed <strong>population</strong> mean (\(\mu\)).</p>
<p>\(s\) is the <a href="statistics_standard_deviation.php">sample standard deviation</a>.</p>
<p>\(n\) is the sample size.</p>
<p>In our example:</p>
<div class="w3-code notranslate">
  <p>The claimed (\(H_{0}\)) population mean (\(\mu\)) was \( 60 \)</p>
  <p>The sample mean (\(\bar{x}\)) was \(62.1\)</p>
  <p>The sample standard deviation (\(s\)) was \(13.46\)</p>
  <p>The sample size (\(n\)) was \(30\)</p>
</div>
<p>So the test statistic (TS) is then:</p>
<p>\(\displaystyle \frac{62.1-60}{13.46} \cdot \sqrt{30} = \frac{2.1}{13.46} \cdot \sqrt{30}  \approx 0.156 \cdot 5.477 = \underline{0.855}\)</p>
<p>You can also calculate the test statistic using programming language functions:</p>
<div class="w3-example">
  <h3>Example</h3>
    <p>With Python use the scipy and math libraries to calculate the test statistic.</p>
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_mean_test_statistic2">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
    <p>With R use built-in math and statistics functions to calculate the test statistic.</p>
  <div class="w3-code notranslate pythonHigh">
    # Specify the sample mean (x_bar), the sample standard deviation (s), the mean claimed in the null-hypothesis (mu_null), and the sample size (n)
    <br>
    x_bar &lt;- 62.1
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_mean_test_statistic2">Try it Yourself &raquo;</a>
</div>
<hr>
<h2>5. Concluding</h2>
...
<p>For a population mean test, the critical value (CV) is a <strong>T-value</strong> from a <a href="students_t_distribution.php">student's t-distribution</a>.</p>
<p>This critical T-value (CV) defines the <strong>rejection region</strong> for the test.</p>
<p>The rejection region is an area of probability in the tails of the standard normal distribution.</p>
<p>Because the claim is that the population proportion is <strong>different</strong> from 60, the rejection region is split into both the left and right tail:</p>

<p><img src="img_critical_value_2tail2.svg" alt="Student's T-Distribution with a left and right tail areas (rejection region) denoted as the greek symbol alpha"></p>
<p>The size of the rejection region is decided by the significance level (\(\alpha\)).
<p>The student's t-distribution is adjusted for the uncertainty from smaller samples.</p>
<p>This adjustment is called degrees of freedom (df), which is the sample size \((n) - 1\)</p>
<p>In this case the degrees of freedom (df) is: \(30 - 1 = \underline{29} \)</p>
<p>Choosing a significance level (\(\alpha\)) of 0.05, or 5&#37;, we can find the critical T-value from a <a href="statistics_t_table.php">T-table</a>, or with a programming language function:</p>
<div class="w3-panel ws-note">
    <p><strong>Note:</strong> Because this is a two-tailed test the tail area (\(\alpha\)) needs to be split in half (divided by 2).</p>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <p>With Python use the Scipy Stats library <code class="w3-codespan">t.ppf()</code> function find the T-Value for an \(\alpha\)/2 = 0.025 at 29 degrees of freedom (df).</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(stats.t.ppf(0.025, 29))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_t_score4">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <p>With R use the built-in <code class="w3-codespan">qt()</code> function to find the t-value for an \(\alpha\)/ = 0.025 at 29 degrees of freedom (df).</p>
  <div class="w3-code notranslate pythonHigh">
    qt(0.025, 29)
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_t_score4">Try it Yourself &raquo;</a>
</div>
<p>Using either method we can find that the critical T-Value is \(\approx \underline{-2.045}\)</p>
...
<p>Here, the test statistic (TS) was \(\approx \underline{0.855}\) and the critical value was \(\approx \underline{-2.045}\)</p>
<p>Here is an illustration of this test in a graph:</p>
<p><img src="img_critical_value_mean_2tail.svg" alt="Student's T-Distribution with a left and right tail area (rejection region) equal to 0.05, a critical value of 2.045, and a test statistic of 0.855"></p>
<p>Since the test statistic is <strong>between</strong> the critical values we <strong>keep</strong> the null hypothesis.</p>
<p>This means that the sample data does not support the alternative hypothesis.</p>
<p>And we can summarize the conclusion stating:</p>
<div class="w3-code notranslate">
  <p>The sample data does <strong>not</strong> support the claim that "The average age of Nobel Prize winners when they received the prize is not 60" at a <strong>5&#37; significance level</strong>.</p>
</div>
...
<p>The test statistic was found to be \( \approx \underline{0.855} \)</p>
<p>For a population proportion test, the test statistic is a T-Value from a <a href="students_t_distribution.php">student's t-distribution</a>.</p>
<p>Because this is a <strong>two-tailed</strong> test, we need to find the P-value of a T-value <strong>bigger</strong> than 0.855 and <strong>multiply it by 2</strong>.</p>
<p>The student's t-distribution is adjusted according to degrees of freedom (df), which is the sample size \((30) - 1 = \underline{29}\)</p>
<p>We can find the P-value using a <a href="statistics_t_table.php">T-table</a>, or with a programming language function:</p>
<div class="w3-example">
  <h3>Example</h3>
    <p>With Python use the Scipy Stats library <code class="w3-codespan">t.cdf()</code> function find the P-value of a T-value bigger than 0.855 for a two tailed test at 29 degrees of freedom (df):</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(2*(1-stats.t.cdf(0.855, 29)))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_p_value6">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
    <p>With R use the built-in <code class="w3-codespan">pt()</code> function find the P-value of a T-Value bigger than 0.855 for a two tailed test at 29 degrees of freedom (df):</p>
  <div class="w3-code notranslate pythonHigh">
    2*(1-pt(0.855, 29))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_p_value6">Try it Yourself &raquo;</a>
</div>
<p>Using either method we can find that the P-value is \(\approx \underline{0.3996}\)</p>
<p>This tells us that the significance level (\(\alpha\)) would need to be smaller 0.3996, or 39.96&#37;, to <strong>reject</strong> the null hypothesis.</p>
<p>Here is an illustration of this test in a graph:</p>
<p><img src="img_p_value_mean_2tail.svg"></p>
<p>This P-value is <strong>bigger</strong> than any of the common significance levels (10&#37;, 5&#37;, 1&#37;).</p>
<p>So the null hypothesis is <strong>kept</strong> at all of these significance levels.</p>
<p>And we can summarize the conclusion stating:</p>
<div class="w3-code notranslate">
  <p>The sample data does <strong>not</strong> support the claim that "The average age of Nobel Prize winners when they received the prize is not 60" at a <strong>10&#37;, 5&#37;, or 1&#37; significance level</strong>.</p>
</div>
<hr>
...
<div class="w3-example">
  <h3>Example</h3>
    <p>With Python use the scipy and math libraries to calculate the P-value for a two tailed hypothesis test for a mean.</p>
    <p>Here, the sample size is 30, the sample mean is 62.1, the sample standard deviation is 13.46, and the test is for a mean different from 60.</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    import math
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_mean_test3">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
    <p>With R use built-in math and statistics functions find the P-value for a two tailed hypothesis test for a mean.</p>
    <p>Here, the sample size is 30, the sample mean is 62.1, the sample standard deviation is 13.46, and the test is for a mean different from 60.</p>
  <div class="w3-code notranslate pythonHigh">
    # Specify the sample mean (x_bar), the sample standard deviation (s), the mean claimed in the null-hypothesis (mu_null), and the sample size (n)
    <br>
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_mean_test3">Try it Yourself &raquo;</a>
</div>
<hr>
<h2>Left-Tailed and Two-Tailed Tests</h2>
<p>This was an example of a <strong>left</strong> tailed test, where the alternative hypothesis claimed that parameter is <strong>smaller</strong> than the null hypothesis claim.</p>
<p>You can check out an equivalent step-by-step guide for other types here:</p>
<ul>
  <li><a href="statistics_hypothesis_testing_mean.php">Right-Tailed Test</a></li>
  <li><a href="statistics_hypothesis_testing_mean_2tail.php">Two-Tailed Test</a></li>
</ul>
<br>
<div class="w3-clear nextprev">
    <a class="w3-left w3-btn" href="statistics_hypothesis_testing_mean_left.php">&#10094; Previous</a>
    <a class="w3-right w3-btn" href="statistics_exam.php">Next &#10095;</a>
</div>
<div
