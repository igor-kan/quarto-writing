<!DOCTYPE html>
<html lang="en-US">
<head><title>Statistics - Hypothesis Testing a Mean (Left Tailed)</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Statistics - Hypothesis Testing a Mean (Left Tailed)</h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="statistics_hypothesis_testing_proportion_2tail.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="statistics_hypothesis_testing_mean_2tail.php">Next &#10095;</a>
</div>
<hr>
...
<div class="w3-code notranslate">
  <p>"The average age of Nobel Prize winners when they received the prize is <strong>less</strong> than 60"</p>
</div>
<p>By taking a sample of 30 randomly selected Nobel Prize winners we could find that:</p>
<div class="w3-code notranslate">
  <p>The mean age in the sample (\(\bar{x}\)) is 62.1</p>
  <p>The standard deviation of age in the sample (\(s\)) is 13.46</p>
</div>
...
<p>Because the claim is that the population mean is <strong>less</strong> than 60, the rejection region is in the left tail:</p>
<p><img src="img_critical_value_left_prop.svg" alt="Student's T-Distribution with a left tail area (rejection region) denoted as the greek symbol alpha"></p>
...
<div class="w3-example">
  <h3>Example</h3>
  <p>With Python use the Scipy Stats library <code class="w3-codespan">t.ppf()</code> function find the T-Value for an \(\alpha\) = 0.05 at 29 degrees of freedom (df).</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(stats.t.ppf(0.05, 29))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_t_score3">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <p>With R use the built-in <code class="w3-codespan">qt()</code> function to find the t-value for an \(\alpha\) = 0.05 at 29 degrees of freedom (df).</p>
  <div class="w3-code notranslate pythonHigh">
    qt(0.05, 29)
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_t_score3">Try it Yourself &raquo;</a>
</div>
<p>Using either method we can find that the critical T-Value is \(\approx \underline{-1.699}\)</p>
...
<p>If the test statistic is smaller the critical value, the test statistic is in the <strong>rejection region</strong>.</p>
<p>When the test statistic is in the rejection region, we <strong>reject</strong> the null hypothesis (\(H_{0}\)).</p>
<p>Here, the test statistic (TS) was \(\approx \underline{0.855}\) and the critical value was \(\approx \underline{-1.699}\)</p>
<p>Here is an illustration of this test in a graph:</p>
<p><img src="img_critical_value_left_mean.svg" alt="Student's T-Distribution with a left tail area (rejection region) equal to 0.01, a critical value of 2.462, and a test statistic of 2.889"></p>
<p>Since the test statistic was <strong>bigger</strong> than the critical value we <strong>keep</strong> the null hypothesis.</p>
...
<div class="w3-code notranslate">
  <p>The sample data does <strong>not</strong> support the claim that "The average age of Nobel Prize winners when they received the prize is less than 60" at a <strong>5&#37; significance level</strong>.</p>
</div>
...
<p>Because this is a <strong>left</strong> tailed test, we need to find the P-value of a t-value <strong>smaller</strong> than 0.855.</p><p>The student's t-distribution is adjusted according to degrees of freedom (df), which is the sample size \((30) - 1 = \underline{29}\)</p>
<p>We can find the P-value using a <a href="statistics_t_table.php">T-table</a>, or with a programming language function:</p>
<div class="w3-example">
  <h3>Example</h3>
    <p>With Python use the Scipy Stats library <code class="w3-codespan">t.cdf()</code> function find the P-value of a T-value smaller than 0.855 at 29 degrees of freedom (df):</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(stats.t.cdf(0.855, 29))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_p_value5">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
    <p>With R use the built-in <code class="w3-codespan">pt()</code> function find the P-value of a T-Value smaller than 0.855 at 29 degrees of freedom (df):</p>
  <div class="w3-code notranslate pythonHigh">
    pt(0.855, 29)
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_p_value5">Try it Yourself &raquo;</a>
</div>
<p>Using either method we can find that the P-value is \(\approx \underline{0.800}\)</p>
<p>This tells us that the significance level (\(\alpha\)) would need to be smaller 0.80, or 80&#37;, to <strong>reject</strong> the null hypothesis.</p>
<p>Here is an illustration of this test in a graph:</p>
<p><img src="img_p_value_left_mean.svg"></p>
<p>This P-value is far <strong>bigger</strong> than any of the common significance levels (10&#37;, 5&#37;, 1&#37;).</p>
<p>So the null hypothesis is <strong>kept</strong> at all of these significance levels.</p>
<p>And we can summarize the conclusion stating:</p>
<div class="w3-code notranslate">
  <p>The sample data does <strong>not</strong> support the claim that "The average age of Nobel Prize winners when they received the prize is less than 60" at a <strong>10&#37;, 5&#37;, or 1&#37; significance level</strong>.</p>
</div>
<hr>
...
<div class="w3-example">
  <h3>Example</h3>
    <p>With Python use the scipy and math libraries to calculate the P-value for a left tailed hypothesis test for a mean.</p>
    <p>Here, the sample size is 30, the sample mean is 62.1, the sample standard deviation is 13.46, and the test is for a mean smaller 60.</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    import math
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_mean_test2">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
    <p>With R use built-in math and statistics functions find the P-value for a left tailed hypothesis test for a mean.</p>
    <p>Here, the sample size is 30, the sample mean is 62.1, the sample standard deviation is 13.46, and the test is for a mean smaller 60.</p>
  <div class="w3-code notranslate pythonHigh">
    # Specify the sample mean (x_bar), the sample standard deviation (s), the mean claimed in the null-hypothesis (mu_null), and the sample size (n)
    <br>
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_mean_test2">Try it Yourself &raquo;</a>
</div>
<hr>
