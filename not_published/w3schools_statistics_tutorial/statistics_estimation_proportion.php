<!DOCTYPE html>
<html lang="en-US">
<head><title>Statistics - Estimating Population Proportions</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Statistics - Estimating Population Proportions</h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="statistics_estimation.php">&#10094; Previous</a>
...
<p class="intro">Confidence intervals are used to <a href="statistics_estimation.php">estimate</a> population proportions. </p>
<hr>
<h2>Estimating Population Proportions</h2>
...
<p>We can take a sample and see how many of them were born in the US.</p>
<p>The sample data is used to make an estimation of the share of <strong>all</strong> the Nobel Prize winners born in the US.</p>
<p>By randomly selecting 30 Nobel Prize winners we could find that:</p>
<div class="w3-code notranslate">
  <p>6 out of 30 Nobel Prize winners in the sample were born in the US</p>
</div>
...
<p>In our example, we randomly selected 6 people that were born in the US.</p>
<p>The rest were not born in the US, so there are 24 in the other category.</p>
<p>The conditions are fulfilled in this case.</p>
<div class="w3-panel ws-note">
  <p><strong>Note:</strong> It is possible to calculate a confidence interval without having 5 of each category. But special adjustments need to be made.</p>
</div>
<hr>
...
<p>The point estimate is the sample proportion (\(\hat{p}\)).</p>
<p>The formula for calculating the sample proportion is the number of 
occurrences (\(x\)) divided by the sample size (\(n\)):</p>
<p>\(\displaystyle \hat{p} =\frac{x}{n}\)</p>
<p>In our example, 6 out of 30 were born in the US: \(x\) is 6, and \(n\) is 30.</p>
<p>So the point estimate for the proportion is:</p>
<p>\(\displaystyle \hat{p} = \frac{x}{n} = \frac{6}{30} = \underline{0.2} = 20\%\)</p>
<p>So 20&#37; of the sample were born in the US.</p>
<hr>
...
<p>We use the <a href="statistics_standard_normal_distribution.php">standard normal distribution</a> to find the <strong>margin of error</strong> for the confidence interval.</p>
<p>The remaining probabilities (\(\alpha\)) are divided in two so that half is in each tail area of the distribution.</p>
<p>The values on the z-value axis that separate the tails area from the middle are called <strong>critical z-values</strong>.</p>
<p>Below are graphs of the standard normal distribution showing the tail areas (\(\alpha\)) for different confidence levels.</p>
<p><img src="img_z_ci.svg" style="width:100%; max-width:1200px; height:auto" alt="Standard Normal Distributions with two tail areas, with different sizes."></p>
...
<p>The margin of error (\(E\)) for a proportion is calculated with a <a href="statistics_z_table.php">critical z-value</a> and the <strong>standard error</strong>:</p>
<p>\(\displaystyle E = Z_{\alpha/2} \cdot \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \)</p>
<p>The critical z-value \(Z_{\alpha/2} \) is calculated from the standard normal distribution and the confidence level.</p>
<p>The standard error \(\sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \) is calculated from the point estimate (\(\hat{p}\)) and sample size (\(n\)).</p>
<p>In our example with 6 US-born Nobel Prize winners out of a sample of 30 the standard error is:</p>
...
<p>So we need to find the critical z-value \(Z_{0.05/2} = Z_{0.025}\)</p>
<p>The critical z-value can be found using a <a href="statistics_z_table.php">Z-table</a> or with a programming language function:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>With Python use the Scipy Stats library <code class="w3-codespan">norm.ppf()</code> function find the Z-value for an \(\alpha\)/2 = 0.025</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    print(stats.norm.ppf(1-0.025))
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_z_score">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <p>With R use the built-in <code class="w3-codespan">qnorm()</code> function to find the Z-value for an \(\alpha\)/2 = 0.025</p>
  <div class="w3-code notranslate pythonHigh">
    qnorm(1-0.025)
  </div>
  <a target="_blank" class="w3-btn w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_z_score">Try it Yourself &raquo;</a>
</div>

<p>Using either method we can find that the critical Z-value \( Z_{\alpha/2} \) is \(\approx \underline{1.96} \)</p>
<p>The standard error \(\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}\) was \( \approx \underline{0.073}\)</p>
...
<p>The lower and upper bounds of the confidence interval are found by subtracting and adding the margin of error (\(E\)) from the point estimate (\(\hat{p}\)).</p>
...
<div class="w3-code notranslate">
  <p>The <strong>95&#37;</strong> confidence interval for the proportion of Nobel Prize winners born in the US is between <strong>5.7&#37; and 34.4&#37;</strong></p>
</div>
<hr>
...
<div class="w3-example">
  ...
  <p>Here, the sample size is 30 and the occurrences is 6.</p>
  <div class="w3-code notranslate pythonHigh">
    import scipy.stats as stats
    <br>
    import math
  ...
  <a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_stat_python_prop_ci">Try it Yourself &raquo;</a>
</div>
<div class="w3-example">
  <h3>Example</h3>
  <p>With R, use the built-in math and statistics functions to calculate the confidence interval for an estimated proportion.</p>
  <p>Here, the sample size is 30 and the occurrences is 6.</p>
  <div class="w3-code notranslate pythonHigh">
    # Specify sample occurrences (x), sample size (n) and confidence level
    <br>
  ...
  <a target="_blank" class="w3-btn w3-margin-top w3-margin-bottom" href="tryr.asp?filename=demo_stat_r_prop_ci">Try it Yourself &raquo;</a>
</div>
<br>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="statistics_estimation.php">&#10094; Previous</a>
