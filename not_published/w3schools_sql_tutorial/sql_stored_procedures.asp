...
<h1>SQL <span class="color_h1">Stored Procedures</span></h1>
...
<p>A stored procedure is a precompiled SQL code that can be saved and reused.</p>
<p>If you have an SQL query that you write over and over again, save 
it as a stored procedure, and then just call it to execute it.</p>
<p>A stored procedure can also have parameters, so it can act based on the parameter value(s) 
that is passed.</p>
<hr>

<h2>Key Benefits of Stored Procedures</h2>
<p>Stored procedures are widely used in database management, and have the following benefits:</p>
<ul>
  <li>Code Reusability - The same procedure can be called from various applications</li>
  <li>Improved Performance - Stored procedures are precompiled and runs faster</li>
  <li>Database Security - You can set users permission to run a specific procedure (limits direct access to tables)</li>
  <li>Easy Maintenance - When updating a procedure, it automatically updates all its use</li>
</ul>
<hr>

<h2>Stored Procedure Syntax (SQL Server)</h2>
<p>To create a stored procedure, use the <code class="w3-codespan">CREATE PROCEDURE</code> statement:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
CREATE PROCEDURE <em>procedure_name</em><br>
&nbsp; <em>@param1 datatype</em>,<br>
&nbsp; <em>@param2 datatype</em><br>
AS<br>
BEGIN<br>
&nbsp; -- SQL_statements to be executed<br>
&nbsp; SELECT <em>column1</em>, <em>column2</em><br>
&nbsp; FROM <em>table_name</em><br>
&nbsp; WHERE <em>columnN</em> = <em>@paramN</em>;
<br>END;
</div></div>
<p><strong>Tip:</strong> To see the syntax for MySQL database, look at 
<a href="/mysql/mysql_stored_procedures.asp">MySQL Stored Procedures</a>.</p>
<hr>

<h2>Execute a Stored Procedure</h2>
<p>To run a stored procedure, use the <code class="w3-codespan">EXEC</code> 
statement:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
EXEC <em>procedure_name @param1</em> = <em>'value1'</em>, <em>@param2</em> = <em>'value2'</em>;
</div></div>
<hr>

<h2>Drop a Stored Procedure</h2>
<p>To delete a stored procedure, use the <code class="w3-codespan">DROP PROCEDURE</code> statement:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
  DROP PROCEDURE <em>procedure_name</em>;</div></div>

<p><strong>Tip:</strong> To ensure that <code class="w3-codespan">DROP PROCEDURE</code> does not 
return an error, if the procedure is missing, add the <code class="w3-codespan">IF EXISTS</code> clause:</p>
<div class="w3-example">
<div class="w3-code notranslate sqlHigh">
  DROP PROCEDURE IF EXISTS <em>procedure_name</em>;</div></div>
<hr>
...
<p>The following SQL creates a stored procedure named "GetCustomersByCity"  
that can be used to select Customers from a particular City in the "Customers" table:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate sqlHigh">
  CREATE PROCEDURE 
  GetCustomersByCity<br>&nbsp; @City nvarchar(50)<br>AS<br>BEGIN<br>&nbsp; SELECT * FROM Customers<br>&nbsp; WHERE 
  City = @City;<br>END;</div>
</div>
<p>Here we execute the stored procedure by passing a city ('London') as a parameter, 
and the stored procedure returns the relevant details from the "Customers" table:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate sqlHigh">
  EXEC 
  GetCustomersByCity @City = 'London';</div>
</div>
<hr>
<h2>Stored Procedure With Multiple Parameters</h2>
<p>Adding multiple parameters is easy. Just list each parameter and the 
data type separated by a comma as shown below.</p>
<p>The following SQL creates a stored procedure  
that selects Customers from a particular City with a particular PostalCode from the "Customers" table:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate sqlHigh">
  CREATE PROCEDURE 
  GetCustomersByCity<br>&nbsp; @City nvarchar(50),<br>&nbsp; @PostalCode nvarchar(10)<br>AS<br>
  BEGIN<br>&nbsp; SELECT * FROM Customers<br>&nbsp; WHERE 
  City = @City AND PostalCode = @PostalCode;<br>END;</div>
</div>
...
<div class="w3-code notranslate sqlHigh">
  EXEC 
  GetCustomersByCity @City = 'London', @PostalCode = 'WA1 1DP';</div>
</div>
<hr>
<div id="exercisecontainer" src="xrcise_stored_procedures.js"></div>
<hr>
