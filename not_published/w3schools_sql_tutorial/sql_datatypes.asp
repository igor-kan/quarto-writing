...
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="sql_exam.asp">&#10094; Previous</a>
...
<p>An SQL developer must decide what type of data that will be stored inside each column when creating a table. The data type is a 
guideline for SQL to understand what type of data is expected inside of each column, and it also identifies how SQL will interact with the stored data.</p>
<div class="w3-panel ws-note">
<p><strong>Always check the documentation!</strong> Data types might have different names in different databases.
And even if the name is the same, the size and other details may be different!
</p>
</div>
<hr>

<h2>MySQL Data Types (Version 8.4)</h2>
...
<table class="ws-table-all notranslate">
    ...
    <tr>
      ...
      <em>size</em> specifies the column length in characters, 
      from 0 to   255. Default <em>size</em> is 1</td>
    </tr>
    <tr>
      <td>VARCHAR(size)</td>
      <td>A variable-length string (can contain letters, numbers, and special 
      characters). <em>size</em> specifies the maximum column 
      length in characters, from 0 to 65535</td>
    </tr>
    <tr>
      <td>BINARY(size)</td>
      <td>Similar to CHAR(), but stores binary byte strings. <em>size</em> 
      specifies the column length in bytes. Default <em>size</em> is 1</td>
    </tr>
    <tr>
      <td>VARBINARY(size)</td>
      <td>Similar to VARCHAR(), but stores binary byte strings. <em>size</em> specifies the maximum column length in bytes.</td>
    </tr>
    <tr>
      <td>BLOB(size)</td>
      <td>A BLOB column with a maximum length of 65535 bytes</td>
    </tr>
    <tr>
      <td>TINYBLOB</td>
      <td>A BLOB column with a maximum length of 255 bytes</td>
    </tr>
    <tr>
      <td>MEDIUMBLOB</td>
      <td>A BLOB column with a maximum length of 16777215 bytes</td>
    </tr>
    <tr>
      <td>LONGBLOB</td>
      <td>A BLOB column with a maximum length of 4294967295 or 4GB bytes</td>
    </tr>
    <tr>
      <td>TEXT(size)</td>
      <td>Holds a string with a maximum length of 65535 bytes</td>
    </tr>
    <tr>
      <td>TINYTEXT</td>
      <td>A TEXT column with a maximum length of 255 characters</td>
    </tr>
    <tr>
      <td>MEDIUMTEXT</td>
      <td>A TEXT column with a maximum length of 16777215 characters</td>
    </tr>
    <tr>
      <td>LONGTEXT</td>
      <td>A TEXT column with a maximum length of 4294967295 or 4GB bytes</td>
    </tr>
    <tr>
      <td>ENUM(val1, val2, val3, ...)</td>
      <td>A string object that can have only one value, chosen from the list of possible values 
      (val1, val2, val3,..). You can list up to 65535 values. If a value is 
      inserted that is not in the list, a blank value will be inserted</td>
    </tr>
    <tr>
      <td>SET(val1, val2, val3, ...)</td>
      <td>A string object that can have zero or more values, chosen from the list of 
      possible values (val1, val2, val3,..). You can list up to 64 values</td>
    </tr>
</table>
</div>
...
<table class="ws-table-all notranslate">
    ...
    <tr>
      ...
      <td>A bit-value type. <em>size</em> indicates the number of bits per value, from 1 to 64. The default 
    ...
    <tr>
      ...
      is from 0 to 255. <em>size</em> specifies the minimum 
      display width (max display width is 255)</td>
    </tr>
    <tr>
      <td>BOOL</td>
      <td>A value of zero is considered as false, nonzero values are considered as true. </td>
    </tr>
    <tr>
      <td>BOOLEAN</td>
      <td>Equal to BOOL</td>
    ...
    <tr>
      ...
      is from 0 to 65535. <em>size</em> specifies the minimum 
      display width (max display width is 255)</td>
    </tr>
    <tr>
      <td>MEDIUMINT(<em>size</em>)</td>
      <td>A medium-sized integer. Signed range is from -8388608 to 8388607. Unsigned 
      range is from 0 to 16777215. <em>size</em> specifies the 
      minimum display width (max display width is 255)</td>
    </tr>
    <tr>
      <td>INT(<em>size</em>)</td>
      <td>A normal-size integer. Signed range is from -2147483648 to 2147483647. 
      Unsigned range is from 0 to 4294967295. <em>size</em>  
      specifies minimum display width (max display width is 255)</td>
    </tr>
    <tr>
      <td>INTEGER(<em>size</em>)</td>
      <td>Synonym for INT(<em>size</em>)</td>
    </tr>
    <tr>
      <td>BIGINT(<em>size</em>)</td>
      <td>A large integer. Signed range is from -9223372036854775808 to 
      9223372036854775807. Unsigned range is from 0 to 18446744073709551615. <em>size</em> specifies the 
      minimum display width (max display width is 255)</td>
    </tr>
    <tr>
      <td>DECIMAL(<em>size</em>, <em>d</em>)</td>
      <td>An exact fixed-point number. <em>size</em> indicates the   total number of digits 
      (max 65). <em>d</em> indicates the   number of digits after the decimal point (max 
      30). The default value for <em>size</em> 
      is 10. The default value for <em>d</em> is 0.</td>
    </tr>
    <tr>
      <td>DEC(<em>size</em>, <em>d</em>)</td>
      <td>Synonym for DECIMAL(<em>size</em>, <em>d</em>)</td>
    </tr>
    <tr>
      <td>FLOAT(<em>size</em>, <em>d</em>)</td>
      <td>A small floating-point number. FLOAT(<em>size</em>, <em>d</em>) is deprecated in MySQL 8.0.17, 
      and it will be removed in future MySQL versions</td>
    </tr>
    <tr>
      <td>FLOAT(<em>p</em>)</td>
      <td>A floating-point number. The <em>p</em> value determines 
    ...
    <tr>
      ...
      <td>A normal-size floating-point number. Permissible values are -1.7976931348623157E+308 to -2.2250738585072014E-308,
      0, and 2.2250738585072014E-308 to 1.7976931348623157E+308. The total number of digits is specified in 
      <em>size</em>. The number of digits after the decimal point is specified in the <em>d</em> parameter</td>
    </tr>
    <tr>
      <td>DOUBLE PRECISION(<em>size</em>, <em>d</em>)</td>
      <td>Synonym for DOUBLE(<em>size</em>, <em>d</em>)</td>
    </tr>
    </table>
    </div>
...
<table class="ws-table-all notranslate">
    ...
    <tr>
      ...
      <td>A timestamp. Format: YYYY-MM-DD hh:mm:ss. TIMESTAMP values are stored as the number of seconds since the Unix epoch ('1970-01-01 00:00:00' UTC). The supported range is from '1970-01-01 00:00:01'   UTC to '2038-01-09 03:14:07' UTC. 
    ...
    <tr>
      <td>YEAR</td>
      <td>A year in a four-digit format (YYYY). Values allowed in four-digit format: 1901 to 2155, and 0000.</td>
    </tr>
</table>
</div>
<hr>
...
<p>In SQL Server there are three main data types: String, Numeric, and Date and Time.</p>
<h3>String Data Types</h3>
<div class="w3-responsive">
<table class="ws-table-all notranslate">
    <tr>
      ...
      <th>Description</th>
      <th>Max char length</th>
      <th>Storage</th>
    </tr>
    <tr>
      <td>char(n)</td>
      <td>Fixed-length non-Unicode character data (n must be between 1 and 8000)</td>
      <td>8,000</td>
      <td>n bytes (uses one byte for each character)</td>
    </tr>
    <tr>
      <td>varchar(n)</td>
      <td>Variable-length non-Unicode character data (n must be between 1 and 8000)</td>
      <td>8,000</td>
      <td>n bytes + 2 bytes</td>
    </tr>
    <tr>
      <td>varchar(max)</td>
      <td>Variable-length non-Unicode character data</td>
      <td>&nbsp;</td>
      <td>up to 2 GB</td>
    </tr>
    <tr>
      <td>nchar(n)</td>
      <td>Fixed-length Unicode character data (n must be between 1 and 4000)</td>
      <td>4,000</td>
      <td>&nbsp;2 * n bytes (uses two bytes for each character)</td>
    </tr>
    <tr>
      <td>nvarchar(n)</td>
      <td>Variable-length Unicode character data (n must be between 1 and 4000)</td>
      <td>4,000</td>
      <td>2 * n bytes + 2 bytes (uses two bytes for each character)</td>
    </tr>
    <tr>
      <td>nvarchar(max)</td>
      <td>Variable-length Unicode character data</td>
      <td>&nbsp;</td>
      <td>up to 2 GB</td>
    </tr>
    <tr>
      <td>binary(n)</td>
      <td>Fixed-length binary data (n must be between 1 and 8000)</td>
      <td>8,000</td>
      <td>n bytes</td>
    </tr>
    <tr>
      <td>varbinary(n)</td>
      <td>Variable-length binary data (n must be between 1 and 8000)</td>
      <td>8,000</td>
      <td>actual length of data entered + 2 bytes</td>
    </tr>
    <tr>
      <td>varbinary(max)</td>
