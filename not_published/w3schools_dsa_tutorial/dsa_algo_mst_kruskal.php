...
<html lang="en-US">
<head>
<title>DSA Kruskal's Algorithm</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
.thickEdge {
  stroke-width: 4; /* Makes the line thicker */
  /*stroke: blue;  Change color to highlight */
}

.cutLine {
  stroke-width: 1;
  stroke: rgb(213,20,102);
}

.shadowEffect {
  filter: drop-shadow(0px 0px 4px rgba(0, 0, 0, 0.4));
}.darkpagetheme .shadowEffect {
  filter: drop-shadow(0px 0px 4px rgba(245, 245, 245, 0.4));
}
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Kruskal's Algorithm</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_mst_prim.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_theory_graphs_maxflow.php">Next &#10095;</a>
</div>
<hr>

<h2>Kruskal's Algorithm</h2>
<p>Kruskal's algorithm finds the Minimum Spanning Tree (MST), or Minimum Spanning Forest, in an undirected graph.</p>
<div id="vueApp">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" class="dsa_svg">
      <transition-group>
        ...
        <line v-for="edge in posEdge" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="`pos-${edge.from}-${edge.to}`" opacity="1" class="posEdge" />
        <line v-for="edge in mstEdges" 
          ...
          :class="['mstEdges', edge.isMostRecent ? 'thickEdge shadowEffect' : '']"/>
      ...
      <!-- <line v-if="!isConnected" -->
      <line v-if="false" :x1="line.ax" :y1="line.ay" :x2="line.bx" :y2="line.by"  :key="9999" opacity="0.5" class="cutLine" stroke-dasharray="5,5" />
    </svg>
    <br>
    <div class="btnDiv">
      <p><label><input :disabled="inpIsDisabled" v-model="isConnected" type="checkbox"> Connected</label></p>
...
<script>
  ...
  watch: {
    ...
    isConnected(val){
      if(val){ // means that the graph is connected
        this.mstEdgeLength = this.dice.length-1;
        this.line.ax = -100;
        this.line.ay = -100;
        this.line.bx = -100;
        this.line.by = -100;
        ...
        this.mstEdgeLength = this.dice.length-2;
        this.initializeLine();
        this.updateEdgesFromMatrix();
      }
    }
  ...
  methods: {
    ...
    async runKruskalsAlgorithm() {
      await new Promise(resolve => setTimeout(resolve, 500));
      const parent = Array.from({length: this.dice.length}, (_, i) => i);
      this.mstEdges = [];
      ...
      for (const edge of this.edges) {
        ...
        if (find(from) !== find(to)) {
          union(from, to);
          this.mstEdges.push(edge); // Add edge to MST
          this.mstEdges[this.mstEdges.length-1].isMostRecent = true;
          await this.delayMS(500); // Visualize edge addition
          this.mstEdges[this.mstEdges.length-1].isMostRecent = false;
          // Mark the vertices as part of the MST in your visualization
          // Note: This step depends on how you're visualizing vertices
          //this.dice[from].isFinished = true;
          //this.dice[to].isFinished = true;
        }
        else {
          this.posEdge[0] = edge;
          await new Promise(resolve => setTimeout(resolve, 500));
        ...
        if(this.mstEdges.length >= this.mstEdgeLength){
          break;
        }
      }
      this.msgDone = 'Done!';
    ...
    initializeLine() {
      // Example initialization of a line
      // This could be more complex based on how you want to divide the graph
      const centerX = 200; // Assuming the center of your graphing area
      const centerY = 200;
      const angle = Math.random() * 2 * Math.PI; // Random angle

      this.line = {
        ax: centerX + Math.cos(angle) * 300, // Starting point x
        ay: centerY + Math.sin(angle) * 300, // Starting point y
        bx: centerX - Math.cos(angle) * 300, // Ending point x
        by: centerY - Math.sin(angle) * 300, // Ending point y
      };
    },
    intersects(ax, ay, bx, by, cx, cy, dx, dy) {
            ...
            for (let j = i + 1; j < numVertices; j++) { // Avoid duplicating efforts for undirected graph
                ...
                if(!this.isConnected){
                  if (matrix[i][j] !== Infinity) {
                    const x1 = this.dice[i].xpos, y1 = this.dice[i].ypos;
                    const x2 = this.dice[j].xpos, y2 = this.dice[j].ypos;
                    if (this.intersects(this.line.ax, this.line.ay, this.line.bx, this.line.by, x1, y1, x2, y2)) {
                      // If the edge intersects with the line, set it to Infinity
                      matrix[i][j] = Infinity;
                      matrix[j][i] = Infinity;
                    }
                  }
...
<p>The MST (or MSTs) found by Kruskal's algorithm is the collection of edges that connect all vertices (or as many as possible) with the minimum total edge weight.</p>
<p>Kruskal's algorithm adds edges to the MST (or Minimum Spanning Forest), starting with the edges with the lowest edge weights.</p>
<p>Edges that would create a cycle are not added to the MST. These are the red blinking lines in the animation above.</p>
<p>Kruskal's algorithm checks all edges in the graph, but the animation above is made to stop when the MST or Minimum Spanning forest is completed, so that you don't have to wait for the longest edges to be checked.</p>
<div class="ws-note">
  <p><strong>Minimum Spanning Forest</strong> is what it is called when a graph has more than one Minimum Spanning Tree. This happens when a graph is not connected. Try it yourself by using the checkbox in the animation above.</p>
  <p>Unlike Prim's algorithm, Kruskal's algorithm can be used for such graphs that are not connected, which means that it can find more than one MST, and that is what we call a Minimum Spanning Forest.</p>
</div>
<p>To find out if an edge will create a cycle, we will use <a href="dsa_algo_graphs_cycledetection.php#unionfind">Union-Find cycle detection</a> inside Kruskal's algorithm.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Sort the edges in the graph from the lowest to the highest edge weight.</li>
    <li>For each edge, starting with the one with the lowest edge weight: 
      <ol>
        <li type="a">Will this edge create a cycle in the current MST?
          <ul>
            <li>If no: Add the edge as an MST edge.</li>
          </ul>
        </li>
      </ol>
    </li>
...
<p>Let's run through Kruskal's algorithm manually on the graph below, so that we understand the detailed step-by-step operations before we try to program it.</p>
<p>The first three edges are added to the MST. These three edges have the lowest edge weights and do not create any cycles:</p>
<ul>
  <li>C-E, weight 2</li>
  <li>D-E, weight 3</li>
  <li>A-B, weight 4</li>
</ul>
<p>After that, edge C-D (indicated in red) cannot be added as it would lead to a cycle.</p>
<div id="vueApp_still1" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 230" class="dsa_svg">
    <transition-group>
        <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges" />
        <line v-for="edge in posEdge" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="`pos-${edge.from}-${edge.to}`" opacity="1" class="posEdge" />
        <line v-for="edge in mstEdges" 
...
<script>
  const app_still1 = Vue.createApp({
    ...
    return {
      ...
      line: {},
      edges: [],
      adjMatrix: [/*        B        C        D        E        F        G*/
      /*A*/[Infinity,       4,       9,Infinity,Infinity,Infinity,      10],
      /*B*/[       4,Infinity,       8,Infinity,Infinity,Infinity,Infinity],
      /*C*/[       9,       8,Infinity,       5,       2,Infinity,       7],
      /*D*/[Infinity,Infinity,       5,Infinity,       3,       7,Infinity],
      /*E*/[Infinity,Infinity,       2,       3,Infinity,Infinity,       6],
      /*F*/[Infinity,Infinity,Infinity,       7,Infinity,Infinity,      11],
      /*G*/[      10,Infinity,       7,Infinity,       6,      11,Infinity]
      ],
      posEdge: [],
      mstEdges: [],
      keyNumber: 0,
  ...
  methods: {
    ...
    addDie50() {
      const vertices = [
        {"name": "A", "x": 0.5, "y": 4, "offset": [-5,0]},
        {"name": "B", "x": 0.8, "y": 9, "offset": [-5,0]},
        {"name": "C", "x": 8.7, "y": 7.6, "offset": [0,0]},
        {"name": "D", "x": 12.5, "y": 8.9, "offset": [0,0]},
        {"name": "E", "x": 10.5, "y": 6.7, "offset": [21,-20]},
        {"name": "F", "x": 19.1, "y": 6.7, "offset": [0,0]},
        {"name": "G", "x": 9.9, "y": 0.7, "offset": [21,-20]}
      ];
      this.dice = [];
    ...
    updateEdgesFromMatrix() {
        ...
        this.edges.sort((a, b) => a.weight - b.weight);
        this.edges[0].offset = [-2,-5];    // C-E
        this.edges[1].offset = [10,0];   // E-D
        this.edges[2].offset = [10,8];   // A-B
        this.edges[3].offset = [-5,18];   // C-D
        this.edges[4].offset = [10,5];   // E-G
        this.edges[5].offset = [-7,-4];   // C-G
        this.edges[6].offset = [0,18];   // D-F
        this.edges[7].offset = [0,16];  // B-C
        this.edges[8].offset = [3,-5];   // A-C
        this.edges[9].offset = [-2,-7];   // A-G
        this.edges[10].offset = [10,-5];   // G-F
        this.buttonText = "Run Kruskal's";
        this.btnIsDisabled = false;
    },
    adjacencyMatrix() {
...
<p>The next four edges Kruskal's algorithm tries to add to the MST are:</p>
<ul>
  <li>E-G, weight 6</li>
  <li>C-G, weight 7 (not added)</li>
  <li>D-F, weight 7</li>
  <li>B-C, weight 8</li>
</ul>
<p>Edge C-G (indicated in red) cannot be added to the MST because it would create a cycle.</p>
...
<script>
  ...
  methods: {
    ...
    addDie50() {
      ...
      this.mstEdges.push({from: 2, to: 1});
      this.spreadOut();
      this.posEdge[0] = this.edges[5];
    },
    updateEdgesFromMatrix() {
        //this.adjacencyMatrix();
        this.edges = [];
...
<p>As you can see, the MST is already created at this point, but Kruskal's algorithm will continue to run until all edges are tested to see if they can be added to the MST.</p>
<p>The last three edges Kruskal's algorithm tries to add to the MST are the ones with the highest edge weights:</p>
<ul>
  <li>A-C, weight 9 (not added)</li>
  <li>A-G, weight 10 (not added)</li>
  <li>F-G, weight 11 (not added)</li>
</ul>
<p>Each of these edges would create a cycle in the MST, so they cannot be added.</p>
...
<script>
  ...
  methods: {
    ...
    addDie50() {
      ...
      this.mstEdges.push({from: 2, to: 1});
      this.spreadOut();
      this.posEdge[0] = this.edges[8];
      this.posEdge[1] = this.edges[9];
      this.posEdge[2] = this.edges[10];
    },
    updateEdgesFromMatrix() {
        //this.adjacencyMatrix();
        this.edges = [];
...
<p>Kruskal's algorithm is now finished.</p>
<p>Run the simulation below to see Kruskal's algorithm doing the manual steps that we have just done.</p>
<div id="vueApp2">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 230" class="dsa_svg">
<transition-group>
        <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges" />
...
<div class="ws-note">
  <p><strong>Note: </strong>Although Kruskal's algorithm checks all edges in the graph, the animation at the top of this page stops right after the last edge is added to the MST or Minimum Spanning Forest so that we don't have to look at all the red edges that can't be added. </p>
  <p>This is possible because for a connected graph, there is just one MST, and the search can stop when the number of edges in the MST is one less than there are vertices in the graph (\(V-1\)). For the unconnected graph, there are two MSTs in our animation, and the algorithm stops when the MSTs have reached a size of \(V-2\) edges in total.</p>
</div>
<hr>

<a id="kruskalImpl"></a>
<h2>Implementation of Kruskal's Algorithm</h2>
<p>For Kruskal's algorithm to find a Minimum Spanning Tree (MST), or a Minimum Spanning Forest, we create a <code class="w3-codespan">Graph</code> class. We will use the methods inside this <code class="w3-codespan">Graph</code> class later to create the graph from the example above, and to run Kruskal's algorithm on it.</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-line="8,12"><code>class Graph:
...
<p><strong>Line 8 and 12: </strong>Checks if the input arguments <code class="w3-codespan">u</code>, <code class="w3-codespan">v</code>, and <code class="w3-codespan">vertex</code>, are within the possible range of index values.</p>
<p>To do Union-Find cycle detection in Kruskal's algorithm, these two methods <code class="w3-codespan">find</code> and <code class="w3-codespan">union</code> are also defined inside the <code class="w3-codespan">Graph</code> class:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="15" data-line-offset="15" data-line="15-18, 20-29"><code>    def find(self, parent, i):
        if parent[i] == i:
            return i
    ...
    def union(self, parent, rank, x, y):
        ...
        if rank[xroot] < rank[yroot]:
            ...
            parent[yroot] = xroot
            rank[xroot] += 1</code></pre>
</div>
<p><strong>Line 15-18: </strong>The <code class="w3-codespan">find</code> method uses the <code class="w3-codespan">parent</code> array to recursively find the root of a vertex. For each vertex, the <code class="w3-codespan">parent</code> array holds a pointer (index) to the parent of that vertex. The root vertex is found when the <code class="w3-codespan">find</code> method comes to a vertex in the <code class="w3-codespan">parent</code> array that points to itself. Keep reading to see how the <code class="w3-codespan">find</code> method and the <code class="w3-codespan">parent</code> array are used inside the <code class="w3-codespan">kruskals_algorithm</code> method.</p>
<p><strong>Line 20-29: </strong>When an edge is added to the MST, the <code class="w3-codespan">union</code> method uses the <code class="w3-codespan">parent</code> array to merge (union) two trees. The <code class="w3-codespan">rank</code> array holds a rough estimate of the tree height for every root vertex. When merging two trees, the root with a lesser rank becomes a child of the other tree's root vertex.</p>
<p>Here is how Kruskal's algorithm is implemented as a method inside the <code class="w3-codespan">Graph</code> class:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="31" data-line-offset="31" data-line="35, 40-41,44-45, 47-51"><code>    def kruskals_algorithm(self):
        result = []  # MST
        i = 0 # edge counter
        ...
        for u, v, weight in result:
            print(f"{self.vertex_data[u]}-{self.vertex_data[v]} \t{weight}")</code></pre>
</div>
<p><strong>Line 35: </strong>The edges must be sorted before Kruskal's algorithm starts trying to add the edges to the MST.</p>
<p><strong>Line 40-41: </strong>The <code class="w3-codespan">parent</code> and <code class="w3-codespan">rank</code> arrays are initialized. To start with, every vertex is its own root (every element in the <code class="w3-codespan">parent</code> array points to itself), and every vertex has no height (<code class="w3-codespan">0</code> values in the <code class="w3-codespan">rank</code> array).</p>
<p><strong>Line 44-45: </strong>Pick the smallest edge, and increment <code class="w3-codespan">i</code> so that the correct edge is picked in the next iteration.</p>
<p><strong>Line 47-51: </strong>If the vertices <code class="w3-codespan">u</code> and <code class="w3-codespan">v</code> at each end of the current edge have different roots <code class="w3-codespan">x</code> and <code class="w3-codespan">y</code>, it means there will be no cycle for the new edge and the trees are merged. To merge the trees, the current edge is added to the <code class="w3-codespan">result</code> array, and we run the <code class="w3-codespan">union</code> method to make sure the trees are merged correctly, so that there is only one root vertex in the resulting merged tree.</p>

<p>Now let's create the graph from the "Manual Run Through" above and run Kruskal's algorithm on it:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
...
g.kruskals_algorithm()</code></pre>
<a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_mst_kruskal" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>


<h2>Time Complexity for Kruskal's Algorithm</h2>
...
<p>With \(E\) as the number of edges in our graph, the time complexity for Kruskal's algorithm is</p>
<p><span class="math display">\[ O( E \cdot log{E} ) \]</span></p>
<p>We get this time complexity because the edges must be sorted before Kruskal's can start adding edges to the MST. Using a fast algorithm like <a href="dsa_algo_quicksort.php">Quick Sort</a> or <a href="dsa_algo_mergesort.php">Merge Sort</a> gives us a time complexity of \( O( E \cdot log{E} ) \) for this sorting alone.</p>
<p>After the edges are sorted, they are all checked one by one, to see if they will create a cycle, and if not, they are added to the MST.</p>
<p>Although it looks like a lot of work to check if a cycle will be created using the <code class="w3-codespan">find</code> method, and then to include an edge to the MST using the <code class="w3-codespan">union</code> method, this can still be viewed as one operation. The reason we can see this as just one operation is that it takes approximately constant time. That means that the time this operation takes grows very little as the graph grows, and so it does actually not contribute to the overall time complexity.</p>
<p>Since the time complexity for Kruskal's algorithm only varies with the number of edges \(E\), it is especially fast for sparse graphs where the ratio between the number of edges \(E\) and the number of vertices \(V\) is relatively low.</p>
<hr>
