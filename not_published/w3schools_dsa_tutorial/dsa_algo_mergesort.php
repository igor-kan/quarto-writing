...
.v-enter-from, .manSim-enter-from {
  scale: 100% 0%;
  opacity: 0;
}

.v-enter-to, .manSim-enter-to {
  scale: 100% 100%;
  opacity: 1;
}
...
.v-leave-active, .manSim-leave-active {
  position: absolute;
}

.v-leave-from, .manSim-leave-from {
  scale: 100% 100%;
  opacity: 1;
}

.v-leave-to, .manSim-leave-to {
  scale: 100% 0%;
  opacity: 0;
}
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Merge Sort</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_radixsort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_linearsearch.php">Next &#10095;</a>
</div>
<hr>

<h2>Merge Sort</h2>
<p>The Merge Sort algorithm is a divide-and-conquer algorithm that sorts an array by first breaking it down into smaller arrays, and then building the array back together the correct way so that it is sorted.</p>
<div id="vueApp">
  <p>Speed: <input v-model="inpVal" type="range" min="50" max="500"><!-- {{ delay }}--></p>
...
const app = Vue.createApp({
  ...
  methods: {
    ...
    async mergeSort(start, end) {
      ...
      await this.mergeSort(start, mid); // Sort the first half
      await this.mergeSort(mid, end);   // Sort the second half
      
      await this.merge(start, mid, end); // Merge them
    ...
    async merge(start, mid, end){
      ...
      for(let t=start; t<end; t++){
        this.dice[t].top -= 10;
        this.dice[t].rightMargin = false;
        this.dice[t].isCandidate = false;
...
<p><strong>Divide: </strong>The algorithm starts with breaking up the array into smaller and smaller pieces until one such sub-array only consists of one element.</p>
<p><strong>Conquer: </strong>The algorithm merges the small pieces of the array back together by putting the lowest values first, resulting in a sorted array.</p>
<p>The breaking down and building up of the array to sort the array is done recursively.</p>
<p>In the animation above, each time the bars are pushed down represents a recursive call, splitting the array into smaller pieces. When the bars are lifted up, it means that two sub-arrays have been merged together.</p>
<p>The Merge Sort algorithm can be described like this:</p>
<div class="ws-info">
...
<p>Take a look at the drawing below to see how Merge Sort works from a different perspective. As you can see, the array is split into smaller and smaller pieces until it is merged back together. And as the merging happens, values from each sub-array are compared so that the lowest value comes first.</p>
<img src="img_mergesort_long.png" alt="Merge Sort" class="imgTut">
...
<p>Let's try to do the sorting manually, just to get an even better understanding of how Merge Sort works before actually implementing it in a programming language.</p>
<p><strong>Step 1:</strong> We start with an unsorted array, and we know that it splits in half until the sub-arrays only consist of one element. The Merge Sort function calls itself two times, once for each half of the array. That means that the first sub-array will split into the smallest pieces first.</p>
...
<hr>
<p><strong>Step 2:</strong> The splitting of the first sub-array is finished, and now it is time to merge. 8 and 9 are the first two elements to be merged. 8 is the lowest value, so that comes before 9 in the first merged sub-array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 12] [<mark class="dsa_mark"> 8</mark>,<mark class="dsa_mark"> 9</mark>] [ 3, 11, 5, 4]</code>
</div>
<hr>
<p><strong>Step 3:</strong> The next sub-arrays to be merged is [ 12] and [ 8, 9]. Values in both arrays are compared from the start. 8 is lower than 12, so 8 comes first, and 9 is also lower than 12.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 4:</strong> Now the second big sub-array is split recursively.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 5:</strong> 3 and 11 are merged back together in the same order as they are shown because 3 is lower than 11.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 8, 9, 12] [<mark class="dsa_mark"> 3</mark>,<mark class="dsa_mark"> 11</mark>] [ 5, 4]</code><br>
</div>
<hr>
<p><strong>Step 6:</strong> Sub-array with values 5 and 4 is split, then merged so that 4 comes before 5.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 7:</strong> The two sub-arrays on the right are merged. Comparisons are done to create elements in the new merged array:</p>
<ol>
  <li>3 is lower than 4</li>
  <li>4 is lower than 11</li>
  <li>5 is lower than 11</li>
  <li>11 is the last remaining value</li>
</ol>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 8:</strong> The two last remaining sub-arrays are merged. Let's look at how the comparisons are done in more detail to create the new merged and finished sorted array:</p>
<p>3 is lower than 8:</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 9:</strong> 4 is lower than 8:</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 10:</strong> 5 is lower than 8:</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 11:</strong> 8 and 9 are lower than 11:</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 12:</strong> 11 is lower than 12:</p>
<div class="w3-code notranslate">
...
const app2 = Vue.createApp({
  ...
  methods: {
    ...
    async selSort() {
      ...
      // Split nr. 1
      this.dice[6].dieNmbr = '] [';
      this.dice[7].dieNmbr = ' 3';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Split nr. 2
      this.dice[2].dieNmbr = '] [';
      this.dice[3].dieNmbr = ' 8';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Split nr. 3
      tmp = this.dice.splice(4,1)[0];
      tmp.dieNmbr = '] [';
      this.dice[4].dieNmbr = ' 9';
      this.dice.splice(4,0,tmp);
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Merge 8, 9
      this.dice[3].isActive = true;
      this.dice[5].isActive = true;
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      tmp = this.dice.splice(4,1)[0];
      tmp.dieNmbr = ',';
      this.dice.splice(4,0,tmp);
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[3].isActive = false;
      this.dice[5].isActive = false;

      // Move 12 in with [8,9]
      this.dice[1].isActive = true;
      this.dice[3].isActive = true;
      this.dice[5].isActive = true;
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      let swapEl = this.dice.splice(1,1)[0]; // Remove 12
      let swapEl2 = this.dice.splice(1,1)[0]; // Remove '] ['
      swapEl2.dieNmbr = ',';
      this.dice.splice(4,0,swapEl)  // Add 12
      this.dice.splice(4,0,swapEl2)  // Add ','
      this.dice[5].isActive = true;
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[5].isActive = false;
      this.dice[3].isActive = false;
      this.dice[1].isActive = false;
      this.dice[0].dieNmbr = '[';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Split [3,11,5,4]
      this.dice[10].dieNmbr = '] [';
      this.dice[11].dieNmbr = ' 5';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Split [3,11]
      this.dice[8].dieNmbr = '] [';
      this.dice[9].dieNmbr = ' 11';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Merge [3][11]
      this.dice[7].isActive = true;
      this.dice[9].isActive = true;
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      tmp = this.dice.splice(8,1)[0];
      tmp.dieNmbr = ',';
      this.dice.splice(8,0,tmp);
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[7].isActive = false;
      this.dice[9].isActive = false; // 11

      // Split [5,4]
      tmp = this.dice.splice(12,1)[0]; // Ta ut ',' mellom 5 og 4
      tmp.dieNmbr = '] [';
      this.dice[12].dieNmbr = ' 4';
      this.dice.splice(12,0,tmp);
      await new Promise(resolve => setTimeout(resolve, this.delay*2));/**/

      // Merge [5][4]
      this.dice[11].isActive = true; // 5
      this.dice[13].isActive = true; // 4
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      tmp = this.dice.splice(12,1)[0]; // Ta ut '] ['
      swapEl = this.dice.splice(12,1)[0]; // Ta ut '5'
      tmp.dieNmbr = ',';
      this.dice.splice(11,0,swapEl);
      this.dice.splice(12,0,tmp);
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[11].isActive = false;
      this.dice[13].isActive = false; // 11

      // Merge [3,11] with [4,5]
      this.dice[7].isActive = true;  // 3
      this.dice[9].isActive = true;  // 11
      this.dice[11].isActive = true; // 4
      this.dice[13].isActive = true; // 5
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(11,1)[0]; // Ta ut '4'
      swapEl2 = this.dice.splice(11,1)[0]; // Ta ut ','
      this.dice.splice(9,0,swapEl); // Stapp 4 inn etter 3,
      this.dice.splice(10,0,swapEl2); // Stapp , inn etter 4
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(13,1)[0]; // Ta ut '5'
      swapEl2 = this.dice.splice(13,1)[0]; // Ta ut ']'
      swapEl2.dieNmbr = ',';
      this.dice[12].dieNmbr = ']';
      this.dice.splice(11,0,swapEl); // Stapp 5 inn etter 4,
      this.dice.splice(12,0,swapEl2); // Stapp , inn etter 5
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[7].isActive = false;  // 3
      this.dice[9].isActive = false;  // 4
      this.dice[11].isActive = false; // 5
      this.dice[13].isActive = false; // 11
      await new Promise(resolve => setTimeout(resolve, this.delay*2));

      // Merge [8,9,12] with [3,4,5,11]
      this.dice[1].isActive = true;  // 8
      this.dice[7].isActive = true;  // 3
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(7,1)[0]; // Ta ut '3'
      swapEl2 = this.dice.splice(7,1)[0]; // Ta ut ','
      this.dice.splice(1,0,swapEl); // Stapp 3 inn før 8
      this.dice.splice(2,0,swapEl2); // Stapp , inn etter 3
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[1].isActive = false;  // 8
      this.dice[9].isActive = true;  // 4
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(9,1)[0]; // Ta ut '4'
      swapEl2 = this.dice.splice(9,1)[0]; // Ta ut ','
      this.dice.splice(3,0,swapEl); // Stapp 4 inn etter 3,
      this.dice.splice(4,0,swapEl2); // Stapp , inn etter 3
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[3].isActive = false;  // 4
      this.dice[11].isActive = true;  // 5
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(11,1)[0]; // Ta ut '5'
      swapEl2 = this.dice.splice(11,1)[0]; // Ta ut ','
      this.dice.splice(5,0,swapEl); // Stapp 5 inn etter 4,
      this.dice.splice(6,0,swapEl2); // Stapp , inn etter 3
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[5].isActive = false;  // 5
      this.dice[13].isActive = true;  // 11
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[7].isActive = false;  // 8
      this.dice[9].isActive = true;  // 9
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[9].isActive = false;  // 9
      this.dice[11].isActive = true;  // 12
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      swapEl = this.dice.splice(13,1)[0]; // Ta ut '11'
      swapEl2 = this.dice.splice(13,1)[0]; // Ta ut ']'
      swapEl2.dieNmbr = ',';
      this.dice.splice(11,0,swapEl); // Stapp 11 inn før 12
      this.dice.splice(12,0,swapEl2); // Stapp , inn etter 11
      this.dice[14].dieNmbr = ']';
      await new Promise(resolve => setTimeout(resolve, this.delay*2));
      this.dice[11].isActive = false;  // 11
      this.dice[13].isActive = false;  // 12
...
<p>We see that the algorithm has two stages: first splitting, then merging.</p>
<p>Although it is possible to implement the Merge Sort algorithm without recursion, we will use recursion because that is the most common approach.</p>
<p>We cannot see it in the steps above, but to split an array in two, the length of the array is divided by two, and then rounded down to get a value we call "mid". This "mid" value is used as an index for where to split the array.</p>
<p>After the array is split, the sorting function calls itself with each half, so that the array can be split again recursively. The splitting stops when a sub-array only consists of one element.</p>
...
<p>To implement the Merge Sort algorithm we need:</p>
<ol>
  <li>An array with values that needs to be sorted.</li>
  <li>A function that takes an array, splits it in two, and calls itself with each half of that array so that the arrays are split again and again recursively, until a sub-array only consist of one value.</li>
  <li>Another function that merges the sub-arrays back together in a sorted way.</li>
</ol>
<p>The resulting code looks like this:</p>

<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate line-numbers w3-white" data-line="6-7,26-27"><code>def mergeSort(arr):
    if len(arr) <= 1:
        return arr
...
print("Sorted array:", sortedArr)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_mergesort" target="_blank">Try it Yourself &raquo;</a>
</div>
<p><strong>On line 6</strong>, arr[:mid] takes all values from the array up until, but not including, the value on index "mid".</p>
<p><strong>On line 7</strong>, arr[mid:] takes all values from the array, starting at the value on index "mid" and all the next values.</p>
<p><strong>On lines 26-27</strong>, the first part of the merging is done. At this this point the values of the two sub-arrays are compared, and either the left sub-array or the right sub-array is empty, so the result array can just be filled with the remaining values from either the left or the right sub-array. These lines can be swapped, and the result will be the same.</p>
<hr>

<h2>Merge Sort without Recursion</h2>
<p>Since Merge Sort is a divide and conquer algorithm, recursion is the most intuitive code to use for implementation. The recursive implementation of Merge Sort is also perhaps easier to understand, and uses less code lines in general.</p>
<p>But Merge Sort can also be implemented without the use of recursion, so that there is no function calling itself.</p>
<p>Take a look at the Merge Sort implementation below, that does not use recursion:</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white" data-line="22-33"><code>def merge(left, right):
    result = []
    i = j = 0
...
print("Sorted array:", sortedArr)
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_mergesort2" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>You might notice that the merge functions are exactly the same in the two Merge Sort implementations above, but in the implementation right above here the while loop inside the mergeSort function is used to replace the recursion. The while loop does the splitting and merging of the array in place, and that makes the code a bit harder to understand.</p>
<p>To put it simply, the while loop inside the mergeSort function uses short step lengths to sort tiny pieces (sub-arrays) of the initial array using the merge function. Then the step length is increased to merge and sort larger pieces of the array until the whole array is sorted.</p>
<hr>
...
<p class="fontComment">For a more thorough and detailed explanation of Merge Sort time complexity, visit <a href="dsa_timecomplexity_mergesort.php">this page</a>.</p>

<p>The time complexity for Merge Sort is</p>
<p><span class="math display">\[ O( n \cdot \log n ) \]</span></p>
<p>And the time complexity is pretty much the same for different kinds of arrays. The algorithm needs to split the array and merge it back together whether it is already sorted or completely shuffled.</p>
<p>The image below shows the time complexity for Merge Sort.</p>
<img src="img_runtime_mergesort.png" class="imgTut" alt="Time Complexity">

<p>Run the simulation below for different number of values in an array, and see how the number of operations Merge Sort needs on an array of \(n\) elements is \(O(n \log n)\):</p>
<div id="vueApp4">
  <div class="frameDivOuter">
    <div id="leftSpace"></div>
    <div id="inpValues">
      <p><label>Set values: <input type="range" v-model="userX" min="100" max="480" :disabled="isRangeDisabled"></label> {{ this.userX }}</p>
    </div>
  </div>
  <div class="frameDivOuter">
    <div id="inpDiv">
      <p><label><input type="radio" value="single" v-model="selectedRunType" :disabled="isLoading"> Random</label></p>
      <p><label><input type="radio" value="worstCase" v-model="selectedRunType" :disabled="isLoading"> Descending</label></p>
      <p><label><input type="radio" value="bestCase" v-model="selectedRunType" :disabled="isLoading"> Ascending</label></p>
      <p><label><input type="radio" value="random30" v-model="selectedRunType" :disabled="isLoading"> 10 Random</label></p>
    </div>
    <div id="frameDiv2">
      <div id="graphDiv">
...
<script>
  ...
  for (let x = 0; x <= 500; x += 10) {
    curve.push({ x: x, y: 3.5*x * Math.log2(x) });
  }
  let ctx = null;
  let myChart = null;
  ...
  methods: {
    newArray(){
      ...
      this.array.splice(Math.floor(Math.random() * this.array.length), 0, { value: 100, active: false });
      this.runBtn = true;
    },
    presort() {
      this.operations = 0;
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performMergeSort();
    ...
    async performMergeSort() {
      ...
      const merge = async (left, right) => {
        ...
        while (i < left.length && j < right.length) {
          this.operations += 2;  // For two comparisons
          if (this.operations % 20 === 0) {
            // Highlight the active elements (just picking the first from both arrays for now)
            left[0].active = true;
            right[0].active = true;
            await new Promise(resolve => setTimeout(resolve, 1));
            // Remove the highlights
            left[0].active = false;
            right[0].active = false;
          }
      ...
      while (step < length) {
        for (let i = 0; i < length; i += 2 * step) {
          let left = this.array.slice(i, i + step);
          let right = this.array.slice(i + step, i + 2 * step);
          
          let merged = await merge(left, right);
          
          for (let j = 0; j < merged.length; j++) {
            this.array[i + j] = merged[j];
            this.operations++;  // For each merge operation
          }
        }
        step *= 2;  // Double the sub-array size for the next iteration
      }
...
<p>If we hold the number of values \(n\) fixed, the number of operations needed for the "Random", "Descending" and "Ascending" is almost the same.</p>
<p>Merge Sort performs almost the same every time because the array is split, and merged using comparison, both if the array is already sorted or not.</p>
<hr>
