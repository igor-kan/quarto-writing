...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Hash Tables</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_data_queues.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_data_hashsets.php">Next &#10095;</a>
</div>
<hr>
...
<p>In a <a href="dsa_theory_linkedlists.php">Linked List</a>, finding a person "Bob" takes time because we would have to go from one node to the next, checking each node, until the node with "Bob" is found.</p>
<p>And finding "Bob" in an <a href="dsa_data_arrays.php">Array</a> could be fast if we knew the index, but when we only know the name "Bob", we need to compare each element (like with Linked Lists), and that takes time.</p>
...
<p>We will build the Hash Set in 5 steps:</p>
<ol>
...
<h3>Step 1: Starting with an array</h3>
<p>Using an array, we could store names like this:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_array = ['Pete', 'Jones', 'Lisa', 'Bob', 'Siri']</code></pre>
</div>
<p>To find "Bob" in this array, we need to compare each name, element by element, until we find "Bob".</p>
<p>If the array was sorted alphabetically, we could use Binary Search to find a name quickly, but inserting or deleting names in the array would mean a big operation of shifting elements in memory.</p>
<p>To make interacting with the list of names really fast, let's use a Hash Table for this instead, or a Hash Set, which is a simplified version of a Hash Table.</p>
<p>To keep it simple, let's assume there is at most 10 names in the list, so the array must be a fixed size of 10 elements. When talking about Hash Tables, each of these elements is called a <strong>bucket</strong>.</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_hash_set = [None,None,None,None,None,None,None,None,None,None]</code></pre>
</div>
<hr>

<h3>Step 2: Storing names using a hash function</h3>
<p>Now comes the special way we interact with the Hash Set we are making.</p>
...
<p>A hash function can be made in many ways, it is up to the creator of the Hash Table. A common way is to find a way to convert the value into a number that equals one of the Hash Set's index numbers, in this case a number from 0 to 9. In our example we will use the Unicode number of each character, summarize them and do a modulo 10 operation to get index numbers 0-9.</p>
<div class="w3-example">
  <h3>Example</h3>
<pre class="language-python notranslate w3-white" data-line=""><code>def hash_function(value):
    sum_of_chars = 0
    for char in value:
...
print("'Bob' has hash code:",hash_function('Bob'))</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_hashset_0" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>The character "B" has Unicode code point 66, "o" has 111, and "b" has 98. Adding those together we get 275. Modulo 10 of 275 is 5, so "Bob" should be stored as an array element at index 5.</p>
<p>The number returned by the hash function is called the <strong>hash code</strong>.</p>
<div class="ws-note">
  <p><strong>Unicode number: </strong>Everything in our computers are stored as numbers, and the Unicode code point is a unique number that exist for every character. For example, the character <code class="w3-codespan">A</code> has Unicode number (also called Unicode code point) <code class="w3-codespan">65</code>. Just try it in the simulation below. See <a href="/charsets/ref_utf_basic_latin.asp">this page</a> for more information about how characters are represented as numbers.</p>
...
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_hash_set = [None,None,None,None,None,'Bob',None,None,None,None]</code></pre>
</div>
<p>We can use the hash function to find out where to store the other names "Pete", "Jones", "Lisa", and "Siri" as well.</p>
<p>After using the hash function to store those names in the correct position, our array looks like this:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_hash_set = [None,'Jones',None,'Lisa',None,'Bob',None,'Siri','Pete',None]</code></pre>
</div>
...
<h3>Step 3: Looking up a name using a hash function</h3>
<p>We have now established a super basic Hash Set, because we do not have to check the array element by element anymore to find out if "Pete" is in there, we can just use the hash function to go straight to the right element!</p>
<p>To find out if "Pete" is stored in the array, we give the name "Pete" to our hash function, we get back hash code 8, we go directly to the element at index 8, and there he is. We found "Pete" without checking any other elements.</p>
<div class="w3-example">
  <h3>Example</h3>
<pre class="language-python notranslate w3-white" data-line=""><code>my_hash_set = [None,'Jones',None,'Lisa',None,'Bob',None,'Siri','Pete',None]
...
print("'Pete' is in the Hash Set:",contains('Pete'))</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_hashset_1" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>When deleting a name from our Hash Set, we can also use the hash function to go straight to where the name is, and set that element value to <code class="w3-codespan">None</code>.</p>
<hr>

<h3>Step 4: Handling collisions</h3>
<p>Let's also add "Stuart" to our Hash Set.</p>
<p>We give "Stuart" to our hash function, and we get the hash code 3, meaning "Stuart" should be stored at index 3.</p>
<p>Trying to store "Stuart" creates what is called a <strong>collision</strong>, because "Lisa" is already stored at index 3.</p>
...
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_hash_set = [
    [None],
    ['Jones'],
    [None],
    ['Lisa', 'Stuart'],
...
<p>Searching for "Stuart" in our Hash Set now means that using the hash function we end up directly in bucket 3, but then be must first check "Lisa" in that bucket, before we find "Stuart" as the second element in bucket 3.</p>
<hr>

<h3>Step 5: Hash Set code example and simulation</h3>
<p>To complete our very basic Hash Set code, let's have functions for adding and searching for names in the Hash Set, which is now a two dimensional array.</p>
<p>Run the code example below, and try it with different values to get a better understanding of how a Hash Set works.</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white line-numbers"><code>my_hash_set = [
    [None],
    ['Jones'],
    [None],
    ['Lisa'],
...
print(my_hash_set)
print('Contains Stuart:',contains('Stuart'))</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_hashset_2" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>The next two pages show better and more detailed implementations of Hast Sets and Hash Tables.</p>
<p>Try the Hash Set simulation below to get a better idea of how a Hash Set works in principle.</p>
<div id="vueApp">
  <!-- Buckets animated independently to make animation of removing names work properly -->
  <div id="hashset">
    <p style="font-weight: bold;">Hash Set</p>
    <div>
      ...
      <transition-group name="list-0">
        <div v-for="el in hashSet[0]" :key="el.name" :class="{'name': true, 'activeEl': el.active }">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-1">
        <div v-for="el in hashSet[1]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-2">
        <div v-for="el in hashSet[2]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-3">
        <div v-for="el in hashSet[3]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-4">
        <div v-for="el in hashSet[4]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-5">
        <div v-for="el in hashSet[5]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-6">
        <div v-for="el in hashSet[6]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-7">
        <div v-for="el in hashSet[7]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-8">
        <div v-for="el in hashSet[8]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
    <div>
      ...
      <transition-group name="list-9">
        <div v-for="el in hashSet[9]" :key="el.name" :class="{'name': true, 'activeEl': el.active}">
          {{ el.name }}
        </div>
      </transition-group>
    </div>
  ...
  <input type="text" size="15" v-model="inputVal" ref="inputRef" :disabled="disabledInpBtns" placeholder="Write a name..."><br>
  <button @click="contains()" :disabled="inpIsEmpty">contains()</button>
  <button @click="add()" :disabled="inpIsEmpty">add()</button>
  <button @click="remove()" :disabled="inpIsEmpty">remove()</button>
  <button @click="hashSetSize()" :disabled="disabledInpBtns">size()</button>
</div>
<script>
const app = Vue.createApp({
  data() {
    return {
      disabledInpBtns: false,
      inputVal: '',
      sumOfAscii: 0,
      hashSet: Array.from({ length: 10 }, () => []), // Initialize a Hash Set with 10 buckets
      resultText: 'Try interacting with the Hash Set'
    }
  },
  watch: {
    inputVal(newVal) {
  ...
  computed: {
    inpIsEmpty(){
      return this.disabledInpBtns || this.inputVal==='';
    },
    currHashCode() {
      // Recompute hash code whenever inputVal changes
      let hashCode = this.inputVal ? this.hashFunction(this.inputVal) : 0;
      let bucketDivs = document.querySelectorAll('#hashset > div');
      this.sumOfAscii = 0;
      bucketDivs.forEach(div => {
        div.classList.remove('currentBucket');
      });
      if(this.inputVal !== ''){
        bucketDivs[hashCode].classList.add('currentBucket');
        this.sumOfAscii = Array.from(this.inputVal).reduce((acc, char) => acc + char.charCodeAt(0), 0);
      }
      return hashCode;
    }
  },
  methods: {
    ...
    hashFunction(value) {
      ...
      return Array.from(value).reduce((acc, char) => acc + char.charCodeAt(0), 0) % this.hashSet.length;
    },
    async add(value = this.inputVal) {
      if(value !== '') {
        this.resultText = "";
        const hashCode = this.currHashCode; // Use the computed property
        const bucket = this.hashSet[hashCode];
        // Check if the bucket already contains an object with the name property equal to value
        const isContained = bucket.some(nameObj => nameObj.name === value);
        if (!isContained) {
          // Push an object with the name and initially set active to false
          bucket.push({name: value, active: true});
          this.resultText = "Added: ";
          this.disabledInpBtns = true;
          await new Promise(resolve => setTimeout(resolve, 500));
          this.disabledInpBtns = false;
          this.resultText += value;
          this.inputVal = '';
        } else {
          this.resultText = "Already in the Hash Set: ";
          this.hashSet.forEach(bucket => {
            bucket.forEach(nameObj => {
              if(nameObj.name === this.inputVal){
                nameObj.active = false;
              }
            });
          });
    ...
    async remove() {
      if(this.inputVal !== ''){
        ...
        const index = bucket.findIndex(nameObj => nameObj.name === this.inputVal);
        this.resultText = '';
        if (index > -1) {
          bucket.splice(index, 1); // Remove the object at the found index
          this.resultText = "Removed: ";
          this.disabledInpBtns = true;
          await new Promise(resolve => setTimeout(resolve, 500));
          this.disabledInpBtns = false;
          this.resultText += this.inputVal;
          this.inputVal = '';
        } else {
    ...
    async contains() {
      if(this.inputVal !== ''){
        this.resultText = "Contains "+this.inputVal+": ";
        const hashCode = this.currHashCode; // Use the computed property
        const bucket = this.hashSet[hashCode];
        // Check if any object in the bucket has a name property matching the inputVal
        const contains = bucket.some(nameObj => nameObj.name === this.inputVal);
        if(contains){
          this.hashSet.forEach(bucket => {
            bucket.forEach(nameObj => {
              if(nameObj.name === this.inputVal){
        ...
        this.resultText += `${contains ? 'True' : 'False'}`;
        //this.inputVal = '';
      }
      else {
        this.$refs.inputRef.focus();
    ...
    addElements() {
      this.hashSet = [
        [{ name: 'Thomas', active: false },{ name: 'Jens', active: false }],
        [],
        [{ name: 'Peter', active: false }],
        [{ name: 'Lisa', active: false }],
        [{ name: 'Charlotte', active: false }],
        [{ name: 'Adele', active: false },{ name: 'Bob', active: false }],
        [],
        [],
        [{ name: 'Michaela', active: false }],
        []
      ];
    },
    createObserver() {
...
<p>The most important reason why Hash Tables are great for these things is that Hash Tables are very fast compared Arrays and Linked Lists, especially for large sets. Arrays and Linked Lists have time complexity \(O(n)\) for search and delete, while Hash Tables have just \(O(1)\) on average! Read more about time complexity <a href="dsa_timecomplexity_theory.php">here</a>.</p>
<hr>

<h2>Hash Set vs. Hash Map</h2>
<p>A Hash Table can be a Hash Set or a Hash Map. The next two pages describe these data structures in more detail.</p>
<p>Here's how Hash Sets and Hash Maps are different and similar:</p>
...
<p>Every Hash Table element has a part that is unique that is called the <strong>key</strong>.</p>
<p>A <strong>hash function</strong> takes the key of an element to generate a <strong>hash code</strong>.</p>
<p>The hash code says what bucket the element belongs to, so now we can go directly to that Hash Table element: to modify it, or to delete it, or just to check if it exists. Specific hash functions are explained in detail on the next two pages.</p>
<p>A <strong>collision</strong> happens when two Hash Table elements have the same hash code, because that means they belong to the same <strong>bucket</strong>. A collision can be solved in two ways.</p>
<p><strong>Chaining</strong> is the way collisions are solved in this tutorial, by using arrays or linked lists to allow more than one element in the same bucket.</p>
<p><strong>Open Addressing</strong> is another way to solve collisions. With open addressing, if we want to store an element but there is already an element in that bucket, the element is stored in the next available bucket. This can be done in many different ways, but we will not explain open addressing any further here.</p>
<hr>

<h2>Conclusion</h2>
<p>Hash Tables are powerful tools in programming, helping you to manage and access data efficiently.</p>
<p>Whether you use a Hash Set or a Hash Map depends on what you need: just to know if something is there, or to find detailed information about it.</p>
<hr>
