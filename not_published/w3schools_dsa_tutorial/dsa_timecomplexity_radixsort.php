...
<html lang="en-US">
<head>
<title>DSA Radix Sort Time Complexity</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Radix Sort Time Complexity</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_timecomplexity_countsort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_timecomplexity_mergesort.php">Next &#10095;</a>
</div>
<hr>
...
<p>The <a href="dsa_algo_radixsort.php">Radix Sort</a> algorithm sorts non negative integers, one digit at a time.</p>
<p>There are \(n\) values that need to be sorted, and \(k\) is the number of digits in the highest value.</p>
<p>When Radix Sort runs, every value is moved to the radix array, and then every value is moved back into the initial array. So \(n\) values are moved into the radix array, and \(n\) values are moved back. This gives us \(n + n=2 \cdot n\) operations.</p>
<p>And the moving of values as described above needs to be done for every digit. This gives us a total of \(2 \cdot n \cdot k\) operations.</p>
<p>This gives us time complexity for Radix Sort:</p>
<p><span class="math display">\[
O(2 \cdot n \cdot k) = \underline{\underline{O(n \cdot k)}} 
\]</span></p>
<p>Radix Sort is perhaps the fastest sorting algorithms there is, as long as the number of digits \(k\) is kept relatively small compared to the number of values \(n\).</p>
<p>We can imagine a scenario where the number of digits \(k\) is the same as the number of values \(n\), in such a case we get time complexity \(O(n \cdot k)=O(n^2)\) which is quite slow, and has the same time complexity as for example Bubble Sort.</p>
<p>We can also image a scenario where the number of digits \(k\) grow as the number of values \(n\) grow, so that \(k(n)= \log n\). In such a scenario we get time complexity \(O(n \cdot k)=O(n \cdot \log n )\), which is the same as for example Quicksort.</p>
<p>See the time complexity for Radix Sort in the image below.</p>
<img src="img_runtime_radixsort.png" class="imgTut" alt="Time Complexity" style="max-width:100%">
<hr>

<h2>Radix Sort Simulation</h2>
...
<div id="vueApp4">
  <div class="frameDivOuter">
    ...
    <div id="inpValues">
      ...
      <p><label>Digits (k): <input type="range" v-model="userK" min="2" max="7" step="1" :disabled="isLoading"></label> {{ this.userK }}</p>
    </div>
  </div>
  <div class="frameDivOuter">
    <div id="inpDiv">
