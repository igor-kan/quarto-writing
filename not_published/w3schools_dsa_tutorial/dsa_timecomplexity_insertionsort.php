...
<html lang="en-US">
<head>
<title>DSA Insertion Sort Time Complexity</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Insertion Sort Time Complexity</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_timecomplexity_selsort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_timecomplexity_quicksort.php">Next &#10095;</a>
</div>
<hr>
...
<p>The worst case scenario for <a href="dsa_algo_insertionsort.php">Insertion Sort</a> is if the array is already sorted, but with the highest values first. That is because in such a scenario, every new value must "move through" the whole sorted part of the array.</p>
<p>These are the operations that are done by the Insertion Sort algorithm for the first elements:</p>
<ul>
  <li>The 1st value is already in the correct position.</li>
  <li>The 2nd value must be compared and moved past the 1st value.</li>
  <li>The 3rd value must be compared and moved past two values.</li>
  <li>The 3rd value must be compared and moved past three values.</li>
  <li>And so on..</li>
</ul>
<p>If we continue this pattern, we get the total number of operations for \(n\) values:</p>
<p><span class="math display">\[1+2+3+...+(n-1)\]</span></p>
<p>This is a well known series in mathematics that can be written like this:</p>
<p><span class="math display">\[ \frac{n(n-1)}{2} = \frac{n^2}{2} - \frac{n}{2} \]</span></p>
<p>For very large \(n\), the \(\frac{n^2}{2}\) term dominates, so we can simplify by removing the second term \(\frac{n}{2}\).</p>
<p>Using Big O notation, we get this time complexity for the Insertion Sort algorithm:</p>
<p><span class="math display">\[ O(\frac{n^2}{2}) = O(\frac{1}{2} \cdot n^2) = \underline{\underline{O(n^2)}} \]</span></p>
<p>The time complexity can be displayed like this:</p>
<img src="img_runtime_bubbleSort.png" alt="Time Complexity for Insertion Sort">
<p>As you can see, the time used by Insertion Sort increases fast when the number of values is \(n\) increased.</p>
<hr>

<h2>Insertion Sort Simulation</h2>
...
<p>The red line above represents the theoretical upper bound time complexity \(O(n^2)\), and the actual function in this case is \(1.07 \cdot n^2\).</p>
<p>Remember that a function \(f(n)\) is said to be \(O(g(n))\) if we have a positive constant \(C\) so that \(C \cdot g(n)>f(n)\).</p>
<p>In this case \(f(n)\) is the number of operations used by Insertion Sort, \(g(n)=n^2\) and \(C=1.07\).</p>
