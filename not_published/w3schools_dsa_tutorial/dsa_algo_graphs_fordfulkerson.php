...
<html lang="en-US">
<head>
<title>DSA Ford-Fulkerson Algorithm</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Ford-Fulkerson Algorithm</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_theory_graphs_maxflow.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_graphs_edmondskarp.php">Next &#10095;</a>
</div>

<div class="ws-info">
  <p>The Ford-Fulkerson algorithm solves the maximum flow problem.</p>
...
<p>The Ford-Fulkerson algorithm solves <a href="dsa_theory_graphs_maxflow.php">the maximum flow problem</a> for a directed graph.</p>
...
const app2 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
/*0*/   { from: 0, to: 1, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
...
/*5*/   { from: 1, to: 4, capacity: 4, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
...
/*10*/  { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
...
/*15*/  { from: 4, to: 5, capacity: 6, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
  ...
  methods: {
    async action(){
      if(this.btnText === 'Run Ford-Fulkerson'){
        this.btnIsDisabled = true;
        this.runFordFulkerson();
      }
      else {
        this.btnIsDisabled = true;
        this.statusText = '';
        ...
        this.btnText = 'Run Ford-Fulkerson';
    ...
    calculateEdgeCenters() {
      ...
      this.edges[1].centerY += 12;
      this.edges[4].centerX -= 10;
      this.edges[4].centerY -= 8;
      this.edges[5].centerX += 10;
      this.edges[7].centerX += 10;
      this.edges[8].centerX -= 10;
      this.edges[8].centerY -= 8;
      this.edges[10].centerX += 10;
      this.edges[11].centerX += 10;
      this.edges[15].centerX += 10;
      this.edges[15].centerY += 10;
    },
    ...
    findPath(sourceKey, sinkKey, visited = new Set()) {
        ...
        for (let edge of outgoingEdges) {
            const resultPath = this.findPath(edge.to, sinkKey, visited);
            if (resultPath) {
                return [sourceKey, ...resultPath];
            }
        }
    ...
    async updateFlowAndResidual(path, additionalFlow) {
        //console.log("Augmented path: ["+path+"], flow: "+additionalFlow);
        for (let i = 0; i < path.length - 1; i++) {
            const fromKey = path[i];
            const toKey = path[i + 1];
...
<p>The Ford-Fulkerson algorithm works by looking for a path with available capacity from the source to the sink (called an <i>augmented path</i>), and then sends as much flow as possible through that path.</p>
<p>The Ford-Fulkerson algorithm continues to find new paths to send more flow through until the maximum flow is reached.</p>
<p>In the simulation above, the Ford-Fulkerson algorithm solves the maximum flow problem: It finds out how much flow can be sent from the source vertex \(s\), to the sink vertex \(t\), and that maximum flow is 8.</p>
...
<div class="ws-note">
  <p><strong>Note: </strong>The Ford-Fulkerson algorithm is often described as a <i>method</i> instead of as an <i>algorithm</i>, because it does not specify how to find a path where flow can be increased. This means it can be implemented in different ways, resulting in different time complexities. But for this tutorial we will call it an algorithm, and use Depth-First-Search to find the paths.</p>
</div>
<p>You can see the basic step-by-step description of how the Ford-Fulkerson algorithm works below, but we need to go into more detail later to actually understand it.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Start with zero flow on all edges.</li>
    <li>Find an <i>augmented path</i> where more flow can be sent.</li>
...
<h2>Residual Network in Ford-Fulkerson</h2>
<p>The Ford-Fulkerson algorithm actually works by creating and using something called a <i>residual network</i>, which is a representation of the original graph.</p>
...
<h2>Reversed Edges in Ford-Fulkerson</h2>
<p>The Ford-Fulkerson algorithm also uses something called <i>reversed edges</i> to send flow back. This is useful to increase the total flow.</p>
<p>For example, the last augmented path \(s \rightarrow v_2 \rightarrow v_4 \rightarrow v_3 \rightarrow t\) in the animation above and in the manual run through below shows how the total flow is increased by one more unit, by actually sending flow back on edge \( v_4 \rightarrow v_3 \), sending the flow in the reverse direction.</p>
<p>Sending flow back in the reverse direction on edge \( v_3 \rightarrow v_4 \) in our example meas that this 1 unit of flow going out of vertex \( v_3 \), now leaves \( v_3 \) on edge \( v_3 \rightarrow t \) instead of \( v_3 \rightarrow v_4 \).</p>
...
<p>A reversed edge has no flow or capacity, just residual capacity. The residual capacity for a reversed edge is always the same as the flow in the corresponding original edge. In our example, the edge \( v_3 \rightarrow v_4 \) has a flow of 2, which means there is a residual capacity of 2 on the corresponding reversed edge \( v_4 \rightarrow v_3 \).</p>
<p>This just means that when there is a flow of 2 on the original edge \( v_3 \rightarrow v_4 \), there is a possibility of sending that same amount of flow back on that edge, but in the reversed direction. Using a reversed edge to push back flow can also be seen as undoing a part of the flow that is already created.</p>
<p>The idea of a residual network with residual capacity on edges, and the idea of reversed edges, are central to how the Ford-Fulkerson algorithm works, and we will go into more detail about this when we implement the algorithm further down on this page.</p>
<hr>

<h2>Manual Run Through</h2>
<p>There is no flow in the graph to start with.</p>
<p>To find the maximum flow, the Ford-Fulkerson algorithm must increase flow, but first it needs to find out where the flow can be increased: it must find an augmented path.</p>
<p>The Ford-Fulkerson algorithm actually does not specify how such an augmented path is found (that is why it is often described as a method instead of an algorithm), but we will use <a href="dsa_algo_graphs_traversal.php#dfsTraversal">Depth First Search (DFS)</a> to find the augmented paths for the Ford-Fulkerson algorithm in this tutorial.</p>
<p>The first augmented path Ford-Fulkerson finds using DFS is \(s \rightarrow v_1 \rightarrow v_3 \rightarrow v_4 \rightarrow t\).</p>
<p>And using the bottleneck calculation, Ford-Fulkerson finds that 3 is the highest flow that can be sent through the augmented path, so the flow is increased by 3 for all the edges in this path.</p>
...
const app3 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
/*0*/   { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 3, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
/*5*/   { from: 1, to: 4, capacity: 4, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*10*/  { from: 3, to: 4, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 0, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*15*/  { from: 4, to: 5, capacity: 6, flow: 3, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 4, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false }
      ]
    }
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,1,3,4,5];
  }
});
app3.mount('#vueApp3');
</script>

<p>The next iteration of the Ford-Fulkerson algorithm is to do these steps again:</p>
<ol start="2">
  <li>Find a new augmented path</li>
  <li>Find how much the flow in that path can be increased</li>
  <li>Increase the flow along the edges in that path accordingly</li>
</ol>
<p>The next augmented path is found to be \(s \rightarrow v_2 \rightarrow v_1 \rightarrow v_4 \rightarrow v_3 \rightarrow t\), which includes the reversed edge <span style="color: #c11515;">\(v_4 \rightarrow v_3\)</span>, where flow is sent back.</p>
<p>The Ford-Fulkerson concept of reversed edges comes in handy because it allows the path finding part of the algorithm to find an augmented path where reversed edges can also be included.</p>
<p>In this specific case that means that a flow of 2 can be sent back on edge \(v_3 \rightarrow v_4\), going into \(v_3 \rightarrow t\) instead.</p>
<p>The flow can only be increased by 2 in this path because that is the capacity in the \( v_3 \rightarrow t \) edge.</p>
...
const app4 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
/*0*/   { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 2, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 3, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
/*5*/   { from: 1, to: 4, capacity: 4, flow: 2, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 2, to: 1, capacity: 5, flow: 2, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*10*/  { from: 3, to: 4, capacity: 3, flow: 1, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
...
/*15*/  { from: 4, to: 5, capacity: 6, flow: 3, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,2,1,4,3,5];
  }
});
app4.mount('#vueApp4');
</script>

<p>The next augmented path is found to be \(s \rightarrow v_2 \rightarrow v_1 \rightarrow v_4 \rightarrow t\).</p>
<p>The flow can be increased by 2 in this path. The bottleneck (limiting edge) is \( v_1 \rightarrow v_4 \) because there is only room for sending two more units of flow in that edge.</p>
...
const app5 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
/*0*/   { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 4, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 3, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
/*5*/   { from: 1, to: 4, capacity: 4, flow: 4, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 2, to: 1, capacity: 5, flow: 4, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*10*/  { from: 3, to: 4, capacity: 3, flow: 1, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        ...
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*15*/  { from: 4, to: 5, capacity: 6, flow: 5, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 4, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false }
      ]
    }
...
<p>The next and last augmented path is \(s \rightarrow v_2 \rightarrow v_4 \rightarrow t\).</p>
<p>The flow can only be increased by 1 in this path because of edge \( v_4 \rightarrow t \) being the bottleneck in this path with only space for one more unit of flow (\(capacity-flow=1\)).</p>
...
/*5*/   { from: 1, to: 4, capacity: 4, flow: 4, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 2, to: 1, capacity: 5, flow: 4, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 1, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*10*/  { from: 3, to: 4, capacity: 3, flow: 1, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*15*/  { from: 4, to: 5, capacity: 6, flow: 6, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 4, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false }
      ]
    }
...
<p>At this point, a new augmenting path cannot be found (it is not possible to find a path where more flow can be sent through from \(s\) to \(t\)), which means the max flow has been found, and the Ford-Fulkerson algorithm is finished.</p>
...
<h2>Implementation of The Ford-Fulkerson Algorithm</h2>
<p>To implement the Ford-Fulkerson algorithm, we create a <code class="w3-codespan">Graph</code> class. The <code class="w3-codespan">Graph</code> represents the graph with its vertices and edges:</p>
...
<p>The <code class="w3-codespan">Graph</code> class also contains the <code class="w3-codespan">dfs</code> method to find augmented paths, using Depth-First-Search:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="14" data-line-offset="14" data-line="15-18, 20-21, 23-24, 26-30, 32"><code>    def dfs(self, s, t, visited=None, path=None):
        if visited is None:
...
<p><strong>Line 15-18: </strong>The <code class="w3-codespan">visited</code> array helps to avoid revisiting the same vertices during the search for an augmented path. Vertices that belong to the augmented path are stored in the <code class="w3-codespan">path</code> array.</p>
<p><strong>Line 20-21: </strong>The current vertex is marked as visited, and then added to the path.</p>
<p><strong>Line 23-24: </strong>If the current vertex is the sink node, we have found an augmented path from the source vertex to the sink vertex, so that path can be returned.</p>
<p><strong>Line 26-30: </strong>Looping through all edges in the adjacency matrix starting from the current vertex <code class="w3-codespan">s</code>, <code class="w3-codespan">ind</code> represents an adjacent node, and <code class="w3-codespan">val</code> is the residual capacity on the edge to that vertex. If the adjacent vertex is not visited, and has residual capacity on the edge to it, go to that node and continue searching for a path from that vertex.</p>
<p><strong>Line 32: </strong><code class="w3-codespan">None</code> is returned if no path is found.</p>

<p>The <code class="w3-codespan">fordFulkerson</code> method is the last method we add to the <code class="w3-codespan">Graph</code> class:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="34" data-line-offset="34" data-line="37,39-42,44-47,49,51-52"><code>    def fordFulkerson(self, source, sink):
        max_flow = 0
...
<p>Initially, the <code class="w3-codespan">max_flow</code> is <code class="w3-codespan">0</code>, and the <code class="w3-codespan">while</code> loop keeps increasing the <code class="w3-codespan">max_flow</code> as long as there is an augmented path to increase flow in.</p>
<p><strong>Line 37: </strong>The augmented path is found.</p>
<p><strong>Line 39-42: </strong>Every edge in the augmented path is checked to find out how much flow can be sent through that path.</p>
<p><strong>Line 44-46: </strong>The residual capacity (capacity minus flow) for every forward edge is reduced as a result of increased flow.</p>
<p><strong>Line 47: </strong>This represents the reversed edge, used by the Ford-Fulkerson algorithm so that flow can be sent back (undone) on the the original forward edges. It is important to understand that these reversed edges are not in the original graph, they are fictitious edges introduced by Ford-Fulkerson to make the algorithm work.</p>
<p><strong>Line 49: </strong>Every time flow is increased over an augmented path, <code class="w3-codespan">max_flow</code> is increased by the same value.</p>
<p><strong>Line 51-52: </strong>This is just for printing the augmented path before the algorithm starts the next iteration.</p>

<p>After defining the <code class="w3-codespan">Graph</code> class, the vertices and edges must be defined to initialize the specific graph, and the complete code for the Ford-Fulkerson algorithm example looks like this:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
...
print("The maximum possible flow is %d " % g.fordFulkerson(source, sink))</code></pre>
<a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_fordfulkerson" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Time Complexity for The Ford-Fulkerson Algorithm</h2>
<p>The time complexity for the Ford-Fulkerson varies with the number of vertices \(V\), the number of edges \(E\), and it actually varies with the maximum flow \(f\) in the graph as well.</p>
<p>The reason why the time complexity varies with the maximum flow \(f\) in the graph, is because in a graph with a high throughput, there will be more augmented paths that increase flow, and that means the DFS method that finds these augmented paths will have to run more times.</p>
<p>Depth-first search (DFS) has time complexity \(O(V+E)\).</p>
<p>DFS runs once for every new augmented path. If we assume that each augmented graph increase flow by 1 unit, DFS must run \(f\) times, as many times as the value of maximum flow.</p>
<p>This means that time complexity for the Ford-Fulkerson algorithm, using DFS, is</p>
<p><span class="math display">\[ O( (V+E) \cdot f ) \]</span></p>
<p>For <i>dense graphs</i>, where \( E > V \), time complexity for DFS can be simplified to \(O(E)\), which means that the time complexity for the Ford-Fulkerson algorithm also can be simplified to</p>
<p><span class="math display">\[ O( E \cdot f ) \]</span></p>
<p>A dense graph does not have an accurate definition, but it is a graph with many edges.</p>
<p>The next algorithm we will describe that finds maximum flow is <a href="dsa_algo_graphs_edmondskarp.php">the Edmonds-Karp algorithm</a>.</p>
<p>The Edmonds-Karp algorithm is very similar to Ford-Fulkerson, but it uses BFS instead of DFS to find augmented paths, which leads to fewer iterations to find maximum flow.</p>
<hr>

<br>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_graphs_maxflow.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_graphs_edmondskarp.php">Next &#10095;</a>
</div>
<div
