...
ny:-->
    ...
    <h1><span class="color_h1">Memoization</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_ref_knapsack.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_ref_tabulation.php">Next &#10095;</a>
</div>
<hr>

<h2>Memoization</h2>
<p>Memoization is a technique where results are stored to avoid doing the same computations many times.</p>
<p>When Memoization is used to improve recursive algorithms, it is called a "top-down" approach because of how it starts with the main problem and breaks it down into smaller subproblems.</p>
<p>Memoization is used in <a href="dsa_ref_dynamic_programming.php">Dynamic Programming</a>.</p>
<hr>

<a id="fibmemo"></a>
<h2>Using Memoization To Find The \(n\)th Fibonacci Number</h2>
<p>The \(n\)th Fibonacci number can be found using recursion. Read more about how that is done on <a href="dsa_algo_simple.php#nthfibo">this page</a>.</p>
<p>The problem with this implementation is that the number of computations and recursive calls "explodes" when trying to find a higher Fibonacci number, because the same computations are done over and over again.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Find the 6th Fibonacci number with recursion:</p>
...
print('F(6) = ',F(6))</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_findfibo_memo" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>As you can see from running the example above, there are 25 computations, with the same computations done many times, even for just finding the 6th Fibonacci number.</p>
<p>But using memoization can help finding the \(n\)th Fibonacci number using recursion much more effectively.</p>
<p>We use memoization by creating an array <code class="w3-codespan">memo</code> to hold the Fibonacci numbers, so that Fibonacci number <code class="w3-codespan">n</code> can be found as element <code class="w3-codespan">memo[n]</code>. And we only compute the Fibonacci number if it does not already exist in the <code class="w3-codespan">memo</code> array.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Find the 6th Fibonacci number with recursion, but using memoization to avoid unnecessary recursive calls:</p>
  <pre class="language-python notranslate w3-white"><code>def F(n):
...
memo = [None]*7
print('F(6) = ',F(6))
print('memo = ',memo)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_findfibo_memo2" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>As you can see by running the examples above, memoization is very helpful to reduce the number of computations. </p>
<p>The number of computations is reduced from 25 in the initial code, to just 7 in the last example using memoization, and the benefit of using memoization increases really fast by how high the Fibonacci number we want to find is.</p>
<p>Finding the 30th Fibonacci number requires 2,692,537 computations in the initial code, but it just requires 31 computations in the algorithm implemented using memoization!</p>
<p>You get this result by running the code below.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>See the difference in calculations for finding the 30th Fibonacci number, with and without memoization:</p>
  <pre class="language-python notranslate w3-white"><code>computation_count = 0
def F(n):
...
memo = [None]*31
print('F(30) = ',F_mem(30))
print(f'Number of computations with memoiztion: {computation_count_mem}')</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_findfibo_memo3" target="_blank">Try it Yourself &raquo;</a>
</div>
...
<h2>Memoization in AVL Trees</h2>
<p>For more details about what an AVL Tree is, please see <a href="dsa_data_avltrees.php">this page</a>.</p>
<p>An AVL tree is a type of Binary Tree that is self-balancing.</p>
<p>Every time a node is inserted or deleted from an AVL tree, the balancing factor must be calculated for all ancestors, using the height of the left and right subtrees to find out if a rotation is needed to restore balance.</p>
<p>To avoid calculating the height of each node (going all the way down to the leaf nodes) to calculate the balancing factors, each node has its subtree height stored.</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white" data-line="6"><code>class TreeNode:
    def __init__(self, data):
        self.data = data
        self.left = None
        self.right = None
        self.height = 1</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_avl_insert" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>This means that to find the balance factor for a node, the already stored left child's height is subtracted from the already stored right child's height, no other calculations needed.</p>
<p>Storing height in AVL trees is a form of memoization, because values are stored to avoid re-calculating them. In AVL trees, when the height is stored like this, it is called an <strong>augmented property</strong>.</p>
<p>An augmented property is a property of an element that does not have to be stored, but is stored to make operations more efficient.</p>
<p>The node heights must be calculated at some point of course, but that is only done when strictly needed, using <a href="dsa_data_avltrees.php#retracing">retracing</a>.</p>
<hr>
