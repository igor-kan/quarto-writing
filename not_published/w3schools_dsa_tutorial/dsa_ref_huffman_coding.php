...
<html lang="en-US">
<head>
<title>DSA Huffman Coding</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
body.darkpagetheme #verticalDivider {
  background-color: #38444d;
}
.codeWrapper, .codeWrapper2, .codeWrapper3 {
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  margin-left: 25px;
...
.branchText {
  fill: black;
  font-size: 12px;
}.darkpagetheme .branchText {
  fill: #ddd;
}
#vueApp1 input {
  font-family: monospace;
}
.darkpagetheme .highlight code {
  color: crimson;
}
</style>
</head>
...
ny:-->
    ...
    <h1><span class="color_h1">Huffman Coding</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_ref_euclidean_algorithm.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_ref_traveling_salesman.php">Next &#10095;</a>
</div>
<hr>

<h2>Huffman Coding</h2>
<p>Huffman Coding is an algorithm used for lossless data compression.</p>
<p>Huffman Coding is also used as a component in many different compression algorithms. It is used as a component in lossless compressions such as zip, gzip, and png, and even as part of lossy compression algorithms like mp3 and jpeg.</p>
<p>Use the animation below to see how a text can be compressed using Huffman Coding.</p>
<div id="vueApp1">
  <div id="graph2" ref="svgEl" class="svgWrapper" :style="{ height: svgHeight + 'px' }"></div>
  <div class="textWrapper">
      <div style="margin-right: 10px;">Text: </div>
      
      <!-- Show charWrapper when state is not 'writing' -->
      <div v-if="state !== 'writing'" class="charWrapper">
        <div v-for="el in compInputText" @click="handleLetterClick(el.letter)" :key="el.letter" class="charDiv clickable" :class="{ highlight: el.isCurrent }">
          {{ el.letter }}
        </div>
      </div>
      
      <!-- Show input field when state is 'writing' -->
      <div v-if="state === 'writing'">
        <input type="text" v-model="inputText" ref="myInput" @input="checkInputLength">
      </div>
    </div>
    <button class="btnDiv" @click="action" :disabled="state === 'building' || btnIsDisabled">{{btnText}}</button>
    <i style="margin-left: 5px;">{{inpComment}}</i>
    <hr>
    <div style="position: relative; width: 100%; margin-left: 10px;" ref="resWrap">
      <div class="resultWrapper" >
        <div class="resultSection" :style="{ opacity: state === 'finished' ? 1 : 0.5 }">
          Huffman code: <br>
          <div class="codeWrapper2">
            <div v-for="el in huffmanCode" @click="handleLetterClick(el.letter)" :class="{ highlight: el.isCurrent }" class="nodeCode">{{ el.code }}</div>
          </div>
        </div>
        ...
        <div class="resultSection">
          ...
          <div class="codeWrapper">
            <div v-for="el in utf8BinaryCode" @click="handleLetterClick(el.letter)" :class="{ highlight: el.isCurrent }">{{ el.code }}</div>
          </div>
        </div>
      </div>
      <div class="resultWrapper" >
        <div class="resultSection">
          <p v-show="state === 'finished'" style="text-align: right; margin: 0; padding: 0;">{{ huffmanBitCount }} bits</p>
        </div>
        <div class="resultSection">
          {{ utf8BitCount }} bits<!--<p></p>-->
        </div>
      </div>
      <div id="verticalDivider" :style="{ height: verticalHeight + 'px' }"></div>
    </div>
...
<!-- <p class="adtext">Advertisement</p> -->
    ...
    <div>
      <p style="padding: 20px; margin: 0;">
        <span v-show="state !== 'finished'" style="opacity:0.5;">Result</span>
        <span v-show="state === 'finished'">The Huffman code is {{compression}}% of the original size.</span>
      </p>
    </div>
</div>
...
*/
  const app1 = Vue.createApp({
    ...
    watch: {
      utf8BinaryCode(){
        this.$nextTick(() => {
          this.verticalHeight = this.$refs.resWrap.clientHeight-20;
        });
      },
      currentChar(char){
        this.tree.forEach(node => {
          if (node.isLetter) {
            node.isCurrent = (node.letter === char);
          }
        });
      }
    },
    computed: {
      compression(){
        if(this.state !== 'finished'){
          ...
          return Math.round(100*this.huffmanBitCount/this.utf8BitCount);
        }
      },
      viewBox() {
      ...
      nodePositions() {
        ...
        x.domain([0, this.width]);
        y.domain([0, this.height]);
        ...
        this.tree.forEach((node, index) => {
          if (node.left) {
            lines.push({
              ...
              endX: node.left.x,
              endY: node.left.y,
              key: `line-${node.dummy || index}-left-${node.left.letter || 'dummy'}`, // Unique key for left connection
              label: '0'
            });
          }
          if (node.right) {
            lines.push({
              ...
              endX: node.right.x,
              endY: node.right.y,
              key: `line-${node.dummy || index}-right-${node.right.letter || 'dummy'}`, // Unique key for right connection
              label: '1'
            });
          }
        });
      ...
      levels() {
        const levels = Array(10).fill(0); // Initialize with zeros for up to 10 levels
        const countLevels = (node, level) => {
          if (!node) return;
          if (!levels[level]) {
            levels[level] = 0;
          }
          levels[level]++;
          if (node.left) countLevels(node.left, level + 1);
          if (node.right) countLevels(node.right, level + 1);
        };
        const rootNodes = this.tree.filter(node => !node.parent);
        rootNodes.forEach(root => countLevels(root, 1)); // Start counting from level 1
        return levels;
      },
      utf8BinaryCode() {
        const toUTF8Binary = (char) => {
          const codePoint = char.codePointAt(0);

          if (codePoint <= 0x7F) {
            return codePoint.toString(2).padStart(8, '0');
          } else if (codePoint <= 0x7FF) {
            return (0xC0 | (codePoint >> 6)).toString(2).padStart(8, '0') + ' ' +
                  (0x80 | (codePoint & 0x3F)).toString(2).padStart(8, '0');
          } else if (codePoint <= 0xFFFF) {
            return (0xE0 | (codePoint >> 12)).toString(2).padStart(8, '0') + ' ' +
                  (0x80 | ((codePoint >> 6) & 0x3F)).toString(2).padStart(8, '0') + ' ' +
                  (0x80 | (codePoint & 0x3F)).toString(2).padStart(8, '0');
          } else {
            return (0xF0 | (codePoint >> 18)).toString(2).padStart(8, '0') + ' ' +
        ...
        return splitString(this.inputText).map(char => ({
          letter: char,
          code: toUTF8Binary(char),
          isCurrent: this.state === 'finished' && char === this.currentChar
        }));
      },
      utf8BitCount() {
        return this.utf8BinaryCode.reduce((total, current) => total + current.code.length, 0);
      },
      huffmanCode() {
        const splitString = (str) => {
          const result = [];
        ...
        return splitString(this.inputText).map(char => {
          const node = this.tree.find(n => n.letter === char);
          return {
            letter: char,
            code: node ? node.code : '',
            isCurrent: this.state === 'finished' && char === this.currentChar
      ...
      huffmanBitCount() {
        return this.huffmanCode.reduce((total, current) => total + current.code.length, 0);
      },
      compInputText() {
        const splitString = (str) => {
          const result = [];
        ...
        return splitString(this.inputText).map(char => {
          return {
            letter: char,
            isCurrent: char === this.currentChar
          };
        });
      },
      checkInputLength() {
        const maxLength = 14;
        const uniqueCharLimit = 10;
        
        let charCount = 0;
        let uniqueChars = new Set();
        
        // Iterate through the input text, accounting for surrogate pairs
        for (let i = 0; i < this.inputText.length; i++) {
          ...
          // Stop if either the max length or unique char limit is exceeded
          if (charCount > maxLength || uniqueChars.size > uniqueCharLimit) {
            this.inputText = this.inputText.slice(0, i);
            break;
          }
        }
        ...
        if (uniqueChars.size < 2) {
          ...
          this.inpComment = '(Min. 2 unique characters)';
        } else {
    ...
    methods: {
      createGraph() {
        if (!this.svg) {
          ...
          // Create layers
          this.svg.append("g").attr("id", "linesLayer");
          this.svg.append("g").attr("id", "nodesLayer");
        }
        //this.updateGraph(500);
      },
      updateGraph(delayMs) {
        const nodePositions = this.tree.filter(d => d.visible);
        const lineData = this.lines.filter(d => {
            const startNode = this.tree.find(n => n.x === d.startX && n.y === d.startY);
            const endNode = this.tree.find(n => n.x === d.endX && n.y === d.endY);
            return startNode && endNode && startNode.visible && endNode.visible;
        });

        // Update lines
        const lines = this.svg.select("#linesLayer").selectAll("line").data(lineData, d => d.key);
        ...
        linesEnter.merge(lines).transition().duration(delayMs)
            .attr("x1", d => d.startX)
            .attr("y1", d => d.startY)
            .attr("x2", d => d.endX)
            .attr("y2", d => d.endY);
        ...
        // Update line labels
        const lineLabels = this.svg.select("#linesLayer").selectAll("text.branchText").data(lineData, d => `label-${d.key}`);

        const lineLabelsEnter = lineLabels.enter().append("text")
            .attr("class", "branchText")
            .attr("x", d => (d.startX + d.endX) / 2 + (d.label === '1' ? 3 : -10))
            .attr("y", d => (d.startY + d.endY) / 2 + 200)
            .text(d => d.label);

        const lineLabelsUpdate = lineLabelsEnter.merge(lineLabels);

        lineLabelsUpdate.transition().duration(delayMs)
            .attr("x", d => (d.startX + d.endX) / 2 + (d.label === '1' ? 3 : -10))
        ...
        // Update nodes
        const circleNodes = this.svg.select("#nodesLayer").selectAll("circle").data(nodePositions, d => d.letter || d.dummy);

        const circleNodesEnter = circleNodes.enter().append("circle")
            .attr("cx", (d, i) => 70 + i * 20)
            .attr("cy", 300)
            .attr("r", 10)
            .attr("class", "nodeCircle")
            .attr("stroke-width", 1)
            .attr("stroke-dasharray", d => !d.letter ? '5,3' : '0')
            .on('click', (event, d) => {
                if (d.isLetter) {
                    this.setActiveNode(d.letter);
                }
            });

        const circleNodesUpdate = circleNodesEnter.merge(circleNodes);

        circleNodesUpdate.transition().duration(delayMs)
            .attr("cx", d => d.x)
            .attr("cy", d => d.y)
            .attr("r", d => d.isCurrent ? 17 : 15)
            .attr("stroke", "black")
            .attr("stroke-width", d => d.isCurrent ? 2 : 1)
            .attr("stroke-dasharray", d => !d.letter ? '5,3' : '0');

        circleNodesUpdate.classed("highlight", d => d.isCurrent)
            .classed("clickable", d => d.isLetter); // Add or remove the 'clickable' class based on the 'isLetter' property

        circleNodes.exit().remove();

        // Update letter texts
        const letterTexts = this.svg.select("#nodesLayer").selectAll("text.nodeLetter").data(nodePositions.filter(d => d.letter), d => d.letter);

        const letterTextsEnter = letterTexts.enter().append("text")
            .attr("x", (d, i) => 70 - 10 + i * 20)
            .attr("y", 300 + 5)
            .attr("class", "nodeLetter")
            .text(d => d.letter)
            .on('click', (event, d) => {
                if (d.isLetter) {
                    this.setActiveNode(d.letter);
        ...
        const letterTextsUpdate = letterTextsEnter.merge(letterTexts);

        letterTextsUpdate.transition().duration(delayMs)
            .attr("x", d => d.x - 10)
            .attr("y", d => d.y + 5);

        letterTextsUpdate.classed("highlight", d => d.isCurrent)
            .classed("clickable", d => d.isLetter);

        letterTexts.exit().remove();

        // Update frequency texts
        const frequencyTexts = this.svg.select("#nodesLayer").selectAll("text.smallText").data(nodePositions, d => d.letter || d.dummy);

        const frequencyTextsEnter = frequencyTexts.enter().append("text")
            .attr("x", (d, i) => 70 + 1 + i * 20)
            .attr("y", 300 + 10)
        ...
        const frequencyTextsUpdate = frequencyTextsEnter.merge(frequencyTexts);

        frequencyTextsUpdate.transition().duration(delayMs)
            .attr("x", d => d.freq > 9 ? d.x - 2 : d.x + 1)
            .attr("y", d => d.y + 10);

        frequencyTextsUpdate.classed("clickable", d => d.isLetter)
            .classed("highlight", d => d.isCurrent);

        frequencyTexts.exit().remove();

        // Update code texts (ensure they are only for letter nodes and have non-empty codes)
        const codeTexts = this.svg.select("#nodesLayer").selectAll("text.nodeCode").data(nodePositions.filter(d => d.isLetter && d.code !== ''), d => d.letter);

        const codeTextsEnter = codeTexts.enter().append("text")
            .attr("x", (d, i) => 70 -10 + i * 20)
            .attr("y", 300 + 25)
            .attr("class", "nodeCode")
            .text(d => d.code)
        ...
        const codeTextsUpdate = codeTextsEnter.merge(codeTexts);

        codeTextsUpdate.transition().duration(delayMs)
            .attr("x", d => d.x - 10)
            .attr("y", d => d.isCurrent ? d.y + 30 : d.y + 28)
            .text(d => d.code);

        codeTextsUpdate.classed("clickable", d => d.isLetter);

        codeTexts.exit().remove();
      },
      updateNodePosition() {
        this.nodes[1].y = this.yValue;
        this.updateGraph(500);
      },
      getRandomPosition() {
        return {
      ...
      updateNodesRandomly() {
        this.nodes[0] = { ...this.nodes[0], ...this.getRandomPosition() };
        this.nodes[1] = { ...this.nodes[1], ...this.getRandomPosition() };
        this.updateGraph(500);
      },
      setActiveNode(letter) {
        this.currentChar = letter;
        this.tree.forEach(node => {
          node.isCurrent = (node.letter === letter);
        });
        this.inpComment = '';
        this.updateGraph(200);
      },
      ...
      async action(){
        if(this.state === 'writing'){
          ...
          // remove nodes, send off, then delete?
          // delete text? --> this.inputText = '';
          this.tree.forEach(node => {
            node.y += 400;
          });
          this.updateGraph(300);
          this.state = 'writing';
          this.btnText = 'Create Huffman Code';
      ...
      async generateHuffmanTree() {
        this.state = 'building';
        this.nodes = []; //{x: 50, y: 250}
        this.tree = [];
        this.dummyNumber = 0;
        this.adjustSvgHeight();
        await this.calculateFrequency(this.inputText);
        while (this.nodes.length > 1) {
          ...
          //this.fixLayout2();
          //this.avoidCrossings();
          this.inOrderTraversalForXPositioning();
          this.correctLeaves();
          this.correctParents();
          await new Promise(resolve => setTimeout(resolve, 1000));
          this.updateGraph(500);
          this.tree.forEach(node => node.visible = true);
          await new Promise(resolve => setTimeout(resolve, 500));
        }
        await new Promise(resolve => setTimeout(resolve, 500));
        this.updateGraph(500);
        ...
        const root = this.tree.find(node => !node.parent && (node.left || node.right));
        if (root) {
          this.assignCodes(root);
        }
        this.updateGraph(500);
        await new Promise(resolve => setTimeout(resolve, 2000));
        this.setActiveNode(this.tree.find(node => node.isLetter).letter);
        this.state = 'finished';
        this.btnText = 'Try again';
        this.inpComment = '(Click any letter, letter-node, or code.)'/**/
      },
      async calculateFrequency(text) {
        const freqMap = {};
        ...
        for (const char of text) {
          if (freqMap[char] > 0) {
            ...
            freqMap[char] = 0;  // Prevent duplicates
            x += 5;
            await new Promise(resolve => setTimeout(resolve, 500));
            this.tree[this.tree.length-1].y = 30;
            this.tree[this.tree.length-1].x = this.tree.length*40;
            this.updateGraph(500);
          }
        }
        this.updateGraph(500);
      ...
      buildHuffmanTree() {
        ...
        const letterNodesWithoutParents = this.tree.filter(node => node.isLetter && !node.parent);

        const newNode = {
          isLetter: false,
          letter: null,
          freq: left.freq + right.freq,
          x: 200, // Placeholder for x, will be fixed in layout
          y: letterNodesWithoutParents.length > 0 ? 80 : 30, // Set y based on letter nodes without parents
          left,
          right,
          parent: null,
        ...
        left.level = newNode.level + 1;
        right.level = newNode.level + 1;
      ...
      fixLayout() {
        const layout = (node, x, y, level = 1) => {
          if (node !== null) {
            node.x = x;
            node.y = y;
            node.level = level;
            const childXOffset = 90 / level; // Adjust this value to space out children nodes
            const childYOffset = 60; // Vertical gap between levels

            if (node.left) layout(node.left, x - childXOffset, y + childYOffset, level + 1);
            if (node.right) layout(node.right, x + childXOffset, y + childYOffset, level + 1);
          }
        };

        const rootNodes = this.tree.filter(node => node.level === 1);
        const totalWidth = 420;
        const spacing = totalWidth / (rootNodes.length * 2);
        const noParentNodes = this.tree.filter(node => !node.parent && node.isLetter);

        const startY = noParentNodes.length === 0 ? 30 : 120;

        rootNodes.forEach((root, index) => {
          layout(root, index * 2 * spacing + spacing, startY); // Start root at y: 120 or 30
        });
      },
      correctLeaves() {
        const leafNodes = this.tree.filter(node => !node.left && !node.right && node.parent);
        const totalWidth = 420;

        // Sort leaf nodes by their current x position
      ...
      correctParents() {
        const parentNodes = this.tree.filter(node => node.left && node.right);
        ...
        const letterNodesWithoutParents = this.tree.filter(node => node.isLetter && !node.parent);
        const rootNodes = this.tree.filter(node => !node.parent && (node.left || node.right));

        if (rootNodes.length > 1 && letterNodesWithoutParents.length === 0) {
          this.tree.forEach(node => {
            node.y += 60;
          });
        }
      },
      inOrderTraversalForXPositioning() {
        let xCounter = 0;
        const totalNodes = this.tree.filter(node => node.parent || !node.isLetter).length; // Count only nodes that have a parent or are non-letter nodes
        const spacing = 420 / (totalNodes + 1); // Adjust the width (420) as per your SVG width
        ...
        const inOrderTraversal = (node) => {
          ...
          // Skip letter nodes without a parent
          if (node.parent || !node.isLetter) {
            // Set the x-position and increment the counter
            node.x = (xCounter + 1) * spacing;
            xCounter++;
          }
        ...
        // Find the root node(s) and start in-order traversal
        const rootNodes = this.tree.filter(node => !node.parent);
        rootNodes.forEach(root => inOrderTraversal(root));
      },
      ...
      adjustSvgHeight() {
        //const uniqueChars = new Set(this.inputText).size;
        //this.svgHeight = Math.max(200, 20 + uniqueChars * 60);
        this.svgHeight = Math.max(200, 20 + this.calculateHuffmanTreeHeight(this.inputText) * 60);
        let ratio = this.$refs.svgEl.clientWidth/420;
        this.svgHeight = this.svgHeight*ratio;/**/
      },
      calculateHuffmanTreeHeight(text) {
        const calculateFrequency = (text) => {
          ...
          return Object.entries(freqMap).map(([letter, freq]) => ({
              letter,
              freq,
              left: null,
              right: null
        ...
        const nodes = calculateFrequency(text);
    ...
    beforeUnmount() {
      window.removeEventListener('resize', this.adjustSvgHeight);
    }
  });

  app1.mount('#vueApp1');
</script>
<p>The animation shows how the letters in a text are normally stored using <a href="../charsets/ref_html_utf8.asp">UTF-8</a>, and how Huffman Coding makes it possible to store the same text with fewer bits.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Count how often each piece of data occurs.</li>
    <li>Build a <a href="dsa_data_binarytrees.php">binary tree</a>, starting with the nodes with the lowest count. The new parent node has the combined count of its child nodes.</li>
    <li>The edge from a parent gets '0' for the left child, and '1' for the edge to the right child.</li>
    <li>In the finished binary tree, follow the edges from the root node, adding '0' or '1' for each branch, to find the new Huffman code for each piece of data.</li>
    <li>Create the Huffman code by converting the data, piece-by-piece, into a binary code using the binary tree.</li>
  </ol>
</div>
<p>Huffman Coding uses a variable length of bits to represent each piece of data, with a shorter bit representation for the pieces of data that occurs more often.</p>
<p>Furthermore, Huffman Coding ensures that no code is the prefix of another code, which makes the compressed data easy to decode.</p>
<div class="ws-note">
  <p><strong>Data compression</strong> is when the original data size is reduced, but the information is mostly, or fully, kept. Sound or music files are for example usually stored in a compressed format, roughly just 10% of the original data size, but with most of the information kept.</p>
  <p><strong>Lossless</strong> means that even after the data is compressed, all the information is still there. This means that for example a compressed text still has all the same letters and characters as the original.</p>
  <p><strong>Lossy</strong> is the other variant of data compression, where some of the original information is lost, or sacrificed, so that the data can be compressed even more. Music, images, and video is normally stored and streamed with lossy compression like mp3, jpeg, and mp4.</p>
</div>
<hr>

<h2>Creating A Huffman Code Manually</h2>
<p>To get a better understanding of how Huffman Coding works, let's create a Huffman code manually, using the same text as in the animation: 'lossless'.</p>
<p>A text is normally stored in the computer using <a href="../charsets/ref_html_utf8.asp">UTF-8</a>, which means that each letter is stored using 8 bits for normal latin letters, like we have in 'lossless'. Other letters or symbols such as '€' or '🦄' are stored using more bits.</p>
<p>To compress the text 'lossless' using Huffman Coding, we start by counting each letter.</p>
<div id="vueApp2"><!--  viewBox="0 0 420 260" :width="420" :height="svgHeight" -->
  <div class="svgWrapper" style="width: 200px; height: 40px;">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 40" ref="svgEl">
      <transition-group>
        <path v-for="line in lines" 
          :key="line.key"
          :d="`M${line.startX},${line.startY} L${line.endX},${line.endY}`" 
          class="branch" fill="none" />
        <text v-for="line in lines"
          :key="`label-${line.key}`"
          :x="(line.startX + line.endX) / 2 + (line.label === '1' ? 3 : -10)"
          :y="(line.startY + line.endY) / 2"
          font-size="12" class="branchText">
          {{ line.label }}
        </text>
        <circle v-for="node in tree" 
          :key="`node-${node.freq}-${node.letter}-${node.dummy}`"
          :cx="node.x" 
          :cy="node.y" 
          :stroke-dasharray="!node.letter ? '5,3' : '0'"
          :r="node.isCurrent ? 17 : 15"
          :stroke-width="node.isCurrent ? 3 : 2"
          :class="{ highlight: node.isCurrent }"
          class="nodeCircle"  />
        <text v-for="node in tree"
          :x="node.x-10"
          :y="node.y+5"
          :key="`letter-${node.freq}-${node.letter}-${node.dummy}`"
          class="nodeLetter"
          :class="{ highlight: node.isCurrent }">
          {{node.letter}}
        </text>
        <text v-for="node in tree" 
          class="smallText"
          :class="{ highlight: node.isCurrent }"
          :x="node.freq > 9 ? node.x - 2 : node.x + 1"
          :y="node.y+10"
          :key="`freq-${node.freq}-${node.letter}-${node.dummy}`">
          {{node.freq}}
        </text>
        <template v-for="node in tree">
          <text
            v-if="node.isLetter"
            :x="node.x - 10"
            :y="node.isCurrent ? node.y + 30 : node.y + 28"
            :key="`code-${node.letter}`"
            class="nodeCode" font-size="12" font-family="monospace">
            {{ node.code }}
          </text>
        </template>
      </transition-group>
...
<script>
  const app2 = Vue.createApp({
    data() {
      return {
        inputText: 'lossless', // senselessness, ouagadougou, luxembourg, lossless
        tree: [],
        nodes: [],
        dummyNumber: 0,
        state: 'writing',
    ...
    methods: {
      checkInputLength() {
        const uniqueChars = new Set(this.inputText).size;
        if (this.inputText.length > 14 || uniqueChars > 10) {
      ...
      async generateHuffmanTree() {
        ...
        await new Promise(resolve => setTimeout(resolve, 2000));
        this.currentChar = this.tree.find(node => node.isLetter).letter;
        this.state = 'finished';
        this.btnText = 'Try again';
        this.inpComment = '(Click any letter, letter-node, or code.)'*/
      },
      async calculateFrequency(text) {
        ...
        let x = 90;
        for (const [letter, freq] of Object.entries(freqMap)) {
          //await new Promise(resolve => setTimeout(resolve, 1000));
          this.currentChar = letter;
          this.tree.push({
            isLetter: false,
      ...
      fixLayout2() {
        this.levels.forEach((count, level) => {
          if (level > 1 && count > 0) {
            if (count > 2 ** (level - 1)) {
              const levelNodes = this.tree.filter(node => node.level === level);
              const spacing = 420 / (count * 2);
              levelNodes.forEach((node, index) => {
                const offset = spacing;
                node.x = node.parent.x + (node === node.parent.left ? -offset : offset);
              });
            } else {
              const levelNodes = this.tree.filter(node => node.level === level);
              const spacing = 420 / (2 ** (level - 1) + 1);
              levelNodes.forEach((node, index) => {
...
<p>As you can see in the nodes above, 's' occurs 4 times, 'l' occurs 2 times, and 'o' and 'e' occurs just 1 time each.</p>
<p>We start building the tree with the least occurring letters 'o' and 'e', and their parent node gets count '2', because the counts for letter 'o' and 'e' are summarized.</p>
<div id="vueApp3"><!--  viewBox="0 0 420 260" :width="420" :height="svgHeight" -->
  <div class="svgWrapper" style="width: 100%; max-width: 300px;">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 150" ref="svgEl">
      <transition-group>
        <path v-for="line in lines" 
          :key="line.key"
          :d="`M${line.startX},${line.startY} L${line.endX},${line.endY}`" 
  ...
  const app3 = Vue.createApp({
    ...
    methods: {
      ...
      async generateHuffmanTree() {
        ...
        this.dummyNumber = 0;
        this.adjustSvgHeight();
        this.calculateFrequency(this.inputText);
...
<p>The next nodes that get a new parent node, are the nodes with the lowest count: 'l', and the parent node of 'o' and 'e'.</p>
<div id="vueApp4"><!--  viewBox="0 0 420 210" :width="420" :height="svgHeight" -->
  <div class="svgWrapper" style="width: 100%; max-width: 300px;">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 210" ref="svgEl">
      <transition-group>
        <path v-for="line in lines" 
          :key="line.key"
          :d="`M${line.startX},${line.startY} L${line.endX},${line.endY}`" 
  ...
  const app4 = Vue.createApp({
    ...
    methods: {
      ...
      async generateHuffmanTree() {
        ...
        this.tree[0].y -= 50; this.tree[0].x -= 50;
        this.tree[1].y -= 50; this.tree[1].x -= 50;
        this.tree[2].x -= 50;
        this.tree[3].y -= 50; this.tree[3].x -= 50;
        this.tree[4].y -= 50; this.tree[4].x -= 50;
        this.tree[5].y -= 50; this.tree[5].x -= 50;
...
<p>Now, the last node 's' must be added to the binary tree. Letter node 's' and the parent node with count '4' get a new parent node with count '8'.</p>
<div id="vueApp5"><!--  viewBox="0 0 420 260" :width="420" :height="svgHeight" -->
  <div class="svgWrapper" style="width: 100%; max-width: 300px;">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 220" ref="svgEl">
      <transition-group>
        <path v-for="line in lines" 
          :key="line.key"
          :d="`M${line.startX},${line.startY} L${line.endX},${line.endY}`" 
  ...
  const app5 = Vue.createApp({
    ...
    methods: {
      ...
      async generateHuffmanTree() {
        ...
        this.tree[0].x -= 40; this.tree[0].y -= 10;
        this.tree[1].x -= 40; this.tree[1].y -= 10;
        this.tree[2].x -= 40; this.tree[2].y -= 10;
        this.tree[3].x -= 40; this.tree[3].y -= 10;
        this.tree[4].x -= 40; this.tree[4].y -= 10;
        this.tree[5].x -= 40; this.tree[5].y -= 10;
        this.tree[6].x -= 40; this.tree[6].y -= 10;
...
<p>Following the edges from the root node, we can now determine the Huffman code for each letter in the word 'lossless'.</p>
<div id="vueApp6"><!--  viewBox="0 0 420 260" :width="420" :height="svgHeight" -->
  <div class="svgWrapper" style="width: 100%; max-width: 300px;">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 230" ref="svgEl">
      <transition-group>
        <path v-for="line in lines" 
          :key="line.key"
          :d="`M${line.startX},${line.startY} L${line.endX},${line.endY}`" 
...
<p>The Huffman code for each letter can now be found under each letter node in the image above. A good thing about Huffman coding is that the most used data pieces get the shortest code, so just '0' is the code for the letter 's'.</p>
<p>As mentioned earlier, such normal latin letters are usually stored with UTF-8, which means they take up 8 bits each. So for example the letter 'o' is stored as '01101111' with UTF-8, but it is stored as '110' with our Huffman code for the word 'lossless'.</p>
<div class="ws-note">
  <p><strong>Note: </strong>With UTF-8, a letter has always the same binary code, but with Huffman code, the binary code for each letter (piece of data) changes with text (data set) we are compressing.</p>
</div>
<p>To summarize, we have now compressed the word 'lossless' from its UTF-8 code</p>
<p><code class="w3-codespan">01101100 01101111 01110011 01110011 01101100 01100101 01110011 01110011</code></p>
<p>to just</p>
<p><code class="w3-codespan">10 110 0 0 10 111 0 0</code></p>

<p>using Huffman Coding, which is a huge improvement.</p>
<p>But if data is stored with Huffman Coding as <code class="w3-codespan">10 110 0 0 10 111 0 0</code>, or the code is sent to us, how can it be decoded so that we see what information the Huffman code contains?</p>
<p>Furthermore, the binary code is really <code class="w3-codespan">10110001011100</code>, without the spaces, and with variable bit lengths for each piece of data, so how can the computer understand where the binary code for each piece of data starts and ends?</p>
<hr>

<h2>Decoding Huffman Code</h2>
<p>Just like with code stored as UTF-8, which our computers can already decode to the correct letters, the computer needs to know which bits represent which piece of data in the Huffman code.</p>
<p>So along with a Huffman code, there must also be a conversion table with information about what the Huffman binary code is for each piece of data, so that it can be decoded.</p>
<p>So, for this Huffman code:</p>
<p><code class="w3-codespan">100110110</code></p>
<p>With this conversion table:</p>
<table class="ws-table-all notranslate" style="width: auto;">
  <tbody><tr>
...
<p>Are you able to decode the Huffman code?</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Start from the left in the Huffman code, and look up each bit sequence in the table.</li>
    <li>Match each code to the corresponding letter.</li>
    <li>Continue until the entire Huffman code is decoded.</li>
  </ol>
</div>
<p>We start with the first bit:</p>
<div class="codeWrapper3">
  <div class="highlight"><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">1</code></div>
...
<p>There is no letter in the table with just <code class="w3-codespan">1</code> as the Huffman code, so we continue and include the next bit as well.</p>
<div class="codeWrapper3">
  <div class="highlight"><code class="w3-codespan">1</code></div>
  <div class="highlight"><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
...
<p>We can see from the table that <code class="w3-codespan">10</code> is 'b', so now we have the first letter. We check the next bit:</p>
<div class="codeWrapper3">
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div class="highlight"><code class="w3-codespan">0</code></div>
...
<p>We find that <code class="w3-codespan">0</code> is 'a', so now we have the two first letters 'ba' stored in the Huffman code.</p>
<p>We continue looking up Huffman codes in the table:</p>
<div class="codeWrapper3">
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">0</code></div>
...
<p>Code <code class="w3-codespan">11</code> is 'n'.</p>
<div class="codeWrapper3">
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">0</code></div>
...
<p>Code <code class="w3-codespan">0</code> is 'a'.</p>
<div class="codeWrapper3">
  <div><code class="w3-codespan">1</code></div>
  <div><code class="w3-codespan">0</code></div>
  <div><code class="w3-codespan">0</code></div>
...
<p>Code <code class="w3-codespan">0</code> is 'a'.</p>
<p>The Huffman code is now decoded, and the word is 'banana'!</p>
<hr>

<h2>Huffman Code Prefixes</h2>
<p>An interesting and very useful part of the Huffman coding algorithm is that it ensures that there is no code that is the prefix of another code.</p>
<div class="ws-warning">
  <p>Just image if the conversion table we just used, looked like this:</p>
  <table class="ws-table-all notranslate" style="width: auto;">
    <tbody><tr>
      <th style="width: 10ch;">Letter</th>
      <th style="width: 15ch;">Huffman Code</th>
  ...
  <p>If this was the case, we would get confused right from the start of the decoding, right?</p>
  <div class="codeWrapper3">
    <div class="highlight"><code class="w3-codespan">1</code></div>
    <div><code class="w3-codespan">0</code></div>
    <div><code class="w3-codespan">0</code></div>
  ...
  <p>Because how would we know if the first bit <code class="w3-codespan">1</code> represents the letter 'a' or if it is the first bit for the letter 'b' or 'c'?</p>
</div>
<p>This property, that no code is the prefix of another code, makes it possible to decode. And it is especially important in Huffman Coding because of the variable bit lengths.</p>
<hr>

<h2>Huffman Coding Implementation</h2>
<p>The correct word for creating Huffman code based on data or text is "encoding", and the opposite would be "decoding", when the original data or text is recreated based on the code.</p>
<p>The code example below takes a word, or any text really, and compress it using Huffman Coding.</p>
...
print("Conversion table:", codes)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_huffman" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Huffman Decoding Implementation</h2>
<p>In addition to encode data using Huffman coding, we should also have a way to decode it, to recreate the original information.</p>
<p>The implementation below is basically the same as the previous code example, but with an additional function for decoding the Huffman code.</p>
<p>The <code class="w3-codespan">huffman_decoding</code> function takes the Huffman code, and the <code class="w3-codespan">codes</code> <a href="../python/python_dictionaries.asp">Python dictionary</a> (a <a href="dsa_data_hashmaps.php">hashmap</a>) with the characters and their corresponding binary codes. The Function then reverse the mapping, and checks the Huffman code bit-by-bit to recreate the original text.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Huffman Decoding.</p>
  <pre class="language-python notranslate w3-white" data-line="51-64"><code>class Node:
    def __init__(self, char=None, freq=0):
        self.char = char
        self.freq = freq
        self.left = None
...
print("Decoded word:", decoded_word)
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_huffman_decoding" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>You have now seen how a text can be compressed using Huffman coding, and how a Huffman code can be decoded to recreate the original text.</p>
<div class="ws-note">
  <p><strong>Note: </strong>Huffman Coding can be used for lossless compression of any kind of data, not just text. Huffman Coding is also used as a component in other compression algorithms like zip, and even in lossy compressions like jpeg and mp3.</p>
</div>
<hr>
