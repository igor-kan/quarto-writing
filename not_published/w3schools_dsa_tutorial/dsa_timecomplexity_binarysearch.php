...
<html lang="en-US">
<head>
<title>DSA Selection Sort Time Complexity</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Selection Sort Time Complexity</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_timecomplexity_linearsearch.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="dsa_exam.php">Next &#10095;</a>
</div>
<hr>
...
<p><a href="dsa_algo_binarysearch.php">Binary Search</a> finds the target value in an already sorted array by checking the center value. If the center value is not the target value, Linear Search selects the left or right sub-array and continues the search until the target value is found.</p>

<p>To find the time complexity for Binary Search, let's see how many compare operations are needed to find the target value in an array with \(n\) values.</p>
<p>The <strong>best case scenario</strong> is if the first middle value is the same as the target value. If this happens the target value is found straight away, with only one compare, so the time complexity is \(O(1)\) in this case.</p>
<p>The <strong>worst case scenario</strong> is if the search area must be cut in half over and over until the search area is just one value. When this happens, it does not affect the time complexity if the target value is found or not.</p>
<p>Let's consider array lengths that are powers of 2, like 2, 4, 8, 16, 32 64 and so on.</p>
<p>How many times must 2 be cut in half until we are looking at just one value? It is just one time, right?</p>
<p>How about 8? We must cut an array of 8 values in half 3 times to arrive at just one value.</p>
<p>An array of 32 values must be cut in half 5 times.</p>
<p>We can see that \(2=2^1\), \(8=2^3\) and \(32=2^5\). So the number of times we must cut an array to arrive at just one element can be found in the power with base 2. Another way to look at it is to ask "how many times must I multiply 2 with itself to arrive at this number?". Mathematically we can use the base-2 logarithm, so that we can find out that an array of length \(n\) can be split in half \( \log_{2}(n)\) times.</p>
<p>This means that time complexity for Binary Search is</p>
<p><span class="math display">\[ \underline{\underline{O( \log_{2} n )}} \]</span></p>
<p>The <strong>average case scenario</strong> is not so easy to pinpoint, but since we understand time complexity of an algorithm as the upper bound of the worst case scenario, using Big O notation, the average case scenario is not that interesting.</p>
<div class="ws-note">
  <p><strong>Note: </strong>Time complexity for Binary Search \(O( \log_{2}n)\) is a lot faster than Linear Search \(O(n)\), but it is important to remember that Binary Search requires a sorted array, and Linear Search does not.</p>
</div>
...
<h2>Binary Search Simulation</h2>
<p>Run the simulation for different number of values \(n\) in an array, and see how many compares are needed for Binary Search to find the target value:</p>
<div id="vueApp4">
