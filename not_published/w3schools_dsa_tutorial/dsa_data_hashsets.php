...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Hash Sets</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_theory_hashtables.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_data_hashmaps.php">Next &#10095;</a>
</div>
<hr>

<h2>Hash Sets</h2>
<p>A Hash Set is a form of <a href="dsa_theory_hashtables.php">Hash Table</a> data structure that usually holds a large number of elements.</p>
<p>Using a Hash Set we can search, add, and remove elements really fast.</p>
<p>Hash Sets are used for lookup, to check if an element is part of a set.</p>
<div id="vueApp">
...
<p>A Hash Set stores unique elements in buckets according to the element's hash code.</p>
<ul>
  <li><strong>Hash code: </strong> A number generated from an element's unique value (key), to determine what bucket that Hash Set element belongs to.</li>
  <li><strong>Unique elements: </strong> A Hash Set cannot have more than one element with the same value.</li>
  <li><strong>Bucket: </strong>A Hash Set consists of many such buckets, or containers, to store elements. If two elements have the same hash code, they belong to the same bucket. The buckets are therefore often implemented as arrays or linked lists, because a bucket needs to be able to hold more than one element.</li>
</ul>
<hr>

<h2>Finding The Hash Code</h2>
<p>A hash code is generated by a <strong>hash function</strong>.</p>
<p>The hash function in the animation above takes the name written in the input, and sums up the Unicode code points for every character in that name.</p>
<p>After that, the hash function does a modulo 10 operation (<code class="w3-codespan">% 10</code>) on the sum of characters to get the hash code as a number from 0 to 9.</p>
<p>This means that a name is put into one of ten possible buckets in the Hash Set, according to the hash code of that name. The same hash code is generated and used when we want to search for or remove a name from the Hash Set.</p>
<p>The Hash Code gives us instant access as long as there is just one name in the corresponding bucket.</p>
<div class="ws-note">
  <p><strong>Unicode code point: </strong>Everything in our computers are stored as numbers, and the Unicode code point is a unique number that exist for every character. For example, the character <code class="w3-codespan">A</code> has Unicode code point <code class="w3-codespan">65</code>. Just try it in the simulation above. See <a href="/charsets/ref_utf_basic_latin.asp">this page</a> for more information about how characters are represented as numbers.</p>
...
<h2>Direct Access in Hash Sets</h2>
<p>Searching for <code class="w3-codespan">Peter</code> in the Hash Set above, means that the hash code <code class="w3-codespan">2</code> is generated (<code class="w3-codespan">512 % 10</code>), and that directs us right to the bucket <code class="w3-codespan">Peter</code> is in. If that is the only name in that bucket, we will find <code class="w3-codespan">Peter</code> right away.</p>
<p>In cases like this we say that the Hash Set has constant time \(O(1)\) for searching, adding, and removing elements, which is really fast.</p>
<p>But, if we search for <code class="w3-codespan">Jens</code>, we need to search through the other names in that bucket before we find <code class="w3-codespan">Jens</code>. In a worst case scenario, all names end up in the same bucket, and the name we are searching for is the last one. In such a worst case scenario the Hash Set has time complexity \(O(n)\), which is the same time complexity as arrays and linked lists.</p>
<p>To keep Hash Sets fast, it is therefore important to have a hash function that will distribute the elements evenly between the buckets, and to have around as many buckets as Hash Set elements.</p>
<p>Having a lot more buckets than Hash Set elements is a waste of memory, and having a lot less buckets than Hash Set elements is a waste of time.</p>
<hr>

<h2>Hash Set Implementation</h2>
<p>Hash Sets in Python are typically done by using Python's own <a href="python/python_sets.asp"><code class="w3-codespan">set</code> data type</a>, but to get a better understanding of how Hash Sets work we will not use that here.</p>
<p>To implement a Hash Set in Python we create a class <code class="w3-codespan">SimpleHashSet</code>.</p>
<p>Inside the <code class="w3-codespan">SimpleHashSet</code> class we have a method <code class="w3-codespan">__init__</code> to initialize the Hash Set, a method <code class="w3-codespan">hash_function</code> for the hash function, and methods for the basic Hash Set operations: <code class="w3-codespan">add</code>, <code class="w3-codespan">contains</code>, and <code class="w3-codespan">remove</code>.</p>
<p>We also create a method <code class="w3-codespan">print_set</code> to better see how the Hash Set looks like.</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white line-numbers"><code>class SimpleHashSet:
...
<p>Using the <code class="w3-codespan">SimpleHashSet</code> class we can create the same Hash Set as in the top of this page:</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white line-numbers" id="codeBlock" style="height: 32em; overflow-y: scroll;"><code>class SimpleHashSet:
    def __init__(self, size=100):
        self.size = size
        self.buckets = [[] for _ in range(size)]  # A list of buckets, each is a list (to handle collisions)
...
print("'Adele' has hash code:",hash_set.hash_function('Adele'))</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_hashset" target="_blank">Try it Yourself &raquo;</a>
</div>
<script>
