...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Counting Sort</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_quicksort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_radixsort.php">Next &#10095;</a>
</div>
<hr>

<h2>Counting Sort</h2>
<p>The Counting Sort algorithm sorts an array by counting the number of times each value occurs.</p>
<div id="vueApp">
  ...
  <div id="frameDiv">
    <transition-group>
      <div v-for="(x, index) in count" class="countDivParent" :key="index-1000">
        <div class="numberDiv1" :key="index-2000">
          {{ x.countValue }}
        </div>
        <div :key="x.keyNmbr" 
        class="countDiv" 
        :class="{ 
          ...
          'diceDiv-nextSort': x.isNextSort,
        }" 
        :style="{
          height: x.countValue*5 + 'px',
    ...
    <transition-group>
      <div v-for="(x,index) in dice" 
        ...
        :style="{
          height: x.dieNmbr*19 + 'px',
          marginTop: 100-x.dieNmbr*19 + 'px',
          transition: 'all ' + delay
        }"
      >
      </div>
...
const app = Vue.createApp({
  ...
  methods: {
    ...
    action(){
      if(this.buttonText === "Counting Sort") {
        this.countSort();
      }
      else {
        this.addDie50();
    ...
    async countSort() {
      ...
      while(this.dice.length > 0) {
        ...
        let removedVal = this.dice.splice(0, 1)[0].dieNmbr;
        this.count[removedVal-1].isActive = true;
        this.count[removedVal-1].countValue++;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        this.count[removedVal-1].isActive = false;
      }
      await new Promise(resolve => setTimeout(resolve, (this.delay)*3));
      for (let i=0; i<this.count.length; i++){
        this.count[i].isActive = true;
        await new Promise(resolve => setTimeout(resolve, (this.delay)*3));
        while(this.count[i].countValue>0){
          this.count[i].countValue--;
          const newDie = {
            dieNmbr: i+1,
          ...
          this.dice[this.dice.length-1].isFinished = true;
          await new Promise(resolve => setTimeout(resolve, this.delay));
          this.keyNumber++;
        }
        this.count[i].isActive = false;
      }
      this.msgDone = 'Done!';
    ...
    addDie() {
      const newDie = {
        dieNmbr: Math.ceil(Math.random() * 5),
        keyNmbr: this.keyNumber,
        isActive: false,
        isFinished: false,
        isNextSort: false
    ...
    addDie50() {
      ...
      //this.dice[0].isNextSort = true;
      this.buttonText = "Counting Sort";
      this.msgDone = '';
    },
    compareFunc(a, b) {
...
<p>Run the simulation to see how 17 integer values from 1 till 5 are sorted using Counting Sort.</p>
<p>Counting Sort does not compare values like the previous sorting algorithms we have looked at, and only works on non negative integers.</p>
<p>Furthermore, Counting Sort is fast when the range of possible values \(k\) is smaller than the number of values \(n\).</p>
<div class="ws-info">
...
<h2>Conditions for Counting Sort</h2>
<p>These are the reasons why Counting Sort is said to only work for a limited range of non-negative integer values:</p>
<ul>
  <li><strong>Integer values: </strong>Counting Sort relies on counting occurrences of distinct values, so they must be integers. With integers, each value fits with an index (for non negative values), and there is a limited number of different values, so that the number of possible different values \(k\) is not too big compared to the number of values \(n\).</li>
  <li><strong>Non negative values: </strong>Counting Sort is usually implemented by creating an array for counting. When the algorithm goes through the values to be sorted, value x is counted by increasing the counting array value at index x. If we tried sorting negative values, we would get in trouble with sorting value -3, because index -3 would be outside the counting array.</li>
  <li><strong>Limited range of values: </strong>If the number of possible different values to be sorted \(k\) is larger than the number of values to be sorted \(n\), the counting array we need for sorting will be larger than the original array we have that needs sorting, and the algorithm becomes ineffective.</li>
</ul>
<hr>

<h2>Manual Run Through</h2>
<p>Before we implement the Counting Sort algorithm in a programming language, let's manually run through a short array, just to get the idea.</p>
<p><strong>Step 1:</strong> We start with an unsorted array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [ 2, 3, 0, 2, 3, 2]</code>
</div>
<hr>
<p><strong>Step 2:</strong> We create another array for counting how many there are of each value. The array has 4 elements, to hold values 0 through 3.</p>
<div class="w3-code notranslate">
<code class="language-python w3-white">myArray = [ 2, 3, 0, 2, 3, 2]</code><br>
  <code class="language-python w3-white">countArray = [ 0, 0, 0, 0]</code>
</div>
<hr>
<p><strong>Step 3:</strong> Now let's start counting. The first element is 2, so we must increment the counting array element at index 2.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [<mark class="dsa_mark"> 2</mark>, 3, 0, 2, 3, 2]</code><br>
  <code class="language-python w3-white">countArray = [ 0, 0,<mark class="dsa_mark"> 1</mark>, 0]</code>
</div>
<hr>
<p><strong>Step 4:</strong> After counting a value, we can remove it, and count the next value, which is 3.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [<mark class="dsa_mark"> 3</mark>, 0, 2, 3, 2]</code><br>
  <code class="language-python w3-white">countArray = [ 0, 0, 1,<mark class="dsa_mark"> 1</mark>]</code>
</div>
<hr>
<p><strong>Step 5:</strong> The next value we count is 0, so we increment index 0 in the counting array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [<mark class="dsa_mark"> 0</mark>, 2, 3, 2]</code><br>
  <code class="language-python w3-white">countArray = [<mark class="dsa_mark"> 1</mark>, 0, 1, 1]</code>
</div>
<hr>
<p><strong>Step 6:</strong> We continue like this until all values are counted.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [ ]</code><br>
  <code class="language-python w3-white">countArray = [<mark class="dsa_mark"> 1, 0, 3, 2</mark>]</code>
</div>
<hr>
<p><strong>Step 7:</strong> Now we will recreate the elements from the initial array, and we will do it so that the elements are ordered lowest to highest.</p>
<p>The first element in the counting array tells us that we have 1 element with value 0. So we push 1 element with value 0 into the array, and we decrease the element at index 0 in the counting array with 1.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [<mark class="dsa_mark"> 0</mark>]</code><br>
  <code class="language-python w3-white">countArray = [<mark class="dsa_mark"> 0</mark>, 0, 3, 2]</code>
</div>
<hr>
<p><strong>Step 8:</strong> From the counting array we see that we do not need to create any elements with value 1.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [ 0]</code><br>
  <code class="language-python w3-white">countArray = [ 0,<mark class="dsa_mark"> 0</mark>, 3, 2]</code>
</div>
<hr>
<p><strong>Step 9:</strong> We push 3 elements with value 2 into the end of the array. And as we create these elements we also decrease the counting array at index 2.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [ 0,<mark class="dsa_mark"> 2, 2, 2</mark>]</code><br>
  <code class="language-python w3-white">countArray = [ 0, 0,<mark class="dsa_mark"> 0</mark>, 2]</code>
</div>
<hr>
<p><strong>Step 10:</strong> At last we must add 2 elements with value 3 at the end of the array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [0, 2, 2, 2,<mark class="dsa_mark"> 3, 3</mark>]</code><br>
  <code class="language-python w3-white">countArray = [ 0, 0, 0,<mark class="dsa_mark"> 0</mark>]</code>
</div>
...
<div id="vueApp3">
  ...
  <div id="frameDiv3">
    myArray = 
    [<div class="diceDiv3" id="ghostDiv3"></div>
    <transition-group name="v3">
      <div v-for="(x, index) in myArray" 
        :key="x.keyNmbr" 
        class="diceDiv3"
        :class="{ 
          'diceDiv3-active': x.isActive
        }" 
        :style="{
          transition: 'all ' + delay
        }"
      >
      {{ x.dieNmbr }}<template v-if="index < myArray.length - 1">,</template>
      </div>
      <div class="diceDiv3" :key="-1000">]</div>
    </transition-group>
    <br>
    countArray = 
    [
    <transition-group name="v3">
      <div v-for="(x, index) in countArray" 
        :key="1000+x.keyNmbr" 
        class="diceDiv3"
        :class="{ 
          'diceDiv3-active': x.isActive
      ...
      >
      {{ x.dieNmbr }}<template v-if="index < countArray.length - 1">,</template>
      </div>
    </transition-group>
    ]
  </div>
  ...
  methods: {
    ...
    updateMoveDuration() {
      ...
      for (let i = 0; i < stylesheets.length; i++) {
        let rules = stylesheets[i].cssRules || stylesheets[i].rules;
        for (let j = 0; j < rules.length; j++) {
          if (rules[j].selectorText === '.v3-move') {
            rules[j].style.transitionDuration = this.delay + 'ms';
            break;
          }
        }
    ...
    async countSort() {
      ...
      while (this.myArray.length > 0) {
        let value = this.myArray[0].dieNmbr;
        this.myArray[0].isActive = true;
        this.countArray[value].isActive = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        this.myArray.splice(0,1);
        this.countArray[value].dieNmbr++;
        this.countArray[value].isActive = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        this.countArray[value].isActive = false;
        this.removeActive();
      }
      await new Promise(resolve => setTimeout(resolve, this.delay));
      await new Promise(resolve => setTimeout(resolve, this.delay));
      for(let i=0; i<this.countArray.length; i++){
        this.countArray[i].isActive = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        while(this.countArray[i].dieNmbr>0){
          const newDie = {
            dieNmbr: i,
            keyNmbr: this.keyNumber,
            isActive: true
          ...
          this.countArray[i].dieNmbr--;
          await new Promise(resolve => setTimeout(resolve, this.delay));
          await new Promise(resolve => setTimeout(resolve, this.delay));
          this.myArray[this.myArray.length-1].isActive = false;
          this.keyNumber++;
        }
        this.countArray[i].isActive = false;
        await new Promise(resolve => setTimeout(resolve, this.delay));
      }
      this.msgDone = 'Done!';
    ...
    addDie50() {
      ...
      let myCount_array = [0,0,0,0];
      for(let i=0; i<myCount_array.length; i++){
        const newDie = {
          dieNmbr: myCount_array[i],
          keyNmbr: this.keyNumber,
          isActive: false
        };
        this.countArray.push(newDie);
        this.keyNumber++;
      }
      this.buttonText = "Counting Sort";
      this.msgDone = '';
...
<p>Before we implement the algorithm in a programming language we need to go through what happened above in more detail.</p>
<p>We have seen that the Counting Sort algorithm works in two steps:</p>
<ol>
  <li>Each value gets counted by incrementing at the correct index in the counting array. After a value is counted, it is removed.</li>
  <li>The values are recreated in the right order by using the count, and the index of the count, from the counting array.</li>
</ol>
<p>With this in mind, we can start implementing the algorithm using Python.</p>
<hr>

<h2>Counting Sort Implementation</h2>
<p>To implement the Counting Sort algorithm in a programming language, we need:</p>
<ol>
  <li>An array with values to sort.</li>
  <li>A 'countingSort' method that receives an array of integers.</li>
  <li>An array inside the method to keep count of the values.</li>
  <li>A loop inside the method that counts and removes values, by incrementing elements in the counting array.</li>
  <li>A loop inside the method that recreates the array by using the counting array, so that the elements appear in the right order.</li>
</ol>
...
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white"><code>def countingSort(arr):
    if not arr:
        return arr
...
print("Sorted array:", sortedArr)
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_countingsort" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Counting Sort Time Complexity</h2>
...
<p class="fontComment">For a more thorough and detailed explanation of Counting Sort time complexity, visit <a href="dsa_timecomplexity_countsort.php">this page</a>.</p>

<p>How fast the Counting Sort algorithm runs depends on both the range of possible values \(k\) and the number of values \(n\).</p>
<p>In general, time complexity for Counting Sort is \(O(n+k)\).</p>
<p>In a best case scenario, the range of possible different values \(k\) is very small compared to the number of values \(n\) and Counting Sort has time complexity \(O(n)\).</p>
<p>But in a worst case scenario, the range of possible different values \(k\) is very big compared to the number of values \(n\) and Counting Sort can have time complexity \(O(n^2)\) or even worse.</p>
<p>The plot below shows how much the time complexity for Counting Sort can vary.</p>
<img src="img_runtime_countingSort.png" class="imgTut" alt="Time Complexity">
<p>As you can see, it is important to consider the range of values compared to the number of values to be sorted before choosing Counting Sort as your algorithm. Also, as mentioned at the top of the page, keep in mind that Counting Sort only works for non negative integer values.</p>

<p>Run different simulations of Counting Sort to see how the number of operations falls between the worst case scenario \(O(n^2)\) (red line) and best case scenario \(O(n)\) (green line).</p>
<div id="vueApp4">
  <div class="frameDivOuter">
    <div id="leftSpace"></div>
    <div id="inpValues">
      <p><label>Set values (n): <input type="range" v-model="userX" min="100" max="480" :disabled="isRangeDisabled"></label> {{ this.userX }}</p>
      <p><label>Range (k), from 0 to: <input type="range" v-model="userK" min="100" max="1500" :disabled="isLoading"></label> {{ this.userK }}</p>
    </div>
  </div>
  <div class="frameDivOuter">
    <div id="inpDiv">
      ...
      <div id="graphDiv">
        <transition-group name="miniSort">
          <div v-for="x in array"
            :key="keyNmbr"
            :style="{
              height: Math.floor(x.value/15) + 'px',
  ...
  methods: {
    newArray(){
      ...
      for (let k = 0; k < this.userX-1; k++) {
        let randNumber = Math.ceil(Math.random() * this.userK);
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performCountingSort();
    ...
    async performCountingSort() {
      this.operations = 0;
      
      // Find the maximum value in the array
      let max_val = Math.max(...this.array.map(item => item.value));
      let count = Array(max_val + 1).fill(0);
      this.operations += max_val + 1;
      let len = this.array.length;
      this.operations += len;
      // Count occurrences of each element
      for(let i = len-1; i >= 0; i--) {
        let num = this.array[i].value;
...
<p>As mentioned previously: if the numbers to be sorted varies a lot in value (large \(k\)), and there are few numbers to sort (small \(n\)), the Counting Sort algorithm is not effective.</p>
<p>If we hold \(n\) and \(k\) fixed, the "Random", "Descending" and "Ascending" alternatives in the simulation above results in the same number of operations. This is because the same thing happens in all three cases: A counting array is set up, the numbers are counted, and the new sorted array is created.</p>
<hr>

<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_algo_countingsort1" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>Using Counting Sort on this array:</p>
<code>[1,0,5,3,3,1,3,3,4,4]</code>
<p>How does the counting array look like?</p>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">
[<input name="ex1" maxlength="1" style="width: 2ch;">,<input name="ex2" maxlength="1" style="width: 2ch;">,<input name="ex3" maxlength="1" style="width: 2ch;">,4,2,1]
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_algo_countingsort1">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
