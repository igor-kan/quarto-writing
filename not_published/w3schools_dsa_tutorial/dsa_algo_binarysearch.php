...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Binary Search</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_linearsearch.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_theory_linkedlists.php">Next &#10095;</a>
</div>
<hr>

<h2>Binary Search</h2>
<p>The Binary Search algorithm searches through an array and returns the index of the value it searches for.</p>
<div id="vueApp">
  <p>Speed: <input v-model="inpVal" type="range" min="150" max="500"><!-- {{ delay }}--></p>
...
const app = Vue.createApp({
  ...
  methods: {
    action(){
      if(this.buttonText === "Binary Search") {
        this.binarySearch();
      }
      else {
        this.addDie50();
...
<p>Let's try to do the searching manually, just to get an even better understanding of how Binary Search works before actually implementing it in a programming language. We will search for value 11.</p>
...
<p>To start with, the algorithm has two variables "left" and "right".</p>
<p>"left" is 0 and represents the index of the first value in the array, and "right" is 6 and represents the index of the last value in the array.</p>
<p>\((left+right)/2=(0+6)/2=3\) is the first index used to check if the middle value (7) is equal to the target value (11).</p>
<p>7 is lower than the target value 11, so in the next loop the search area must be limited to the right side of the middle value: [ 11, 15, 25], on index 4-6.</p>
<p>To limit the search area and find a new middle value, "left" is updated to index 4, "right" is still 6. 4 and 6 are the indexes for the first and last values in the new search area, the right side of the previous middle value. The new middle value index is \((left+right)/2=(4+6)/2=10/2=5\).</p>
<p>The new middle value on index 5 is checked: 15 is higher than 11, so if the target value 11 exists in the array it must be on the left side of index 5. The new search area is created by updating "right" from 6 to 4. Now both "left" and "right" is 4, \((left+right)/2=(4+4)/2=4\), so there is only index 4 left to check. The target value 11 is found at index 4, so index 4 is returned.</p>
<p>In general, this is the way the Binary Search algorithm continues to halve the array search area until the target value is found.</p>
<p>When the target value is found, the index of the target value is returned. If the target value is not found, -1 is returned.</p>
<hr>
...
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white"><code>def binarySearch(arr, targetVal):
    left = 0
    right = len(arr) - 1
...
if result != -1:
    ...
    print("Target not found in array.")</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_binarysearch" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>
...
<p class="fontComment">For a more thorough and detailed explanation of Insertion Sort time complexity, visit <a href="dsa_timecomplexity_binarysearch.php">this page</a>.</p>
...
<p>Time complexity for Binary Search is</p>
<p><span class="math display">\[ O( \log_{2} n ) \]</span></p>
<div class="ws-note">
...
<img src="img_binarysearch_timecomplexity.png" class="imgTut" alt="Binary Search Time Complexity">

<p>Run the Binary Search simulation below for different number of values \(n\) in an array, and see how many compares are needed for Binary Search to find the target value:</p>
<div id="vueApp4">
  ...
  <div class="frameDivOuter">
    <div id="inpDiv">
      ...
      <p><label><input type="radio" value="random30" v-model="selectedRunType" :disabled="isLoading"> 10 Ascending</label></p>
      <p style="opacity: 0;"><label><input type="radio" value="single" v-model="selectedRunType" disabled> Random</label></p>
      <p style="opacity: 0;"><label><input type="radio" value="worstCase" v-model="selectedRunType" disabled> Descending</label></p>
    </div>
    <div id="frameDiv9">
      <div id="graphDiv">
...
<script>
  ...
  for (let x = 0; x <= 500; x += 10) {
    curve.push({ x: x, y: 1.1*Math.log2(x) });
  }
  let ctx = null;
  let myChart = null;
  ...
  methods: {
    newArray(){
      ...
      if(Math.random()>0.9){
        ...
        this.array[Math.floor(Math.random() * this.userX)] = newObj;
      }
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performBinarySearch();
      if(this.notFound2){
    ...
    async performBinarySearch() {
      ...
      while (left <= right) {
        ...
        // Highlight the active (middle) element
        this.array[mid].active = true;
        await new Promise(resolve => setTimeout(resolve, 200));
        

        if (this.array[mid].value === this.targetVal) {
          // this.operations++; // Increment for successful comparison
          foundIndex = mid;
          this.array[mid].active = true;
          await new Promise(resolve => setTimeout(resolve, 200));
          let kk = 0;
          while(kk < this.array.length){
            if(kk !== foundIndex){
              this.array[kk].done = true;
            }
            kk++;
          }
          break; // Exit the loop since we've found the target
        } else if (this.array[mid].value < this.targetVal) {
          // Disregard values to the left
          left = mid + 1;
          for(let ll=0; ll<mid; ll++){
            this.array[ll].done = true;
          }
        } else {
          // Disregard values to the right
          right = mid - 1;
          for(let ll=this.array.length-1; ll>mid; ll--){
            this.array[ll].done = true;
          }
        }
        await new Promise(resolve => setTimeout(resolve, 200));
        // Remove the highlight from the active element
        this.array[mid].active = false;
      }
      ...
      // Return the number of operations. If it's -1, the target was not found.
...
<p>As you can see when running simulations of Binary Search, the search requires very few compares, even if the the array is big and the value we are looking for is not found.</p>
<hr>

<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_algo_binarysearch1" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>What kind of array?</p>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">
For the Binary Search algorithm to work,
the array must already be <input name="ex1" maxlength="6" style="width: 7ch;">.
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_algo_binarysearch1">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
