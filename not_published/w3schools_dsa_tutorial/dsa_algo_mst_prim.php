...
<html lang="en-US">
<head>
<title>DSA Prim's Algorithm</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Prim's Algorithm</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_theory_mst_minspantree.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_mst_kruskal.php">Next &#10095;</a>
</div>
<div class="ws-info">
  <p>Prim's algorithm was invented in 1930 by the Czech mathematician Vojtěch Jarník.</p>
  <p>The algorithm was then rediscovered by Robert C. Prim in 1957, and also rediscovered by Edsger W. Dijkstra in 1959. Therefore, the algorithm is also sometimes called "Jarník's algorithm", or the "Prim-Jarník algorithm".</p>
</div>

<h2>Prim's Algorithm</h2>
<p>Prim's algorithm finds the Minimum Spanning Tree (MST) in a connected and undirected graph.</p>
<div id="vueApp">
    ...
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" class="dsa_svg">
      <transition-group>
...
<script>
  ...
  methods: {
    ...
    async spreadOut() {
      ...
      for (let i = 0; i < this.dice.length; i++) {
        ...
        do {
          ...
          const newX = Math.ceil(Math.random() * 360) + 20;
    ...
    async runPrimsAlgorithm() {
      await new Promise(resolve => setTimeout(resolve, 500));
      const numVertices = this.dice.length;
      const inMST = new Array(numVertices).fill(false);
      const keyValues = new Array(numVertices).fill(Infinity);
      const parents = new Array(numVertices).fill(-1);
      keyValues[0] = 0; // Start from the first vertex
      this.dice[0].isCurrent = true; // Initially mark the first vertex as current
      
      for (let i = 0; i < numVertices; i++) {
        let u = this.findMinKeyVertex(keyValues, inMST);
        inMST[u] = true;
        
        // Before exploring new edges, reset the current vertex marker
        this.dice.forEach(vertex => {
          vertex.isCurrent = false;
        });
        
        // Mark the new current vertex
        this.dice[u].isCurrent = true;

        for (let v = 0; v < numVertices; v++) {
          if (this.adjacencyMatrix[u][v] && !inMST[v] && this.adjacencyMatrix[u][v] < keyValues[v]) {
            parents[v] = u;
            keyValues[v] = this.adjacencyMatrix[u][v];
          }
        }

        // Add delay here for visual effect
        await this.delayMS(500); // Adjust delay time as needed
        this.updateMSTEdges(parents);
        this.dice[u].isFinished = true;
        await this.delayMS(500); // Additional delay to visualize the current vertex change
      }

      // Reset the current vertex marker after completing the algorithm
      this.dice.forEach(vertex => {
        vertex.isCurrent = false;
      });
      this.dice.forEach(vertex => {
    ...
    updateMSTEdges(parents) {
      this.mstEdges = [];
      for (let i = 1; i < parents.length; i++) { // Skip the first vertex
        if (parents[i] !== -1) {
          const edge = {
            from: parents[i],
            to: i,
...
<p>The MST found by Prim's algorithm is the collection of edges in a graph, that connects all vertices, with a minimum sum of edge weights.</p>
<p>Prim's algorithm finds the MST by first including a random vertex to the MST. The algorithm then finds the vertex with the lowest edge weight from the current MST, and includes that to the MST. Prim's algorithm keeps doing this until all nodes are included in the MST.</p>
<p>Prim's algorithm is greedy, and has a straightforward way to create a minimum spanning tree.</p>
<p>For Prim's algorithm to work, all the nodes must be connected. To find the MST's in an unconnected graph, <a href="dsa_algo_mst_kruskal.php">Kruskal's algorithm</a> can be used instead. You can read about Kruskal's algorithm on the next page.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Choose a random vertex as the starting point, and include it as the first vertex in the MST.</li>
    <li>Compare the edges going out from the MST. Choose the edge with the lowest weight that connects a vertex among the MST vertices to a vertex outside the MST.</li>
    <li>Add that edge and vertex to the MST.</li>
    <li>Keep doing step 2 and 3 until all vertices belong to the MST.</li>
  </ol>
</div>
<div class="ws-note">
  <p><strong>NOTE: </strong>Since the starting vertex is chosen at random, it is possible to have different edges included in the MST for the same graph, but the total edge weight of the MST will still have the same minimum value.</p>
</div>
...
<p>Let's run through Prim's algorithm manually on the graph below, so that we understand the detailed step-by-step operations before we try to program it.</p>
<p>Prim's algorithm starts growing the Minimum Spanning Tree (MST) from a random vertex, but for this demonstration vertex A is chosen as the starting vertex.</p>
<div id="vueApp_still1" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
      <transition-group>
        <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges" />
        <line v-for="edge in mstEdges" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
          :x2="dice[edge.to].xpos" :y2="dice[edge.to].ypos"
          :key="`mst-${edge.from}-${edge.to}`" class="mstEdges"/>
        <text v-for="edge in edges" 
          :key="`text-${edge.key}`" 
          :x="edge.wPos[0]+edge.offset[0]" 
          :y="edge.wPos[1]+edge.offset[1]" 
          text-anchor="middle" 
          class="svgText">
            {{ edge.weight }}
        </text>
        ...
        />
        <text v-for="el in dice"
          :key="el.keyNmbr*1000"
          :x="el.xpos-10+el.offset[0]"
          :y="el.ypos+23+el.offset[1]"
          class="svgText">
          {{el.name}}
        </text>
      </transition-group>
...
<script>
  const app_still1 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
      ...
      buttonText: 'New graph',
      adjacencyMatrix: [
        [Infinity,       4,Infinity,       3,Infinity,Infinity,Infinity,Infinity],
        [       4,Infinity,       3,       5,       6,Infinity,Infinity,Infinity],
        [Infinity,       3,Infinity,Infinity,       4,Infinity,Infinity,       2],
        [       3,       5,Infinity,Infinity,       7,       4,Infinity,Infinity],
        [Infinity,       6,       4,       7,Infinity,       5,       3,Infinity],
        [Infinity,Infinity,Infinity,       4,       5,Infinity,       7,Infinity],
        [Infinity,Infinity,Infinity,Infinity,       3,       7,Infinity,       5],
        [Infinity,Infinity,       2,Infinity,       3,       7,       5,Infinity]
      ]
    }
  },
  methods: {
    ...
    addDie(name, x, y, offs) {
      ...
      this.dice.push(newDie);
      this.dice[0].isCurrent = true;
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 0, to: 3});
      this.keyNumber++;
    },
    addDie50() {
      const vertices = [
        {"name": "A", "x": 0.4, "y": 3, "offset": [0,0]},
        {"name": "B", "x": 3.6, "y": 0.6, "offset": [-12,-20]},
        {"name": "C", "x": 6.6, "y": 0.2, "offset": [0,0]},
        {"name": "D", "x": 2.2, "y": 5.4, "offset": [0,0]},
        {"name": "E", "x": 8.9, "y": 3.4, "offset": [12,0]},
        {"name": "F", "x": 5.7, "y": 7.3, "offset": [0,0]},
        {"name": "G", "x": 11.9, "y": 3.9, "offset": [0,0]},
        {"name": "H", "x": 8.6, "y": 0.1, "offset": [0,0]}
      ];
      this.dice = [];
      for (let i = 0; i < vertices.length; i++) { //18
        this.addDie(vertices[i].name, vertices[i].x, vertices[i].y, vertices[i].offset); 
      }
      //this.buttonText = "Run Prim's";
      //this.msgDone = 'Done!';
      this.spreadOut();
        ...
        for (let i = 0; i < this.adjacencyMatrix.length; i++) {
            for (let j = i + 1; j < this.adjacencyMatrix[i].length; j++) {
                if (this.adjacencyMatrix[i][j] !== Infinity) {
                    ...
                    const midpointX = (this.dice[i].xpos + this.dice[j].xpos) / 2;
                    const midpointY = (this.dice[i].ypos + this.dice[j].ypos) / 2;
                    this.edges.push({
                        x1: this.dice[i].xpos,
                        y1: this.dice[i].ypos,
                        x2: this.dice[j].xpos,
                        ...
                        wPos: [midpointX,midpointY],
                        offset: [0,0]
                    });
                }
            }
        }
        // move text out from edges
        this.edges[0].offset = [-10,0];    // A-B
        this.edges[1].offset = [-10,10];   // A-C
        this.edges[2].offset = [0,-5];   // B-C
        this.edges[3].offset = [10,0];   // B-D
        this.edges[4].offset = [5,-3];   // B-E
        this.edges[5].offset = [7,0];   // C-E
        this.edges[6].offset = [0,-5];   // C-H
        this.edges[7].offset = [0,-4];  // D-E
        this.edges[8].offset = [9,-1];   // D-F
        this.edges[9].offset = [-5,-5];   // E-F
        this.edges[10].offset = [7,17];   // E-G
        this.edges[11].offset = [10,14];   // F-G
        this.edges[12].offset = [10,0];   // G-H
        this.buttonText = "Run Prim's";
        this.btnIsDisabled = false;
    },
  },
  ...
  app_still1.mount('#vueApp_still1')
</script>
<p>From vertex A, the MST grows along the edge with the lowest weight. So vertices A and D now belong to the group of vertices that belong to the Minimum Spanning Tree.</p>
<div id="vueApp_still2" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
...
<script>
  const app_still2 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
  ...
  methods: {
    ...
    addDie50() {
      ...
      //this.buttonText = "Run Prim's";
      //this.msgDone = 'Done!';
      this.dice[0].isFinished = true;
      this.dice[3].isCurrent = true;
      this.mstEdges.push({from: 0, to: 3});
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 3, to: 4});
      this.mstEdges.push({from: 3, to: 5});
      this.spreadOut();
    },
    updateEdgesFromMatrix() {
        this.edges = [];
  ...
  app_still2.mount('#vueApp_still2')
</script>
<p>A <code class="w3-codespan">parents</code> array is central to how Prim's algorithm grows the edges in the MST.</p>
<p>At this point, the <code class="w3-codespan">parents</code> array looks like this:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white"><code>parents = [-1,  0, -1,  0,  3,  3, -1, -1]
#vertices [ A,  B,  C,  D,  E,  F,  G,  H]
</code></pre>
</div>
<p>Vertex A, the starting vertex, has no parent, and has therefore value <code class="w3-codespan">-1</code>. Vertex D's parent is A, that is why D's parent value is <code class="w3-codespan">0</code> (vertex A is located at index 0). B's parent is also A, and D is the parent of E and F.</p>
<p>The <code class="w3-codespan">parents</code> array helps us to keep the MST tree structure (a vertex can only have one parent).</p>
<p>Also, to avoid cycles and to keep track of which vertices are currently in the MST, the <code class="w3-codespan">in_mst</code> array is used.</p>
<p>The <code class="w3-codespan">in_mst</code> array currently looks like this:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white"><code> in_mst = [ true, false, false,  true, false, false, false, false]
#vertices [    A,     B,     C,     D,     E,     F,     G,     H]
</code></pre>
</div>
<p>The next step in Prim's algorithm is to include one more vertex as part of the MST, and the vertex closest to the current MST nodes A and D is chosen.</p>
<p>Since both A-B and D-F have the same lowest edge weight <code class="w3-codespan">4</code>, either B or F can be chosen as the next MST vertex. We choose B as the next MST vertex for this demonstration.</p>
<div id="vueApp_still3" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
...
<script>
  const app_still3 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
  ...
  methods: {
    ...
    addDie50() {
      ...
      //this.msgDone = 'Done!';
      this.dice[0].isFinished = true;
      this.dice[3].isFinished = true;
      this.dice[1].isCurrent = true;
      this.mstEdges.push({from: 0, to: 3});
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 1, to: 4});
      this.mstEdges.push({from: 1, to: 2});
      this.mstEdges.push({from: 3, to: 5});
      this.spreadOut();
    },
    updateEdgesFromMatrix() {
  ...
  app_still3.mount('#vueApp_still3')
</script>
<p>As you can see, the MST edge to E came from vertex D before, now it comes from vertex B, because B-E with weight <code class="w3-codespan">6</code> is lower than D-E with weight <code class="w3-codespan">7</code>. Vertex E can only have one parent in the MST tree structure (and in the <code class="w3-codespan">parents</code> array), so B-E and D-E cannot both be MST edges to E.</p>
<p>The next vertex in the MST is vertex C, because edge B-C with weight <code class="w3-codespan">3</code> is the shortest edge weight from the current MST vertices.</p>
<div id="vueApp_still4" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
...
<script>
  const app_still4 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
  ...
  methods: {
    ...
    addDie50() {
      ...
      this.dice[0].isFinished = true;
      this.dice[3].isFinished = true;
      this.dice[1].isFinished = true;
      this.dice[2].isCurrent = true;
      this.mstEdges.push({from: 0, to: 3});
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 2, to: 4});
      this.mstEdges.push({from: 1, to: 2});
      this.mstEdges.push({from: 3, to: 5});
      this.mstEdges.push({from: 2, to: 7});
      this.spreadOut();
    },
    updateEdgesFromMatrix() {
        this.edges = [];
  ...
  app_still4.mount('#vueApp_still4')
</script>
<p>As vertex C is included in the MST, edges out from C are checked to see if there are edges with a lower weight from this MST vertex to vertices outside the MST. Edge C-E has a lower weight (<code class="w3-codespan">3</code>) than the previous B-E MST edge (<code class="w3-codespan">6</code>), and the C-H edge gets included in the MST with edge weight <code class="w3-codespan">2</code>.</p>
<p>Vertex H is the next to be included in the MST, as it has the lowest edge weight <code class="w3-codespan">6</code>, and vertex H becomes the parent of vertex G in the <code class="w3-codespan">parents</code> array.</p>
<div id="vueApp_still5" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
...
<script>
  const app_still5 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
  ...
  methods: {
    ...
    addDie50() {
      ...
      this.dice[3].isFinished = true;
      this.dice[1].isFinished = true;
      this.dice[2].isFinished = true;
      this.dice[7].isCurrent = true;
      this.mstEdges.push({from: 0, to: 3});
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 2, to: 4});
      this.mstEdges.push({from: 1, to: 2});
      this.mstEdges.push({from: 3, to: 5});
      this.mstEdges.push({from: 2, to: 7});
      this.mstEdges.push({from: 6, to: 7});
      this.spreadOut();
    },
    updateEdgesFromMatrix() {
        this.edges = [];
  ...
  app_still5.mount('#vueApp_still5')
</script>
<p>The next vertex to be included in the MST is either E or F because they have both the lowest edge weight to them: <code class="w3-codespan">4</code>.</p>
<p>We choose vertex E as the next vertex to be included in the MST for this demonstration.</p>
<div id="vueApp_still6" class="vueApp_still">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 340" class="dsa_svg">
...
<script>
  const app_still6 = Vue.createApp({
    data() {
    return {
      btnIsDisabled: true,
      msgDone: '',
  ...
  methods: {
    ...
    addDie50() {
      ...
      this.dice[1].isFinished = true;
      this.dice[2].isFinished = true;
      this.dice[7].isFinished = true;
      this.dice[4].isCurrent = true;
      this.mstEdges.push({from: 0, to: 3});
      this.mstEdges.push({from: 0, to: 1});
      this.mstEdges.push({from: 2, to: 4});
      this.mstEdges.push({from: 1, to: 2});
      this.mstEdges.push({from: 3, to: 5});
      this.mstEdges.push({from: 4, to: 6});
      this.mstEdges.push({from: 2, to: 7});
      this.spreadOut();
    },
    updateEdgesFromMatrix() {
  ...
  app_still6.mount('#vueApp_still6')
</script>
<p>The next and last two vertices to be added to the MST are vertices F and G. D-F is the MST edge to F, and E-G is the MST edge to G because these edges are the edges with the lowest weight from the current MST.</p>
<p>Run the simulation below to see Prim's algorithm doing the manual steps that we have just done.</p>
<div id="vueApp2">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 350" class="dsa_svg">
      <transition-group>
      <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges" />
        <line v-for="edge in mstEdges" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
  ...
  const app2 = Vue.createApp({
    ...
    return {
      ...
      adjacencyMatrix: [
        ...
        [Infinity,Infinity,Infinity,       4,       5,Infinity,       7,Infinity],
        [Infinity,Infinity,Infinity,Infinity,       3,       7,Infinity,       5],
        [Infinity,Infinity,       2,Infinity,Infinity,Infinity,       5,Infinity]
      ]
    }
  },
  computed: {
...
<a id="dijkstraImpl"></a>
<h2>Implementation of Prim's Algorithm</h2>
<p>For Prim's algorithm to find a Minimum Spanning Tree (MST), we create a <code class="w3-codespan">Graph</code> class. We will use the methods inside this <code class="w3-codespan">Graph</code> class later to create the graph from the example above, and to run Prim's algorithm on it.</p>
<div class="w3-example">
...
<p><strong>Line 3-5: </strong>At first, the adjacency matrix is empty, meaning there are no edges in the graph. Also, the vertices have no names to start with.</p>
<p><strong>Line 7-10: </strong>The <code class="w3-codespan">add_edge</code> method is for adding an edge, with an edge weight value, to the undirected graph.</p>
<p><strong>Line 12-14: </strong>The <code class="w3-codespan">add_vertex_data</code> method is used for giving names to the vertices, like for example 'A' or 'B'.</p>
<p>Now that the structure for creating a graph is in place, we can implement Prim's algorithm as a method inside the <code class="w3-codespan">Graph</code> class:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="16" data-line-offset="16" data-line="17-19,21,25,32-35"><code>    def prims_algorithm(self):
        ...
        for _ in range(self.size):
            ...
            for v in range(self.size):
                if 0 < self.adj_matrix[u][v] < key_values[v] and not in_mst[v]:
                    ...
                    parents[v] = u</code></pre>
</div>
<p><strong>Line 17: </strong>The <code class="w3-codespan">in_mst</code> array holds the status of which vertices are currently in the MST. Initially, none of the vertices are part of the MST.</p>
<p><strong>Line 18: </strong>The <code class="w3-codespan">key_values</code> array holds the current shortest distance from the MST vertices to each vertex outside the MST.</p>
<p><strong>Line 19: </strong>The MST edges are stored in the <code class="w3-codespan">parents</code> array. Each MST edge is stored by storing the parent index for each vertex.</p>
<p><strong>Line 21: </strong>To keep it simple, and to make this code run like in the "Manual Run Through" animation/example above, the first vertex (vertex A at index <code class="w3-codespan">0</code>) is set as the staring vertex. Changing the index to <code class="w3-codespan">4</code> will run Prim's algorithm from vertex E, and that works just as well.</p>
<p><strong>Line 25: </strong>The index is found for the vertex with the lowest key value that is not yet part of the MST. Check out these explanations for <code class="w3-codespan"><a href="/python/ref_func_min.asp">min</a></code> and <code class="w3-codespan"><a href="/python/python_lambda.asp">lambda</a></code> to better understand this Python code line.</p>
<p><strong>Line 32-35: </strong>After a new vertex is added to the MST (line 27), this part of the code checks to see if there are now edges from this newly added MST vertex that can lower the key values to other vertices outside the MST. If that is the case, the <code class="w3-codespan">key_values</code> and <code class="w3-codespan">parents</code> arrays are updated accordingly. This can be seen clearly in the animation when a new vertex is added to the MST and becomes the active (current) vertex.</p>
<p>Now let's create the graph from the "Manual Run Through" above and run Prim's algorithm on it:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
<pre class="language-python notranslate w3-white line-numbers" data-line="32" style="height: 400px; overflow: auto;"><code>class Graph:
    def __init__(self, size):
        self.adj_matrix = [[0] * size for _ in range(size)]
        self.size = size
        self.vertex_data = [''] * size
...
g.prims_algorithm()</code></pre>
<a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_mst_prim" target="_blank">Try it Yourself &raquo;</a>
</div>
<div class="ws-note">
  <p><strong>Line 32: </strong>We can actually avoid the last loop in Prim's algorithm by changing this line to <code class="w3-codespan">for _ in range(self.size - 1):</code>. This is because when there is just one vertex not yet in the MST, the parent vertex for that vertex is already set correctly in the <code class="w3-codespan">parents</code> array, so the MST is actually already found at this point.</p>
</div>
<hr>

<script src="https://cdnjs.cloudflare.com/polyfill/v3/polyfill.min.js?features=es6"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.7/MathJax.js?config=TeX-MML-AM_CHTML"></script>
<h2>Time Complexity for Prim's Algorithm</h2>
<p style="font-style: oblique;">For a general explanation of what time complexity is, visit <a href="dsa_timecomplexity_theory.php">this page</a>.</p>
<p>With \(V\) as the number of vertices in our graph, the time complexity for Prim's algorithm is</p>
<p><span class="math display">\[ O( V^2 ) \]</span></p>
<p>The reason why we get this time complexity is because of the nested loops inside the Prim's algorithm (one for-loop with two other for-loops inside it).</p>
<p>The first for-loop (line 24) goes through all the vertices in the graph. This has time complexity \(O(V)\).</p>
<p>The second for-loop (line 25) goes through all the adjacent vertices in the graph to find the vertex with the lowest key value that is outside the MST, so that it can be the next vertex included in the MST. This has time complexity \(O(V)\).</p>
<p>After a new vertex is included in the MST, a third for-loop (line 32) checks all other vertices to see if there are outgoing edges from the newly added MST vertex to vertices outside the MST that can lead to lower key values and updated parent relations. This also has time complexity \(O(V)\).</p>
<p>Putting the time complexities together we get:</p>
...
\end{equation}
\]</span></p>
<p>By using a priority queue data structure to manage key values, instead of using an array like we do here, the time complexity for Prim's algorithm can be reduced to:</p>
<p><span class="math display">\[ O( E \cdot \log{V}) \]</span></p>
<p>Where \(E\) is the number of edges in the graph, and \(V\) is the number of vertices.</p>
<p>Such an implementation of Prim's algorithm using a priority queue is best for sparse graphs. A graph is sparse when the each vertex is just connected to a few of the other vertices.</p>
<hr>
