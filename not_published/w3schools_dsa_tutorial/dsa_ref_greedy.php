...
<html lang="en-US">
<head>
<title>DSA Greedy Algorithms</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1><span class="color_h1">DSA Greedy Algorithms</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_ref_dynamic_programming.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_examples.php">Next &#10095;</a>
</div>
<hr>

<h2>Greedy Algorithms</h2>
<p>A greedy algorithm decides what to do in each step, only based on the current situation, without a thought of how the total problem looks like.</p>
<p>In other words, a greedy algorithm makes the locally optimal choice in each step, hoping to find the global optimum solution in the end.</p>
<p>In <a href="dsa_algo_graphs_dijkstra.php">Dijkstra's algorithm</a> for example, the next vertex to be visited is always the next unvisited vertex with the currently shortest distance from the source, as seen from the current group of visited vertices.</p>
<div id="vueApp">
...
<p>So Dijkstra's algorithm is greedy because the choice of which vertex to visit next is only based on the currently available information, without considering the overall problem or how this choice might affect future decisions or the shortest paths in the end.</p>
<p>Choosing a greedy algorithm is a design choice, just like <a href="dsa_ref_dynamic_programming.php">Dynamic Programming</a> is another algorithm design choice.</p>
<div class="ws-info">
  <p>Two properties must be true for a problem for a greedy algorithm to work:</p>
  <ul>
    <li><strong>Greedy Choice Property: </strong>Means that the problem is so that the solution (the global optimum) can be reached by making greedy choices in each step (locally optimal choices).</li>
    <li><strong>Optimal Substructure: </strong>Means that the optimal solution to a problem, is a collection of optimal solutions to sub-problems. So solving smaller parts of the problem locally (by making greedy choices) contributes to the overall solution.</li>
  </ul>
</div>

<p>Most of the problems in this tutorial, like sorting an array, or <a href="dsa_theory_graphs_shortestpath.php">finding the shortest paths</a> in a graph, have these properties, and those problems can therefore be solved by greedy algorithms like <a href="dsa_algo_selectionsort.php">Selection sort</a> or <a href="dsa_algo_graphs_dijkstra.php">Dijkstra's algorithm</a>.</p>


<p>But problems like <a href="dsa_ref_traveling_salesman.php">The Traveling Salesman</a>, or the <a href="dsa_ref_knapsack.php">0/1 Knapsack Problem</a>, do not have these properties, and so a greedy algorithm can not be used to solve them. These problems are discussed further down.</p>
<p>In addition, even if a problem can be solved by a greedy algorithm, it can also be solved by non-greedy algorithms.</p>
...
<h2>Algorithms That Are Not Greedy</h2>
<p>Below are algorithms that are not greedy, meaning they do not only rely on doing locally optimal choices in each step:</p>
<ul>
  <li><strong><a href="dsa_algo_mergesort.php">Merge Sort</a>: </strong>Splits the array in halves over and over again, and then merges the array parts together again in a way that results in a sorted array. These operations are not a series of locally optimal choices like greedy algorithms are.</li>
  <li><strong><a href="dsa_algo_quicksort.php">Quick Sort</a>: </strong>The choice of pivot element, the arranging of elements around the pivot element, and the recursive calls to do the same with the left and right side of the pivot element &mdash; those actions do not rely on making greedy choices.</li>
  <li><strong><a href="dsa_algo_graphs_traversal.php#bfsTraversal">BFS</a> and <a href="dsa_algo_graphs_traversal.php#dfsTraversal">DFS</a> Traversal: </strong>These algorithms traverse a graph without making a choice locally in each step on how to continue with the traversal, and so they are not greedy algorithms.</li>
  <li><strong><a href="dsa_ref_memoization.php#fibmemo">Finding the nth Fibonacci number using memoization</a>: </strong>This algorithm belongs to a way of solving problems called <a href="dsa_ref_dynamic_programming.php">Dynamic Programming</a>, which solves overlapping sub-problems, and then pieces them back together. Memoization is used in each step to optimize the overall algorithm, which means that at each step, this algorithm does not only consider what is the locally optimal solution, but it also takes into account that a result computed in this step, might be used in later steps.</li>
</ul>
<hr>

<h2>The 0/1 Knapsack Problem</h2>
<p>The <a href="dsa_ref_knapsack.php">0/1 Knapsack Problem</a> cannot be solved by a greedy algorithm because it does not fulfill the greedy choice property, and the optimal substructure property, as mentioned earlier.</p>
<div class="ws-info">
...
<p>This problem cannot be solved by a greedy algorithm, because choosing the item with the highest value, the lowest weight, or the highest value to weight ratio, in each step (local optimal solution, greedy), does not guarantee the optimal solution (global optimum).</p>
<p>Let's say your backpack's limit is 10 kg, and you have these three treasures in front of you:</p>
<table class="ws-table-all notranslate" id="treasureTable">
  <tbody>
    <tr>
...
<p>Making the greedy choice by taking the most valuable thing first, the horse figure with value $600, means that you can not bring any of the other things without breaking the weight limit.</p>
<p>So by trying to solve this problem in a greedy way you end up with a metal horse with value $600.</p>
<p>But the best solution in this case is taking the shield and the pot, maximizing the total value in the backpack to be $800, while still being under the weight limit.</p>
<p><span style="font-style: italic;">What about always taking the treasure with the lowest weight? Or always taking the treasure with the highest value to weight ratio?</span> </p>
<p>While following those principles would actually lead us to the best solution in this specific case, we could not guarantee that those principles would work if the values and weights in this example were changed.</p>
<p>This means that The 0/1 Knapsack Problem cannot be solved with a greedy algorithm.</p>
<p>Read more about The 0/1 Knapsack Problem <a href="dsa_ref_knapsack.php">here</a>.</p>
<hr>

<h2>The Traveling Salesman</h2>
<p><a href="dsa_ref_traveling_salesman.php">The Traveling Salesman Problem</a> is a famous problem that also cannot be solved by a greedy algorithm, because it does not fulfill the greedy choice property, and the optimal substructure property, as mentioned earlier.</p>
<p>The Traveling Salesman Problem states that you are a salesperson with a number of cities or towns you must visit to sell your things.</p>
<div class="ws-info">
...
<p>Using a greedy algorithm here, you would always go to the next unvisited city that is closest to the city you are currently in. But this would in most cases not lead you to the optimal solution with the shortest total path.</p>
<p>The simulation below shows how it looks like when a greedy algorithm tries to solve The Traveling Salesman Problem. </p>
<p>Running the simulation, it might not always be obvious that the algorithm is flawed, but you might notice how sometimes the lines are crossing, creating a longer total distance, when that is clearly not necessary.</p>
<div id="vueApp2">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <p style="color: black; margin: 15px 15px 0 15px;">A greedy algorithm trying to solve The Traveling Salesman Problem.</p>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 400" class="dsa_svg">
      <transition-group>
        <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges2" />
        <line v-for="edge in dijkstraEdges" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
          :x2="dice[edge.to].xpos" :y2="dice[edge.to].ypos"
          :key="`mst-${edge.from}-${edge.to}`" class="dijkstraEdges2"/>
        <circle v-for="el in dice" 
          :key="el.keyNmbr"
          :cx="el.xpos" 
          :cy="el.ypos"
    ...
    <br>
    <div class="btnDiv">
      <button style="margin-right: 5px;" @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button>
    </div>
</div>
<script>
...
<p>While running a greedy approach to The Traveling Salesman Problem sometimes gives us a pretty good approximation to the shortest possible route, a greedy algorithm will not be able to solve The Traveling Salesman Problem in general.</p>
<p>The Traveling Salesman Problem does not fulfill the properties needed so that it can be solved by a greedy algorithm.</p>
<div class="ws-note">
  <p><strong>Note: </strong>There is actually no algorithm that finds the shortest route in The Traveling Salesman Problem efficiently. We just have to check all possible routes! This gives us a time complexity of \(O(n!)\), which means the number of calculations explodes when the number of cities (\(n\)) is increased.</p>
</div>
<p>Read more about the Traveling Salesman Problem <a href="dsa_ref_traveling_salesman.php">here</a>.</p>
<hr>
