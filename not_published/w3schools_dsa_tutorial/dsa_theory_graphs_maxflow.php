...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Maximum Flow</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_mst_kruskal.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_graphs_fordfulkerson.php">Next &#10095;</a>
</div>
<hr>

<h2>The Maximum Flow Problem</h2>
<p>The Maximum Flow problem is about finding the maximum flow through a directed graph, from one place in the graph to another.</p>
<p>More specifically, the flow comes from a source vertex \(s\), and ends up in a sink vertex \(t\), and each edge in the graph is defined with a flow and a capacity, where the capacity is the maximum flow that edge can have.</p>
<div id="vueApp2">
...
<p>A <strong>flow network</strong> if often what we call a directed graph with a flow flowing through it.</p>
<p>The <strong>capacity</strong> \(c\) of an edge tells us how much flow is allowed to flow through that edge.</p>
<p>Each edge also has a <strong>flow</strong> value that tells how much the current flow is in that edge.</p>
<div class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 40" class="dsa_svg">
    <text class="svgBalFac redText" opacity="1" x="190" y="12">0/7</text>
    <line x1="120" y1="20" x2="280" y2="20" fill="none" stroke="gray" stroke-width="1" stroke-dasharray="4" opacity="1"></line>
    <path d="M151,19.000000000000014 L280,19 L280,21 L151,21.000000000000014 L151,22.000000000000014 L135,19.999999999999996 L151,18.000000000000014 Z" fill="gray" stroke="none" opacity="0"></path>
    <path d="M246,24 L120,24 L120,16 L246,16 L246,12 L264,20 L246,28 Z" fill-opacity="0.2" stroke-width="1" opacity="1" class="capEdge"></path>
    <path d="M151,19.000000000000014 L280,19 L280,21 L151,21.000000000000014 L151,22.000000000000014 L136,19.999999999999996 L151,18.000000000000014 Z" fill-opacity="0.2" stroke-width="1" opacity="0" class="capEdge"></path>
    <circle cx="120" cy="20" r="15" stroke-width="1" class="vertex"></circle>
    <circle cx="280" cy="20" r="15" stroke-width="1" class="vertex"></circle>
    <text x="120" y="25" class="svgText">v1</text>
    <text x="280" y="25" class="svgText">v2</text>
  </svg>
</div>
<p>The edge in the image above \( v_1 \rightarrow v_2 \), going from vertex \( v_1 \) to vertex \( v_2 \), has its flow and capacity described as <code class="w3-codespan">0/7</code>, which means the flow is <code class="w3-codespan">0</code>, and the capacity is <code class="w3-codespan">7</code>. So the flow in this edge can be increased up to 7, but not more.</p>
<p>In its simplest form, flow network has one <strong>source vertex</strong> \(s\) where the flow comes out, and one <strong>sink vertex</strong> \(t\) where the flow goes in. The other vertices just have flow passing through them.</p>
<p>For all vertices except \(s\) and \(t\), there is a <strong>conservation of flow</strong>, which means that the same amount of flow that goes into a vertex, must also come out of it.</p>
<p>The maximum flow is found by algorithms such as Ford-Fulkerson, or Edmonds-Karp, by sending more and more flow through the edges in the flow network until the capacity of the edges are such that no more flow can be sent through. Such a path where more flow can be sent through is called an <strong>augmented path</strong>.</p>
<p>The Ford-Fulkerson and Edmonds-Karp algorithms are implemented using something called a <strong>residual network</strong>. This will be explained in more detail on the next pages.</p>
<p>The <strong>residual network</strong> is set up with the <strong>residual capacities</strong> on each edge, where the residual capacity of an edge is the capacity on that edge, minus the flow. So when flow is increased in a an edge, the residual capacity is decreased with the same amount.</p>
<p>For each edge in the residual network, there is also a <strong>reversed edge</strong> that points in the opposite direction of the original edge. The residual capacity of a reversed edge is the flow of the original edge. Reversed edges are important for sending flow back on an edge as part of the maximum flow algorithms.</p>
<p>The image below shows the reversed edges in the graph from the simulation at the top of this page. Each reversed edge points in the opposite direction, and because there is no flow in the graph to begin with, the residual capacities for the reversed edges are 0.</p>
<div id="vueApp3" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isActive, redText: edge.rev }">{{edge.capacity}}</text>

    <!-- Default paths for empty edges -->
    <line v-for="edge in flowEdges" :key="`generateEdgeKey('empty',edge)`"  
      :x1="vertices[edge.from].x" :y1="vertices[edge.from].y" 
      :x2="vertices[edge.to].x" :y2="vertices[edge.to].y"
      fill="none" stroke="black" stroke-dasharray="5,3" class="capEdge2"/>

    <!-- Paths for flow edges
    <path v-for="edge in flowEdges" :key="`generateEdgeKey('flow',edge)`" :d="edge.path" fill="gray" stroke="none" :opacity="edge.opacity"/> -->

    <!-- Paths for capacity edges -->
    <path v-for="edge in capacityEdges" :key="`generateEdgeKey('cap',edge)`" :d="edge.path" class="capEdge2"/>
...
const app3 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
/*0*/   { from: 1, to: 0, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
/*5*/   { from: 4, to: 1, capacity: 0, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*7*/   { from: 1, to: 2, capacity: 0, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*10*/  { from: 4, to: 3, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
/*15*/  { from: 5, to: 4, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 4, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false }
      ]
    }
  ...
  computed: {
    capacityEdges() {
        return this.edges.map(edge => {
            ...
            // Offset for the thick line's corners
            const offsetX = 2*capacity * Math.sin(angle);
            const offsetY = 2*capacity * Math.cos(angle);

            // Points for the thick line (rectangle)
            const p1 = { x: fromVertex.x - offsetX, y: fromVertex.y + offsetY };
            const p2 = { x: fromVertex.x + offsetX, y: fromVertex.y - offsetY };
            const adjustedEndX = fromVertex.x + (length - capacity - toVertex.radius+7) * Math.cos(angle); // Shorten line to accommodate arrowhead
            const adjustedEndY = fromVertex.y + (length - capacity - toVertex.radius+7) * Math.sin(angle);
            const p3 = { x: adjustedEndX + offsetX, y: adjustedEndY - offsetY };
            const p4 = { x: adjustedEndX - offsetX, y: adjustedEndY + offsetY };
            ...
            const leftCorner = { x: p3.x + offsetX, y: p3.y - offsetY };
            const rightCorner = { x: p4.x - offsetX, y: p4.y + offsetY };
            const arrowHeadPath = `M${leftCorner.x},${leftCorner.y} L${arrowTip.x},${arrowTip.y} L${rightCorner.x},${rightCorner.y}`;
  ...
  methods: {
    calculateEdgeCenters() {
      ...
      this.edges[0].centerX -= 12;
      this.edges[1].centerX -= 10;
      this.edges[1].centerY += 12;
      this.edges[4].centerX += 5;
      this.edges[4].centerY -= 3;
      this.edges[5].centerX += 5;
      this.edges[7].centerX += 5;
      this.edges[8].centerX += 5;
      this.edges[8].centerY -= 3;
      this.edges[10].centerX += 5;
      this.edges[11].centerX += 5;
      this.edges[15].centerX += 5;
      this.edges[15].centerY += 10;
    },
    generateEdgeKey(type, edge) {
      // Generate a unique key for each edge based on its properties and the type
...
<p>Some of these concepts, like the residual network and the reversed edge, can be hard to understand. That is why these concepts are explained more in detail, and with examples, on the next two pages.</p>
<p>When the maximum flow is found, we get a value for how much flow can be sent through the flow network in total.</p>
<hr>

<h2>Multiple Source and Sink Vertices</h2>
...
<p>To modify the graph so that you can run the Ford-Fulkerson or Edmonds-Karp algorithm on it, create an extra super-source vertex if you have multiple source vertices, and create an extra super-sink vertex if you have multiple sink-vertices.</p>
...
<p>The image below shows such a graph with two sources \(s_1\) and \(s_2\), and three sinks \(t_1\), \(t_2\), and \(t_3\).</p>
<p>To run Ford-Fulkerson or Edmonds-Karp on this graph, a super source \(S\) is created with edges with infinite capacities to the original source nodes, and a super sink \(T\) is created with edges with infinite capacities to it from the original sinks.</p>
<div id="vueApp4" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 220" class="dsa_svg">

    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.super ? 1 : 0" :class="{ boldText: edge.isActive, redText: edge.rev }">inf</text>

    <!-- Default paths for empty edges -->
    <line v-for="edge in flowEdges" :key="`generateEdgeKey('empty',edge)`"  
      :x1="vertices[edge.from].x" :y1="vertices[edge.from].y" 
      :x2="vertices[edge.to].x" :y2="vertices[edge.to].y"
      :class="edge.super ? 'capEdge2' : 'capEdge3'"
      :stroke-dasharray="edge.super ? '3,3' : ''"/>

    <!-- Paths for flow edges
    <path v-for="edge in flowEdges" :key="`generateEdgeKey('flow',edge)`" :d="edge.path" fill="gray" stroke="none" :opacity="edge.opacity"/> -->

    <!-- Paths for capacity edges -->
    <path v-for="edge in capacityEdges" :key="`generateEdgeKey('cap',edge)`" :d="edge.path" 
    :class="edge.super ? 'capEdge2' : 'capEdge3'"/>
    ...
    <circle v-for="vertex in vertices" 
        ...
        :r="vertex.radius"
        :stroke-width="vertex.isAug ? 2 : 1"
        :stroke-dasharray="vertex.super ? '3,3' : ''"
        class="vertex"/>
    ...
    <text v-for="vertex in vertices" :key="`name-${vertex.key}`" :x="vertex.x" :y="vertex.y+5" class="svgText" font-size="smaller">{{vertex.name}}</text>
  </svg>
</div>
<script>
const app4 = Vue.createApp({
  data() {
    return {
      ...
      augPathVertices: [],
      vertices: [
        { key: 0, x: 80, y: 50, radius: 12, name: 's1',   opacity: 1, isAug: true },
        { key: 1, x: 190, y: 20, radius: 12, name: 'v1',  opacity: 0, isAug: false },
        { key: 2, x: 130, y: 100, radius: 12, name: 'v2', opacity: 0, isAug: false },
        { key: 3, x: 280, y: 20, radius: 12, name: 'v3',  opacity: 0, isAug: false },
        { key: 4, x: 210, y: 150, radius: 12, name: 'v4', opacity: 0, isAug: false },
        { key: 5, x: 290, y: 120, radius: 12, name: 't2',  opacity: 1, isAug: true },
        { key: 6, x: 110, y: 200, radius: 12, name: 's2',  opacity: 1, isAug: true },
        { key: 7, x: 330, y: 180, radius: 12, name: 't3',  opacity: 1, isAug: true },
        { key: 8, x: 340, y: 50, radius: 12, name: 't1',  opacity: 1, isAug: true },
        { key: 9, x: 20, y: 120, radius: 18, name: 'S',  opacity: 0, isAug: false, super: true },
        { key: 10, x: 380, y: 120, radius: 18, name: 'T',  opacity: 0, isAug: false, super: true },
      ],
      edges: [
        { from: 0, to: 1, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 0, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 0, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 0, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 0, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 0, flow: 0, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 6, to: 2, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 6, to: 4, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 7, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 8, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 9, to: 0, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false, super: true },
        { from: 9, to: 6, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false, super: true },
        { from: 8, to: 10, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false, super: true },
        { from: 5, to: 10, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false, super: true },
        { from: 7, to: 10, capacity: 0, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false, super: true },
      ]
    }
  },
  watch: {
  ...
  computed: {
    capacityEdges() {
        return this.edges.map(edge => {
            ...
            const p1 = { x: fromVertex.x - offsetX, y: fromVertex.y + offsetY };
            const p2 = { x: fromVertex.x + offsetX, y: fromVertex.y - offsetY };
            const adjustedEndX = fromVertex.x + (length - capacity - 8) * Math.cos(angle); // Shorten line to accommodate arrowhead
            const adjustedEndY = fromVertex.y + (length - capacity - 8) * Math.sin(angle);
            const p3 = { x: adjustedEndX + offsetX, y: adjustedEndY - offsetY };
            const p4 = { x: adjustedEndX - offsetX, y: adjustedEndY + offsetY };
  ...
  methods: {
    calculateEdgeCenters() {
      ...
      this.edges[13].centerY += 10;
      this.edges[14].centerY -= 5;
      this.edges[15].centerY -= 5;
      this.edges[16].centerX -= 10;
      this.edges[16].centerY -= 3;
      this.edges[17].centerY += 10;
    },
    generateEdgeKey(type, edge) {
      // Generate a unique key for each edge based on its properties and the type
      return `${type}-${edge.from}-${edge.to}`;
...
<p>The Ford-Fulkerson or Edmonds-Karp algorithm is now able to find maximum flow in a graph with multiple source and sink vertices, by going from the super source \(S\), to the super sink \(T\).</p>
<hr>

<h2>The Max-flow Min-cut Theorem</h2>
<p>To understand what this theorem says we first need to know what a cut is.</p>
<p>We create two sets of vertices: one with only the source vertex inside it called "S", and one with all the other vertices inside it (including the sink vertex) called "T".</p>
<p>Now, starting in the source vertex, we can choose to expand set S by including adjacent vertices, and continue to include adjacent vertices as much as we want as long as we do not include the sink vertex.</p>
<p>Expanding set S will shrink set T, because any vertex belongs either to set S or set T.</p>
<p>In such a setup, with any vertex belonging to either set S or set T, there is a "cut" between the sets. The cut consists of all the edges stretching from set S to set T.</p>
<p>If we add all the capacities from edges going from set S to set T, we get the capacity of the cut, which is the total possible flow from source to sink in this cut.</p>
<p>The minimum cut is the cut we can make with the lowest total capacity, which will be the bottleneck.</p>
<p>In the image below, three different cuts are done in the graph from the simulation in the top of this page.</p>
<div id="vueApp5" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">   
    ...
    <!-- Paths for capacity edges -->
    <path v-for="edge in capacityEdges" :key="`generateEdgeKey('cap',edge)`" :d="edge.path" fill-opacity="0.2" stroke-width="1" :opacity="edge.opacity" class="capEdge" :class="{ augPath: edge.isAug, redEdge: edge.rev }"/>
    ...
    <rect width="400" height="250" x="0" y="0" class="opacityScreen" opacity="0.5"></rect>
    <!-- Cut line A -->
    <line x1="180" x2="40" y1="0" y2="230" stroke="hotpink" stroke-width="3" stroke-dasharray="10,4"></line>
    <text x="50" y="230" stroke="hotpink" fill="hotpink">A</text>
    <!-- Cut line B -->
    <line x1="220" x2="150" y1="0" y2="250" stroke="hotpink" stroke-width="3" stroke-dasharray="10,4"></line>
    <text x="160" y="245" stroke="hotpink" fill="hotpink">B</text>
    <!-- Cut line C -->
    <line x1="340" x2="320" y1="20" y2="235" stroke="hotpink" stroke-width="3" stroke-dasharray="10,4"></line>
    <text x="325" y="230" stroke="hotpink" fill="hotpink">C</text>
  </svg>
</div>
<script>
...
<p><strong>Cut A: </strong>This cut has vertices \(s\) and \(v_1\) in set S, and the other vertices are in set T. The total capacity of the edges leaving set S in this cut, from sink to source, is 3+4+7=14. We are not adding the capacity from edge \(v_2 \rightarrow v_1\), because this edge goes in the opposite direction, from sink to source. So the maximum possible flow across cut A is 14.</p>
<p><strong>Cut B: </strong>The maximum possible flow is 3+4+3=10 across cut B.</p>
<p><strong>Cut C: </strong>The maximum possible flow is 2+6=8 across cut C. If we checked all other cuts in the graph, we would not find a cut with a lower total capacity. This is the minimum cut. Have you run the simulation finding the maximum flow in the top of this page? Then you also know that 8 is the maximum flow, which is exactly what the max-flow min-cut theorem says.</p>
<p>The max-flow min-cut theorem says that finding the minimum cut in a graph, is the same as finding the maximum flow, because the value of the minimum cut will be the same value as the maximum flow.</p>
<hr>

<h2>Practical Implications of The Max-flow Min-cut Theorem</h2>
<p>Finding the maximum flow in a graph using an algorithm like Ford-Fulkerson also helps us to understand where the minimum cut is: The minimum cut will be where the edges have reached full capacity.</p>
<p>The minimum cut will be where the bottleneck is, so if we want to increase flow beyond the maximum limit, which is often the case in practical situations, we now know which edges in the graph that needs to be modified to increase the overall flow.</p>
...
<h2>The Maximum Flow Problem Described Mathematically</h2>
...
<p><span class="math display">\[ \forall (u,v) \in E: f(u,v) \leq c(u,v)  \]</span></p>
...
<p><span class="math display">\[ \forall (u,v) \in E: f(u,v) = -f(v,u) \]</span></p>
...
<p><span class="math display">\[ \forall u \in V \setminus \{s,t\} \Rightarrow \sum_{w \in V} f(u,w) = 0  \]</span></p>
...
<p><span class="math display">\[ \sum_{(s,u) \in E} f(s,u) = \sum_{(v,t) \in E} f(v,t) \]</span></p>
