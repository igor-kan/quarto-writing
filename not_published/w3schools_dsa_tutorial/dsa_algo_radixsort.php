...
.v2-enter-active,.v4-enter-active,
.v2-leave-active,.v4-leave-active,
.v2-move,.v4-move {
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Radix Sort</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_countingsort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_mergesort.php">Next &#10095;</a>
</div>
<hr>

<h2>Radix Sort</h2>
<p>The Radix Sort algorithm sorts an array by individual digits, starting with the least significant digit (the rightmost one).</p>
<div id="vueApp">
  <p>Click the button to do Radix Sort, one step (digit) at a time.</p>
  <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}<br>
  <div id="frameDiv2">
    <div id="rightDiv">
      <transition-group name="v3">
        ...
        >
        <span v-for="(digit, dIndex) in String(x.dieNmbr).split('')" 
              :class="{ 'highlight': dIndex === String(x.dieNmbr).length - 1 - currentDigitPosition }">
          {{ digit }}
        </span>
      </div>
      </transition-group>
...
const app = Vue.createApp({
  ...
  methods: {
    ...
    async radixSort() {
      ...
      if(this.currentDigitPosition === 5){
        this.currentDigitPosition = -1;
      }
      this.currentDigitPosition++;
      await new Promise(resolve => setTimeout(resolve, 500));
      while(this.dice.length > 0) {
        this.dice[0].isActive = true;
        let removedObj = this.dice.splice(0, 1)[0];
        removedObj.digits = String(removedObj.dieNmbr).split('').map(Number);
        let n = this.currentDigitPosition+1;
        let digit = Math.floor(removedObj.dieNmbr / Math.pow(10, n - 1)) % 10;
        this.count[digit].push(removedObj);
        this.count[digit][this.count[digit].length-1].isActive = false;
      }
      for(let i=this.count.length-1; i>=0; i--){
        while(this.count[i].length>0){
          this.count[i][this.count[i].length-1].isActive = true;
          let removedObj = this.count[i].pop();
          this.dice.splice(0,0,removedObj);
          this.dice[0].isActive = false;
        }
      }
      await new Promise(resolve => setTimeout(resolve, 1500));
      if(this.currentDigitPosition === 2) {
...
<p>The radix (or base) is the number of unique digits in a number system. In the decimal system we normally use, there are 10 different digits from 0 till 9.</p>
<p>Radix Sort uses the radix so that decimal values are put into 10 different buckets (or containers) corresponding to the digit that is in focus, then put back into the array before moving on to the next digit.</p>
<p>Radix Sort is a non comparative algorithm that only works with non negative integers.</p>
<p>The Radix Sort algorithm can be described like this:</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Start with the least significant digit (rightmost digit).</li>
    <li>Sort the values based on the digit in focus by first putting the values in the correct bucket based on the digit in focus, and then put them back into array in the correct order.</li>
    <li>Move to the next digit, and sort again, like in the step above, until there are no digits left.</li>
  </ol>
</div>
...
<div id="vueApp2">
  <div v-bind:class="{ unstableClass: !stableBool }">
  <p>Speed: <input v-model="inpVal" type="range" min="50" max="300"><!-- {{ delay }}--></p>
  <p>Stable sort?
    <label class="dsa_label">
      <input type="checkbox" v-model="stableBool" :disabled="chbxDisabled">
      {{ isStable }}
    </label>
  </p>
  <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}<br>
  <div id="frameDiv3">
    <div id="leftDiv">
      <div v-for="(x, index) in count" class="countDivParent2" :key="index-1000">
        <div class="numberDiv2" :key="index-3000">
          {{ index }}
        </div>
        <transition-group name="v2">
          <div v-for="y in count[index]" :key="y.keyNmbr" 
          class="diceDiv3" 
          :class="{ 
            'diceDiv2-active': y.isActive, 
            'diceDiv2-finished': y.isFinished
          }" 
          :style="{
            //width: 3+x.countValue*25 + 'px',
            transition: 'all ' + delay
          }">
          <span v-for="(digit, dIndex) in y.digits" 
                :class="{ 'highlight2': dIndex === y.digits.length - 1 - currentDigitPosition }">
            {{ digit }}
          </span>
          </div>
        </transition-group>
    ...
    <div id="rightDiv2">
      <transition-group name="v4">
        <div v-for="(x,index) in dice" 
          :key="x.keyNmbr"
          class="diceDiv3" 
          :class="{ 
            'diceDiv2-active': x.isActive,
            'diceDiv2-finished': x.isFinished
          }" 
          :style="{
            transition: 'all ' + delay
          }"
        >
        <span v-for="(digit, dIndex) in String(x.dieNmbr).split('')" 
              :class="{ 'highlight2': dIndex === String(x.dieNmbr).length - 1 - currentDigitPosition }">
          {{ digit }}
        </span>
      </div>
      </transition-group>
...
<p>Let's try to do the sorting manually, just to get an even better understanding of how Radix Sort works before actually implementing it in a programming language.</p>
<p><strong>Step 1:</strong> We start with an unsorted array, and an empty array to fit values with corresponding radices 0 till 9.</p>
...
<hr>
<p><strong>Step 2:</strong> We start sorting by focusing on the least significant digit.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 3:</strong> Now we move the elements into the correct positions in the radix array according to the digit in focus. Elements are taken from the start of myArray and pushed into the correct position in the radixArray.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 4:</strong> We move the elements back into the initial array, and the sorting is now done for the least significant digit. Elements are taken from the end radixArray, and put into the start of myArray.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 5:</strong> We move focus to the next digit. Notice that values 45 and 25 are still in the same order relative to each other as they were to start with, because we sort in a stable way.</p>
<div class="w3-code notranslate">
...
<hr>
<p><strong>Step 6:</strong> We move elements into the radix array according to the focused digit.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">myArray = [ ]</code><br>
...
<hr>
<p><strong>Step 7:</strong> We move elements back into the start of myArray, from the back of radixArray.</p>
<div class="w3-code notranslate">
...
<p>The sorting is finished!</p>
<hr>
<p>Run the simulation below to see the steps above animated:</p>

<div id="vueApp3">
  <!--<p>Delay: <input v-model="delay" type="range" min="50" max="500"> {{ delay }}</p>-->
  <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}<br>
  <div id="frameDiv4">
    myArray = 
    [<div class="diceDiv4" id="ghostDiv3"></div>
    <transition-group name="v5">
      <div v-for="(x, index) in myArray" 
        :key="x.keyNmbr" 
        class="diceDiv4"
        :class="{ 
          'diceDiv4-active': x.isActive
        }" 
        :style="{
          transition: 'all ' + delay
        }"
      >
      <span v-for="(digit, dIndex) in String(x.dieNmbr).split('')" 
            :class="{ 'highlight': (dIndex === String(x.dieNmbr).length - 1 - currentDigitPosition) && this.btnIsDisabled }">
        {{ digit }}
      </span>
      <template v-if="index < myArray.length - 1">,</template>
      </div>
      <div class="diceDiv4" :key="-1000">]</div>
    </transition-group>
    <br>
    radixArray = 
    [
    <template v-for="(x,index) in radixArray">
      [
      <transition-group name="v5">
        <div v-for="(y, indexY) in radixArray[index]" 
          :key="1000+y.keyNmbr" 
          class="diceDiv4"
          :class="{ 
            'diceDiv4-active': y.isActive
          }" 
          :style="{
            transition: 'all ' + delay
          }"
        >
        <span v-for="(digit, dIndex) in String(y.dieNmbr).split('')" 
              :class="{ 'highlight': dIndex === String(y.dieNmbr).length - 1 - currentDigitPosition && this.btnIsDisabled }">
          {{ digit }}
        </span>
        <template v-if="indexY < radixArray[index].length - 1">,</template>
        </div>
      </transition-group>
      ],
    </template>
    [ ]
    <div class="diceDiv4">] </div>
  </div>
</div>
<script>
  ...
  methods: {
    ...
    updateMoveDuration() {
      ...
      for (let i = 0; i < stylesheets.length; i++) {
        let rules = stylesheets[i].cssRules || stylesheets[i].rules;
        for (let j = 0; j < rules.length; j++) {
          if (rules[j].selectorText === '.v5-move') {
            rules[j].style.transitionDuration = this.delay + 'ms';
            break;
          }
        }
    ...
    async radixSort() {
      ...
      for(let k=0; k<2; k++){
        while(this.myArray.length>0){
          this.myArray[0].isActive = true;
          await new Promise(resolve => setTimeout(resolve, this.delay));
          let removedObj = this.myArray.splice(0, 1)[0];
          removedObj.digits = String(removedObj.dieNmbr).split('').map(Number);
          let n = this.currentDigitPosition+1;
          let digit = Math.floor(removedObj.dieNmbr / Math.pow(10, n - 1)) % 10;
          this.radixArray[digit].push(removedObj);
          await new Promise(resolve => setTimeout(resolve, this.delay));
          this.radixArray[digit][this.radixArray[digit].length-1].isActive = false;
        }
        for(let i=this.radixArray.length-1; i>=0; i--){
          while(this.radixArray[i].length>0){
            this.radixArray[i][this.radixArray[i].length-1].isActive = true;
            await new Promise(resolve => setTimeout(resolve, this.delay));
            let removedObj = this.radixArray[i].pop();
            this.myArray.splice(0,0,removedObj);
            this.myArray[0].isActive = false;
          }
        }
        this.currentDigitPosition++;
        await new Promise(resolve => setTimeout(resolve, this.delay));
    ...
    removeActive() {
      for(let i=0; i<this.myArray.length; i++){
        this.myArray[i].isActive = false;
      };
      for(let i=0; i<this.radixArray.length; i++){
        this.radixArray[i].isActive = false;
      }
    },
    addDie50() {
      this.myArray = [];
      this.radixArray = [[],[],[],[],[],[],[],[],[],[]];
      let my_array = [33, 45, 40, 25, 17, 24];
...
<h2>Manual Run Through: What Happened?</h2>
<p>We see that values are moved from the array and placed in the radix array according to the current radix in focus. And then the values are moved back into the array we want to sort.</p>
<p>This moving of values from the array we want to sort and back again must be done as many times as the maximum number of digits in a value. So for example if 437 is the highest number in the array that needs to be sorted, we know we must sort three times, once for each digit.</p>
<p>We also see that the radix array needs to be two-dimensional so that more than one value on a specific radix, or index.</p>
<p>And, as mentioned earlier, we must move values between the two arrays in a way that keeps the order of values with the same radix in focus, so the the sorting is stable.</p>
<hr>

<h2>Radix Sort Implementation</h2>
<p>To implement the Radix Sort algorithm we need:</p>
<ol>
  <li>An array with non negative integers that needs to be sorted.</li>
  <li>A two dimensional array with index 0 to 9 to hold values with the current radix in focus.</li>
  <li>A loop that takes values from the unsorted array and places them in the correct position in the two dimensional radix array.</li>
  <li>A loop that puts values back into the initial array from the radix array.</li>
  <li>An outer loop that runs as many times as there are digits in the highest value.</li>
</ol>
<p>The resulting code looks like this:</p>

<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate line-numbers w3-white" data-line="7,11"><code>myArray = [170, 45, 75, 90, 802, 24, 2, 66]
...
print("Sorted array:", myArray)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_radixsort" target="_blank">Try it Yourself &raquo;</a>
</div>
<p><strong>On line 7</strong>, we use floor division ("//") to divide the maximum value 802 by 1 the first time the while loop runs, the next time it is divided by 10, and the last time it is divided by 100. When using floor division "//", any number beyond the decimal point are disregarded, and an integer is returned.</p>
<p><strong>On line 11</strong>, it is decided where to put a value in the radixArray based on its radix, or digit in focus. For example, the second time the outer while loop runs exp will be 10. Value 170 divided by 10 will be 17. The "%10" operation divides by 10 and returns what is left. In this case 17 is divided by 10 one time, and 7 is left. So value 170 is placed in index 7 in the radixArray.</p>
<hr>

<h2>Radix Sort Using Other Sorting Algorithms</h2>
<p>Radix Sort can actually be implemented together with any other sorting algorithm as long as it is stable. This means that when it comes down to sorting on a specific digit, any stable sorting algorithm will work, such as counting sort or bubble sort.</p>
<p>This is an implementation of Radix Sort that uses Bubble Sort to sort on the individual digits:</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white" data-line="1-6,20"><code>def bubbleSort(arr):
    n = len(arr)
    for i in range(n):
...
print("Sorted array:", myArray)
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_radixsortbbl" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Radix Sort Time Complexity</h2>
<p class="fontComment">For a general explanation of what time complexity is, visit <a href="dsa_timecomplexity_theory.php">this page</a>.</p>
<p class="fontComment">For a more thorough and detailed explanation of Radix Sort time complexity, visit <a href="dsa_timecomplexity_radixsort.php">this page</a>.</p>

<p>The time complexity for Radix Sort is:</p>
<p><span class="math display">\[ \underline{\underline{O(n \cdot k)}} \]</span></p>
<p>This means that Radix Sort depends both on the values that need to be sorted \(n\), and the number of digits in the highest value \(k\).</p>
<p>A best case scenario for Radix Sort is if there are lots of values to sort, but the values have few digits. For example if there are more than a million values to sort, and the highest value is 999, with just three digits. In such a case the time complexity \(O(n \cdot k)\) can be simplified to just \(O(n)\).</p>
<p>A worst case scenario for Radix Sort would be if there are as many digits in the highest value as there are values to sort. This is perhaps not a common scenario, but the time complexity would be \(O(n^2)\)in this case.</p>
<p>The most average or common case is perhaps if the number of digits \(k\) is something like \(k(n)= \log n\). If so, Radix Sort gets time complexity \(O(n \cdot \log n )\). An example of such a case would be if there are 1000000 values to sort, and the values have 6 digits.</p>
<p>See different possible time complexities for Radix Sort in the image below.</p>
<img src="img_runtime_radixsort.png" class="imgTut" alt="Time Complexity">

<p>Run different simulations of Radix Sort to see how the number of operations falls between the worst case scenario \(O(n^2)\) (red line) and best case scenario \(O(n)\) (green line).</p>
<div id="vueApp4">
  <div class="frameDivOuter">
    <div id="leftSpace"></div>
    <div id="inpValues">
      <p><label class="dsa_label">Set values (n): <input type="range" v-model="userX" min="100" max="480" :disabled="isRangeDisabled"></label> {{ this.userX }}</p>
      <p><label class="dsa_label">Digits (k): <input type="range" v-model="userK" min="2" max="7" step="1" :disabled="isLoading"></label> {{ this.userK }}</p>
    </div>
  </div>
  <div class="frameDivOuter">
    <div id="inpDiv">
      <p><label class="dsa_label"><input type="radio" value="single" v-model="selectedRunType" :disabled="isLoading"> Random</label></p>
      <p><label class="dsa_label"><input type="radio" value="worstCase" v-model="selectedRunType" :disabled="isLoading"> Descending</label></p>
      <p><label class="dsa_label"><input type="radio" value="bestCase" v-model="selectedRunType" :disabled="isLoading"> Ascending</label></p>
      <p><label class="dsa_label"><input type="radio" value="random30" v-model="selectedRunType" :disabled="isLoading"> 10 Random</label></p>
    </div>
    <div id="frameDiv9">
      <div id="graphDiv">
        <transition-group name="miniSort">
          <div v-for="x in array"
            :key="keyNmbr"
            :style="{
              height: Math.round(0.14*this.userK*x.value/(Math.pow(10,this.userK-2))) + 'px',
              boxShadow: x.active ? '0px 0px 5px 2px rgba(144, 238, 144, 0.9)' : 'none'
            }"
          >
            <!-- {{ x.dieNmbr }} -->
          </div>
  ...
  <button @click="runSelected" :disabled="isLoading" style="width: 105px;">{{runBtnText}}</button> &nbsp;
...
<script>
  ...
  for (let x = 0; x <= 500; x += 10) {
    curve.push({ x: x, y: 3 * x });
  }
  let worstCurve = [];
  for (let x = 0; x <= 500; x += 10) {
    worstCurve.push({ x: x, y: 0.3*x * x });
  }
  let ctx = null;
  let myChart = null;
  const app4 = Vue.createApp({
    ...
    return {
      ...
      runBtn: true,
      isLoading: false,
      selectedRunType: 'single'
    };
  },
  computed: {
    isRangeDisabled() {
      return this.isLoading || this.selectedRunType === 'random30';
    },
    runBtnText(){
  ...
  methods: {
    newArray(){
      ...
      this.operations = 0;
      this.keyNmbr = 15;
      for (let k = 0; k < this.userX-1; k++) {
        let randNumber = Math.floor(Math.random() * Math.pow(10,this.userK));
        this.array.push({ value: randNumber, active: false });
      }
      this.runBtn = true;
    },
    presort() {
      this.operations = 0;
      if (this.selectedRunType === "worstCase") {
        this.array.sort(function (a, b) {
          return b.value - a.value;
        });
      } 
      else if (this.selectedRunType === "bestCase") {
        this.array.sort(function (a, b) {
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performRadixSort();
      myChart.data.datasets[1].data.push({ x: parseFloat(this.userX), y: operations });
      myChart.update();
    ...
    async add30Points(){
      ...
      for(let i=0; i<9; i++){
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        this.userX = Math.ceil(Math.random()*380)+100;
        await new Promise(resolve => setTimeout(resolve, 500));
        this.newArray();
      }
      let operations = await this.performRadixSort();
    ...
    async addWorstCase(){
      this.isLoading = true;
      let operations =  await this.performRadixSort();
      myChart.data.datasets[1].data.push({ x: this.userX, y: operations });
      myChart.update(); // Update the chart
      this.clearBtnDisabled = false;
      this.isLoading = false;
    },
    ...
    async performRadixSort() {
      this.operations = 0;
      let maxVal = Math.max(...this.array.map(item => item.value));
      this.operations++;  // For finding max value
      let exp = 1;
      
      while (Math.floor(maxVal / exp) > 0) {
        // Initialize counting array and output array
        let count = Array(10).fill(0);
        let output = Array(this.array.length).fill({ value: 0, active: false });

        // Count occurrences of each digit
        for (let i = 0; i < this.array.length; i++) {
          let index = Math.floor((this.array[i].value / exp) % 10);
          count[index]++;
          this.operations++;  // For counting digits
        }

        // Update count[i] to store the position in output
        for (let i = 1; i < count.length; i++) {
          count[i] += count[i - 1];
          this.operations++;  // For updating counts
        }
        ...
        for (let i = this.array.length - 1; i >= 0; i--) {
          ...
          count[index]--;
          this.operations++;  // For filling output array
        ...
        for (let i = 0; i < this.array.length; i++) {
          ...
          this.operations++;  // For copying back to original array
        }
    ...
    runSelected() {
      if(this.runBtnText === 'Run'){
        switch (this.selectedRunType) {
          case 'single':
            this.addPoint();
            break;
          case 'worstCase':
          ...
          case 'random30':
            ...
            break;
          default:
            console.error('Invalid run type selected');
        }
      }
      else if(this.runBtnText === 'New Values') {
        this.newArray();
        this.presort();
      }
    }
...
<p>The bars representing the different values are scaled to fit the window, so that it looks ok. This means that values with 7 digits look like they are just 5 times bigger than values with 2 digits, but in reality, values with 7 digits are actually 5000 times bigger than values with 2 digits!</p>
<p>If we hold \(n\) and \(k\) fixed, the "Random", "Descending" and "Ascending" alternatives in the simulation above results in the same number of operations. This is because the same thing happens in all three cases.</p>
<hr>

<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_algo_radixsort1" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>To sort an array with Radix Sort, what property must the sorting have for the sorting to be done properly?</p>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">
Radix Sort must use a <input name="ex1" maxlength="6" style="width: 7ch;"> 
sorting algorithm.
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_algo_radixsort1">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
