...
<html lang="en-US">
<head>
<title>DSA Bubble Sort Time Complexity</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Bubble Sort Time Complexity</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_timecomplexity_theory.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_timecomplexity_selsort.php">Next &#10095;</a>
</div>
<hr>
<p>See <a href="dsa_timecomplexity_theory.php">the previous page</a> for a general explanation of what time complexity is.</p>
<hr>

<h2>Bubble Sort Time Complexity</h2>
<p><a href="dsa_algo_bubblesort.php">The Bubble Sort algorithm</a> goes through an array of \(n\) values \(n-1\) times in a worst case scenario.</p>
<p>The first time the algorithm runs through the array, every value is compared to the next, and swaps the values if the left value is larger than the right. This means that the highest value bubbles up, and the unsorted part of the array becomes shorter and shorter until the sorting is done. So on average, \(\frac{n}{2}\) elements are considered when the algorithm goes through the array comparing and swapping values.</p>
<p>We can start calculating the number of operations done by the Bubble Sort algorithm on \(n\) values:</p>
<p><span class="math display">\[Operations = (n-1)\cdot \frac{n}{2} = \frac{n^2}{2} - \frac{n}{2} \]</span></p>
<p>When looking at the time complexity for algorithms, we look at very large data sets, meaning \(n\) is a very big number. And for a very big number \(n\), the term \(\frac{n^2}{2}\) becomes a lot bigger than the term \(\frac{n}{2}\). So large in fact, that we can approximate by simply removing that second term \(\frac{n}{2}\).</p>
...
<p>When we are looking at time complexity like we are here, using Big O notation, factors are disregarded, so factor \(\frac{1}{2}\) is omitted. This means that the run time for the Bubble Sort algorithm can be described with time complexity, using Big O notation like this:</p>
<p><span class="math display">\[ O( \frac{1}{2} \cdot n^2) = \underline{\underline{O(n^2)}} \]</span></p>
<p>And the graph describing the Bubble Sort time complexity looks like this:</p>
...
<p>Luckily there are sorting algorithms that are faster than this, like <a href="dsa_algo_quicksort.php">Quicksort</a>.</p>
<hr>

<h2>Bubble Sort Simulation</h2>
<p>Choose the number of values in an array, and run this simulation to see how the number of operations Bubble Sort needs on an array of \(n\) elements is \(O(n^2)\):</p>
<div id="vueApp3">
...
<p>The red line above represents the upper bound time complexity \(O(n^2)\), and the actual function in this case is \(1.05 \cdot n^2\).</p>
<p>A function \(f(n)\) is said to be \(O(g(n))\) if we have a positive constant \(C\) so that \(C \cdot g(n)>f(n)\) for a large number of values \(n\).</p>
<p>In this case \(f(n)\) is the number of operations used by Buble Sort, \(g(n)=n^2\) and \(C=1.05\).</p>
<p>Read more about Big O notation and time complexity on <a href="dsa_timecomplexity_theory.php">this page</a>.</p>
