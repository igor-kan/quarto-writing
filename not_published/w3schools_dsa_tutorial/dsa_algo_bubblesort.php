...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Bubble Sort</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_data_arrays.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_selectionsort.php">Next &#10095;</a>
</div>
<hr>
...
<p>Continue reading to fully understand the Bubble Sort algorithm and how to implement it yourself.</p>
...
<hr>
<p>Run the simulation below to see the 8 steps above animated:</p>
<div id="vueApp2">
<!--
...
<script>
  ...
  methods: {
    action(){
      if(this.buttonText === "Run Through Once") {
        this.bblSort();
      }
      else {
        this.addDie50();
    ...
    async bblSort() {
      this.btnIsDisabled = true;
      for (var j = 0; j < (this.dice.length - 1); j++) {
        this.dice[j].isSwapped = true;
        this.dice[j + 1].isSwapped = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        if (this.dice[j].dieNmbr > this.dice[j + 1].dieNmbr) {
      ...
      this.msgDone = 'First Run Through Finished!';
    ...
    addDie50() {
      ...
      this.buttonText = "Run Through Once";
      this.msgDone = '';
    }
  },
  mounted() {
...
<p>We must understand what happened in this first run through to fully understand the algorithm, so that we can implement the algorithm in a programming language.</p>
<p>Can you see what happened to the highest value 12? It has bubbled up to the end of the array, where it belongs. But the rest of the array remains unsorted.</p>
<p>So the Bubble Sort algorithm must run through the array again, and again, and again, each time the next highest value bubbles up to its correct position. The sorting continues until the lowest value 3 is left at the start of the array. This means that we need to run through the array 4 times, to sort the array of 5 values.</p>
<p>And each time the algorithm runs through the array, the remaining unsorted part of the array becomes shorter.</p>
<p>This is how a full manual run through looks like:</p>
<div id="vueApp4">
<!--
  <p>Delay: <input v-model="delay" type="range" min="50" max="999"> {{ delay }}</p>
-->
...
print("Sorted array:", my_array)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_bubblesort" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>
...
<div class="w3-example">
  <pre class="language-python notranslate w3-white"><code>my_array = [7, 3, 9, 12, 11]</code></pre>
</div>
...
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white" data-line="5,9-11"><code>my_array = [7, 3, 9, 12, 11]
...
print("Sorted array:", my_array)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_bubblesort2" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>
...
<p class="fontComment">For a more thorough and detailed explanation of Bubble Sort time complexity, visit <a href="dsa_timecomplexity_bblsort.php">this page</a>.</p>
...
<p>This means there are \(n \cdot n\) comparisons done in total, so the time complexity for Bubble Sort is:</p>
<p><span class="math display">\[ \underline{\underline{O(n^2)}} \]</span></p>
<p>The graph describing the Bubble Sort time complexity looks like this:</p>
<img src="img_runtime_bubbleSort.png" alt="Bubble Sort time complexity">
<p>As you can see, the run time increases really fast when the size of the array is increased.</p>
<p>Luckily there are sorting algorithms that are faster than this, like <a href="dsa_algo_quicksort.php">Quicksort</a>, that we will look at later.</p>
...
<script>
  ...
  methods: {
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performBubbleSort();
    ...
    async performBubbleSort() {
      ...
      this.operations = 0;
      let swapped;
      if(this.selectedRunType==='bestCase'){
        do {
          swapped = false;
          this.operations++;
          for (let i = 1; i < n; i++) {
        ...
        do {
          ...
          if(n%3===0){
            this.array[n-1].active = true;
            await new Promise(resolve => setTimeout(resolve, 1));
            this.array[n-1].active = false;
          }
          n=n-1;
        } while (swapped);
      }
      
      //onsole.log(this.operations);
      this.clearBtnDisabled = false;
...
<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_algo_bubblesort1" method="post" target="_blank">
...
[<input name="ex1" maxlength="2" style="width: 3ch;">,<input name="ex2" maxlength="2" style="width: 3ch;">,<input name="ex3" maxlength="2" style="width: 3ch;">,<input name="ex4" maxlength="2" style="width: 3ch;">,<input name="ex5" maxlength="2" style="width: 3ch;">]
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_algo_bubblesort1">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
