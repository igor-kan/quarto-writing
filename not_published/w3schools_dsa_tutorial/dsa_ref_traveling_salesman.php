...
<html lang="en-US">
<head>
<title>DSA The Traveling Salesman Problem</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
.dijkstraEdges, .greedyEdges {
  stroke: black; 
  stroke-width: 1px; 
}.darkpagetheme .dijkstraEdges, .darkpagetheme .greedyEdges {
  stroke: whitesmoke;
}
...
.progBar {
  border: solid black 1px;
} .darkpagetheme .progBar {
  border-color: whitesmoke;
}

.progBar > div {
  background-color: lightgreen;
} .darkpagetheme .progBar > div {
  background-color: rgb(84,164,99);
}

body.darkpagetheme #scrollDiv::-webkit-scrollbar {width: 12px;}
body.darkpagetheme #scrollDiv::-webkit-scrollbar-track {background:rgb(21,32,43);}
body.darkpagetheme #scrollDiv::-webkit-scrollbar-thumb {background: #29353e;}
body.darkpagetheme #scrollDiv::-webkit-scrollbar-thumb:hover {background-color: #404e59}
...
ny:-->
    ...
    <h1><span class="color_h1">DSA The Traveling Salesman Problem</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_ref_huffman_coding.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_ref_knapsack.php">Next &#10095;</a>
</div>
<hr>

<h2>The Traveling Salesman Problem</h2>
<p>The Traveling Salesman Problem states that you are a salesperson and you must visit a number of cities or towns.</p>
<div class="ws-info">
  <p><strong>The Traveling Salesman Problem</strong></p>
  <p><u>Rules</u>: Visit every city only once, then return back to the city you started in.</p>
  <p><u>Goal</u>: Find the shortest possible route.</p>
</div>
<p>Except for the Held-Karp algorithm (which is quite advanced and time consuming, (\(O(2^n n^2)\)), and will not be described here), there is no other way to find the shortest route than to check all possible routes.</p>
<p>This means that the time complexity for solving this problem is \(O(n!)\), which means 720 routes needs to be checked for 6 cities, 40,320 routes must be checked for 8 cities, and if you have 10 cities to visit, more than 3.6 million routes must be checked!</p>
<div class="ws-note">
  <p><strong>Note: </strong>"!", or "factorial", is a mathematical operation used in combinatorics to find out how many possible ways something can be done. If there are 4 cities, each city is connected to every other city, and we must visit every city exactly once, there are \(4!= 4 \cdot 3 \cdot 2 \cdot 1 = 24\) different routes we can take to visit those cities.</p>
</div>
<p>The Traveling Salesman Problem (TSP) is a problem that is interesting to study because it is very practical, but so time consuming to solve, that it becomes nearly impossible to find the shortest route, even in a graph with just 20-30 vertices.</p>
<p>If we had an effective algorithm for solving The Traveling Salesman Problem, the consequences would be very big in many sectors, like for example chip design, vehicle routing, telecommunications, and urban planning.</p>
<hr>

<h2>Checking All Routes to Solve The Traveling Salesman Problem</h2>
<p>To find the optimal solution to The Traveling Salesman Problem, we will check all possible routes, and every time we find a shorter route, we will store it, so that in the end we will have the shortest route.</p>
<p><strong>Good: </strong>Finds the overall shortest route.</p>
<p><strong>Bad: </strong>Requires an awful lot of calculation, especially for a large amount of cities, which means it is very time consuming.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Check the length of every possible route, one route at a time.</li>
    <li>Is the current route shorter than the shortest route found so far? If so, store the new shortest route.</li>
    <li>After checking all routes, the stored route is the shortest one.</li>
  </ol>
</div>
<p>Such a way of finding the solution to a problem is called <strong>brute force</strong>.</p>
<p>Brute force is not really an algorithm, it just means finding the solution by checking all possibilities, usually because of a lack of a better way to do it.</p>
<div id="vueApp">
    ...
    <br>-->
    <p style="margin: 15px 15px 0px;">Finding the shortest route in The Traveling Salesman Problem by checking all routes (brute force).</p>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 400" class="dsa_svg">
      <transition-group>
        ...
        <line v-for="edge in shortestRoute" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
          :x2="dice[edge.to].xpos" :y2="dice[edge.to].ypos" :opacity="routeOpacity"
          :key="`best-${edge.from}-${edge.to}`" class="shortestRoute"/>
...
<!-- <p class="adtext">Advertisement</p> -->
    ...
    <div class="btnDiv">
      <p style="margin-bottom: 0;">Progress: {{progress}}%</p>
      <div class="progBar" style="width: 100%; height: 8px; position: relative;">
        <div style="position: absolute; height: 100%; x: 0; y: 0;" v-bind:style="{ width: progress+'%' }"></div>
      </div>
      <details style="margin: 18px 0;">
        <summary>Route distance: <span v-show="routeDist < 999999" style="text-align: right;">{{routeDist}}</span></summary>
        <div style="max-height: 70px; overflow-y: scroll; text-align: right; font-family: monospace; padding-right: 15px; color: gray; position: relative;" id="scrollDiv">
          <span style="position: absolute; top: 0; left: 0;">&nbsp;&nbsp;Log:</span>
          <template v-for="entry in log">
            <span v-html="entry"></span><br>
          </template>
        </div>
      </details>
    </div>
    <hr>
    <div class="btnDiv">
      <p><input v-model="vertices" type="range" id="customRange" min="5" max="10" step="1" @change="resetNodes" :disabled="btnIsDisabled"> n = {{vertices}} cities</p>
      <p>{{vertices}}!={{posRoutes}} possible routes</p>
      <p><label>Show every route: <input v-model="showCompares" type="checkbox"> {{showCompares}}</label></p>
      <button style="margin-right: 5px;" @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button>{{ msgDone }} <br>
      <button style="margin-right: 5px; margin-top: 10px;" @click="resetNodes" :disabled="!running">Reset</button>
    </div>
</div>
<script>
  ...
  computed: {
    ...
    posRoutes(){
      const number = this.factorialIterative(this.vertices);
    ...
    shortestRoute() {
      ...
      for (let child = 0; child < this.bestParents.length; child++) {
        const parent = this.bestParents[child];
  ...
  methods: {
      ...
      for (let i = 0; i < this.dice.length; i++) {
        ...
        do {
          ...
          isOverlap = this.dice.some((otherDie, j) => {
            ...
            const distance = Math.sqrt(dx * dx + dy * dy);
            return distance < 80; // Adjust the distance threshold as needed
          });
    ...
    checkOneRoute(route){
      this.parents = new Array(this.dice.length).fill(-1);
      if(this.showCompares){
        this.parents[0] = route[route.length-2];
        for (let i = 1; i < route.length - 1; i++) { // Skip the last element (return to start)
          const currentVertex = route[i];
          const parentVertex = route[i-1];
          this.parents[currentVertex] = parentVertex;
        }
      }

      let totalDistance = 0;

      // Iterate through the route array, but stop one short of the end since we're looking ahead by one each time
      for (let i = 0; i < route.length - 1; i++) {
        const fromVertex = route[i];
        const toVertex = route[i + 1];

        // Find the edge in the edges array that matches this segment of the route
        // Check for both directions since the graph is undirected
        const edge = this.edges.find(edge => (edge.from === fromVertex && edge.to === toVertex) || (edge.from === toVertex && edge.to === fromVertex));

        // If the edge was found, add its distance to the totalDistance
        if (edge) {
          totalDistance += edge.dist;
        } else {
          // If no direct edge was found, this might indicate an issue with the route or edges data
          console.warn(`No edge found between ${fromVertex} and ${toVertex}. Check the route and edges array.`);
        }
      }
      if(totalDistance < this.routeDist){
        this.routeDist = Math.round(totalDistance * 10) / 10;
        let logEntryText = this.progress +'% -> ';
        const distanceStr = this.routeDist.toFixed(1);
        if(distanceStr.length < 6) {
          logEntryText += '&nbsp;';
        }
        logEntryText += distanceStr;
        logEntryText += '&nbsp;&nbsp;&nbsp;';
        this.log.unshift(logEntryText);
        this.bestParents[0] = route[route.length-2];
        for (let i = 1; i < route.length - 1; i++) { // Skip the last element (return to start)
          const currentVertex = route[i];
          const parentVertex = route[i-1];
          this.bestParents[currentVertex] = parentVertex;
        }
        //this.bestParents = this.parents;
        return true;
      }
      return false;
      //console.log('Total distance of the round route:', totalDistance);
    },
    async runDijkstrasAlgorithm() {
      ...
      for(let i=1; i<this.dice.length; i++){
        verticesIndices.push(i);
      }
      const permutations = generatePermutations(verticesIndices);
      let i = 0;

      for (const permutation of permutations) {
        if (!this.shouldContinue) break;
        const route = [0, ...permutation, 0]; // Start and end at 0
        i++;
        this.progress = Math.round(100*i/permutations.length);
        if(i%1000===0){
          await new Promise(resolve => setTimeout(resolve, 20));
        }

        if(this.checkOneRoute(route)){
          this.parents = new Array(this.dice.length).fill(-1);
          this.routeOpacity = 1;
          await new Promise(resolve => setTimeout(resolve, 800));
          this.routeOpacity = 0.5;
        }
        else if(this.showCompares){
    ...
    addDie50() {
      this.dice = [];
      for (let i = 0; i < this.vertices; i++) { //18
        this.addDie(); 
      }
    ...
    updateEdgesFromMatrix() {
        ...
        this.buttonText = "Find Shortest Route";
        this.btnIsDisabled = false;
    },
    ...
    markFarthestVertex() {
      let randIndex = 0;//Math.floor(Math.random()*this.dice.length);
...
<p>The reason why the brute force approach of finding the shortest route (as shown above) is so time consuming is that we are checking all routes, and the number of possible routes increases really fast when the number of cities increases.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Finding the optimal solution to the Traveling Salesman Problem by checking all possible routes (brute force):</p>
  <pre class="language-python notranslate w3-white"><code>from itertools import permutations
...
print("Route:", route)
print("Total distance:", total_distance)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_tsp_brute_force" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Using A Greedy Algorithm to Solve The Traveling Salesman Problem</h2>
<p>Since checking every possible route to solve the Traveling Salesman Problem (like we did above) is so incredibly time consuming, we can instead find a short route by just going to the nearest unvisited city in each step, which is much faster.</p>
<p><strong>Good: </strong>Finds a solution to the Traveling Salesman Problem much faster than by checking all routes.</p>
<p><strong>Bad: </strong>Does not find the overall shortest route, it just finds a route that is much shorter than an average random route.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Visit every city.</li>
    <li>The next city to visit is always the nearest of the unvisited cities from the city you are currently in.</li>
    <li>After visiting all cities, go back to the city you started in.</li>
  </ol>
</div>
<p>This way of finding an approximation to the shortest route in the Traveling Salesman Problem, by just going to the nearest unvisited city in each step, is called a <strong>greedy algorithm</strong>.</p>
<div id="vueApp2">
    <!--<p>Speed: <input v-model="inpVal" type="range" min="500" max="900"> {{ inpVal }}</p>
    <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
    <br>-->
    <p style="margin: 15px 15px 0 15px;">Finding an approximation to the shortest route in The Traveling Salesman Problem by always going to the nearest unvisited neighbor (greedy algorithm).</p>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 400" class="dsa_svg">
      <transition-group>
        <line v-for="edge in edges" :x1="edge.x1" :y1="edge.y1" :x2="edge.x2" :y2="edge.y2" :key="edge.key" opacity="1" stroke-dasharray="5,5" class="allEdges" />
        <line v-for="edge in dijkstraEdges" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
          :x2="dice[edge.to].xpos" :y2="dice[edge.to].ypos"
          :key="`mst-${edge.from}-${edge.to}`" class="shortestRoute"/>
        <line v-for="edge in greedyEdges" 
          :x1="dice[edge.from].xpos" :y1="dice[edge.from].ypos"
          :x2="dice[edge.to].xpos" :y2="dice[edge.to].ypos"
          :key="`greed-${edge.from}-${edge.to}`" class="greedyEdges"/>
        <circle v-for="el in dice" 
          :key="el.keyNmbr"
          :cx="el.xpos" 
          :cy="el.ypos"
    ...
    <button style="margin: 0 auto; display: block;" @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button>
</div>
<script>
  ...
  computed: {
    ...
    greedyEdges() {
      ...
      // Iterate through the parents array
      for (let child = 0; child < this.posParents.length; child++) {
        const parent = this.posParents[child];
  ...
  methods: {
    ...
    this.visited = new Array(numVertices).fill(false);
    this.parents = new Array(numVertices).fill(-1);
    this.posParents = new Array(numVertices).fill(-1);
    
    // Start from a random vertex or a specific one
    let currentVertexIndex = this.dice.findIndex(vertex => vertex.isCurrent);
    let startingVertexIndex = currentVertexIndex;
    this.visited[currentVertexIndex] = true;

    for (let i = 0; i < numVertices - 1; i++) { // Visit all vertices except the last one
        ...
        for (let j = 0; j < numVertices; j++) {
          if (!this.visited[j]) {
            this.posParents[j] = currentVertexIndex;
            const distance = this.adjacencyMatrix[currentVertexIndex][j];
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        for(let k=0; k<this.posParents.length;k++){
          this.posParents[k]=-1;
        }
        for(let ind=0; ind<this.dice.length; ind++){
          this.dice[ind].isCurrent = false;
        ...
        if (closestVertexIndex !== -1) {
            this.visited[closestVertexIndex] = true;
            // Mark the current vertex as finished and the next one as current
            this.dice[currentVertexIndex].isFinished = true;
            this.dice[closestVertexIndex].isCurrent = true;
            // Store the path
            this.parents[closestVertexIndex] = currentVertexIndex;

            await new Promise(resolve => setTimeout(resolve, 500)); // Visualization delay
    ...
    this.parents[startingVertexIndex] = currentVertexIndex; // Close the loop by setting the parent of the start to the last vertex
    this.dice[currentVertexIndex].isFinished = true; // Mark the last vertex as finished
    this.dice[startingVertexIndex].isCurrent = true; // Optionally, highlight the start vertex again
    ...
    updateEdgesFromMatrix() {
        ...
        */
        this.buttonText = "Greedy Algorithm";
        this.btnIsDisabled = false;
    },
    markFarthestVertex() {
...
<p>As you can see by running this simulation a few times, the routes that are found are not completely unreasonable. Except for a few times when the lines cross perhaps, especially towards the end of the algorithm, the resulting route is a lot shorter than we would get by choosing the next city at random.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Finding a near-optimal solution to the Traveling Salesman Problem using the nearest-neighbor algorithm (greedy):</p>
  <pre class="language-python notranslate w3-white"><code>def nearest_neighbor_tsp(distances):
    n = len(distances)
    visited = [False] * n
    route = [0]
    visited[0] = True
...
print("Route:", route)
print("Total distance:", total_distance)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_tsp_nearest_neighbor" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Other Algorithms That Find Near-Optimal Solutions to The Traveling Salesman Problem</h2>
<p>In addition to using a greedy algorithm to solve the Traveling Salesman Problem, there are also other algorithms that can find approximations to the shortest route.</p>
<p>These algorithms are popular because they are much more effective than to actually check all possible solutions, but as with the greedy algorithm above, they do not find the overall shortest route.</p>
<p>Algorithms used to find a near-optimal solution to the Traveling Salesman Problem include:</p>
<ul>
  <li><strong>2-opt Heuristic: </strong>An algorithm that improves the solution step-by-step, in each step removing two edges and reconnecting the two paths in a different way to reduce the total path length.</li>
  <li><strong>Genetic Algorithm: </strong>This is a type of algorithm inspired by the process of natural selection and use techniques such as selection, mutation, and crossover to evolve solutions to problems, including the TSP.</li>
  <li><strong>Simulated Annealing: </strong>This method is inspired by the process of annealing in metallurgy. It involves heating and then slowly cooling a material to decrease defects. In the context of TSP, it's used to find a near-optimal solution by exploring the solution space in a way that allows for occasional moves to worse solutions, which helps to avoid getting stuck in local minima.</li>
  <li><strong>Ant Colony Optimization: </strong>This algorithm is inspired by the behavior of ants in finding paths from the colony to food sources. It's a more complex probabilistic technique for solving computational problems which can be mapped to finding good paths through graphs.</li>
</ul>
<hr>

<h2>Time Complexity for Solving The Traveling Salesman Problem</h2>
<p>To get a near-optimal solution fast, we can use a greedy algorithm that just goes to the nearest unvisited city in each step, like in the second simulation on this page.</p>
<p>Solving The Traveling Salesman Problem in a greedy way like that, means that at each step, the distances from the current city to all other unvisited cities are compared, and that gives us a time complexity of \(O(n^2) \).</p>
...
<p>See the time complexity for the greedy algorithm \(O(n^2)\), versus the time complexity for finding the shortest route by comparing all routes \(O(n!)\), in the image below.</p>
<img src="img_plot_timecomplexity_tsp.png" alt="Time complexity for checking all routes versus running a greedy algorithm and finding a near-optimal solution instead." style="max-width: 100%;">
<p>But there are two things we can do to reduce the number of routes we need to check.</p>
<p>In the Traveling Salesman Problem, the route starts and ends in the same place, which makes a cycle. This means that the length of the shortest route will be the same no matter which city we start in. That is why we have chosen a fixed city to start in for the simulation above, and that reduces the number of possible routes from \(n!\) to \((n-1)!\).</p>
<p>Also, because these routes go in cycles, a route has the same distance if we go in one direction or the other, so we actually just need to check the distance of half of the routes, because the other half will just be the same routes in the opposite direction, so the number of routes we need to check is actually \( \frac{(n-1)!}{2}\).</p>
<p>But even if we can reduce the number of routes we need to check to \( \frac{(n-1)!}{2}\), the time complexity is still \( O(n!)\), because for very big \(n\), reducing \(n\) by one and dividing by 2 does not make a significant change in how the time complexity grows when \(n\) is increased.</p>
<p>To better understand how time complexity works, go to <a href="dsa_timecomplexity_theory.php">this page</a>.</p>
<hr>

<h2>Actual Traveling Salesman Problems Are More Complex</h2>
<p>The edge weight in a graph in this context of The Traveling Salesman Problem tells us how hard it is to go from one point to another, and it is the total edge weight of a route we want to minimize.</p>
<p>So far on this page, the edge weight has been the distance in a straight line between two points. And that makes it much easier to explain the Traveling Salesman Problem, and to display it.</p>
<p>But in the real world there are many other things that affects the edge weight:</p>
<ol>
  <li><strong>Obstacles: </strong>When moving from one place to another, we normally try to avoid obstacles like trees, rivers, houses for example. This means it is longer and takes more time to go from A to B, and the edge weight value needs to be increased to factor that in, because it is not a straight line anymore.</li>
  <li><strong>Transportation Networks: </strong>We usually follow a road or use public transport systems when traveling, and that also affects how hard it is to go (or send a package) from one place to another.</li>
  <li><strong>Traffic Conditions: </strong>Travel congestion also affects the travel time, so that should also be reflected in the edge weight value.</li>
  <li><strong>Legal and Political Boundaries: </strong>Crossing border for example, might make one route harder to choose than another, which means the shortest straight line route might be slower, or more costly.</li>
  <li><strong>Economic Factors: </strong>Using fuel, using the time of employees, maintaining vehicles, all these things cost money and should also be factored into the edge weights.</li>
</ol>
<p>As you can see, just using the straight line distances as the edge weights, might be too simple compared to the real problem. And solving the Traveling Salesman Problem for such a simplified problem model would probably give us a solution that is not optimal in a practical sense.</p>
<p>It is not easy to visualize a Traveling Salesman Problem when the edge length is not just the straight line distance between two points anymore, but the computer handles that very well.</p>
<hr>
