...
<html lang="en-US">
<head>
<title>DSA The Euclidean Algorithm</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
}.darkpagetheme #vueApp, .darkpagetheme #vueApp1 {
  border-color: #ddd;
}
...
#vueApp input[type=number], #vueApp1 input[type=number] {
...
ny:-->
    ...
    <h1><span class="color_h1">The Euclidean Algorithm</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="dsa_exam.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_ref_huffman_coding.php">Next &#10095;</a>
</div>
<div class="ws-info">
  <p>Named after the ancient Greek mathematician Euclid, the Euclidean algorithm is the oldest known non-trivial algorithm, described in Euclid's famous book "Elements" from 300 BCE.</p>
</div>

<h2>The Euclidean Algorithm</h2>
<p>The Euclidean algorithm finds the greatest common divisor (gcd) of two numbers \(a\) and \(b\).</p>
<p>The greatest common divisor is the largest number that divides both \(a\) and \(b\) without leaving a remainder.</p>
<div id="vueApp1">
  <p style="margin: 0;">Finding the greatest common divisor using division.</p>
  <div class="fixedFrame">
    <div class="frameDiv" v-bind:style="{height: wrapperHeight+'px'}">
      <transition-group name="v3">
        <div v-for="x in dice" 
          ...
          :class="{ 
            ...
            'diceDiv-ghost': x.isGhost,
          }" 
          :style="{
            height: x.dieNmbr*this.fac + 'px',
            width: this.diceWidth +'px',
            margin: '0 '+this.diceMargin +'px',
          }"
        >
          <!-- {{ x.dieNmbr }} -->
        </div>
    ...
    <div id="divisionDiv" 
          :style="{
            left: divWrapperPos + 'px',
            height: divWrapperHeight*this.fac + 'px'
          }">
      <!--<div class="diceDiv2" style="height: 40px; bottom: 0;"></div>-->
      <transition-group name="v2">
        <div v-for="(x,index) in dice2" 
          :key="x.keyNmbr" 
          class="diceDiv2" 
          :class="{ 
            'diceDiv-active': x.isActive
        ...
        >
          <!-- {{ 190-((index+1)*(x.dieNmbr*this.fac)) }} -->
        </div>
      </transition-group>
    </div> 
  </div>

  <!-- <p>Speed: <input v-model="inpVal" type="range" min="50" max="900">{{ delay }}</p>-->
  <p><label>\(a=\)<span class="nmbrSpan">{{nmbr1}}</span><input type="range" v-model.number="nmbr1" step="1" min="2" max="50" :disabled="inpIsDisabled"></label></p>
  <p><label>\(b=\)<span class="nmbrSpan">{{nmbr2}}</span><input type="range" v-model.number="nmbr2" step="1" min="2" max="50" :disabled="inpIsDisabled"></label></p>
  <p>Result: <span v-html="result"></span></p>
  <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}<br><hr style="width: 95%; display:block; margin: 20px auto;">
  <details>
    <summary>Calculations</summary>
    <ol v-show="calculations">
      <li v-for="(step,index) in calculations">
        <span v-html="step"></span><span v-html="remainders[index]"></span>
      </li>
    </ol>
  </details>
</div>
...
const app1 = Vue.createApp({
  data() {
    return {
      nmbr1: 78,
      nmbr2: 48,
      nmbrCombinations: [[50,15],[48,11],[20,18],[50,25],[39,24]],
      rollingIndex: 0,
      wrapperHeight: 191,
      inpIsDisabled: false,
      btnIsDisabled: false,
  ...
  computed: {
    ...
    fac(){
      let highest = Math.max(this.nmbr1,this.nmbr2);
      return 190/highest;
    },
    divWrapperPos(){
      return (this.divWrapperColumn)*14;
    },
    divWrapperHeight(){
      if(this.dice.length>1){
        return this.dice[this.divWrapperColumn].dieNmbr;
      }
      else {
        return 0;
      }
    ...
    diceWidth(){
      const nonGhostElementsCount = this.dice.filter(element => !element.isGhost).length;
      if (nonGhostElementsCount < 24) {
        return 10;
      } else if (nonGhostElementsCount < 33) {
        return 8;
      } else if (nonGhostElementsCount < 55) {
        return 5;
      } else if (nonGhostElementsCount < 100) {
        return 3;
      } else {
        return 2;
      }
  ...
  watch: {
    ...
    nmbr1(newVal) {
      this.dice[0].dieNmbr = newVal;
    },
    nmbr2(newVal) {
      this.dice[1].dieNmbr = newVal;
    }
  },
  methods: {
    action(){
      if(this.buttonText === "Find GCD") {
        this.findGCD();
      }
      else {
        this.addDie50();
    ...
    async findGCD() {
      ...
      let divisionDelay = 500;
      let length = this.dice.length;
      await new Promise(resolve => setTimeout(resolve, this.delay));
      let a = this.dice[length-2].dieNmbr;
      let b = this.dice[length-1].dieNmbr;
      if(a%b===0){
        this.remainders.push(a%b);
        this.calculations.push('&nbsp; &nbsp; &nbsp;'+a+' = '+Math.floor(a/b)+' * <span class="hashcode">'+b+'</span> + ');
      }
      let remainder = a%b;
      while(remainder != 0){
        this.dice[this.dice.length-1].isActive = true;
        await new Promise(resolve => setTimeout(resolve, divisionDelay));
        let b_in_a = Math.floor(a/b);
        this.remainders.push(a%b);
        this.calculations.push('&nbsp; &nbsp; &nbsp;'+a+' = '+Math.floor(a/b)+' &#183; '+b+' + ');
        for(let i=0; i<b_in_a; i++){
          this.addDie2(b,true);
          await new Promise(resolve => setTimeout(resolve, divisionDelay));
          
          this.dice2[this.dice2.length-1].bottom = this.divWrapperHeight*this.fac-((this.dice2.length)*(this.dice2[this.dice2.length-1].dieNmbr*this.fac));
          await new Promise(resolve => setTimeout(resolve, divisionDelay));
          this.dice2[this.dice2.length-1].isActive = false;
        }
        
        this.dice[this.dice.length-1].isActive = false;
        await new Promise(resolve => setTimeout(resolve, divisionDelay));
        await new Promise(resolve => setTimeout(resolve, divisionDelay));
        ...
        this.wrapperHeight = this.dice[this.dice.length-1].dieNmbr*this.fac+0.7;
        await new Promise(resolve => setTimeout(resolve, 100));
        this.addDie(remainder,true);
        await new Promise(resolve => setTimeout(resolve, divisionDelay));
        this.dice[this.dice.length-1].isActive = false;
        await new Promise(resolve => setTimeout(resolve, divisionDelay));
        this.dice2 = [];
      ...
      this.dice[this.dice.length-1].isActive = true;
      await new Promise(resolve => setTimeout(resolve, divisionDelay));
      this.result = 'The GCD of '+this.nmbr1+' and '+this.nmbr2+' is <span class="hashcode">'+b+'</span>.';
      this.remainders[this.remainders.length-2] = '<span class="hashcode">'+b+'</span>';
    ...
    addGhost() {
      ...
      this.dice.splice(this.dice.length-1,0,newDie);
      this.keyNumber++;
    },
    removeGhost() {
      if(this.dice[this.dice.length-1].isGhost){
        this.dice.splice(this.dice.length-1,1);
      }
    },
    addDie50() {
      this.result = '';
      this.divWrapperColumn = 0;
      this.inpIsDisabled = false;
      this.wrapperHeight = 191;
      this.nmbr1 = this.nmbrCombinations[this.rollingIndex][0];
      this.nmbr2 = this.nmbrCombinations[this.rollingIndex][1];
      if(this.rollingIndex >= this.nmbrCombinations.length-1){
        this.rollingIndex = 0;
      }
      else {
        this.rollingIndex++;
      ...
      this.remainders = [];
      this.dice = [];
      this.addDie(this.nmbr1);
      this.addDie(this.nmbr2);
      this.ghostDivIsFat = false;
      this.buttonText = "Find GCD";
      this.msgDone = '';
    },
    compareFunc(a, b) {
...
<p>The algorithm uses division with remainder. It takes the remainder from the previous step to set up the calculation in the next step.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Start with the two initial numbers \(a\) and \(b\).</li>
    <li>Do a division with remainder: \(a=q_0 \cdot b + r_0\)</li>
    <li>Use the remainder (\(r_0\)) and the divisor (\(b\)) from the last calculation to set up the next calculation: \(b=q_1 \cdot r_0 + r_1\)</li>
    <li>Repeat steps 2 and 3 until the remainder is \(0\).</li>
    <li>The second last remainder calculated is the greatest common divisor.</li>
  </ol>
</div>
<p>Continue reading to see how the Euclidean algorithm can be done by hand, with programming, and to understand how and why the algorithm actually works.</p>
...
<h2>Mathematical Terminology</h2>
<p>Below are words used to describe the Euclidean Algorithm that you need to know to understand the explanations on this page.</p>
<p><strong>Divisor: </strong>A number we can use to divide a number by, without leaving a remainder. We say that 3 is a divisor of 6 because \(6/3=2\), without leaving a remainder (the remainder is 0).</p>
<p><strong>Remainder: </strong>The part you are left with after dividing a number with another number. Dividing 7 by 3 is 2, with a remainder of 1. (So 3 is not a divisor of 7.)</p>
<p><strong>Common divisor: </strong>For numbers \(a\) and \(b\), a common divisor is a number that can divide both \(a\) and \(b\) without leaving a remainder. The common divisors of 18 and 12 are 2, 3, and 6, because both 18 and 12 can be divided by 2, 3, and 6 without producing a remainder.</p>
<p><strong>Greatest common divisor: </strong>The largest of the common divisors. The greatest common divisor of 18 and 12 is 6 because that is the largest of the common divisors 2, 3, and 6.</p>
<p>The greatest common divisor is used in the mathematical field of Number Theory, and in cryptography for encrypting messages.</p>
<div class="ws-note">
  <p><strong>Note: </strong>All numbers used by the Euclidean algorithm are integers.</p>
</div>
<hr>

<h2>Manual Run Through</h2>
<p>To understand how the Euclidean algorithm works, and to write the code for it, let's first run it manually to find the greatest common divisor of \(120\) and \(25\).</p>
<p>To do this we use division with remainder.</p>
<p><strong>Step 1: </strong>We start with dividing \(120\) with \(25\):</p>
...
\end{equation}
\]</span></p>
<p>How many times can we fit \(25\) inside \(120\)? It is \(4\) times, right? \(4 \cdot 25\) is \(100\). We get the remainder \(20\) by subtracting \(100\) from \(120\).</p>
<p><strong>Step 2: </strong>We use the previous remainder \(20\) in the next step to divide \(25\):</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
25 & = 1 \cdot 20 + 5
...
\end{equation}
\]</span></p>
<p>We can fit \(20\) inside \(25\) one time. We get the remainder \(5\) by subtracting \(20\) from \(25\).</p>
<p><strong>Step 3: </strong>In the next calculation we divide \(20\) with the previous remainder \(5\):</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
20 & = 4 \cdot 5 + 0
...
\end{equation}
\]</span></p>
<p>We get \(0\) as the remainder, and that means that we are done with the calculations.</p>
<p>The greatest common divisor of \(120\) and \(25\) is \(5\).</p>
<hr>

<h2>Implementation of The Euclidean Algorithm</h2>
<p>To find the greatest common divisor using division, we continue running the algorithm until the remainder calculated is \(0\).</p>
<p>This is the same as saying we continue to run the algorithm as long as \(b\) is not \(0\). That is why <code class="w3-codespan">b != 0</code> is the condition in the <code class="w3-codespan">while</code> loop below.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Finding the greatest common divisor of 120 and 25 using the Euclidean algorithm:</p>
  <pre class="language-python notranslate w3-white"><code>def gcd_division(a, b):
    while b != 0:
        remainder = a % b
...
print(f"The GCD of {a} and {b} is: {gcd_division(a, b)}")</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_gcd_division" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>The Original Euclidean Algorithm</h2>
<p>Instead of using division like we did above, the original Euclidean algorithm as described in the book "Elements" over 2000 years ago uses subtraction.</p>
<div id="vueApp">
  <p style="margin: 0;">Finding the greatest common divisor using subtraction.</p>
  <div class="fixedFrame">
    <div class="frameDiv" v-bind:style="{height: wrapperHeight+'px'}">
      <transition-group>
        <div v-for="x in dice" 
  ...
  <details>
    ...
    <ol v-show="calculations">
      <li v-for="(step,index) in calculations">
        <span v-html="step"></span><span v-html="differences[index]"></span>
      </li>
    </ol>
  </details>
</div>
...
const app = Vue.createApp({
  ...
  watch: {
    ...
    nmbr1(val) {
      this.dice[0].dieNmbr = val;
    },
    nmbr2(val) {
      this.dice[1].dieNmbr = val;
    }
  },
  methods: {
    action(){
    ...
    async findGCD() {
      ...
      while(a!==b){
        let calc = '';
        if(this.dice[this.dice.length-1].dieNmbr > this.dice[this.dice.length-2].dieNmbr){
          let tmpDice = this.dice.splice(this.dice.length-2,1)[0];
          this.dice.push(tmpDice);
          await new Promise(resolve => setTimeout(resolve, this.delay));
        }
        a = this.dice[this.dice.length-2].dieNmbr;
        b = this.dice[this.dice.length-1].dieNmbr;
        if(a > b){
          calc = '&nbsp; &nbsp; &nbsp;'+a+' - '+b+' = ';
          this.differences.push(a-b);
          a = a - b;
          this.addDie(a,true);
        }
        else {
          calc = '&nbsp; &nbsp; &nbsp;'+b+' - '+a+' = ';
          this.differences.push(b-a);
          b = b - a;
          this.addDie(b,true);
        }
        this.calculations.push(calc);
        // New version
        this.wrapperHeight = (this.dice[this.dice.length-1].dieNmbr+this.dice[this.dice.length-2].dieNmbr)*this.fac+0.1;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        this.addGhost(); // difference falls down
        await new Promise(resolve => setTimeout(resolve, 200)); // Allow the difference to fall down nicely
        this.wrapperHeight = (this.dice[this.dice.length-3].dieNmbr+this.dice[this.dice.length-2].dieNmbr)*this.fac+1.5;
        this.removeGhost();
        this.dice[this.dice.length-1].isActive = false;
        //await new Promise(resolve => setTimeout(resolve, this.delay));
      }
      await new Promise(resolve => setTimeout(resolve, 500));
      this.dice[this.dice.length-1].isActive = true;
      this.calculations.push('&nbsp; &nbsp; &nbsp;'+a+' - '+b+' = 0');
      this.differences[this.differences.length-1] = '<span class="hashcode">'+a+'</span>';
      this.result = 'The GCD of '+this.nmbr1+' and '+this.nmbr2+' is '+'<span class="hashcode">'+a+'</span>.';
    ...
    removeGhost() {
      this.dice.splice(this.dice.length-2,1);
    },
    addDie50() {
      this.result = '';
      this.inpIsDisabled = false;
...
<div class="ws-info">
  <p><strong>How the Euclidean algorithm with subtraction works:</strong></p>
  <ol>
    <li>Start with the two initial numbers \(a\) and \(b\).</li>
    <li>Find the difference \( a-b=c\). The difference \(c\) shares the same greatest common divisor as \(a\) and \(b\).</li>
    <li>Take the two lowest numbers of \(a\), \(b\), and \(c\), and find the difference between them.</li>
    <li>Repeat steps 2 and 3 until the difference is \(0\).</li>
    <li>The second last difference calculated is the greatest common divisor.</li>
  </ol>
</div>
<p>Using subtraction instead of division is not as fast, but both the division method and the subtraction method uses the same mathematical principle:</p>
<div class="ws-info">
  <p><i>The greatest common divisor of numbers \(a\) and \(b\), is also the greatest common divisor of the difference between \(a\) and \(b\).</i></p>
</div>
<p>This can be shown in just a few lines.</p>
<p>Numbers \(a\) and \(b\) have a greatest common divisor \(x\).</p>
<p>This means that both \(a\) and \(b\) can be factorized like this:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
a & = k \cdot x \\
...
\end{equation}
\]</span></p>
<p>After factorization, subtracting \(b\) from \(a\) gives us a very interesting result:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
...
\end{equation}
\]</span></p>
<p>We can see that the greatest common divisor (\(x\)) of \(a\) and \(b\) is also the greatest common divisor of the difference between \(a\) and \(b\)!</p>
<p>This principle is why the Euclidean algorithm works, it is what makes it possible.</p>
<hr>

<h2>Finding The Greatest Common Divisor Using Subtraction</h2>
<p>Using the principle described above, that the difference between \(a\) and \(b\) also shares the same greatest common divisor, we can use subtraction to find the greatest common divisor, like Euclid's original algorithm does.</p>
<p>Let's find the greatest common divisor of \(120\) from \(25\) using subtraction.</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
120 - 25 & = 95 
...
\end{equation}
\]</span></p>
<p>According to the mathematical principle already described, the numbers \(120\), \(25\), and \(95\) all share the same greatest common divisor.</p>
<p>This means we can further reduce our problem by subtracting \(25\) from \(95\):</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
95 - 25 & = 70 
...
\end{equation}
\]</span></p>
<p>If we continue like this, always taking the two lowest numbers from the previous step and finding the difference between them, we get these calculations:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
70 - 25   & = 45 \\
...
\end{equation}
\]</span></p>
<p>The Euclidean algorithm using subtraction is done when the difference is \(0\).</p>
<p>The greatest common divisor of \(120\) and \(25\) can be found in the previous step, it is \(5\).</p>
<p>Now that we can calculate the greatest common divisor using subtraction by hand, it is easier to implement it in a programming language.</p>
<hr>

<h2>Implementation of The Euclidean Algorithm Using Subtraction</h2>
<p>To find the greatest common divisor using subtraction, we continue running the algorithm until the difference between \(a\) and \(b\) is \(0\), like we have just seen.</p>
<p>This is the same as saying we continue running the algorithm as long as \(a\) and \(b\) are different values. That is why <code class="w3-codespan">a != b</code> is the condition in the <code class="w3-codespan">while</code> loop below.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Finding the greatest common divisor of 120 and 25 using the Euclidean algorithm with subtraction:</p>
  <pre class="language-python notranslate w3-white"><code>def gcd_subtraction(a, b):
    while a != b:
        if a > b:
            print(f"{a} - {b} = {a-b}")
            a = a - b
...
print(f"The GCD of {a} and {b} is: {gcd_subtraction(a, b)}")</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_gcd_subtraction" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Comparing The Subtraction Method With The Division Method</h2>
<p>To see how good the division method can be to find the greatest common divisor, and how the methods are similar, we will:</p>
<ol>
  <li>Use subtraction to find the greatest common divisor of \(120\) and \(25\).</li>
  <li>Use division with remainder to find the greatest common divisor of \(120\) and \(25\).</li>
  <li>Compare the subtraction and division methods.</li>
</ol>
<hr>

<h3>1. Using Subtraction</h3>
<p>Finding the greatest common divisor of \(120\) and \(25\) using subtraction:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
120 - 25  & = 95 \\
...
\end{equation}
\]</span></p>
<p>Using subtraction, the algorithm is finished when the difference is \(0\).</p>
<p>In the second last calculation we see that the greatest common divisor of \(120\) and \(25\) is \(5\).</p>
<p>Notice how \(25\) and \(5\) must be subtracted many times, until finding the GCD.</p>
<hr>

<h3>2. Using Division</h3>
<p>Finding the greatest common divisor of \(120\) and \(25\) using division with remainder looks like this:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
120  & = 4 \cdot 25 + 20 \\
...
\end{equation}
\]</span></p>
<p>Using division, the Euclidean algorithm is finished when the remainder is \(0\).</p>
<p>The previous remainder \(5\) is the greatest common divisor of \(120\) and \(25\).</p>
<hr>

<h3>3. Comparison</h3>
<p>Take a look at the subtraction and division methods above.</p>
<p>To easier see that the division calculations are basically the same as the subtraction calculations, we can write the division with remainder calculations like this:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
120 - 4 \cdot 25  & = 20 \\
...
\end{equation}
\]</span></p>
<p>Using subtraction, \(25\) is subtracted from \(120\) a total of \(4\) times, while the division method does this in just one step.</p>
<p>Similarly, the subtraction method subtracts \(5\) a total of \(4\) times, while the division method does the same in just one calculation.</p>
<p>As you can see, the methods do the same thing, the division method just does many subtractions in the same calculation, so that it finds the greatest common divisor faster.</p>
<hr>
