...
<html lang="en-US">
<head>
<title>DSA Array Implementation</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Array Implementation</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_binarytrees_postorder.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_data_binarysearchtrees.php">Next &#10095;</a>
</div>
...
<p>To avoid the cost of all the shifts in memory that we get from using Arrays, it is useful to implement Binary Trees with pointers from one element to the next, just like Binary Trees are implemented before this point, especially when the Binary Tree is modified often.</p>
<p>But in case we read from the Binary Tree a lot more than we modify it, an Array implementation of a Binary Tree can make sense as it needs less memory, it can be easier to implement, and it can be faster for certain operations due to cache locality.</p>

<div class="ws-note">
  <p><strong>Cache Locality </strong>is when the fast cache memory in the computer stores parts of memory that was recently accessed, or when the cache stores parts of memory that is close to the address that is currently accessed. This happens because it is likely that the CPU needs something in the next cycle that is close to what it used in the previous cycle, either close in time or close in space.</p>
  <p>Since Array elements are stored contiguously in memory, one element right after the other, computers are sometimes faster when reading from Arrays because the next element is already cached, available for fast access in case the CPU needs it in the next cycle.</p>
  <p>How arrays are stored in memory is explained more in detail <a href="dsa_theory_linkedlists_memory.php#arraysInMemory">here</a>.</p>
</div>
...
<p>This Binary Tree can be stored in an Array starting with the root node R on index 0. The rest of the tree can be built by taking a node stored on index \(i\), and storing its left child node on index \(2\cdot i+1\), and its right child node on index \(2\cdot i+2\).</p>
<p>Below is an Array implementation of the Binary Tree.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white"><code>binary_tree_array = ['R', 'A', 'B', 'C', 'D', 'E', 'F', None, None, None, None, None, None, 'G']
...
print("root.right.left.data:", data)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_binarytree_arrayimpl" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>In this Array implementation, since the Binary Tree nodes are placed in an array, much of the code is about accessing nodes using indexes, and about how to find the correct indexes.</p>
<p>Let's say we want to find the left and right child nodes of node B. Because B is on index 2, B's left child is on index \(2\cdot 2+1=5\), which is node E, right? And B's right child is on index \(2\cdot 2+2=6\), which is node F, and that also fits with the drawing above, right?</p>
<p>As you can see on line 1, this implementation requires empty array elements where nodes have no child nodes. So to avoid wasting space on empty Array elements, Binary Trees stored using Array implementation should be a "perfect" Binary Tree, or a nearly perfect one.</p>
<p>A perfect Binary Tree is when every internal node have exactly two child nodes, and all leaf nodes are on the same level.</p>
<p>If we remove the G node in the Binary Tree above, it looks like this:</p>
<div id="svgDiv"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 350 240" class="svgAnim">
  <!-- 'R' node -->
  <g class="root parent">
    <circle cx="170" cy="30" r="20" class="node" />
    <text x="164" y="37" class="svgDataText">R</text>
...
<p>And the first line in the code above can be written without wasting space on empty Array elements:</p>
<div class="w3-example">
  <pre class="language-python notranslate line-numbers w3-white"><code>binary_tree_array = ['R', 'A', 'B', 'C', 'D', 'E', 'F']</code></pre>
</div>
<p>This is how the three different DFS traversals can be done on an Array implementation of a Binary Tree.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white"><code>binary_tree_array = ['R', 'A', 'B', 'C', 'D', 'E', 'F', None, None, None, None, None, None, 'G']
