...
<html lang="en-US">
<head>
<title>DSA Counting Sort Time Complexity</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Counting Sort Time Complexity</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_timecomplexity_quicksort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_timecomplexity_radixsort.php">Next &#10095;</a>
</div>
<hr>
...
<p><a href="dsa_algo_countingsort.php">Counting Sort</a> works by first counting the occurrence of different values, and then uses that to recreate the array in a sorted order.</p>

<p>As a rule of thumb, the Counting Sort algorithm runs fast  when the range of possible values \(k\) is smaller than the number of values \(n\).</p>
<p>To represent the time complexity with Big O notation we need to first count the number of operations the algorithm does:</p>
<ul>
  <li>Finding the maximum value: Every value must be evaluated once to find out if it is the maximum value, so \(n\) operations are needed.</li>
  <li>Initializing the counting array: With \(k\) as the maximum value in the array, we need \(k+1\) elements in the counting array to include 0. Every element in the counting array must be initialized, so \(k+1\) operations are needed.</li>
  <li>Every value we want to sort is counted once, then removed, so 2 operations per count, \(2 \cdot n\) operations in total.</li>
  <li>Building the sorted array: Create \(n\) elements in the sorted array: \(n\) operations.</li>
</ul>
<p>In total we get:</p>
...
\end{equation}
\]</span></p>
<p>Based on what have seen about time complexity earlier, we can create a simplified expression using Big O notiation to represent time complexity:</p>
<p><span class="math display">\[
\begin{equation}
\begin{aligned}
...
\end{equation}
\]</span></p>
<p>It has already been mentioned that Counting Sort is effective when the range of different values \(k\) is relatively small compared to the total number of values to be sorted \(n\). We can now see this directly from the Big O expression \(O(n+k)\).</p>
<p>Just imagine for example that the range of different numbers \(k\) is 10 times as large as the number of the values to be sorted. In such a case we can see that the algorithm uses most of its time on handling the range of different numbers \(k\), although the actual number of values to be sorted \(n\) is small in comparison.</p>
<p>It is not straight forward to show the time complexity for Counting Sort in a graph, or to have a simulation for the time complexity as we have had for the previous algorithms, because the time complexity is so greatly affected by the range of values \(k\).</p>
<p>Below is a plot that shows how much the time complexity for Counting Sort can vary, followed by an explanation for the best and worst case scenarios.</p>
<img src="img_runtime_countingSort.png" class="imgTut" alt="Time Complexity">
<p>The <strong>best case</strong> scenario for Counting Sort would be to have the range \(k\) just a fraction of \(n\), let's say \(k(n)=0.1 \cdot n\). As an example of this, for 100 values, the range would be from 0 to 10, or for 1000 values the range would be from 0 to 100. In this case we get time complexity \(O(n+k)=O(n+0.1 \cdot n) = O(1.1 \cdot n)\) which is simplified to \(O(n)\).</p>
<p>The <strong>worst case</strong> however would be if the range is a lot larger than the input. Let's say for an input of just 10 values the the range is between 0 and 100, or similarly, for an input of 1000 values, the range is between 0 and 1000000. In such a scenario, the growth of \(k\) is quadratic with respect to \(n\), like this: \(k(n)=n^2\), and we get time complexity \(O(n+k)=O(n+n^2)\) which is simplified to \(O(n^2)\). A case that is even worse than this could also be constructed, but this case is chosen because it is relatively easy to understand, and perhaps not that unrealistic either.</p>
...
<h2>Counting Sort Simulation</h2>
