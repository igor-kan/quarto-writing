...
<html lang="en-US">
<head>
<title>DSA Edmonds-Karp Algorithm</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
.dsa_svg path, .dsa_svg line, .dsa_svg circle, .dsa_svg text {
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Edmonds-Karp Algorithm</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_graphs_fordfulkerson.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_timecomplexity_theory.php">Next &#10095;</a>
</div>
<div class="ws-info">
  <p>The Edmonds-Karp algorithm solves the maximum flow problem.</p>
  <p>Finding the maximum flow can be helpful in many areas: for optimizing network traffic, for manufacturing, for supply chain and logistics, or for airline scheduling.</p>
</div>

<h2>The Edmonds-Karp Algorithm</h2>
<p>The Edmonds-Karp algorithm solves <a href="dsa_theory_graphs_maxflow.php">the maximum flow problem</a> for a directed graph.</p>
<p>The flow comes from a source vertex (\(s\)) and ends up in a sink vertex (\(t\)), and each edge in the graph allows a flow, limited by a capacity.</p>
<p>The Edmonds-Karp algorithm is very similar to <a href="dsa_algo_graphs_fordfulkerson.php">the Ford-Fulkerson algorithm</a>, except the Edmonds-Karp algorithm uses <a href="dsa_algo_graphs_traversal.php#bfsTraversal">Breadth First Search (BFS)</a> to find augmented paths to increase flow.</p>
<div id="vueApp2">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
  <transition-group>    

    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isActive, redText: edge.rev }">{{edge.flow}}/{{edge.capacity}}</text>

    <!-- Default paths for empty edges -->
    <line v-for="edge in flowEdges" :key="`generateEdgeKey('empty',edge)`"  
      :x1="vertices[edge.from].x" :y1="vertices[edge.from].y" 
      :x2="vertices[edge.to].x" :y2="vertices[edge.to].y"
      fill="none" stroke="gray" stroke-width="1" stroke-dasharray="4" :opacity="edge.opacity"/>

    <!-- Paths for flow edges -->
    <path v-for="edge in flowEdges" :key="`generateEdgeKey('flow',edge)`" :d="edge.path" fill="gray" stroke="none" :opacity="edge.opacity"/>

    <!-- Paths for capacity edges -->
    <path v-for="edge in capacityEdges" :key="`generateEdgeKey('cap',edge)`" :d="edge.path" fill-opacity="0.2" :opacity="edge.opacity" class="capEdge" :class="{ augPath: edge.isAug, redEdge: edge.rev }"/>

    <!-- Circles for vertices -->
    <circle v-for="vertex in vertices" 
        :key="vertex.key"
        :cx="vertex.x" 
        :cy="vertex.y"
        :r="vertex.radius"
        :stroke-width="vertex.isAug ? 2 : 1"
        class="vertex"/>

    <!-- Circles for vertices in augmented path -->
    <circle v-for="vertex in vertices"
        :key="`overlay-${vertex.key}`"
        :cx="vertex.x" 
        :cy="vertex.y"
        :r="vertex.radius-0.5"
    ...
    <text v-for="vertex in vertices" :key="`name-${vertex.key}`" :x="vertex.x" :y="vertex.y+5" class="svgText">{{vertex.name}}</text>

  </transition-group>
  </svg>
  <div class="btnDiv">
    <p>Max flow: {{maxFlow}}</p>
    <button @click="action" :disabled="btnIsDisabled">{{btnText}}</button> {{statusText}}
  </div>
</div>
<script>
const app2 = Vue.createApp({
  data() {
    return {
      btnText: 'Run Edmonds-Karp',
      btnIsDisabled: false,
      statusText: '',
      maxFlow: 0,
      augPathVertices: [],
      vertices: [
        { key: 0, x: 20, y: 120, radius: 15, name: 's',   opacity: 0, isAug: false },
        { key: 1, x: 120, y: 20, radius: 15, name: 'v1',  opacity: 0, isAug: false },
        { key: 2, x: 120, y: 220, radius: 15, name: 'v2', opacity: 0, isAug: false },
        { key: 3, x: 280, y: 20, radius: 15, name: 'v3',  opacity: 0, isAug: false },
        { key: 4, x: 280, y: 220, radius: 15, name: 'v4', opacity: 0, isAug: false },
        { key: 5, x: 380, y: 120, radius: 15, name: 't',  opacity: 0, isAug: false }
      ],
      edges: [
        // Forward edges:
        { from: 0, to: 1, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 4, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 0, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 6, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        // Reverse edges:
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 3, to: 1, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 4, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 4, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 5, to: 3, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false }
      ]
    }
  },
  watch: {
    augPathVertices(val){
      for(let i=0; i<this.vertices.length; i++){
        ...
        this.vertices[i].isAug = 0;
      }
      for(let i=0; i<val.length; i++){
        this.vertices[val[i]].opacity = 0.2;
        this.vertices[val[i]].isAug = true;
      }
      if(val.every(element => element === 0)){
        this.vertices[0].opacity = 0;
      }

      for(let i=0; i<this.edges.length; i++){
        if(this.edges[i].opacity === 1){
          this.edges[i].isAug = false;
          this.edges[i].rev = false;
        }
      }
      ...
      for (let i = 0; i < val.length - 1; i++) {
        const from = val[i];
        const to = val[i + 1];
        let edge = this.edges.find(e => e.from === from && e.to === to && e.opacity === 1);
        if (edge) {
          edge.isAug = true;
        }
        else {
          edge = this.edges.find(e => e.from === to && e.to === from && e.opacity === 1);
          if(edge){
            edge.isAug = true;
            edge.rev = true;
            // Indicate that its a reverse edge somehow, red background color perhaps?
          }
        }
      }
    }
  ...
  computed: {
    capacityEdges() {
        return this.edges.map(edge => {
            const fromVertex = this.vertices.find(v => v.key === edge.from);
            const toVertex = this.vertices.find(v => v.key === edge.to);
            
            const dx = toVertex.x - fromVertex.x;
            const dy = toVertex.y - fromVertex.y;
            const angle = Math.atan2(dy, dx);
            const length = Math.sqrt(dx * dx + dy * dy) - toVertex.radius;
            const capacity = edge.capacity+1; // Thickness of the edge line
            
            // Offset for the thick line's corners
            const offsetX = capacity * Math.sin(angle);
            const offsetY = capacity * Math.cos(angle);

            // Points for the thick line (rectangle)
            const p1 = { x: fromVertex.x - offsetX, y: fromVertex.y + offsetY };
            const p2 = { x: fromVertex.x + offsetX, y: fromVertex.y - offsetY };
            const adjustedEndX = fromVertex.x + (length - capacity - toVertex.radius) * Math.cos(angle); // Shorten line to accommodate arrowhead
            const adjustedEndY = fromVertex.y + (length - capacity - toVertex.radius) * Math.sin(angle);
            const p3 = { x: adjustedEndX + offsetX, y: adjustedEndY - offsetY };
            const p4 = { x: adjustedEndX - offsetX, y: adjustedEndY + offsetY };

            // Line path
            const linePath = `M${p4.x},${p4.y} L${p1.x},${p1.y} L${p2.x},${p2.y} L${p3.x},${p3.y}`;

            // Arrowhead
            const tipX = toVertex.x - (toVertex.radius+1) * Math.cos(angle);
            const tipY = toVertex.y - (toVertex.radius+1) * Math.sin(angle);
            const arrowTip = { x: tipX, y: tipY };
            const leftCorner = { x: p3.x + offsetX, y: p3.y - offsetY };
            const rightCorner = { x: p4.x - offsetX, y: p4.y + offsetY };
            const arrowHeadPath = `L${leftCorner.x},${leftCorner.y} L${arrowTip.x},${arrowTip.y} L${rightCorner.x},${rightCorner.y}`;

            // Combine line and arrowhead into a single path
            const combinedPath = `${linePath} ${arrowHeadPath} Z`;

            return { ...edge, path: combinedPath };
        });
    },
    flowEdges() {
        return this.edges.map(edge => {
            ...
            const angle = Math.atan2(dy, dx);
            const length = Math.sqrt(dx * dx + dy * dy) - toVertex.radius;
            const flow = edge.flow === edge.capacity ? edge.flow + 1 : edge.flow;
            
            // Offset for the thick line's corners
            const offsetX = flow * Math.sin(angle);
            const offsetY = flow * Math.cos(angle);

            // Points for the thick line (rectangle)
            const p1 = { x: fromVertex.x - offsetX, y: fromVertex.y + offsetY };
            const p2 = { x: fromVertex.x + offsetX, y: fromVertex.y - offsetY };
            const adjustedEndX = fromVertex.x + (length - flow - toVertex.radius) * Math.cos(angle); // Shorten line to accommodate arrowhead
            const adjustedEndY = fromVertex.y + (length - flow - toVertex.radius) * Math.sin(angle);
            const p3 = { x: adjustedEndX + offsetX, y: adjustedEndY - offsetY };
            const p4 = { x: adjustedEndX - offsetX, y: adjustedEndY + offsetY };
            ...
            // Arrowhead
            const tipX = toVertex.x - toVertex.radius * Math.cos(angle);
            const tipY = toVertex.y - toVertex.radius * Math.sin(angle);
            const arrowTip = { x: tipX, y: tipY };
            const leftCorner = { x: p3.x + offsetX, y: p3.y - offsetY };
            const rightCorner = { x: p4.x - offsetX, y: p4.y + offsetY };
            const arrowHeadPath = `L${leftCorner.x},${leftCorner.y} L${arrowTip.x},${arrowTip.y} L${rightCorner.x},${rightCorner.y}`;
  ...
  methods: {
    async action(){
      if(this.btnText === 'Run Edmonds-Karp'){
        ...
        for(let i=0; i<this.edges.length;i++){
          this.edges[i].flow = 0;
          if(this.edges[i].opacity === 0){
            this.edges[i].resCap = 0;
          }
          else {
            this.edges[i].resCap = this.edges[i].capacity;
          }
        }
        await new Promise(resolve => setTimeout(resolve, 500));
        this.btnText = 'Run Edmonds-Karp';
        this.btnIsDisabled = false;
      }
    },
    calculateEdgeCenters() {
      this.edges.forEach(edge => {
        const fromVertex = this.vertices.find(vertex => vertex.key === edge.from);
        const toVertex = this.vertices.find(vertex => vertex.key === edge.to);

        // Calculate the midpoint
        edge.centerX = (fromVertex.x + toVertex.x) / 2;
        edge.centerY = (fromVertex.y + toVertex.y) / 2;
      });
      this.edges[0].centerX -= 30;
      this.edges[1].centerX -= 30;
      this.edges[1].centerY += 15;
      this.edges[2].centerX -= 10;
      this.edges[2].centerY -= 10;
      this.edges[3].centerX += 10;
      this.edges[4].centerX += 10;
      this.edges[5].centerX -= 10;
      this.edges[5].centerY -= 10;
      this.edges[6].centerX += 10;
      this.edges[7].centerX += 5;
      this.edges[7].centerY -= 5;
      this.edges[8].centerX += 5;
      this.edges[8].centerY += 20;
      
    },
    generateEdgeKey(type, edge) {
      // Generate a unique key for each edge based on its properties and the type
      return `${type}-${edge.from}-${edge.to}`;
    },
    async runEdmondsKarp() {
      let maxFlow = 0;
      let path = this.findPath(this.vertices[0].key, this.vertices[this.vertices.length - 1].key); // Assuming the first vertex is the source and the last is the sink
      
      while (path != null) {
        let tempPath = [...path];
        while(tempPath.length<6){
          tempPath.splice(0,0,0);
        }
        this.augPathVertices = tempPath;
        await new Promise(resolve => setTimeout(resolve, 1000));
        // Find the minimum residual capacity (bottleneck) in the found path
        const bottleneck = this.findBottleneck(path);

        // Update flows and residual capacities along the path
        await this.updateFlowAndResidual(path, bottleneck);
        this.maxFlow += bottleneck;

        await new Promise(resolve => setTimeout(resolve, 500));
        this.augPathVertices = [];
        await new Promise(resolve => setTimeout(resolve, 1000));

        // Increment max flow
        maxFlow += bottleneck;

        // Find a new path with updated residual capacities
        path = this.findPath(this.vertices[0].key, this.vertices[this.vertices.length - 1].key);
      }

      //console.log(`Max flow achieved: ${maxFlow}`);
      this.btnText = 'Reset';
      this.statusText = 'Done!';
      this.augPathVertices = [];
      this.btnIsDisabled = false;
    },
    findPath(sourceKey, sinkKey) {
        let visited = new Set();
        let queue = [{ node: sourceKey, path: [] }];

        while (queue.length > 0) {
            let { node, path } = queue.shift(); // Dequeue the first item

            // When the sink is found, return the path
            if (node === sinkKey) {
                return [...path, sinkKey];
            }
            ...
            const outgoingEdges = this.edges.filter(edge =>
                edge.from === node &&
                !visited.has(edge.to) &&
                edge.resCap > 0 // Only consider edges with positive residual capacity
            );

            for (let edge of outgoingEdges) {
                if (!visited.has(edge.to)) {
                    // Add the next node to the queue with the updated path
                    queue.push({ node: edge.to, path: [...path, node] });
                }
            }
        }
    ...
    findBottleneck(path) {
      ...
      // Iterate over the path to find the edge with the minimum residual capacity
      for (let i = 0; i < path.length - 1; i++) {
        const fromKey = path[i];
        const toKey = path[i + 1];
        
        // Find the edge object based on fromKey and toKey
        const edge = this.edges.find(edge => edge.from === fromKey && edge.to === toKey);
        
        // Update the bottleneck if the current edge's residual capacity is smaller
        if (edge && edge.resCap < bottleneck) {
          bottleneck = edge.resCap;
        }
      }

      return bottleneck;
    },
    async updateFlowAndResidual(path, additionalFlow) {
        for (let i = 0; i < path.length - 1; i++) {
            const fromKey = path[i];
            const toKey = path[i + 1];
            const currentEdge = this.edges.find(edge => edge.from === fromKey && edge.to === toKey);
            
            if(currentEdge.opacity===1){ // A forward edge
              currentEdge.isActive = true;
              await new Promise(resolve => setTimeout(resolve, 500));
              currentEdge.flow += additionalFlow;
              currentEdge.resCap -= additionalFlow;
              const revEdge = this.edges.find(edge => edge.from === toKey && edge.to === fromKey);
              revEdge.resCap += additionalFlow;
              await new Promise(resolve => setTimeout(resolve, 1000));
              currentEdge.isActive = false;
            }
            else if(currentEdge.opacity===0){ // A reverse edge
              currentEdge.resCap -= additionalFlow;
              const forwEdge = this.edges.find(edge => edge.from === toKey && edge.to === fromKey);
              forwEdge.isActive = true;
              await new Promise(resolve => setTimeout(resolve, 500));
              forwEdge.flow -= additionalFlow;
              forwEdge.resCap += additionalFlow;
              await new Promise(resolve => setTimeout(resolve, 1000));
              forwEdge.isActive = false;
            }
            else {
              //console.log("?!?!");
            }
...
<p>The Edmonds-Karp algorithm works by using Breadth-First Search (BFS) to find a path with available capacity from the source to the sink (called an <i>augmented path</i>), and then sends as much flow as possible through that path.</p>
<p>The Edmonds-Karp algorithm continues to find new paths to send more flow through until the maximum flow is reached.</p>
<p>In the simulation above, the Edmonds-Karp algorithm solves the maximum flow problem: It finds out how much flow can be sent from the source vertex \(s\), to the sink vertex \(t\), and that maximum flow is 8.</p>
<p>The numbers in the simulation above are written in fractions, where the first number is the flow, and the second number is the capacity (maximum possible flow in that edge). So for example, <code class="w3-codespan">0/7</code> on edge \(s \rightarrow v_2\), means there is <code class="w3-codespan">0</code> flow, with a capacity of <code class="w3-codespan">7</code> on that edge.</p>
<p>You can see the basic step-by-step description of how the Edmonds-Karp algorithm works below, but we need to go into more detail later to actually understand it.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Start with zero flow on all edges.</li>
    <li>Use BFS to find an <i>augmented path</i> where more flow can be sent.</li>
    <li>Do a <i>bottleneck calculation</i> to find out how much flow can be sent through that augmented path.</li>
...
<h2>Residual Network in Edmonds-Karp</h2>
<p>The Edmonds-Karp algorithm works by creating and using something called a <i>residual network</i>, which is a representation of the original graph.</p>
<p>In the residual network, every edge has a <i>residual capacity</i>, which is the original capacity of the edge, minus the the flow in that edge. The residual capacity can be seen as the leftover capacity in an edge with some flow.</p>
<p>For example, if there is a flow of 2 in the \( v_3 \rightarrow v_4 \) edge, and the capacity is 3, the residual flow is 1 in that edge, because there is room for sending 1 more unit of flow through that edge.</p>
<hr>

<h2>Reversed Edges in Edmonds-Karp</h2>
<p>The Edmonds-Karp algorithm also uses something called <i>reversed edges</i> to send flow back. This is useful to increase the total flow.</p>
<p>To send flow back, in the opposite direction of the edge, a reverse edge is created for each original edge in the network. The Edmonds-Karp algorithm can then use these reverse edges to send flow in the reverse direction.</p>
...
<p>In our example, the edge \( v_1 \rightarrow v_3 \) has a flow of 2, which means there is a residual capacity of 2 on the corresponding reversed edge \( v_3 \rightarrow v_1 \).</p>
<p>This just means that when there is a flow of 2 on the original edge \( v_1 \rightarrow v_3 \), there is a possibility of sending that same amount of flow back on that edge, but in the reversed direction. Using a reversed edge to push back flow can also be seen as undoing a part of the flow that is already created.</p>
<p>The idea of a residual network with residual capacity on edges, and the idea of reversed edges, are central to how the Edmonds-Karp algorithm works, and we will go into more detail about this when we implement the algorithm further down on this page.</p>
<hr>

<h2>Manual Run Through</h2>
<p>There is no flow in the graph to start with.</p>
<p>The Edmonds-Karp algorithm starts with using Breadth-First Search to find an augmented path where flow can be increased, which is \(s \rightarrow v_1 \rightarrow v_3 \rightarrow t\).</p>
<p>After finding the augmented path, a bottleneck calculation is done to find how much flow can be sent through that path, and that flow is: 2.</p>
<p>So a flow of 2 is sent over each edge in the augmented path.</p>
<div id="vueApp3" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isAug }">{{edge.flow}}/{{edge.capacity}}</text>
...
const app3 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
        // Forward edges:
        { from: 0, to: 1, capacity: 3, flow: 2, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 3, flow: 2, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 4, flow: 0, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 6, flow: 0, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        // Reverse edges:
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,1,3,5];
  }
});
app3.mount('#vueApp3');
</script>

<p>The next iteration of the Edmonds-Karp algorithm is to do these steps again: Find a new augmented path, find how much the flow in that path can be increased, and increase the flow along the edges in that path accordingly.</p>
<p>The next augmented path is found to be \(s \rightarrow v_1 \rightarrow v_4 \rightarrow t \).</p>
<p>The flow can only be increased by 1 in this path because there is only room for one more unit of flow in the \( s \rightarrow v_1 \) edge.</p>
<div id="vueApp4" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isAug }">{{edge.flow}}/{{edge.capacity}}</text>
...
const app4 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
        // Forward edges:
        { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 0, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 3, flow: 2, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 4, flow: 1, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 6, flow: 1, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        // Reverse edges:
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,1,4,5];
  }
});
app4.mount('#vueApp4');
</script>

<p>The next augmented path is found to be \(s \rightarrow v_2 \rightarrow v_4 \rightarrow t\).</p>
<p>The flow can be increased by 3 in this path. The bottleneck (limiting edge) is \( v_2 \rightarrow v_4 \) because the capacity is 3.</p>
<div id="vueApp5" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isAug }">{{edge.flow}}/{{edge.capacity}}</text>
...
const app5 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
        // Forward edges:
        { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 3, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 3, flow: 2, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 4, flow: 1, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 5, flow: 0, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 6, flow: 4, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        // Reverse edges:
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,2,4,5];
  }
});
app5.mount('#vueApp5');
</script>

<p>The last augmented path found is \(s \rightarrow v_2 \rightarrow v_1 \rightarrow v_4 \rightarrow t\).</p>
<p>The flow can only be increased by 2 in this path because of edge \( v_4 \rightarrow t \) being the bottleneck in this path with only space for 2 more units of flow (\(capacity-flow=1\)).</p>
<div id="vueApp6" class="vueApp_still">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 250" class="dsa_svg">
    <text v-for="edge in capacityEdges" :key="`generateEdgeKey('text',edge)`" :x="edge.centerX" :y="edge.centerY" class="svgBalFac" :opacity="edge.opacity" :class="{ boldText: edge.isAug }">{{edge.flow}}/{{edge.capacity}}</text>
...
const app6 = Vue.createApp({
  data() {
    return {
      ...
      edges: [
        // Forward edges:
        { from: 0, to: 1, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 0, to: 2, capacity: 7, flow: 5, resCap: 7, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 3, capacity: 3, flow: 2, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 1, to: 4, capacity: 4, flow: 3, resCap: 4, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 1, capacity: 5, flow: 2, resCap: 5, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 2, to: 4, capacity: 3, flow: 3, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 4, capacity: 3, flow: 0, resCap: 3, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 3, to: 5, capacity: 2, flow: 2, resCap: 2, opacity: 1, isAug: false, rev: false, isActive: false },
        { from: 4, to: 5, capacity: 6, flow: 6, resCap: 6, opacity: 1, isAug: false, rev: false, isActive: false },
        // Reverse edges:
        { from: 1, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 2, to: 0, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
        { from: 1, to: 2, capacity: 0, flow: 0, resCap: 0, opacity: 0, isAug: false },
  ...
  mounted() {
    this.calculateEdgeCenters();
    this.augPathVertices = [0,2,1,4,5];
  }
});
app6.mount('#vueApp6');
</script>
<p>At this point, a new augmenting path cannot be found (it is not possible to find a path where more flow can be sent through from \(s\) to \(t\)), which means the max flow has been found, and the Edmonds-Karp algorithm is finished.</p>
<p>The maximum flow is 8. As you can see in the image above, the flow (8) is the same going out of the source vertex \(s\), as the flow going into the sink vertex \(t\).</p>
<p>Also, if you take any other vertex than \(s\) or \(t\), you can see that the amount of flow going into a vertex, is the same as the flow going out of it. This is what we call <i>conservation of flow</i>, and this must hold for all such flow networks (directed graphs where each edge has a flow and a capacity).</p>
<hr>

<h2>Implementation of The Edmonds-Karp Algorithm</h2>
<p>To implement the Edmonds-Karp algorithm, we create a <code class="w3-codespan">Graph</code> class.</p>
<p>The <code class="w3-codespan">Graph</code> represents the graph with its vertices and edges:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-line="3-5, 7-8, 10-12"><code>class Graph:
...
<p><strong>Line 3: </strong>We create the <code class="w3-codespan">adj_matrix</code> to hold all the edges and edge capacities. Initial values are set to <code class="w3-codespan">0</code>.</p>
...
<p><strong>Line 7-8: </strong>The <code class="w3-codespan">add_edge</code> method is used to add an edge from vertex <code class="w3-codespan">u</code> to vertex <code class="w3-codespan">v</code>, with capacity <code class="w3-codespan">c</code>.</p>
<p><strong>Line 10-12: </strong>The <code class="w3-codespan">add_vertex_data</code> method is used to add a vertex name to the graph. The index of the vertex is given with the <code class="w3-codespan">vertex</code> argument, and <code class="w3-codespan">data</code> is the name of the vertex.</p>
<p>The <code class="w3-codespan">Graph</code> class also contains the <code class="w3-codespan">bfs</code> method to find augmented paths, using Breadth-First-Search:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="14" data-line-offset="14" data-line="15-18, 20-21, 23-27, 29"><code>    def bfs(self, s, t, parent):
        ...
        return visited[t]</code></pre>
</div>
<p><strong>Line 15-18: </strong>The <code class="w3-codespan">visited</code> array helps to avoid revisiting the same vertices during the search for an augmented path. The <code class="w3-codespan">queue</code> holds vertices to be explored, the search always starts with the source vertex <code class="w3-codespan">s</code>.</p>
<p><strong>Line 20-21: </strong>As long as there are vertices to be explored in the <code class="w3-codespan">queue</code>, take the first vertex out of the <code class="w3-codespan">queue</code> so that a path can be found from there to the next vertex.</p>
<p><strong>Line 23: </strong>For every adjacent vertex to the current vertex.</p>
<p><strong>Line 24-27: </strong>If the adjacent vertex is not visited yet, and there is a residual capacity on the edge to that vertex: add it to the queue of vertices that needs to be explored, mark it as visited, and set the <code class="w3-codespan">parent</code> of the adjacent vertex to be the current vertex <code class="w3-codespan">u</code>.</p>
<p>The <code class="w3-codespan">parent</code> array holds the parent of a vertex, creating a path from the sink vertex, backwards to the source vertex. The <code class="w3-codespan">parent</code> is used later in the Edmonds-Karp algorithm, outside the <code class="w3-codespan">bfs</code> method, to increase flow in the augmented path.</p>
<p><strong>Line 29: </strong>The last line returns <code class="w3-codespan">visited[t]</code>, which is <code class="w3-codespan">true</code> if the augmented path ends in the sink node <code class="w3-codespan">t</code>. Returning <code class="w3-codespan">true</code> means that an augmenting path has been found.</p>

<p>The <code class="w3-codespan">edmonds_karp</code> method is the last method we add to the <code class="w3-codespan">Graph</code> class:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white line-numbers" data-start="31" data-line-offset="31" data-line="35-40, 42, 44-48"><code>    def edmonds_karp(self, source, sink):
        parent = [-1] * self.size
        max_flow = 0
...
<p>Initially, the <code class="w3-codespan">parent</code> array holds invalid index values, because there is no augmented path to begin with, and the <code class="w3-codespan">max_flow</code> is <code class="w3-codespan">0</code>, and the <code class="w3-codespan">while</code> loop keeps increasing the <code class="w3-codespan">max_flow</code> as long as there is an augmented path to increase flow in.</p>
<p><strong>Line 35: </strong>The outer <code class="w3-codespan">while</code> loop makes sure the Edmonds-Karp algorithm keeps increasing flow as long as there are augmented paths to increase flow along.</p>
<p><strong>Line 36-37: </strong>The initial flow along an augmented path is infinite, and the possible flow increase will be calculated starting with the sink vertex.</p>
<p><strong>Line 38-40: </strong>The value for <code class="w3-codespan">path_flow</code> is found by going backwards from the sink vertex towards the source vertex. The lowest value of residual capacity along the path is what decides how much flow can be sent on the path.</p>
<p><strong>Line 42: </strong><code class="w3-codespan">path_flow</code> is increased by the <code class="w3-codespan">path_flow</code>.</p>
<p><strong>Line 44-48: </strong>Stepping through the augmented path, going backwards from sink to source, the residual capacity is decreased with the <code class="w3-codespan">path_flow</code> on the forward edges, and the residual capacity is increased with the <code class="w3-codespan">path_flow</code> on the reversed edges.</p>
<p><strong>Line 50-58: </strong>This part of the code is just for printing so that we are able to track each time an augmented path is found, and how much flow is sent through that path.</p>

<p>After defining the <code class="w3-codespan">Graph</code> class, the vertices and edges must be defined to initialize the specific graph, and the complete code for the Edmonds-Karp algorithm example looks like this:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
...
source = 0; sink = 5
print("The maximum possible flow is %d " % g.edmonds_karp(source, sink))</code></pre>
<a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_edmondskarp" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Time Complexity for The Edmonds-Karp Algorithm</h2>
<p>The difference between Edmonds-Karp and Ford-Fulkerson is that Edmonds-Karp uses Breadth-First Search (BFS) to find augmented paths, while Ford-Fulkerson uses Depth-First Search (DFS).</p>
<p>This means that the time it takes to run Edmonds-Karp is easier to predict than Ford-Fulkerson, because Edmonds-Karp is not affected by the maximum flow value.</p>
<p>With the number of vertices \(V\), the number of edges \(E\), the time complexity for the Edmonds-Karp algorithm is</p>
<p><span class="math display">\[ O(V \cdot E^2) \]</span></p>
<p>This means Edmonds-Karp does not depend on the maximum flow, like Ford-Fulkerson does, but on how many vertices and edges we have.</p>
<p>The reason we get this time complexity for Edmonds-Karp is that it runs BFS which has time complexity \(O(E+V)\).</p>
<p>But if we assume a bad case scenario for Edmonds-Karp, with a dense graph, where the number of edges \(E\) is much greater than the number of vertices \(V\), time complexity for BFS becomes \(O(E)\).</p>
<p>BFS must run one time for every augmented path, and there can actually be found close to \(V \cdot E \) augmented paths during running of the Edmonds-Karp algorithm.</p>
<p>So, BFS with time complexity \(O(E)\) can run close to \(V \cdot E \) times in the worst case, which means we get a total time complexity for Edmonds-Karp: \( O(V \cdot E \cdot E) = O(V \cdot E^2) \).</p>
<hr>
