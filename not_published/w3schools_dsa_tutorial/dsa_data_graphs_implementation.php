...
<html lang="en-US">
<head>
<title>DSA Graphs Implementation</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Graphs Implementation</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_theory_graphs.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_graphs_traversal.php">Next &#10095;</a>
</div>
<hr>

<h2>A Basic Graph Implementation</h2>
<p>Before we can run algorithms on a Graph, we must first implement it somehow.</p>
<p>To implement a Graph we will use an <a href="dsa_theory_graphs.php#adjMatrix">Adjacency Matrix</a>, like the one below.</p>
<figure id="divAnim" class="dsa_figure">
...
<figcaption>An undirected Graph<br>and its adjacency matrix</figcaption>
</figure>
<p>To store data for each vertex, in this case the letters A, B, C, and D, the data is put in a separate array that matches the indexes in the adjacency matrix, like this:</p>
<div class="w3-example">
  <pre class="language-python notranslate w3-white" data-line=""><code>vertexData = [ 'A', 'B', 'C', 'D']</code></pre>
</div>
<p>For an undirected and not weighted Graph, like in the image above, an edge between vertices <code class="w3-codespan">i</code> and <code class="w3-codespan">j</code> is stored with value <code class="w3-codespan">1</code>. It is stored as <code class="w3-codespan">1</code> on both places <code class="w3-codespan">(j,i)</code> and <code class="w3-codespan">(i,j)</code> because the edge goes in both directions. As you can see, the matrix becomes diagonally symmetric for such undirected Graphs.</p>
<p>Let's look at something more specific. In the adjacency matrix above, vertex A is on index <code class="w3-codespan">0</code>, and vertex D is on index <code class="w3-codespan">3</code>, so we get the edge between A and D stored as value <code class="w3-codespan">1</code> in position <code class="w3-codespan">(0,3)</code> and <code class="w3-codespan">(3,0)</code>, because the edge goes in both directions.</p>
<p>Below is a basic implementation of the undirected Graph from the image above.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white"><code>vertexData = ['A', 'B', 'C', 'D']
...
print_adjacency_matrix(adjacency_matrix)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_undirected_adjmatrix" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>This implementation is basically just a two dimensional array, but to get a better sense of how the vertices are connected by edges in the Graph we have just implemented, we can run this function:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white"><code>def print_connections(matrix, vertices):
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_undirected_connections" target="_blank">Try it Yourself &raquo;</a>
</div>
...
<h2>Graph Implementation Using Classes</h2>
<p>A more proper way to store a Graph is to add an abstraction layer using classes so that a Graph's vertices, edges, and relevant methods, like algorithms that we will implement later, are contained in one place.</p>
<p>Programming languages with built-in object-oriented functionality like Python and Java, make implementation of Graphs using classes much easier than languages like C, without this built-in functionality.</p>
...
<p>Here is how the undirected Graph above can be implemented using classes.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white" data-line="9,10,29-32"><code>class Graph:
...
g.print_graph()</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_undirected_class" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>In the code above, the matrix symmetry we get for undirected Graphs is provided for on line 9 and 10, and this saves us some code when initializing the edges in the Graph on lines 29-32.</p>
<hr>

<h2>Implementation of Directed and Weighted Graphs</h2>
<p>To implement a Graph that is directed and weighted, we just need to do a few changes to previous implementation of the undirected Graph.</p>
<p>To create directed Graphs, we just need to remove line 10 in the previous example code, so that the matrix is not automatically symmetric anymore.</p>
<p>The second change we need to do is to add a <code class="w3-codespan">weight</code> argument to the <code class="w3-codespan">add_edge()</code> method, so that instead of just having value <code class="w3-codespan">1</code> to indicate that there is an edge between two vertices, we use the actual weight value to define the edge.</p>
...
<p>Below is the implementation of the directed and weighted Graph above.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate line-numbers w3-white" data-line="3,7,10"><code>class Graph:
    def __init__(self, size):
    ...
    def add_edge(self, u, v, weight):
        if 0 <= u < self.size and 0 <= v < self.size:
            ...
            <del>self.adj_matrix[v][u] = weight</del>
...
g.print_graph()</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_graphs_directed_class" target="_blank">Try it Yourself &raquo;</a>
</div>
<p><strong>Line 3: </strong>All edges are set to <code class="w3-codespan">None</code> initially.</p>
<p><strong>Line 7: </strong>The weight can now be added to an edge with the additional <code class="w3-codespan">weight</code> argument.</p>
<p><strong>Line 10: </strong> By removing line 10, the Graph can now be set up as being directed.</p>
<p>On the next page we will see how Graphs can be traversed, and on the next pages after that we will look at different algorithms that can run on the Graph data structure.</p>
<hr>

<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_theory_graphs2" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>How are the edges in a graph implemented?</p>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">
The edges, and edge weights, 
in a graph are normally 
implemented in an <input name="ex1" maxlength="9" style="width: 10ch;"> matrix.
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_theory_graphs2">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
