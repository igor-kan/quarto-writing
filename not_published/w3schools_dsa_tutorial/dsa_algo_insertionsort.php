...
<html lang="en-US">
<head>
<title>DSA Insertion Sort</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Insertion Sort</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_selectionsort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_quicksort.php">Next &#10095;</a>
</div>
<hr>

<h2>Insertion Sort</h2>
<p>The Insertion Sort algorithm uses one part of the array to hold the sorted values, and the other part of the array to hold values that are not sorted yet.</p>
<div id="vueApp">
  ...
  <div id="frameDiv">
    ...
    <div class="diceDiv" id="ghostDiv" :class="{ 'diceDiv-nextSort': ghostDivIsFat }"></div>
  </div>
</div>
<script>
const app = Vue.createApp({
  ...
  methods: {
    action(){
      if(this.buttonText === "Insertion Sort") {
        this.insertSort();
      }
      else {
        this.addDie50();
    ...
    async insertSort() {
      ...
      this.dice[0].isNextSort = false;
      this.dice[1].isNextSort = true;
      await new Promise(resolve => setTimeout(resolve, this.delay));
      ...
      for (var i = 1; i < length; i++) {
        ...
        await new Promise(resolve => setTimeout(resolve, this.delay));
        this.dice[i].isNextSort = false;
        if(i<length-1){
          this.dice[i+1].isNextSort = true;
        }
        else {
          this.ghostDivIsFat = true;
        }
        await new Promise(resolve => setTimeout(resolve, this.delay));
        for (var j = i; j >= 0; j--) {
          ...
          if (j===0) {
            ...
            this.dice[j].isFinished = true;
            break;
          }
          else if (this.dice[j].dieNmbr < this.dice[j-1].dieNmbr ) {
            let tmp = this.dice.splice(j-1,1)[0];
            this.dice.splice(j,0,tmp);
          }
          else if (this.dice[j].dieNmbr >= this.dice[j-1].dieNmbr ){
            this.dice[j].isActive = false;
            this.dice[j].isFinished = true;
    ...
    addDie50() {
      ...
      this.buttonText = "Insertion Sort";
      this.msgDone = '';
    },
    compareFunc(a, b) {
...
<p>The algorithm takes one value at a time from the unsorted part of the array and puts it into the right place in the sorted part of the array, until the array is sorted.</p>
...
<p>Continue reading to fully understand the Insertion Sort algorithm and how to implement it yourself.</p>
...
<p>Before we implement the Insertion Sort algorithm in a programming language, let's manually run through a short array, just to get the idea.</p>
<p><strong>Step 1:</strong> We start with an unsorted array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7, 12, 9, 11, 3]</code></pre>
</div>
<hr>
<p><strong>Step 2:</strong> We can consider the first value as the initial sorted part of the array. If it is just one value, it must be sorted, right?</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[<mark class="dsa_mark"> 7</mark>, 12, 9, 11, 3]</code>
</div>
<hr>
<p><strong>Step 3:</strong> The next value 12 should now be moved into the correct position in the sorted part of the array. But 12 is higher than 7, so it is already in the correct position.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7,<mark class="dsa_mark"> 12</mark>, 9, 11, 3]</code>
</div>
<hr>
<p><strong>Step 4:</strong> Consider the next value 9.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7, 12,<mark class="dsa_mark"> 9</mark>, 11, 3]</code>
</div>
<hr>
<p><strong>Step 5:</strong> The value 9 must now be moved into the correct position inside the sorted part of the array, so we move 9 in between 7 and 12.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7,<mark class="dsa_mark"> 9</mark>, 12, 11, 3]</code>
</div>
<hr>
<p><strong>Step 6:</strong> The next value is 11.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7, 9, 12, > 11</mark>, 3]</code>
</div>
<hr>
<p><strong>Step 7:</strong> We move it in between 9 and 12 in the sorted part of the array.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7, 9,<mark class="dsa_mark"> 11</mark>, 12, 3]</code>
</div>
<hr>
<p><strong>Step 8:</strong> The last value to insert into the correct position is 3.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 7, 9, 11, 12,<mark class="dsa_mark"> 3</mark>]</code>
</div>
<hr>
<p><strong>Step 9:</strong> We insert 3 in front of all other values because it is the lowest value.</p>
<div class="w3-code notranslate">
  ...
  methods: {
    ...
    async insSort() {
      ...
      for (var i = 1; i < length; i++) {
        ...
        await new Promise(resolve => setTimeout(resolve, this.delay));
        for (var j = i; j > 0; j--) {
          if (this.dice[j-1].dieNmbr > this.dice[j].dieNmbr) {
            let lowerVal = this.dice.splice(j,1)[0]; // Remove lowerVal
            this.dice.splice(j-1,0,lowerVal)
          }
          else {
            break;
          }
...
<p>We must understand what happened above to fully understand the algorithm, so that we can implement the algorithm in a programming language.</p>
<p>The first value is considered to be the initial sorted part of the array.</p>
<p>Every value after the first value must be compared to the values in the sorted part of the algorithm so that it can be inserted into the correct position.</p>
<p>The Insertion Sort Algorithm must run through the array 4 times, to sort the array of 5 values because we do not have to sort the first value.</p>
<p>And each time the algorithm runs through the array, the remaining unsorted part of the array becomes shorter.</p>
<p>We will now use what we have learned to implement the Insertion Sort algorithm in a programming language.</p>
<hr>

<h2>Insertion Sort Implementation</h2>
<p>To implement the Insertion Sort algorithm in a programming language, we need:</p>
<ol>
  <li>An array with values to sort.</li>
  <li>An outer loop that picks a value to be sorted. For an array with \(n\) values, this outer loop skips the first value, and must run \(n-1\) times.</li>
  <li>An inner loop that goes through the sorted part of the array, to find where to insert the value. If the value to be sorted is at index \(i\), the sorted part of the array starts at index \(0\) and ends at index \(i-1\).</li>
</ol>
...
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white"><code>my_array = [64, 34, 25, 12, 22, 11, 90, 5]
...
print("Sorted array:", my_array)
  ...
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_insertionsort" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Insertion Sort Improvement</h2>
<p>Insertion Sort can be improved a little bit more.</p>
...
<p>The problem with this way of programming it is that when removing a value from the array, all elements above must be shifted one index place down:</p>
<img src="img_array_removed_shifting.png" class="imgTut" alt="Removing an element from an array">
<p>And when inserting the removed value into the array again, there are also many shift operations that must be done: all following elements must shift one position up to make place for the inserted value:</p>
<img src="img_array_inserted_shifting.png" class="imgTut" alt="Inserting an element into an array">
<p>These shifting operations can take a lot of time, especially for an array with many elements.</p>
<!--<div class="ws-note">
...
<div class="ws-note">
  <p><strong>Hidden memory shifts: </strong>You will not see these shifting operations happening in the code if you are using a high-level programming language such as Python or JavaScript, but the shifting operations are still happening in the background. Such shifting operations require extra time for the computer to do, which can be a problem.</p>
</div>
<p>You can read more about how arrays are stored in memory <a href="dsa_theory_linkedlists_memory.php#arraysInMemory">here</a>.</p>
<div class="ws-note">
    <p><strong>C and Java code examples above and below are the same: </strong>The issue of memory shifts happening behind the scenes is only relevant for high-level programming languages like Python or JavaScript, where arrays are dynamic, which means you can easily remove and insert elements. In lower-level programming languages like C and Java, where arrays have a fixed length, elements cannot be removed or inserted. As a result, there are no such memory shifts happening, and therefore the example codes above and below for C and Java remain the same.</p>
</div>
<hr>

<h2>Improved Solution</h2>
<p>We can avoid most of these shift operations by only shifting the values necessary:</p>
<img src="img_array_inserted_shifting_improved.png" class="imgTut" alt="Moving an element in an array efficiently">
<p>In the image above, first value 7 is copied, then values 11 and 12 are shifted one place up in the array, and at last value 7 is put where value 11 was before.</p>
<p>The number of shifting operations is reduced from 12 to 2 in this case.</p>

<p>This improvement is implemented in the example below:</p>
<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white" data-line="6,9,11-13"><code>my_array = [64, 34, 25, 12, 22, 11, 90, 5]
...
print("Sorted array:", my_array)</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_insertionsort2" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>What is also done in the code above is to break out of the inner loop. That is because there is no need to continue comparing values when we have already found the correct place for the current value.</p>
<hr>

<h2>Insertion Sort Time Complexity</h2>
...
<p class="fontComment">For a more thorough and detailed explanation of Insertion Sort time complexity, visit <a href="dsa_timecomplexity_insertionsort.php">this page</a>.</p>
<p>Insertion Sort sorts an array of \(n\) values.</p>
<p>On average, each value must be compared to about \(\frac{n}{2}\) other values to find the correct place to insert it.</p>
<p>Insertion Sort must run the loop to insert a value in its correct place approximately \(n\) times.</p>
<p>We get time complexity for Insertion Sort:</p>
<p><span class="math display">\[ O( \frac{n}{2} \cdot n) = \underline{\underline{O(n^2)}} \]</span></p>

<p>The time complexity for Insertion Sort can be displayed like this:</p>
<img src="img_runtime_bubbleSort.png" alt="Time Complexity for Insertion Sort">

<p>Use the simulation below to see how the theoretical time complexity \(O(n^2)\) (red line) compares with the number of operations of actual Insertion Sorts.</p>
<div id="vueApp4">
  ...
  <div class="frameDivOuter">
    <div id="inpDiv">
      ...
      <p><label><input type="radio" value="worstCase" v-model="selectedRunType" :disabled="isLoading"> Worst Case</label></p>
      <p><label><input type="radio" value="bestCase" v-model="selectedRunType" :disabled="isLoading"> Best Case</label></p>
...
<script>
  ...
  methods: {
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.performInsertionSort();
    ...
    async performInsertionSort() {
      let n = this.array.length;
      this.operations = 0;

      for (let i = 1; i < n; i++) {
        this.operations++;  // Increment for loop iteration
        let insert_index = i;
        let current_value = this.array[i].value;

        if(i % 3 === 0) {
          // Highlight the active element
          this.array[i].active = true;
          await new Promise(resolve => setTimeout(resolve, 1));
          // Remove the highlight from the active element
          this.array[i].active = false;
        }

        for (let j = i - 1; j >= 0; j--) {
          this.operations++;  // Increment for loop iteration
          if (this.array[j].value > current_value) {
            this.operations++;  // Increment for comparison
        ...
        // Insert the current_value at the right place
        this.operations++;  // Increment for the insertion operation
        this.array[insert_index].value = current_value;
      }
...
<p>For Insertion Sort, there is a big difference between best, average and worst case scenarios. You can see that by running the different simulations above.</p>
<p>Next up is Quicksort. Finally we will see a faster sorting algorithm!</p>
<hr>
