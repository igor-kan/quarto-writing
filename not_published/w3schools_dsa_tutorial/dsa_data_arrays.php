...
.lowestFound {
  ...
  z-index: 2;
  box-shadow: rgba(144, 238, 144, 0.9) 0px 0px 5px 4px;
}
.dsaDone {
  opacity: 0.5;
}
...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Arrays</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_simple.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_bubblesort.php">Next &#10095;</a>
</div>
<hr>

<h2>Arrays</h2>
<p>An array is a data structure used to store multiple elements.</p>
<p>Arrays are used by many algorithms.</p>
<p>For example, an algorithm can be used to look through an array to find the lowest value, like the animation below shows:</p>
<div id="vueApp2">
  ...
  <p>Lowest value: {{ minVal }}</p>
  <div id="frameDiv2">
    <transition-group>
      <div v-for="x in dice" :key="x.keyNmbr" class="diceDiv" :class="{
    ...
    <div class="diceDiv" id="ghostDiv2"></div>
  </div>
</div>
<script>
  ...
  methods: {
    action(){
      if(this.buttonText === "Find Lowest") {
        this.findLowest();
      }
      else {
        this.addDie50();
    ...
    async findLowest() {
      ...
      this.minVal = this.dice[0].dieNmbr;
      for (var j = 0; j < this.dice.length; j++) {
        this.dice[j].isActive = true;
        this.dice[j].isBorder = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        if (this.dice[j].dieNmbr < this.minVal || j === 0) {
          this.minVal = this.dice[j].dieNmbr;
          this.minIndex = j;
        ...
        this.dice[j].isBorder = false;
      }
      this.msgDone = 'Done!';
      this.buttonText = "New Values";
      this.btnIsDisabled = false;
      this.dice[this.minIndex].isFinished = true;
      this.dice[this.minIndex].isBorder = true;
    },
    removeCandidates() {
      for (var j = 0; j < this.dice.length; j++) {
        this.dice[j].isCandidate = false;
      }
    },
    addDie() {
      const newDie = {
...
<p>In Python, an array can be created like this:</p>
<div class="w3-example">
<pre class="language-python notranslate w3-white" data-line=""><code>my_array = [7, 12, 9, 4, 11]</code></pre>
</div>
<div class="ws-note">
  <p><strong>Note: </strong>The Python code above actually generates a Python 'list' data type, but for the scope of this tutorial the 'list' data type can be used in the same way as an array. Learn more about Python lists <a href="https://www.w3schools.com/python/python_lists.asp">here</a>.</p>
</div>
<p>Arrays are indexed, meaning that each element in the array has an index, a number that says where in the array the element is located. The programming languages in this tutorial (Python, Java, and C) use zero-based indexing for arrays, meaning that the first element in an array can be accessed at index 0.</p>
<p>In Python, this code use index 0 to write the first array element (value 7) to the console:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
<pre class="language-python notranslate w3-white" data-line=""><code>my_array = [7, 12, 9, 4, 11]
print( my_array[0] )</code></pre>
<a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_array_print" target="_blank">Try it Yourself &raquo;</a>
</div>
...
<a id="findLowest"></a>
<h2 >Algorithm: Find The Lowest Value in an Array</h2>
<p>Let's create our first algorithm using the array data structure.</p>
<p>Below is the algorithm to find the lowest number in an array.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Go through the values in the array one by one.</li>
    <li>Check if the current value is the lowest so far, and if it is, store it.</li>
    <li>After looking at all the values, the stored value will be the lowest of all values in the array.</li>
  </ol>
</div>
<p>Try the simulation below to see how the algorithm for finding the lowest value works (the animation is the same as the one on the top of this page):</p>
<div id="vueApp">
  <p>Speed: <input v-model="inpVal" type="range" min="50" max="500"><!-- {{ delay }} --></p>
  <button @click="action" :disabled="btnIsDisabled">{{ buttonText }}</button> {{ msgDone }}
  <p>Lowest value: {{ minVal }}</p>
...
<p>This next simulation also finds the lowest value in an array, just like the simulation above, but here we can see how the numbers inside the array are checked to find the lowest value:</p>
<div id="vueApp3">
<!--
...
<script>
  const app3 = Vue.createApp({
    ...
    return {
      ...
      dice: [],
      keyNumber: 0,
      buttonText: 'Find The Lowest Value'
    }
  },
  computed: {
    formattedDiceArray() {
  ...
  methods: {
    action(){
      if(this.buttonText === "Find The Lowest Value") {
        this.bblSort();
      }
      else {
        this.addDie50();
    ...
    async bblSort() {
      ...
      await new Promise(resolve => setTimeout(resolve, this.delay));
      for (var j = 1; j < (this.dice.length); j++) {
        this.dice[0].isSwapped = true;
        this.dice[j].isSwapped = true;
        await new Promise(resolve => setTimeout(resolve, this.delay));
        if (this.dice[j].dieNmbr < this.dice[0].dieNmbr) {
          this.dice[0].isFinished = false;
          this.dice[j].isFinished = true;
        }
        this.dice[0].isSwapped = false;
        this.dice[j].isSwapped = false;
        await new Promise(resolve => setTimeout(resolve, this.delay));
      }
      this.msgDone = 'Lowest value: 3';
    ...
    addDie50() {
      ...
      this.buttonText = "Find The Lowest Value";
      this.msgDone = '';
    }
  },
  mounted() {
...
<p>Before implementing the algorithm using an actual programming language, it is usually smart to first write the algorithm as a step-by-step procedure.</p>
<p>If you can write down the algorithm in something between human language and programming language, the algorithm will be easier to implement later because we avoid drowning in all the details of the programming language syntax.</p>
<div class="w3-example ws-light-grey">
<ol>
  <li>Create a variable 'minVal' and set it equal to the first value of the array.</li>
  <li>Go through every element in the array.</li>
  <li>If the current element has a lower value than 'minVal', update 'minVal' to this value.</li>
  <li>After looking at all the elements in the array, the 'minVal' variable now contains the lowest value.</li>
</ol>
</div>

<p>You can also write the algorithm in a way that looks more like a programming language if you want to, like this:</p>
<div class="w3-example ws-light-grey">
<p><code>
...
<div class="ws-note">
  <p><strong>Note: </strong>The two step-by-step descriptions of the algorithm we have written above can be called 'pseudocode'. Pseudocode is a description of what a program does, using language that is something between human language and a programming language.</p>
</div>
<p>After we have written down the algorithm, it is much easier to implement the algorithm in a specific programming language:</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Python:</p>
  <pre class="language-python notranslate w3-white"><code>my_array = [7, 12, 9, 4, 11]
...
print('Lowest value: ',minVal) # Step 4</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_findlowest" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>

<h2>Algorithm Time Complexity</h2>
<img class="tutImg" src="img_runtime_findlowest.png" alt="Run Time">
<p>When exploring algorithms, we often look at how much time an algorithm takes to run relative to the size of the data set.</p>
<p>In the example above, the time the algorithm needs to run is proportional, or linear, to the size of the data set. This is because the algorithm must visit every array element one time to find the lowest value. The loop must run 5 times since there are 5 values in the array. And if the array had 1000 values, the loop would have to run 1000 times.</p>
<p>Try the simulation below to see this relationship between the number of compare operations needed to find the lowest value, and the size of the array.</p>
<p>See <a href="dsa_timecomplexity_theory.php">this page</a> for a more thorough explanation of what time complexity is.</p>
<p>Each algorithm in this tutorial will be presented together with its time complexity.</p>
<div id="vueApp4">
  ...
  methods: {
    newArray(){
      ...
      for (let k = 0; k < this.userX-1; k++) {
        let randNumber = Math.ceil(Math.random() * 90)+10;
        this.array.push({ value: randNumber, active: false, done: false, lowestFound: false });
      }
      this.array.splice(Math.floor(Math.random() * this.array.length), 0, { value: 100, active: false, done: false, lowestFound: false });
      this.runBtn = true;
    },
    presort() {
      this.operations = 0;
    ...
    async addPoint() {
      this.isLoading = true;
      let operations = await this.findLowest();
    ...
    async findLowest() {
      ...
      this.operations = 1;
      let min_index = 0;
      let lowest = this.array[0].value;
      this.array[0].active = true;
      this.array[0].lowestFound = true;
      for (let i = 1; i < n; i++) {
        this.operations++; // Increment for loop iteration
        if(this.array[i].value < lowest){
          this.array[min_index].done = true;
          this.array[min_index].active = false;
          this.array[min_index].lowestFound = false;
          lowest = this.array[i].value;
          min_index = i;
          this.array[min_index].active = true;
          this.array[min_index].lowestFound = true;
        }
        else {
...
<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_data_arrays1" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>How can we print value "7" from the array below?</p>
<code>my_array = [7, 12, 9, 4, 11]</code>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">
print(my_array[<input name="ex1" maxlength="1" style="width: 2ch;">])
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_data_arrays1">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
