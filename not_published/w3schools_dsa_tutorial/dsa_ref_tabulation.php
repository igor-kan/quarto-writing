...
ny:-->
    ...
    <h1><span class="color_h1">Tabulation</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_ref_memoization.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_ref_dynamic_programming.php">Next &#10095;</a>
</div>
...
<h2>Tabulation</h2>
<p>Tabulation is a technique used to solve problems.</p>
<p>Tabulation uses a table where the results to the most basic subproblems are stored first. The table then gets filled with more and more subproblem results until we find the result to the complete problem that we are looking for.</p>
<p>The tabulation technique is said to solve problems "bottom-up" because of how it solves the most basic subproblems first.</p>
<p>Tabulation is a technique used in <a href="dsa_ref_dynamic_programming.php">Dynamic Programming</a>, which means that to use tabulation, the problem we are trying to solve must consist of overlapping subproblems.</p>
<hr>

<h2>Using Tabulation To Find The \(n\)th Fibonacci Number</h2>
<p><a href="dsa_algo_simple.php">The Fibonacci numbers</a> are great for demonstrating different programming techniques, also when demonstrating how tabulation works.</p>
<p>Tabulation uses a table that is filled with the lowest Fibonacci numbers \(F(0)=0\) and \(F(1)=1\) first (bottom-up). The next Fibonacci number to be stored in the table is \(F(2)=F(1)+F(0)\).</p>
<p>The next Fibonacci number is always the sum of the two previous numbers:</p>
<p><span class="math display">\[
F(n)=F(n-1)+F(n-2)
\]</span></p>
<p>In this way, the table continues to get filled with next Fibonacci numbers until we find the \(n\)th Fibonacci number that we are looking for.</p>
<div class="w3-example">
  <h3>Example</h3>
  <p>Finding the 10th Fibonacci number using tabulation:</p>
  <pre class="language-python notranslate w3-white"><code>def fibonacci_tabulation(n):
    if n == 0: return 0
    elif n == 1: return 1
...
print(f"\nThe {n}th Fibonacci number is {result}")</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_findfibo_tab" target="_blank">Try it Yourself &raquo;</a>
</div>
<p>Other ways to find the \(n\)th Fibonacci number include <a href="dsa_algo_simple.php#nthfibo">recursion</a>, or the improved version of it using <a href="dsa_ref_memoization.php">memoization</a>.</p>
<hr>

<h2>Tabulation Is A Bottom Up Approach</h2>
<p>See the drawings below to get a better idea of why tabulation is called a "bottom up" approach.</p>
<p>As a reference to compare with, see the drawing of the <a href="dsa_algo_simple.php#nthfibo">"top-down" recursion approach</a> to finding the \(n\)th Fibonacci number.</p>
<div class="figureWrap" >
<figure class="dsa_figure">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 292 380" class="dsa_svg">
  ...
  <g class="root parent" transform="translate(270,200)">
    ...
    <figcaption>The top down recursive approach to finding the 10th Fibonacci number.</figcaption>
  </figure>
</div>
<p>The tabulation approach starts building the table bottom up to find the 10th Fibonacci number, starting with \(F(0)\) and \(F(1)\).</p>
<p>The recursive approach start by trying to find \(F(10)\), but to find that it must call \(F(9)\) and \(F(8)\), and so it goes all the way down to \(F(0)\) and \(F(1)\) before the function calls start returning values that can be put together to the final answer.</p>
...
<h2>Other Problems That Are Solved with Tabulation</h2>
<p>Just like finding the \(n\)th Fibonacci number, tabulation can also be used to find the solution to other problems:</p>
<ul>
  <li><strong>The 0/1 Knapsack Problem </strong>is about having a set of items we can pack in a knapsack (a simple backpack), each item with a different value. To solve the problem we need to find the items that will maximize the total value of items we pack, but we cannot bring all the items we want because the knapsack has a weight limit.</li>
  <li><strong>The Shortest Path Problem </strong>can be solved using <a href="dsa_algo_graphs_bellmanford.php">the Bellman-Ford algorithm</a>, which also uses tabulation to find the shortest paths in a graph. More specifically, the tabulation approach of the Bellman-Ford algorithm is in how the values in the "distances" array gets updated.</li>
  <li><strong>The Traveling Salesman Problem </strong>can be solved precisely using the Held-Karp algorithm, which also uses tabulation. This algorithm is not described in this tutorial as it is although better than brute force \(O(n!)\), still not very effective \(O(2^n n^2)\), and quite advanced.</li>
</ul>
<hr>

<h2>Tabulation in Dynamic Programming</h2>
<p>As mentioned in the top, tabulation (just like memoization) is a technique used in something called <a href="dsa_ref_dynamic_programming.php">Dynamic Programming</a>.</p>
<p>Dynamic Programming is a way of designing algorithms to solve problems.</p>
<p>For Dynamic Programming to work, the problem we want to solve must have these two properties:</p>
<ul>
  <li>The problem must be built up by smaller, <strong>overlapping subproblems</strong>. For example, the solution to Fibonacci number \(F(3)\) overlaps with the solutions to Fibonacci numbers \(F(2)\) and \(F(1)\), because we get \(F(3)\) by combining \(F(2)\) and \(F(1)\).</li>
  <li>The problem must also have an <strong>optimal substructure</strong>, meaning that the solution to the problem can be constructed from the solutions to its subproblems. When finding the \(n\)th Fibonacci number, \(F(n)\) can be found by adding \(F(n-1)\) and \(F(n-2)\). So knowing the two previous numbers is not enough to find \(F(n)\), we must also know the structure so that we know how to put them together.</li>
</ul>
<p>Read more about Dynamic Programming on the <a href="dsa_ref_dynamic_programming.php">next page</a>.</p>
<hr>
