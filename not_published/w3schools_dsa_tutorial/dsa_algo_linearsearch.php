...
ny:-->
    ...
    <h1>DSA <span class="color_h1">Linear Search</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="dsa_algo_mergesort.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="dsa_algo_binarysearch.php">Next &#10095;</a>
</div>
<hr>

<h2>Linear Search</h2>
<p>The Linear Search algorithm searches through an array and returns the index of the value it searches for.</p>
<div id="vueApp">
  <p>Speed: <input v-model="inpVal" type="range" min="50" max="500"><!-- {{ delay }}--></p>
  <p><label>Find value: <input type="number" step="1" min="1" max="10" v-model="targetHeight" :disabled="inpIsDisabled"></label></p>
  <p><p>Current value: {{ currVal }}</p></p>
...
const app = Vue.createApp({
  ...
  methods: {
    action(){
      if(this.buttonText === "Linear Search") {
        this.linearSearch();
      }
      else {
        this.addDie50();
    ...
    async linearSearch() {
      ...
      this.currIndex= 0;
      while(this.currIndex<this.dice.length){
        ...
        if (this.currVal === this.targetHeight) {
          this.currVal = this.dice[this.currIndex].dieNmbr;
          ...
          this.dice[this.currIndex].isDone = true;
        }
        this.dice[this.currIndex].isActive = false;
        this.dice[this.currIndex].isBorder = false;
        this.currIndex++;
      }
      if(this.currIndex===this.dice.length){
...
<p>Run the simulation above to see how the Linear Search algorithm works.</p>
<p>Too see what happens when a value is not found, try to find value 5.</p>
<p>This algorithm is very simple and easy to understand and implement.</p>
<p>If the array is already sorted, it is better to use the much faster Binary Search algorithm that we will explore on the next page.</p>
<p>A big difference between <span style="font-style: italic;">sorting</span> algorithms and <span style="font-style: italic;">searching</span> algorithms is that sorting algorithms modify the array, but searching algorithms leave the array unchanged.</p>
<div class="ws-info">
  <p><strong>How it works:</strong></p>
  <ol>
    <li>Go through the array value by value from the start.</li>
    <li>Compare each value to check if it is equal to the value we are looking for.</li>
    <li>If the value is found, return the index of that value.</li>
    <li>If the end of the array is reached and the value is not found, return -1 to indicate that the value was not found.</li>
  </ol>
</div>
...
<p>Let's try to do the searching manually, just to get an even better understanding of how Linear Search works before actually implementing it in a programming language. We will search for value 11.</p>
<p><strong>Step 1:</strong> We start with an array of random values.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 12, 8, 9, 11, 5, 11]</code>
</div>
<hr>
<p><strong>Step 2:</strong> We look at the first value in the array, is it equal to 11?</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[<mark class="dsa_mark"> 12</mark>, 8, 9, 11, 5, 11]</code>
</div>
<hr>
<p><strong>Step 3:</strong> We move on to the next value at index 1, and compare it to 11 to see if it is equal.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 12,<mark class="dsa_mark"> 8</mark>, 9, 11, 5, 11]</code>
</div>
<hr>
<p><strong>Step 4:</strong> We check the next value at index 2.</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 12, 8,<mark class="dsa_mark"> 9</mark>, 11, 5, 11]</code>
</div>
<hr>
<p><strong>Step 5:</strong> We move on to the next value at index 3. Is it equal to 11?</p>
<div class="w3-code notranslate">
  <code class="language-python w3-white">[ 12, 8, 9,<mark class="dsa_mark"> 11</mark>, 5, 11]</code>
</div>
<p>We have found it!</p>
<p>Value 11 is found at index 3.</p>
<p>Returning index position 3.</p>
<p>Linear Search is finished.</p>
<hr>
...
const app2 = Vue.createApp({
  ...
  methods: {
    ...
    async linearSearch() {
      ...
      this.msgDone = 'Found value 11 at index 3!';
      this.buttonText = "Reset";
      this.btnIsDisabled = false;
    },
    addDie50() {
      this.dice = [];
      let my_array = [ 12, 8, 9, 11, 5, 11];
...
<p>This algorithm is really straight forward.</p>
<p>Every value is checked from the start of the array to see if the value is equal to 11, the value we are trying to find.</p>
<p>When the value is found, the searching is stopped, and the index where the value is found is returned.</p>
<p>If the array is searched through without finding the value, -1 is returned.</p>
<hr>

<h2>Linear Search Implementation</h2>
<p>To implement the Linear Search algorithm we need:</p>
<ol>
  <li>An array with values to search through.</li>
  <li>A target value to search for.</li>
  <li>A loop that goes through the array from start to end.</li>
  <li>An if-statement that compares the current value with the target value, and returns the current index if the target value is found.</li>
  <li>After the loop, return -1, because at this point we know the target value has not been found.</li>
</ol>
<p>The resulting code for Linear Search looks like this:</p>

<div class="w3-example">
  <h3>Example</h3>
  <pre class="language-python notranslate w3-white"><code>def linearSearch(arr, targetVal):
...
if result != -1:
    ...
    print("Value",targetVal,"not found")</code></pre>
  <a class="w3-btn w3-margin-bottom" href="trydsa.php?filename=demo_linearsearch" target="_blank">Try it Yourself &raquo;</a>
</div>
<hr>
...
<p>If Linear Search runs and finds the target value as the first array value in an array with \(n\) values, only one compare is needed.</p>
<p>But if Linear Search runs through the whole array of \(n\) values, without finding the target value, \(n\) compares are needed.</p>
<p>This means that time complexity for Linear Search is</p>
<p><span class="math display">\[ O(n) \]</span></p>
<p>If we draw how much time Linear Search needs to find a value in an array of \(n\) values, we get this graph:</p>
<img src="img_linearsearch_timecomplexity.png" class="imgTut" alt="Time Complexity">
<p>Run the simulation below for different number of values in an array, and see how many compares are needed for Linear Search to find a value in an array of \(n\) values:</p>
<div id="vueApp4">
  ...
  <div class="frameDivOuter">
    ...
    <div id="frameDiv9">
      <div id="graphDiv">
        <transition-group name="miniSort">
          <div v-for="x in array"
            :key="keyNmbr"
            :style="{
              height: Math.round(100*x.value/this.userX) + 'px',
              boxShadow: x.active ? '0px 0px 5px 4px rgba(144, 238, 144, 0.9)' : 'none',
              zIndex: x.active ? 2 : 1,
              backgroundColor: x.active ? 'black' : 'gray',
      ...
      <p>Operations: {{ operations }}<br><span v-show="notFound" id="notFound">Not found!</span></p>
    </div>
  </div>
...
<script>
  ...
  computed: {
    targetVal(){
      return Math.ceil(Math.random() * (this.array.length-1));
    },
    isRangeDisabled() {
  ...
  methods: {
    newArray(){
      ...
      for (let k = 0; k < this.userX-1; k++) {
        let randNumber = Math.ceil(Math.random() * this.userX);
        this.array.push({ 
          value: randNumber, 
          active: false,
          done: false
    ...
    async addPoint() {
      this.isLoading = true;
      let operationsReturn = await this.performLinearSearch();
      if(operationsReturn.foundIt){
        myChart.data.datasets[1].data.push({ x: parseFloat(this.userX), y: operationsReturn.operations });
      }
      else{
        myChart.data.datasets[2].data.push({ x: parseFloat(this.userX), y: operationsReturn.operations });
      }
    ...
    async performLinearSearch() {
      let n = this.array.length;
      this.operations = 0;
      let foundIndex = -1; // Initialize to -1, which means "not found"
      let foundIt = false;
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (this.array[i].value === this.targetVal) {
          this.operations++; // Increment for successful comparison
          foundIndex = i; // Set the found index
          foundIt = true;
          this.array[i].active = true;
          i++;
          while(i<n){
      ...
      // Return the found index. If it's -1, the target was not found.
      return {operations: this.operations, foundIt: foundIt};
    },
    runSelected() {
...
<p>Choosing "Random", "Descending" or "Ascending" in the simulation above has no effect on how fast Linear Search is.</p>
<hr>

<div>
<h2>DSA Exercises</h2>
<form autocomplete="off" spellcheck="false" id="w3-exerciseform" action="exercise.php?filename=exercise_algo_linearsearch2" method="post" target="_blank">
<h2>Test Yourself With Exercises</h2>
<div class="exercisewindow">
<h2>Exercise:</h2>
<p>Complete the code.</p>
<div class="exerciseprecontainer">
<pre style="overflow: auto;">def linearSearch(arr, targetVal):
    for i in range(len(arr)):
        if arr[i] == targetVal:
          <input name="ex1" maxlength="8" style="width: 9ch;">
    return -1
</pre>
</div>
<br>
<button type="submit" class="w3-btn w3-margin-bottom">Submit Answer &raquo;</button>
<p><a target="_blank" href="exercise.php?filename=exercise_algo_linearsearch2">Start the Exercise</a></p>
</div>
</form>
</div>
<hr>
