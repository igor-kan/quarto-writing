...
<a class="w3-left w3-btn" href="js_asynchronous.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="js_async_timeouts.asp">Next &#10095;</a>
</div>
<!--start-->
<div class="ws-info">
<h2>Asynchronous Code</h2>
<p><b>Async code</b> allows a program to start a long-running task (like fetching data from a file).
and continue with other tasks before the first one finishes.</p>
<p><b>Async code</b> prevents the application from freezing, which is critical for user experience.</p>
</div>

<h2>Control Flow</h2>
<p><b>Control Flow</b> is the order in which statements are executed in a program.</p>
<p>By default, JavaScript runs code <b>from top to bottom</b> and left to right.</p>
<p><b>Async programming can change this</b>.</p>

<hr>
<h2>How JavaScript Runs Code</h2>
<p>JavaScript executes code one line at a time.</p>
<p>Each line must finish before the next line runs.</p>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryit.asp?filename=tryjs_synchronous">
Try it Yourself &raquo;</a>
</div>
<p>The output is always A B C.</p>
...
<p>This example will display "<b>Hello Goodbye</b>" because the functions are called in that order:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate jsHigh">
  function myFirst() {<br>&nbsp; myDisplayer(&quot;Hello&quot;);<br>}<br><br>function 
  mySecond() {<br>&nbsp; 
  myDisplayer(&quot;Goodbye&quot;);<br>}<br><br>myFirst();<br>mySecond();
</div>
<p>
<a target="_blank" class="w3-btn" href="tryit.asp?filename=tryjs_function_flow1">
Try it Yourself &raquo;</a>
</p>
</div>

<p>This example will display "<b>Goodbye Hello</b>" because the functions are called in that order:</p>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate jsHigh">
  function myFirst() {<br>&nbsp; myDisplayer(&quot;Hello&quot;);<br>}<br><br>function 
  mySecond() {<br>&nbsp; 
  myDisplayer(&quot;Goodbye&quot;);<br>}<br><br>mySecond();<br>myFirst();</div>
<p>
<a target="_blank" class="w3-btn" href="tryit.asp?filename=tryjs_function_flow2">
Try it Yourself &raquo;</a>
</p>
</div>
...
<p>The examples above are normal <b>synchronous flow</b>.</p>
</div>

<hr>
<h2>Why Async Code</h2>
<p><b>Some tasks take time to finish</b> (network requests, timers, user input).</p>
<p><b>To stay responsive</b>, JavaScript can use <b>async programming</b>.</p>

<p><b>Asynchronous flow</b> refers to how JavaScript allows certain operations to 
<b>run in the background</b> and let their results be <b>handled when they are ready</b>.</p>
<p>If JavaScript waited for these tasks, the page would freeze.</p>
<p>Asych code lets the rest of the program continue to run.</p>
<p>Async code does not run immediately:</p>

<ul class="w3-ul w3-border">
<li><b>Timers</b> run after a specified number of milliseconds</li>
<li><b>Events</b> run when triggered by an event</li>
<li><b>Network requests</b> run when the data arrives</li>
</ul>

<div class="ws-note">
<p>A frozen page is a broken page.</p>
<p>Asynch code does not block execution.</p>
</div>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryit.asp?filename=tryjs_asynchronous">
Try it Yourself &raquo;</a>
</div>
<div class="ws-note">
<p>The output from the above example is A C B.</p>
</div>
...
<p>Result is <code class="w3-codespan">undefined</code> because the async code has not finished yet.</p>

<div class="ws-note">

<p>Beginners expect async results immediately.</p>
</div>

<hr>
<h2>JavaScript Events</h2>
<p><b>Events</b> are actions or occurrences that happen in the browser,
often triggered by user interactions (like clicks, keypresses, or form submissions)
or by the browser itself (like page loading or resizing).</p>
<div class="w3-example">
...
<p>JavaScript handles asynchronus programming using different core concepts.</p>
<div class="w3-stretch">
<table class="ws-table-all">
<tr><th>Concept</th><th>Description</th></tr>
<tr><th>Synchronus</th><td>The JavaScript standard flow is executing line by line</td></tr>
<tr><th>Timers</th><td>Allows code to run while other code is waiting</td></tr>
<tr><th>Callbacks</th><td>Callbacks were the first solution for async JavaScript</td></tr>
<tr><th>Events</th><td>Stores callback function waiting to be executed</td></tr>
<tr><th>Promises</th><td>Tools to handle asynchronous operations cleanly</td></tr>
<tr><th>Async/Await</th><td>The clean and modern way to handle async code</td></tr>
</table>
</div>

<hr>
<h2>Asynchronous vs Parallel</h2>
<p>Parallel means doing multiple things at the same time on different processors.</p>
<p>Asynchronous means switching between tasks, not necessarily running them simultaneously.</p>
<p>A single-threaded JavaScript engine handles asynchronous tasks by using an event
loop to switch between them, rather than utilizing multiple CPU cores.
When a task finishes, it signals the main thread (via a callback, promise, or event) to handle the result.</p>
<div class="w3-stretch">
<table class="ws-table-all">
<tr></tr>
<tr><th>Feature</th><th>Async (Deferred)</th><th>Parallel (Simultaneous)</th></tr>
<tr><th>Goal</th><td>Responsiveness (Don't freeze)</td><td>Performance (Get it done faster)</td></tr>
<tr><th>Execution</th><td>Non-blocking (waiting for I/O)</td><td>Simultaneous (crunching many numbers)</td></tr>
<tr><th>Hardware</th><td>Can run on 1 processor</td><td>Requires multiple processors</td></tr>
<tr><th>Example</th><td>Running code while the user scrolls</td><td>Processing 10,000 images at once</td></tr>
</table>
</div>

<p>In short, asynchronous tells the system:</p>
<ol>
<li>Start this task now</li>
<li>I don't need the result immediately</li>
<li>Notify me later when it's done</li>
</ol>
<div class="ws-note">
<h2>Analogy:</h2>
<p>Ordering food at a restaurant.</p>
<ol>
<li>Place your order (async call)</li>
<li>Sit down and do other things while the chef makes it</li>
<li>The server brings the food (callback)</li>
</ol>
</div>

<hr>
<h2>What You Will Learn</h2>
<p>This tutorial will build the understanding of async programming step by step:</p>

<ul class="w3-ul w3-border">
<li>What are <b>Timeouts</b></li>
<li>Why <b>callbacks</b> were created</li>
<li>How <b>promises</b> represent future values.</li>
<li>Why <code class="w3-codespan">async</code> and <code class="w3-codespan">await</code> are preferred.</li>
<li>How to <b>debug</b> async code.</li>
</ul>
<p>Each of these tools was created to solve problems from the previous tool.</p>

<hr>
<div class="ws-note">
<h2>Next Chapter</h2>
<p>A JavaScript timeout schedules a function to run after a delay in milliseconds.</p>
<p>A timeout is an async operation used to delay code execution without freezing the browser.</p>
<p class="w3-large"><a class="w3-btn" href="js_async_timeouts.asp">JavaScript Timeouts</a></p>
</div>
<hr>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="js_asynchronous.asp">&#10094; Previous</a>
