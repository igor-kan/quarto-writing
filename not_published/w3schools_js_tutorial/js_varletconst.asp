...
<p>The primary difference between <code class="w3-codespan">var</code>, <code class="w3-codespan">let</code>,
and <code class="w3-codespan">const</code> lies in <b>scoping</b> mechanisms, <b>reassignment</b> rules, and how JavaScript handles
them via <b>hoisting</b>.</p>
</div>
...
<tr></tr>
<tr><th></th><th>Scope</th><th>Redeclare</th><th>Reassign</th><th>Hoisted</th><th>NOTE</th></tr>
<tr><th>var</th><td>No</td><td>Yes</td><td>Yes</td><td>Yes</td><td>Undefined</td></tr>
<tr><th>let</th><td>Yes</td><td>No</td><td>Yes</td><td>Yes</td><td>Not initialized</td></tr>
<tr><th>const</th><td>Yes</td><td>No</td><td>No</td><td>Yes</td><td>Not initialized</td></tr>
</table>
</div>
<p>Variables declared with <code class="w3-codespan">var</code> are hoisted but initialized with <code class="w3-codespan">undefined</code>.</p>
<p>Variables declared with <code class="w3-codespan">let</code> and <code class="w3-codespan">const</code> are also hoisted,
but not initialized.</p>
<p>Using a <code class="w3-codespan">let</code> or <code class="w3-codespan">const</code> variable before it is declared will
result in a <code class="w3-codespan">ReferenceError</code>.</p>

<p>The time between the start of the block and the declaration is called the Temporal Dead Zone.</p>
...
<div class="ws-note">
<p>Modern JavaScript standards recommend <b>avoiding var entirely</b> to minimize unintentional bugs.</p>
</div>

<hr>
<h2>Scope Rules</h2>
<p><b><code class="w3-codespan">var</code> is function-scoped</b>:<br>
If you declare a <code class="w3-codespan">var</code> variable inside a loop or an if block,
it "<b>leaks out</b>" and is accessible anywhere outside the block.</p>
<p><b><code class="w3-codespan">let</code> and <code class="w3-codespan">const</code> are block-scoped</b>:<br>
They are confined strictly to the nearest pair of <b>curly braces {}</b> (such as loops, functions, or conditional blocks).</p>
<div class="ws-note">
<p>As a rule of thumb, you should <b>use const by default</b>, and only use <code class="w3-codespan">let</code> when you have to reassign the
variable after initialization.</p>
<p>You should <b>avoid using <code class="w3-codespan">var</code></b>.</p>
</div>
...
<tr><th>Scope</th><td>Function or global scope</td><td>Block-scope { }</td><td>Block-scope { }</td></tr>
<tr><th>Reassignment</th><td>Can be updated</td><td>Can be updated</td><td>Cannot be updated</td></tr>
<tr><th>Redeclaration</th><td>Can be redeclared</td><td>Cannot be redeclared</td><td>Cannot be redeclared</td></tr>
<tr><th>Hoisting</th><td>Initialized as undefined</td><td>Hoisted, not initialized</td><td>Hoisted, not initialized</td></tr>
</table>
</div>
...
<br>
let text1 = text2 = "unknown";<br>
<br>
if (typeof firstName !== "undefined") text1 = firstName;<br>
if (typeof lastName !== "undefined") text2 = lastName;
</div>
<a class="w3-btn w3-margin-bottom" href="tryit.asp?filename=tryjs_block_scope" target="_blank">Try it Yourself &raquo;</a>
</div>

<hr>
<h2>Reassignment vs. Redeclaration</h2>
<p>var allows you to accidentally re-declare the exact same variable name in the same scope, silently overwriting earlier values.</p>
<p>let can be updated/reassigned, but it throws an error if you attempt to re-declare it within the same scope.</p>
<p>const is strictly read-only. It requires an initial value immediately upon declaration and cannot be reassigned later.</p>
<div class="ws-note">
<p><b>Note on const objects:</b><br>
You cannot reassign a const variable with a completely new value,
you can still change the properties of an object or array declared with const.</p>
</div>
<div class="w3-example">
<h3>Example</h3>
<div class="w3-code notranslate jsHigh">
...
// This will work<br>
user.name = "Bob";<br>
let text = user.name + " ";<br>
<br>
// This will not work<br>
try {<br>
&nbsp; user = { name: "Charlie" };<br>
&nbsp; text += user.name;<br>
} catch(err) {<br>
&nbsp; text += err;<br>
}
</div>
<a class="w3-btn w3-margin-bottom" href="tryit.asp?filename=tryjs_const_object3" target="_blank">Try it Yourself &raquo;</a>
</div>

<hr>
<h2>Hoisting and the Temporal Dead Zone (TDZ)</h2>
<p>All variables are hoisted to the top of their scope by the JavaScript engine, but they are treated differently:</p>
<p>var variables are hoisted and automatically assigned a default value of undefined. You can reference them before they are declared in the code without breaking the application.</p>
<p>let and const are hoisted but remain completely uninitialized. The region of code from the start of the block until the line they are declared is called the Temporal Dead Zone. Accessing them here instantly crashes with a ReferenceError.</p>

<hr>
<h2>Best Practice</h2>
<p>Developers should follow these guidelines for modern JavaScript (<a href="js_es6.asp">ES6 - 2015+</a>):</p>
<p><b>Always use const by default.</b><br>
It prevents accidental reassignments and makes your code more predictable.</p>
<p><b>Only use let when:</b><br>
You know its value needs to change later (inside a loop or a mathematical counter).</p>
<p><b>Never use var.</b><br>
Its unpredictable scoping and the redeclaration rules are known to cause bugs.</p>
