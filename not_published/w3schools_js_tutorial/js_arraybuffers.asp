...
<html lang="en-US">
<head>
<title>JavaScript Array Buffer</title>
<meta charset="utf-8">
...
<h1>JavaScript ArrayBuffer</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="js_typed_reference.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="js_dataview.asp">Next &#10095;</a>
</div>

<div class="ws-info">
<p>An <b>ArrayBuffer</b> is fixed a block of memory,
often used to store <b>typed arrays</b>.</p>
<p>On top of this block, you can create different 
<b>views</b> that interpret the bits as numbers, bytes, or other data types.</p>
</div>

<h2>Creating an ArrayBuffer</h2>
<p>Use <code class="w3-codespan">new ArrayBuffer()</code> to create a new ArrayBuffer.</p>
<div class="w3-example">
<h3>Example</h3>
<p>Create a 16-byte ArrayBuffer:</p>
<div class="w3-code notranslate jsHigh">
// Create an ArrayBuffer<br>
const myBuf = new ArrayBuffer(16);<br>
<br>
// Get the length in bytes<br>
let len = myBuf.byteLength;<br>
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_create" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<div class="ws-note">

<p>The <b>size</b> of an ArrayBuffer is specified in <b>bytes</b>.</p>
<p>The <b>byteLength</b> property represents the size.</p>
<p>Once created, the size can not be changed.</p>
</div>

<hr>

<h2>Accessing an ArrayBuffer</h2>
<p>The ArrayBuffer does not have methods to <b>read</b> and <b>write</b> data.</p>
<p>You must always use a <b>view</b> to access the data.</p>
<p><b>Typed Arrays</b> and <b>DataViews</b> provide a way to read and write numeric values to an
ArrayBuffer.</p>
<p>Common typed arrays are:</p>

<ul>
<li>Uint8Array - 8-bit unsigned integers</li>
<li>Int16Array - 16-bit signed integers</li>
<li>Int32Array - 32-bit signed integers</li>
<li>Float32Array - 32-bit floating point numbers</li>
<li>Float64Array - 64-bit floating point numbers</li>
</ul>

<hr>
<h2>Using an Uint8Array</h2>

<div class="w3-example">
<h3>Example</h3>
<p>Each Uint8 uses 1 byte.</p>
<div class="w3-code notranslate jsHigh">
// Create an ArrayBuffer<br>
const myBuf = new ArrayBuffer(8);<br>
<br>
// Create a Uint8Array view<br>
const view = new Uint8Array(myBuf);<br>
<br>
// Write max 8 values<br>
view[0] = 10;<br>
view[2] = 128;<br>
view[1] = 255;<br>
<br>
...
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_unit8" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>
...
<hr>
<h2>Using an Int32Array</h2>
<div class="w3-example">
<h3>Example</h3>

<p>Each Int32 uses 4 bytes.</p>
<div class="w3-code notranslate jsHigh">
// Create an ArrayBuffer<br>
const myBuf = new ArrayBuffer(12);<br>
<br>
// Create an Int32Array view<br>
const view = new Int32Array(myBuf);<br>
<br>
// 12 bytes = max 3 Int32 values<br>
view[0] = 100000;<br>
view[1] = 200000;<br>
view[2] = 300000;<br>
<br>
...
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_int32" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<hr>
<h2>Using a DataView</h2>
<p>A <b>DataView</b> is a more flexible view for an ArrayBuffer.</p>
<p>A <b>DataView</b> lets you read and write values of different types (Int8, Uint16, Float32, etc.).</p>
<p>A <b>DataView</b> also lets you read and write values at any byte offset.</p>
<div class="w3-example">
<h3>Example: Reading and Writing with DataView</h3>
<div class="w3-code notranslate jsHigh">
// Create an ArrayBuffer<br>
const myBuf = new ArrayBuffer(8);<br>
<br>
// Create a DataView<br>
const view = new DataView(myBuf);<br>
<br>
// Write a 32-bit integer at byte offset 0<br>
view.setInt32(0, 123456);<br>
<br>
// Write a 16-bit integer at byte offset 4<br>
view.setInt16(4, 32000);<br>
<br>
// Read the values<br>
let v1 view.getInt32(0);<br>
let v2 = view.getInt16(4);<br>
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_dataview" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<div class="ws-note">

<p>The <b>DataView methods</b> have an optional <b>littleEndian</b> parameter (true/false)
to control the byte order.</p>
</div>

<hr>
<h2 id="slicing">Slicing an ArrayBuffer</h2>
<p>
You can make a copy of a part of an <code class="w3-codespan">ArrayBuffer</code> using
the <code class="w3-codespan">slice()</code> method. It returns a new <code class="w3-codespan">ArrayBuffer</code>
with bytes from the specified range.
</p>

<div class="w3-example">
<h3>Example: ArrayBuffer.slice()</h3>
<div class="w3-code notranslate jsHigh">
// Create an ArrayBuffer<br>
const myBuf = new ArrayBuffer(8);<br>
<br>
// Create a Uint8Array<br>
const view = new Uint8Array(myBuf);<br>
<br>
// Fill with values 0 to 7<br>
for (let i = 0; i &lt; view.length; i++) {<br>
&nbsp;&nbsp;view[i] = i;<br>
}<br>
<br>
// Create a copy of bytes from 2 to 5 (not including 5)<br>
const sliced = myBuf.slice(2, 5);<br>
const slicedView = new Uint8Array(sliced);<br>
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_slice" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<div class="ws-note">

<p>The <code class="w3-codespan">slice()</code> method creates a new buffer.</p>
<p>The <code class="w3-codespan">slice()</code> method does not share memory with the original buffer.</p>
</div>
...
<div class="w3-example">
<h3>Example</h3>
<p>Converting a String to an ArrayBuffer (UTF-8)</p>
<div class="w3-code notranslate jsHigh">
function stringToArrayBuffer(str) {<br>
&nbsp;&nbsp;const encoder = new TextEncoder();<br>
&nbsp;&nbsp;return encoder.encode(str).buffer;<br>
}<br>
<br>
const myBuf = stringToArrayBuffer("Hello");<br>
let len1 = myBuf.byteLength;<br>
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_stringtoarr" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<div class="w3-example">
<h3>Example</h3>
<p>Converting an ArrayBuffer to a String (UTF-8)</p>
<div class="w3-code notranslate jsHigh">
function arrayBufferToString(buffer) {<br>
&nbsp;&nbsp;const decoder = new TextDecoder();<br>
&nbsp;&nbsp;return decoder.decode(new Uint8Array(buffer));<br>
}<br>
<br>
const encoder = new TextEncoder();<br>
const myBuf = encoder.encode("Hello ArrayBuffer").buffer;<br>
<br>
let text = arrayBufferToString(myBuf);
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_arrtostring" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<hr>
<h2>Sharing ArrayBuffer (SharedArrayBuffer)</h2>
<p>An <code class="w3-codespan">ArrayBuffer</code> is not shared between threads by default.</p>
<p>To share memory between workers, JavaScript provides <code class="w3-codespan">SharedArrayBuffer</code>.</p>
<p>It behaves like <code class="w3-codespan">ArrayBuffer</code>, but its contents can be shared and
used with <code class="w3-codespan">Atomics</code>.</p>

<div class="w3-example">
<h3>Example: Creating a SharedArrayBuffer</h3>
<div class="w3-code notranslate jsHigh">
if (Window.crossOriginIsolated) {<br>
&nbsp;&nbsp;buffer = new SharedArrayBuffer(16);<br>
} else {<br>
&nbsp;&nbsp;buffer = new ArrayBuffer(16);<br>
}<br>
<br>
const sharedView = new Int32Array(buffer);<br>
<br>
buffer[0] = 42;<br>
<br>
let numb = sharedView[0];<br>
</div>
<a target="_blank" href="tryit.asp?filename=tryjs_arraybuffer_shared" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>

<div class="ws-note">

<p>Browsers may require special security headers to enable
<code class="w3-codespan">SharedArrayBuffer</code> (COOP/COEP).
</div>

<hr>
<h2>Summary</h2>
<ul>
<li>ArrayBuffer is a low-level object representing a fixed-size block of memory</li>
<li>You cannot read or write directly to an ArrayBuffer</li>
<li>You use views (typed arrays or DataView) to access an ArrayBuffer</li>
<li>Typed arrays are good for uniform numeric data</li>
<li>DataView is good for mixed or structured data</li>
<li>Use slice() to copy parts of an ArrayBuffer</li>
<li>Use a SharedArrayBuffer and Atomics for shared-memory concurrency</li>
</ul>
<hr>
<h2>Common Use Cases</h2>
<ul>
<li>Working with binary data from files (e.g., images, videos, audio).</li>
<li>Handling binary network protocols (WebSockets, WebRTC, etc.).</li>
<li>Performing performance-critical numeric computations.</li>
<li>Interoperability with WebAssembly or other low-level APIs.</li>
</ul>

<hr>
<h2>ArrayBuffer Reference</h2>
...
<td>Creates a new ArrayBuffer</td>
</tr>
<tr>
...
<td>
Returns a new ArrayBuffer as a copy of a portion of this buffer
</td>
</tr>
<tr>
<td>ArrayBuffer.isView()</td>
<td>
Returns true if argument is a view on an ArrayBuffer
</td>
</tr>
</table>
</div>
...
<div class="ws-note">
<h2>Learn More:</h2>
<p class="w3-large"><a href="js_typed_arrays.asp">Typed Array Tutorial</a></p>
