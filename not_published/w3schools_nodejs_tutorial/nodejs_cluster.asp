...
<meta name="title" property="og:title" content="Node.js Cluster Module">
<meta name="Keywords" content="HTML, Python, CSS, SQL, JavaScript, How to, PHP, Java, C, C++, C#, jQuery, Bootstrap, Colors, W3.CSS, XML, MySQL, Icons, NodeJS, React, Graphics, Angular, R, AI, Git, Data Science, Code Game, Tutorials, Programming, Web Development, Training, Learning, Quiz, Exercises, Courses, Lessons, References, Examples, Learn to code, Source code, Demos, Tips, Website">
<meta name="Description" content="Well organized and easy to understand Web building tutorials with lots of examples of how to use HTML, CSS, JavaScript, SQL, Python, PHP, Bootstrap, Java, XML and more.">
<meta property="og:image" content="https://www.w3schools.com/images/og/nodejs/nodejs_cluster.png">
<meta property="og:image:type" content="image/png">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:description" content="The Cluster module provides a way to create multiple worker processes that share the same server port.">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:site" content="@w3schools">
<meta name="twitter:title" content="Node.js Cluster Module">
<meta name="twitter:description" content="The Cluster module provides a way to create multiple worker processes that share the same server port.">
<meta name="twitter:image" content="https://www.w3schools.com/images/og/nodejs/nodejs_cluster.png">
...
<h1>Node.js Cluster Module</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="nodejs_child_process.asp">&lt; Previous</a>
<a class="w3-right w3-btn" href="nodejs_worker_threads.asp">Next &gt;</a>
</div>

<hr>
<h2>What is the Cluster Module?</h2>
<p>The Cluster module provides a way to create multiple worker processes that share the same server port.</p>
<p>Since Node.js is single-threaded by default, the Cluster module helps your application utilize multiple CPU cores, significantly improving performance on multi-core systems.</p>
<p>Each worker runs in its own process with its own event loop and memory space, but they all share the same server port.</p>
<p>The master process is responsible for creating workers and distributing incoming connections among them.</p>

<hr>

<h2>Importing the Cluster Module</h2>

<p>The Cluster module is included in Node.js by default.</p>
<p>You can use it by requiring it in your script:</p>
...
<br>
// Check if this is the master process<br>
if (cluster.isMaster) {<br>
&nbsp;&nbsp;console.log(`Master process ${process.pid} is running`);<br>
} else {<br>
&nbsp;&nbsp;console.log(`Worker process ${process.pid} started`);<br>
}</div>
</div>
...
<p>The Cluster module works by creating a master process that spawns multiple worker processes.</p>
<p>The master process doesn't execute the application code but manages the workers.</p>
<p>Each worker process is a new Node.js instance that runs your application code independently.</p>

<div class="w3-panel ws-note">
<p><strong>Note:</strong> Under the hood, the Cluster module uses the Child Process module's <code class="w3-codespan">fork()</code> method to create new workers.</p>
</div>
...
<table class="ws-table-all">
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>Creating and managing worker processes</li>
        <li>Monitoring worker health</li>
        <li>Restarting crashed workers</li>
        <li>Load balancing (distributing connections)</li>
      </ul>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <ul>
        <li>Running the actual application code</li>
...
<h2>Creating a Basic Cluster</h2>

<p>Here's a simple example of creating a cluster with worker processes for each CPU:</p>
...
<br>
if (cluster.isMaster) {<br>
&nbsp;&nbsp;// This is the master process<br>
<br>
&nbsp;&nbsp;console.log(`Master ${process.pid} is running`);<br>
<br>
&nbsp;&nbsp;// Fork workers for each CPU core<br>
&nbsp;&nbsp;for (let i = 0; i &lt; numCPUs; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;cluster.fork();<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Listen for worker exits<br>
&nbsp;&nbsp;cluster.on('exit', (worker, code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} died`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;// You can fork a new worker to replace the dead one<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Forking a new worker...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;cluster.fork();<br>
&nbsp;&nbsp;});<br>
} else {<br>
&nbsp;&nbsp;// This is a worker process<br>
<br>
&nbsp;&nbsp;// Create an HTTP server<br>
&nbsp;&nbsp;http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;res.writeHead(200);<br>
&nbsp;&nbsp;&nbsp;&nbsp;res.end(`Hello from Worker ${process.pid}\n`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Simulate CPU work<br>
&nbsp;&nbsp;&nbsp;&nbsp;let i = 1e7;<br>
&nbsp;&nbsp;&nbsp;&nbsp;while (i > 0) { i--; }<br>
<br>
&nbsp;&nbsp;}).listen(8000);<br>
...
<p>In this example:</p>
<ol>
  <li>The master process detects the number of CPU cores</li>
  <li>It forks one worker per CPU</li>
  <li>Each worker creates an HTTP server on the same port (8000)</li>
  <li>The cluster module automatically load balances the incoming connections</li>
  <li>If a worker crashes, the master creates a new one</li>
</ol>
...
<p>You can communicate between master and worker processes using the <code class="w3-codespan">send()</code> method and <code class="w3-codespan">message</code> events, similar to how IPC works in the Child Process module.</p>
...
&nbsp;&nbsp;console.log(`Master ${process.pid} is running`);<br>
<br>
&nbsp;&nbsp;// Track request count for each worker<br>
&nbsp;&nbsp;const requestCounts = {};<br>
<br>
&nbsp;&nbsp;// Fork workers<br>
&nbsp;&nbsp;for (let i = 0; i &lt; numCPUs; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;const worker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;requestCounts[worker.id] = 0;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Listen for messages from this worker<br>
&nbsp;&nbsp;&nbsp;&nbsp;worker.on('message', (msg) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (msg.cmd === 'incrementRequestCount') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;requestCounts[worker.id]++;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.id} (pid ${worker.process.pid}) has handled ${requestCounts[worker.id]} requests`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Every 10 seconds, send the request count to each worker<br>
&nbsp;&nbsp;setInterval(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;for (const id in cluster.workers) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;cluster.workers[id].send({<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;cmd: 'requestCount',<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;requestCount: requestCounts[id]<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Total request counts:', requestCounts);<br>
&nbsp;&nbsp;}, 10000);<br>
<br>
&nbsp;&nbsp;// Handle worker exit<br>
&nbsp;&nbsp;cluster.on('exit', (worker, code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} died`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Fork a new worker to replace it<br>
&nbsp;&nbsp;&nbsp;&nbsp;const newWorker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;requestCounts[newWorker.id] = 0;<br>
&nbsp;&nbsp;});<br>
} else {<br>
&nbsp;&nbsp;// Worker process<br>
&nbsp;&nbsp;console.log(`Worker ${process.pid} started`);<br>
<br>
&nbsp;&nbsp;let localRequestCount = 0;<br>
<br>
&nbsp;&nbsp;// Handle messages from the master<br>
&nbsp;&nbsp;process.on('message', (msg) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (msg.cmd === 'requestCount') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} has handled ${msg.requestCount} requests according to master`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// Create an HTTP server<br>
&nbsp;&nbsp;http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Notify the master that we handled a request<br>
&nbsp;&nbsp;&nbsp;&nbsp;process.send({ cmd: 'incrementRequestCount' });<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Increment local count<br>
&nbsp;&nbsp;&nbsp;&nbsp;localRequestCount++;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Send response<br>
&nbsp;&nbsp;&nbsp;&nbsp;res.writeHead(200);<br>
...
<h2>Zero-Downtime Restart</h2>

<p>One of the main benefits of clustering is the ability to restart workers without downtime. This is useful for deploying updates to your application.</p>
...
&nbsp;&nbsp;const workers = [];<br>
<br>
&nbsp;&nbsp;// Fork initial workers<br>
&nbsp;&nbsp;for (let i = 0; i &lt; numCPUs; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;workers.push(cluster.fork());<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Function to restart workers one by one<br>
&nbsp;&nbsp;function restartWorkers() {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Starting zero-downtime restart...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;<br>
&nbsp;&nbsp;&nbsp;&nbsp;let i = 0;<br>
&nbsp;&nbsp;&nbsp;&nbsp;function restartWorker() {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (i >= workers.length) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('All workers restarted successfully!');<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const worker = workers[i++];<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Restarting worker ${worker.process.pid}...`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Create a new worker<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const newWorker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;newWorker.on('listening', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Once the new worker is listening, kill the old one<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;worker.disconnect();<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Replace the old worker in our array<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;workers[workers.indexOf(worker)] = newWorker;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Continue with the next worker<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;setTimeout(restartWorker, 1000);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Start the recursive process<br>
&nbsp;&nbsp;&nbsp;&nbsp;restartWorker();<br>
&nbsp;&nbsp;}<br>
&nbsp;&nbsp;<br>
&nbsp;&nbsp;// Simulate a restart after 20 seconds<br>
&nbsp;&nbsp;setTimeout(restartWorkers, 20000);<br>
<br>
&nbsp;&nbsp;// Handle normal worker exit<br>
&nbsp;&nbsp;cluster.on('exit', (worker, code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (worker.exitedAfterDisconnect !== true) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} died unexpectedly, replacing it...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const newWorker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;workers[workers.indexOf(worker)] = newWorker;<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
...
<ol>
  <li>Creating an initial set of workers</li>
  <li>Replacing each worker one by one</li>
  <li>Ensuring a new worker is listening before disconnecting the old one</li>
  <li>Gracefully handling unexpected worker deaths</li>
</ol>
...
<p>The Cluster module has built-in load balancing for distributing incoming connections among worker processes.</p>
<p>There are two primary strategies:</p>

<h3>Round-Robin (default)</h3>

<p>By default on all platforms except Windows, Node.js distributes connections using a round-robin approach, where the master accepts connections and distributes them across workers in a circular sequence.</p>

<div class="w3-panel ws-note">
<p><strong>Note:</strong> On Windows, the load distribution behaves differently due to how Windows handles ports. In Windows, the workers compete to accept connections.</p>
</div>

<h3>Primary Worker</h3>

<p>You can also let each worker accept connections directly by setting <code class="w3-codespan">cluster.schedulingPolicy</code>:</p>
...
const numCPUs = require('os').cpus().length;<br>
<br>
// Set the scheduling policy to SCHED_NONE (let workers accept connections themselves)<br>
cluster.schedulingPolicy = cluster.SCHED_NONE;<br>
<br>
if (cluster.isMaster) {<br>
&nbsp;&nbsp;console.log(`Master ${process.pid} is running`);<br>
<br>
...
<p>Since each worker runs in its own process with its own memory space, they cannot directly share state via variables. Instead, you can:</p>

<ol>
  <li>Use IPC messaging (as shown in the communication example)</li>
  <li>Use external storage like Redis, MongoDB, or a file system</li>
  <li>Use sticky load balancing for session management</li>
</ol>

<h3>Sticky Sessions Example</h3>

<p>Sticky sessions ensure that requests from the same client always go to the same worker process:</p>
...
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Store worker references by id<br>
&nbsp;&nbsp;const workers = {};<br>
&nbsp;&nbsp;for (const id in cluster.workers) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;workers[id] = cluster.workers[id];<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Create a server to route connections to workers<br>
&nbsp;&nbsp;const server = http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Get client IP<br>
&nbsp;&nbsp;&nbsp;&nbsp;const clientIP = req.connection.remoteAddress || req.socket.remoteAddress;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Simple hash function to determine which worker to use<br>
&nbsp;&nbsp;&nbsp;&nbsp;const workerIndex = clientIP.split('.').reduce((a, b) => a + parseInt(b), 0) % numCPUs;<br>
&nbsp;&nbsp;&nbsp;&nbsp;const workerIds = Object.keys(workers);<br>
&nbsp;&nbsp;&nbsp;&nbsp;const workerId = workerIds[workerIndex];<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Send the request to the selected worker<br>
&nbsp;&nbsp;&nbsp;&nbsp;workers[workerId].send('sticky-session:connection', req.connection);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;res.end(`Request routed to worker ${workerId}`);<br>
&nbsp;&nbsp;}).listen(8000);<br>
<br>
&nbsp;&nbsp;console.log('Master server listening on port 8000');<br>
<br>
&nbsp;&nbsp;// Handle worker exit<br>
&nbsp;&nbsp;cluster.on('exit', (worker, code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} died`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Remove the dead worker<br>
&nbsp;&nbsp;&nbsp;&nbsp;delete workers[worker.id];<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Create a replacement<br>
&nbsp;&nbsp;&nbsp;&nbsp;const newWorker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;workers[newWorker.id] = newWorker;<br>
&nbsp;&nbsp;});<br>
} else {<br>
&nbsp;&nbsp;// Worker process - just demonstrates the concept<br>
&nbsp;&nbsp;// In a real implementation, you'd need more socket handling<br>
<br>
&nbsp;&nbsp;process.on('message', (msg, socket) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (msg === 'sticky-session:connection' && socket) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} received sticky connection`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// In a real implementation, you'd handle the socket here<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// socket.end(`Handled by worker ${process.pid}\n`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// Workers would also run their own server<br>
&nbsp;&nbsp;http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;res.writeHead(200);<br>
...
<p>This is a simplified example showing the concept of sticky sessions. In production, you'd typically:</p>

<ol>
  <li>Use a more sophisticated hashing algorithm</li>
  <li>Use cookies or other session identifiers instead of IP addresses</li>
  <li>Handle socket connections more carefully</li>
</ol>
...
<p>Understanding the worker lifecycle is important for properly managing your cluster:</p>
...
<table class="ws-table-all">
  ...
  <tr>
    <td><code class="w3-codespan">fork</code></td>
    <td>Emitted when a new worker is forked</td>
  </tr>
  <tr>
    <td><code class="w3-codespan">online</code></td>
    <td>Emitted when the worker is running and ready to process messages</td>
  </tr>
  <tr>
    <td><code class="w3-codespan">listening</code></td>
    <td>Emitted when the worker starts listening for connections</td>
  </tr>
  <tr>
    <td><code class="w3-codespan">disconnect</code></td>
    <td>Emitted when a worker's IPC channel is disconnected</td>
  </tr>
  <tr>
    <td><code class="w3-codespan">exit</code></td>
    <td>Emitted when a worker process exits</td>
  </tr>
</table>
</div>
...
&nbsp;&nbsp;const worker = cluster.fork();<br>
<br>
&nbsp;&nbsp;// Listen for all worker lifecycle events<br>
&nbsp;&nbsp;worker.on('fork', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} is being forked`);<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;worker.on('online', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} is online`);<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;worker.on('listening', (address) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} is listening on port ${address.port}`);<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;worker.on('disconnect', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} has disconnected`);<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;worker.on('exit', (code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} exited with code ${code} and signal ${signal}`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp; if (signal) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker was killed by signal: ${signal}`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;} else if (code !== 0) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker exited with error code: ${code}`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;} else {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('Worker exited successfully');<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// After 10 seconds, gracefully disconnect the worker<br>
&nbsp;&nbsp;setTimeout(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Gracefully disconnecting worker...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;worker.disconnect();<br>
&nbsp;&nbsp;}, 10000);<br>
<br>
} else {<br>
...
&nbsp;&nbsp;}).listen(8000);<br>
<br>
&nbsp;&nbsp;// If worker is disconnected, close the server<br>
&nbsp;&nbsp;process.on('disconnect', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} disconnected, closing server...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;// In a real application, you'd want to close all connections and clean up resources<br>
&nbsp;&nbsp;&nbsp;&nbsp;process.exit(0);<br>
&nbsp;&nbsp;});<br>
}</div>
</div>
...
<p>A graceful shutdown is important to allow your worker processes to finish handling existing requests before they exit.</p>
...
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Handle termination signals<br>
&nbsp;&nbsp;process.on('SIGTERM', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Master received SIGTERM, initiating graceful shutdown...');<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Notify all workers to finish their work and exit<br>
&nbsp;&nbsp;&nbsp;&nbsp;Object.values(cluster.workers).forEach(worker => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Sending SIGTERM to worker ${worker.process.pid}`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;worker.send('shutdown');<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Set a timeout to force-kill workers if they don't exit gracefully<br>
&nbsp;&nbsp;&nbsp;&nbsp;setTimeout(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('Some workers did not exit gracefully, forcing shutdown...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Object.values(cluster.workers).forEach(worker => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (!worker.isDead()) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Killing worker ${worker.process.pid}`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;worker.process.kill('SIGKILL');<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Exit the master<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('All workers terminated, exiting master...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;process.exit(0);<br>
&nbsp;&nbsp;}, 5000);<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// Handle worker exits<br>
&nbsp;&nbsp;cluster.on('exit', (worker, code, signal) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} exited (${signal || code})`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// If all workers have exited, exit the master<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (Object.keys(cluster.workers).length === 0) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('All workers have exited, shutting down master...');<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;process.exit(0);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// Log to show the master is ready<br>
&nbsp;&nbsp;console.log(`Master ${process.pid} is ready with ${Object.keys(cluster.workers).length} workers`);<br>
&nbsp;&nbsp;console.log('Send SIGTERM to the master process to initiate graceful shutdown');<br>
<br>
} else {<br>
&nbsp;&nbsp;// Worker process<br>
&nbsp;&nbsp;console.log(`Worker ${process.pid} started`);<br>
<br>
&nbsp;&nbsp;// Track if we're shutting down<br>
&nbsp;&nbsp;let isShuttingDown = false;<br>
&nbsp;&nbsp;let activeConnections = 0;<br>
<br>
&nbsp;&nbsp;// Create HTTP server<br>
&nbsp;&nbsp;const server = http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp; // Track active connection<br>
&nbsp;&nbsp;&nbsp;&nbsp; activeConnections++;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Simulate a slow response<br>
&nbsp;&nbsp;&nbsp;&nbsp;setTimeout(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.writeHead(200);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.end(`Hello from Worker ${process.pid}\n`);<br>
<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;activeConnections--;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// If we're shutting down and no active connections, close the server<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (isShuttingDown && activeConnections === 0) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} has no active connections, closing server...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;server.close(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} closed server, exiting...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;process.exit(0);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;}, 2000);<br>
...
<br>
&nbsp;&nbsp;// Handle shutdown message from master<br>
&nbsp;&nbsp;process.on('message', (msg) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp; if (msg === 'shutdown') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} received shutdown message, stopping new connections...`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Set shutdown flag<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;isShuttingDown = true;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Stop accepting new connections<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;server.close(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} closed server`);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// If no active connections, exit immediately<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (activeConnections === 0) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} has no active connections, exiting...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;process.exit(0);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;} else {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} waiting for ${activeConnections} connections to finish...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
<br>
&nbsp;&nbsp;// Also handle direct termination signal<br>
&nbsp;&nbsp;process.on('SIGTERM', () => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} received SIGTERM directly`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Use the same shutdown logic<br>
&nbsp;&nbsp;&nbsp;&nbsp;isShuttingDown = true;<br>
...
<ul>
  <li><strong>Number of Workers:</strong> In most cases, create one worker per CPU core</li>
  <li><strong>Stateless Design:</strong> Design your application to be stateless to work effectively with clusters</li>
  <li><strong>Graceful Shutdown:</strong> Implement proper shutdown handling to avoid dropping connections</li>
  <li><strong>Worker Monitoring:</strong> Monitor and replace crashed workers promptly</li>
  <li><strong>Database Connections:</strong> Each worker has its own connection pool, so configure database connections appropriately</li>
  <li><strong>Shared Resources:</strong> Be careful with resources shared between workers (e.g., file locks)</li>
  <li><strong>Keep Workers Lean:</strong> Avoid unnecessary memory usage in worker processes</li>
</ul>

<div class="w3-panel w3-warning">
<p><strong>Warning:</strong> Be careful with file-based locking and other shared resources when using multiple workers. Operations that were safe in a single-process application may cause race conditions with multiple workers.</p>
</div>

<hr>

<h2>Alternatives to the Cluster Module</h2>

<p>While the Cluster module is powerful, there are alternatives for running Node.js applications on multiple cores:</p>
...
<table class="ws-table-all">
  ...
  <tr>
    ...
    <td>A process manager for Node.js applications with built-in load balancing and clustering</td>
    <td>Production applications that need robust process management</td>
  </tr>
  <tr>
    <td><strong>Load Balancer</strong></td>
    <td>Running multiple Node.js instances behind a load balancer like Nginx</td>
    <td>Distributing load across multiple servers or containers</td>
  </tr>
  <tr>
    <td><strong>Worker Threads</strong></td>
    <td>Lighter-weight threading for CPU-intensive tasks (Node.js >= 10.5.0)</td>
    <td>CPU-intensive operations within a single process</td>
  </tr>
  <tr>
    <td><strong>Containers</strong></td>
    <td>Running multiple containerized instances (e.g., with Docker and Kubernetes)</td>
    <td>Scalable, distributed applications in modern cloud environments</td>
  </tr>
</table>
</div>
...
<h2>Advanced Load Balancing Strategies</h2>

<p>While the Cluster module's default round-robin load balancing works well for many applications, you might need more sophisticated strategies for specific use cases.</p>

<h3>1. Weighted Round-Robin</h3>
...
&nbsp;&nbsp;console.log(`Master ${process.pid} is running`);<br>
  <br>
&nbsp;&nbsp;// Create workers with different weights<br>
&nbsp;&nbsp;const workerWeights = [3, 2, 1]; // First worker gets 3x more load than the last<br>
&nbsp;&nbsp;const workers = [];<br>
  <br>
&nbsp;&nbsp;// Create workers based on weights<br>
&nbsp;&nbsp;workerWeights.forEach((weight, index) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;for (let i = 0; i &lt; weight; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const worker = cluster.fork({ WORKER_WEIGHT: weight });<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;worker.weight = weight;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;workers.push(worker);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
  <br>
&nbsp;&nbsp;// Track the next worker to use<br>
&nbsp;&nbsp;let workerIndex = 0;<br>
  <br>
&nbsp;&nbsp;// Create a load balancer server<br>
&nbsp;&nbsp;http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Simple round-robin with weights<br>
&nbsp;&nbsp;&nbsp;&nbsp;const worker = workers[workerIndex++ % workers.length];<br>
&nbsp;&nbsp;&nbsp;&nbsp;worker.send('handle-request', req.socket);<br>
&nbsp;&nbsp;}).listen(8000);<br>
  <br>
} else {
&nbsp;&nbsp;// Worker code<br>
&nbsp;&nbsp;process.on('message', (message, socket) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (message === 'handle-request' && socket) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// Handle the request<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbspsocket.end(`Handled by worker ${process.pid}\n`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
}</div>
</div>
...
&nbsp;&nbsp;console.log(`Master ${process.pid} is running`);<br>
  <br>
&nbsp;&nbsp;// Create workers and track their connection counts<br>
&nbsp;&nbsp;const workers = [];<br>
&nbsp;&nbsp;const numCPUs = require('os').cpus().length;<br>
  <br>
&nbsp;&nbsp;for (let i = 0; i &lt; numCPUs; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;const worker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;worker.connectionCount = 0;<br>
&nbsp;&nbsp;&nbsp;&nbsp;workers.push(worker);<br>
    <br>
&nbsp;&nbsp;&nbsp;&nbsp;// Track worker connections<br>
...
&nbsp;&nbsp;http.createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;// Find worker with least connections<br>
&nbsp;&nbsp;&nbsp;&nbsp;let minConnections = Infinity;<br>
&nbsp;&nbsp;&nbsp;&nbsp;let selectedWorker = null;<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;for (const worker of workers) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (worker.connectionCount &lt; minConnections) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;minConnections = worker.connectionCount;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;selectedWorker = worker;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (selectedWorker) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;selectedWorker.send('handle-request', req.socket);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;}).listen(8000);<br>
}</div>
</div>
...
<p>Monitoring your cluster's performance is crucial for maintaining a healthy application. Here's how to implement basic metrics collection:</p>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
const cluster = require('cluster');<br>
const os = require('os');<br>
const promClient = require('prom-client');<br>

if (cluster.isMaster) {<br>
&nbsp;&nbsp;// Create metrics registry<br>
&nbsp;&nbsp;const register = new promClient.Registry();<br>
&nbsp;&nbsp;promClient.collectDefaultMetrics({ register });<br>
  <br>
&nbsp;&nbsp;// Custom metrics<br>
&nbsp;&nbsp;const workerRequests = new promClient.Counter({<br>
&nbsp;&nbsp;&nbsp;&nbsp;name: 'worker_requests_total',<br>
&nbsp;&nbsp;&nbsp;&nbsp;help: 'Total requests handled by worker',<br>
&nbsp;&nbsp;&nbsp;&nbsp;labelNames: ['worker_pid']<br>
&nbsp;&nbsp});<br>
  register.registerMetric(workerRequests);<br>
  <br>
&nbsp;&nbsp;// Fork workers<br>
&nbsp;&nbsp;for (let i = 0; i &lt; os.cpus().length; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;const worker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;worker.on('message', (msg) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (msg.type === 'request_processed') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;workerRequests.inc({ worker_pid: worker.process.pid });<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;}<br>
  <br>
&nbsp;&nbsp;// Expose metrics endpoint<br>
&nbsp;&nbsp;require('http').createServer(async (req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (req.url === '/metrics') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.setHeader('Content-Type', register.contentType);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.end(await register.metrics());<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;}).listen(9090);<br>
} else {<br>
&nbsp;&nbsp;// Worker code<br>
...
<br>
&nbsp;&nbsp;require('http').createServer((req, res) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;requestCount++;<br>
&nbsp;&nbsp;&nbsp;&nbsp;process.send({ type: 'request_processed' });<br>
...
<ul>
  <li><strong>Request Rate:</strong> Requests per second per worker</li>
  <li><strong>Error Rate:</strong> Error responses per second</li>
  <li><strong>Response Time:</strong> P50, P90, P99 response times</li>
  <li><strong>CPU Usage:</strong> Per-worker CPU utilization</li>
  <li><strong>Memory Usage:</strong> Heap and RSS memory per worker</li>
  <li><strong>Event Loop Lag:</strong> Delay in the event loop</li>
</ul>
...
<p>When running in containerized environments like Docker and Kubernetes, consider these best practices:</p>

<h3>1. Process Management</h3>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
// Dockerfile example for a Node.js cluster app<br>
FROM node:16-slim<br>
<br>
WORKDIR /app<br>
...
COPY . .<br>
<br>
# Use the node process as PID 1 for proper signal handling<br>
CMD ["node", "cluster.js"]<br>
<br>
# Health check<br>
HEALTHCHECK --interval=30s --timeout=3s \<br>
...
<div class="w3-example">
<div class="w3-code notranslate yaml">
# k8s-deployment.yaml<br>
apiVersion: apps/v1<br>
kind: Deployment<br>
metadata:<br>
&nbsp;&nbsp;name: node-cluster-app<br>
spec:<br>
&nbsp;&nbsp;replicas: 3  # Number of pods<br>
&nbsp;&nbsp;selector:<br>
&nbsp;&nbsp;&nbsp;&nbsp;matchLabels:<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;app: node-cluster<br>
&nbsp;&nbsp;template:<br>
...
<h3>1. Memory Leaks in Workers</h3>
<p><strong>Problem:</strong> Memory leaks in worker processes can cause gradual memory growth.</p>
<p><strong>Solution:</strong> Implement worker recycling based on memory usage.</p>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
// In worker process<br>
const MAX_MEMORY_MB = 500; // Max memory in MB before recycling<br>
<br>
function checkMemory() {<br>
&nbsp;&nbsp;const memoryUsage = process.memoryUsage();<br>
&nbsp;&nbsp;const memoryMB = memoryUsage.heapUsed / 1024 / 1024;<br>
<br>
&nbsp;&nbsp;if (memoryMB > MAX_MEMORY_MB) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${process.pid} memory ${memoryMB.toFixed(2)}MB exceeds limit, exiting...`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;process.exit(1); // Let cluster restart the worker<br>
&nbsp;&nbsp;}<br>
}<br>
<br>
// Check memory every 30 seconds<br>
setInterval(checkMemory, 30000);
</div>
</div>

<h3>2. Thundering Herd Problem</h3>
<p><strong>Problem:</strong> All workers accepting connections simultaneously after a restart.</p>
<p><strong>Solution:</strong> Implement staggered startup.</p>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
// In master process<br>
if (cluster.isMaster) {<br>
&nbsp;&nbsp;const numWorkers = require('os').cpus().length;<br>
<br>
&nbsp;&nbsp;function forkWorker(delay) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;setTimeout(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const worker = cluster.fork();<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`Worker ${worker.process.pid} started after ${delay}ms delay`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}, delay);<br>
&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;// Stagger worker starts by 1 second<br>
&nbsp;&nbsp;for (let i = 0; i &lt; numWorkers; i++) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;forkWorker(i * 1000);<br>
&nbsp;&nbsp;}<br>
}
</div>
</div>

<h3>3. Worker Starvation</h3>
<p><strong>Problem:</strong> Some workers get more load than others.</p>
<p><strong>Solution:</strong> Implement proper load balancing and monitoring.</p>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
// Track request distribution<br>
const requestDistribution = new Map();<br>
<br>
// In master process<br>
if (cluster.isMaster) {<br>
&nbsp;&nbsp;// ...<br>
<br>
&nbsp;&nbsp;// Monitor request distribution<br>
&nbsp;&nbsp;setInterval(() => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;console.log('Request distribution:');<br>
&nbsp;&nbsp;&nbsp;&nbsp;requestDistribution.forEach((count, pid) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(`  Worker ${pid}: ${count} requests`);<br>
&nbsp;&nbsp;&nbsp;&nbsp;});<br>
&nbsp;&nbsp;}, 60000);<br>
<br>
&nbsp;&nbsp;// Track requests per worker<br>
&nbsp;&nbsp;cluster.on('message', (worker, message) => {<br>
&nbsp;&nbsp;&nbsp;&nbsp;if (message.type === 'request_handled') {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;const count = requestDistribution.get(worker.process.pid) || 0;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;requestDistribution.set(worker.process.pid, count + 1);<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;});<br>
}</div>
</div>
...
<p>The Node.js Cluster module provides an efficient way to scale your application across multiple CPU cores:</p>

<ul>
  <li>Creates a master process that manages multiple worker processes</li>
  <li>Workers share the same server port, allowing load balancing</li>
  <li>Improves application performance and resilience</li>
  <li>Enables zero-downtime restarts and graceful shutdowns</li>
  <li>Uses IPC for communication between master and workers</li>
</ul>

<p>By understanding and properly implementing clustering, you can build high-performance, reliable Node.js applications that efficiently utilize all available CPU resources.</p>

<hr>


<div id="exercisecontainer" src="xrcise_cluster.js" data-extensions="asp"></div>
<hr>
<br>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="nodejs_child_process.asp">&lt; Previous</a>
