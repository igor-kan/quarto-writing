...
<a class="w3-left w3-btn" href="postgresql_groupby.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="postgresql_exists.php">Next &#10095;</a>
</div>
...
<code class="w3-codespan">WHERE</code> clause cannot be used with aggregate functions.</p>

<p>Aggregate functions are often used with <code class="w3-codespan">GROUP BY</code> clauses,
and by adding <code class="w3-codespan">HAVING</code> we can write condition like we do with
<code class="w3-codespan">WHERE</code> clauses.</p>

<div class="w3-example">
  <h3>Example</h3>
  <p>List only countries that are represented more than 5 times:</p>  
  <div class="w3-code notranslate w3-black shellcommands">
        SELECT COUNT(customer_id), country<br>
        FROM customers<br>
        GROUP BY country<br>
        HAVING COUNT(customer_id) > 5;
  </div>
  <a class="w3-btn w3-margin-bottom" href="trypostgresql.php?filename=demo_having" target="_blank">Run Example &raquo;
  </a>  
</div>
...
<p>The following SQL statement lists only orders with a total price of 400$ or more:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate w3-black shellcommands" style="overflow:auto;white-space:nowrap">
SELECT order_id, SUM(order_details.quantity * products.price)<br>
FROM order_details<br>
LEFT JOIN products on products.product_id = order_details.product_id<br>
GROUP BY order_id<br>
HAVING SUM(order_details.quantity * products.price) > 400.00;
  </div>
  <a class="w3-btn w3-margin-bottom" href="trypostgresql.php?filename=demo_having2" target="_blank">Run Example &raquo;
  </a>  
</div>

<p>Lists customers that have ordered for 1000$ or more:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate w3-black shellcommands" style="overflow:auto;white-space:nowrap">
SELECT customers.customer_name,<br>
SUM(order_details.quantity * products.price)<br>
FROM order_details<br>
INNER JOIN products ON order_details.product_id = products.product_id<br>
INNER JOIN orders ON order_details.order_id = orders.order_id<br>
INNER JOIN customers ON orders.customer_id = customers.customer_id<br>
GROUP BY customers.customer_id, customers.customer_name<br>
HAVING SUM(order_details.quantity * products.price) >= 1000;

</div>
  <a class="w3-btn w3-margin-bottom" href="trypostgresql.php?filename=demo_having3" target="_blank">Run Example &raquo;
  </a>  
</div>
...
<div id="exercisecontainer" src="xrcise_having.js" data-extensions="php"></div>
<hr>
