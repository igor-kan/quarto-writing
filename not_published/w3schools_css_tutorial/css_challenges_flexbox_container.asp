...
<html lang="en-US">
<head>
<title>CSS Flex Container Code Challenge</title>
<meta charset="utf-8">
...
uic_prov_pre("challenge","",subjectFolder);
var stickyadstatus = "";
...
<link rel="stylesheet" href="/lib/challenge-engine/main.v1.0.7.css">
</head>
<body>
...
<h1>CSS <span class="color_h1">Flex Container Code Challenge</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="css3_flexbox_container_align.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="css3_flexbox_items.asp">Next &#10095;</a>
</div>
<hr>

<h3>Challenge: Flex Container</h3>
<p>Test your understanding of CSS Flex Container properties by completing a small coding challenge.</p>

<div class="ch-wrap" id="cssChallenges"></div>
...
<script>
  window.CHALLENGE_DATA_URL = "css_challenge_flexbox_container.js";
  window.CHALLENGE_HOST_ID = "cssChallenges";
  window.CHALLENGE_PRISM_LANGUAGE = "markup";
</script>
<script src="/lib/challenge-engine/main.v1.0.7.js"></script>
<!-- CSS Challenge Runner (self-contained) -->
<style>
/* --- Layout: Instructions full-width, Editor+Preview split --- */
.ch-split{ grid-template-columns: 1fr !important; }
.ch-layoutbtn{ display:none !important; }

.ch-editor-preview-grid{
  display:flex;
...
@media (min-width:1201px){
  ...
  .ch-editor-preview-grid .ch-workarea{
    flex:1 1 50%;
    min-width:0;
    overflow:hidden;
  }
  .ch-editor-preview-grid .ch-runner-wrap{
    flex:1 1 50%;
    min-width:0;
    overflow:hidden;
  }
...
.ch-editor-preview-grid .ch-editorpanel,
.ch-editor-preview-grid .ch-runner-preview{
  height:100%;
}
.ch-editor-preview-grid .c-editor-input{ resize:none; }
@media (max-width:1200px){
  .ch-editor-preview-grid .ch-runner-preview{ min-height:500px; }
}

/* Drag splitter between code and result */
.ch-dragbar{
  display:none;
  width:14px;
  cursor:col-resize;
...
.ch-dragbar:hover::after,
.ch-dragbar.active::after{
  color:#04AA6D;
}
@media (min-width:1201px){
  .ch-dragbar{ display:block; }
}
.ch-drag-shield{
  display:none;
  position:fixed;
...
.ch-tab:hover{ color:#E7E9EB; }
.ch-tab.active{
  color:#fff;
  border-bottom-color:#04AA6D;
}

.ch-toolbar-title{ display:none !important; }
.ch-toolbar [data-action='toggleSolution']{ display:none !important; }
.ch-toolbar [data-action='reset']{ display:none !important; }

.ch-toolbar{ flex-wrap:nowrap; background:#282A35 !important; box-shadow:none !important; border-bottom:1px solid rgba(255,255,255,0.08) !important; padding:0 10px 0 0 !important; }
.ch-toolbar-left{ flex:1 1 auto; }
.ch-toolbar-actions{ display:flex; align-items:center; gap:8px; flex-wrap:nowrap; }
.ch-toolbar [data-action='check']{ font-weight:600 !important; }
...
.ch-solution-view.is-visible{ display:block; }
.c-editor-wrap.sol-active > pre.c-editor-highlight,
.c-editor-wrap.sol-active > textarea.c-editor-input{ display:none !important; }
.ch-solution-view pre{
  margin:0;
  padding:12px 14px;
...
.ch-solution-view textarea::selection{
  background:rgba(4,170,109,0.25);
}

/* Runner preview panel */
.ch-runner-wrap{ display:none; }
.ch-runner-preview{
...
.ch-browser-dot.red{ background:#ff5f57; }
.ch-browser-dot.yellow{ background:#febc2e; }
.ch-browser-dot.green{ background:#28c840; }
.ch-browser-url{
  padding:10px 16px;
  font-size:13px;
  font-weight:600;
...
.ch-solutionbox{ display:none !important; }
.c-editor-wrap{ position:relative !important; }
</style>

<div class="ch-dragbar" id="chDragbar"></div>
<div class="ch-drag-shield" id="chDragShield"></div>
<div class="ch-runner-wrap" id="chRunnerWrap">
  <div class="ch-runner-preview">
    <div class="ch-browser-bar">
      <div class="ch-browser-dots">
        <span class="ch-browser-dot red"></span>
        <span class="ch-browser-dot yellow"></span>
        <span class="ch-browser-dot green"></span>
      </div>
      <div class="ch-browser-url">Result</div>
    </div>
    <div class="ch-runner-output" id="chRunnerOutput">
      <div class="ch-runner-placeholder" id="chRunnerPlaceholder">Click <b style="color:#04AA6D;margin:0 4px">Check Code</b> to see the result here.</div>
      <iframe id="chRunnerIframe" name="chRunnerIframe" style="display:none" sandbox="allow-scripts allow-same-origin"></iframe>
    </div>
  </div>
</div>
...
(function(){
  ...
  var hostId = window.CHALLENGE_HOST_ID || "cssChallenges";
  var DATA_URL = window.CHALLENGE_DATA_URL || "";
  var PRISM_LANG = window.CHALLENGE_PRISM_LANGUAGE || "markup";
  var hooked = false;
  var solutionActive = false;

  function getEditorTextarea(){
    var host = document.getElementById(hostId);
    return host ? host.querySelector("textarea.c-editor-input") : null;
  }
  ...
  function restructureLayout(){
    ...
    if (toolbar && toolbarLeft){
      ...
      tabsDiv.innerHTML =
        "<button class='ch-tab active' data-tab='code'>Code</button>" +
        "<button class='ch-tab' data-tab='solution'>Solution</button>";
    ...
    if (editorWrap){
      ...
      solView.innerHTML = "<pre class='c-editor-highlight language-" + PRISM_LANG + "' id='chSolViewCode'></pre>" +
        "<textarea id='chSolViewTa' readonly spellcheck='false'></textarea>";
    ...
    var dragbar = document.getElementById("chDragbar");
    grid.appendChild(dragbar);
    wrap.style.display = "block";
    grid.appendChild(wrap);

    // --- Drag splitter logic ---
    var shield = document.getElementById("chDragShield");
    var dragging = false;
    ...
    function dragMove(e){
      ...
      var x = (e.touches ? e.touches[0].pageX : e.pageX) - rect.left;
      var total = rect.width;
      var barW = dragbar.offsetWidth;
      var minW = total * 0.15;
      var maxW = total * 0.85 - barW;
      var codeW = x - (barW / 2);
      if (codeW < minW) codeW = minW;
      if (codeW > maxW) codeW = maxW;
      var resultW = total - codeW - barW;
      workarea.style.flex = "0 0 " + codeW + "px";
      wrap.style.flex = "0 0 " + resultW + "px";
    }
    function dragEnd(){
      if (!dragging) return;
      dragging = false;
    ...
    var resetBtnEl = card.querySelector("[data-action='reset']");
  }
  ...
  function hookCheckButton(){
    ...
    checkBtn.addEventListener("click", function(){
      setTimeout(function(){ chRunCode(); }, 50);
    });
    hooked = true;
  }
  ...
  window.chRunCode = function(){
    ...
    if (!code.trim() || code === starter){
      ...
      placeholder.innerHTML = "<span style='color:#c00'>No changes have been made. Edit the code and try again.</span>";
    ...
    iframe.onload = function(){};  // srcdoc triggers load
  };
