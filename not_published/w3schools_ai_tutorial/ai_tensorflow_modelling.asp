...
<html lang="en-US">
<head>
<title>TensorFlow Modelling</title>
<meta charset="utf-8">
...
<h1>Example 1 Model</h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="ai_tensorflow_data.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="ai_tensorflow_training.asp">Next &#10095;</a>
</div>
<hr>
<h2>Shuffle Data</h2>
<h4>Always shuffle data before training.</h4>
<p>When a model is trained, the data is divided into small sets (batches).
...
<h2>TensorFlow Tensors</h2>
<p>To use TensorFlow, input data needs to be converted to tensor data:</p>
<div class="w3-example">
<div class="w3-code notranslate jsHigh">
// Map x values to Tensor inputs<br>
const inputs = values.map(obj =&gt; obj.x);<br>
// Map y values to Tensor labels<br>
const labels = values.map(obj =&gt; obj.y);<br>
<br>
// Convert inputs and labels to 2d tensors<br>
const inputTensor = tf.tensor2d(inputs, [inputs.length, 1]);<br>
const labelTensor = tf.tensor2d(labels, [labels.length, 1]);<br>
</div>
</div>
<hr>
<h2>Data Normalization</h2>
<p>Data should be normalized before being used in a neural network.</p>
<p>A range of 0 - 1 using min-max are often best for numerical data:</p>
<div class="w3-example">
<div class="w3-code notranslate jsHigh">
const inputMin = inputTensor.min();<br>
const inputMax = inputTensor.max();<br>
const labelMin = labelTensor.min();<br>
const labelMax = labelTensor.max();<br>
const nmInputs = inputTensor.sub(inputMin).div(inputMax.sub(inputMin));<br>
const nmLabels = labelTensor.sub(labelMin).div(labelMax.sub(labelMin));<br>
</div>
</div>
...
<p>A <b>Machine Learning Model</b> is an algorithm that produces output from input.</p>
<p>This example uses 3 lines to define a <b>ML Model</b>:</p>

<div class="w3-example">
<div class="w3-code notranslate jsHigh">
const model = tf.sequential();<br>
model.add(tf.layers.dense({inputShape: [1], units: 1, useBias: true}));<br>
model.add(tf.layers.dense({units: 1, useBias: true}));<br>
</div>
</div>

<h3>Sequential ML Model</h3>
<p><span class="w3-codespan w3-border">const model = tf.sequential();</span>
creates a <b>Sequential ML Model</b>.</p>
<p>In a sequential model, the input flows directly to the output. 
...
<p><span class="w3-codespan w3-border">model.add()</span> is used to add two layers to the model.</p>
<p><span class="w3-codespan w3-border">tf.layer.dense</span> is a layer type that works in most cases.
It multiplies its inputs by a weight-matrix and adds a number (bias) to the result.</p>

<h3>Shapes and Units</h3>
<p><span class="w3-codespan w3-border">inputShape: [1]</span> because we have 1 input (x = horsepower).
<p><span class="w3-codespan w3-border">units: 1</span> defines the size of the weight matrix:
1 weight for each input (x value).
<hr>
<h2>Compiling a Model</h2>
<p>Compile the model with a specified <b>optimizer</b> and <b>loss</b> function:</p>
<div class="w3-example">
<div class="w3-code notranslate jsHigh">
model.compile({loss: 'meanSquaredError', optimizer:'sgd'});
</div>
</div>
<p>The compiler is set to use the <span class="w3-codespan w3-border">sgd</span> optimizer.
It is simple to use and quite effective.</p>
<p><span class="w3-codespan w3-border">meanSquaredError</span>
