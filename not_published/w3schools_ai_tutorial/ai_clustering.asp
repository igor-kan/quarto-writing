...
<a class="w3-left w3-btn" href="ai_data.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="ai_regressions.asp">Next &#10095;</a>
</div>
<div class="w3-panel ws-info">
<ul>
<li><b>Clusters</b> are collections of similar data</li>
<li><b>Clustering</b> is a type of unsupervised learning</li>
<li>The <b>Correlation Coefficient</b> describes the strength of a relationship.</li>
</ul>
</div>

<h2>Clusters</h2>
<p><b>Clusters</b> are collections of data based on similarity.</p>

<p>Data points clustered together in a graph can often be classified into clusters.</p>
<p>In the graph below we can distinguish 3 different clusters:</p>
<div class="w3-row">
<div class="w3-half">
<canvas id="myCanvas11" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>
<div class="w3-half">
<canvas id="myCanvas12" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>
</div>
<hr>


<h2>Identifying Clusters</h2>
<p>Clusters can hold a lot of valuable information, but clusters come in all sorts of shapes,
so how can we recognize them?</p>
<p>The two main methods are:</p>
<ul>
<li>Using Visualization</li>
<li>Using an Clustering Algorithm</li>
</ul>
<hr>
<h2>Clustering</h2>
<p><b>Clustering</b> is a type of <b>Unsupervised Learning</b>.</p>
<p>Clustering is trying to:</p>
<ul>
<li>Collect similar data in groups</li>
<li>Collect dissimilar data in other groups</li>
</ul>
<hr>
<h2>Clustering Methods</h2>
<ul>
<li>Density Method</li>
<li>Hierarchical Method</li>
<li>Partitioning Method</li>
<li>Grid-based Method</li>
</ul>
<p>The <b>Density Method</b> considers points in a dense regions to have more similarities
and differences than points in a lower dense region.
The density method has a good accuracy. It also has the ability to merge clusters.<br>
Two common algorithms are DBSCAN and OPTICS.</p>

<p>The <b>Hierarchical Method</b> forms the clusters in a tree-type structure.
New clusters are formed using previously formed clusters.<br>
Two common algorithms are CURE and BIRCH.</p>

<p>The <b>Grid-based Method</b> formulates the data into a finite number of cells that form a grid-like structure.<br>
Two common algorithms are CLIQUE and STING</p>

<p>The <b>Partitioning Method</b> partitions the objects into k clusters and each partition forms one cluster.<br>
One common algorithm is CLARANS.</p>
...
<p>The <b>Correlation Coefficient</b> (r) describes the strength and direction of a linear relationship
and x/y variables on a scatterplot.</p>
<p>The value of r is always between -1 and +1:</p>

<table class="ws-table-all">
<tr><td>-1.00</td><td>Perfect downhill</td><td>Negative linear relationship.</td></tr>
<tr><td>-0.70</td><td>Strong downhill</td><td>Negative linear relationship.</td></tr>
<tr><td>-0.50</td><td>Moderate downhill</td><td>Negative linear relationship.</td></tr>
<tr><td>-0.30</td><td>Weak downhill</td><td>Negative linear relationship.</td></tr>
<tr><td>0</td><td></td><td>No linear relationship.</td></tr>
<tr><td>+0.30</td><td>Weak uphill</td><td>Positive linear relationship.</td></tr>
<tr><td>+0.50</td><td>Moderate uphill</td><td>Positive linear relationship.</td></tr>
<tr><td>+0.70</td><td>Strong uphill</td><td>Positive linear relationship.</td></tr>
<tr><td>+1.00</td><td>Perfect uphill</td><td>Positive linear relationship.</td></tr>
</table>

<div class="w3-row">
<div class="w3-half">
<p><b>Perfect Uphill +1.00</b>:</p>
<canvas id="myCanvas2" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>
<div class="w3-half">
<p><b>Perfect Downhill -1.00</b>:</p>
<canvas id="myCanvas3" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>'
</div>

<div class="w3-row">
<div class="w3-half">
<p><b>Strong Uphill +0.61</b>:</p>
<canvas id="myCanvas4" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>
<div class="w3-half">
<p><b>No Relationship</b>:</p>
<canvas id="myCanvas1" width="300px" height="300px" style="width:95%;max-width:300px;border:1px solid black"></canvas>
</div>
</div>
...
<script>
// Create a Plotter
let myPlotter = new XYPlotter("myCanvas1");
myPlotter.transformXY();
// Create some XY Points
numPoints = 500;
let xPoints = [];
let yPoints = [];
...
myPlotter.plotLine(0, 0, myPlotter.xMax-100, myPlotter.yMax, "blue");

myPlotter = new XYPlotter("myCanvas2");
myPlotter.transformXY();
xPoints = [];
yPoints = [];
for (let i = 0; i < numPoints; i+=10) {
  xPoints[i] = Math.random() * myPlotter.xMax;
  yPoints[i] = xPoints[i] * 1.3;
}
myPlotter.plotPoints(numPoints, xPoints, yPoints, "blue");

myPlotter = new XYPlotter("myCanvas3");
xPoints = [];
yPoints = [];
for (let i = 0; i < numPoints; i+=10) {
  xPoints[i] = i;
  yPoints[i] = xPoints[i] * 1.2 + 50;
}
myPlotter.plotPoints(numPoints, xPoints, yPoints, "red");

numPoints = 11;
myPlotter = new XYPlotter("myCanvas4");
myPlotter.transformXY();
xPoints = [50,60,70,80,90,100,110,120,130,140,150];
yPoints = [7,8,8,9,9,9,9,10,11,14,14,15];
for (let i = 0; i < numPoints; i++) {
  xPoints[i] = xPoints[i] * 2.8 - 130;
  yPoints[i] = yPoints[i] * 30 - 150;
}
myPlotter.plotPoints(numPoints, xPoints, yPoints, "blue");
myPlotter.plotLine(xPoints[0], yPoints[0], xPoints[10], yPoints[10], "red");

// Create a Plotter
xPoints = [];
yPoints = [];
let myPlotter1 = new XYPlotter("myCanvas11");
let myPlotter2 = new XYPlotter("myCanvas12");
myPlotter1.transformXY();
myPlotter2.transformXY();
xMax = myPlotter1.xMax;
yMax = myPlotter1.yMax
// Create some XY Points
numPoints = 40;
for (let i = 0; i < numPoints; i++) {
...
myPlotter1.plotPoints(numPoints, xPoints, yPoints, "black", 2);
myPlotter2.plotPoints(numPoints, xPoints, yPoints, "black", 2);

for (let i = 0; i < numPoints; i++) {
  xPoints[i] = Math.random() * xMax/4 + xMax/50;
  yPoints[i] = Math.random() * yMax/4 + yMax/2;;
}
myPlotter1.plotPoints(numPoints, xPoints, yPoints, "black", 2);
myPlotter2.plotPoints(numPoints, xPoints, yPoints, "red");

for (let i = 0; i < numPoints; i++) {
  xPoints[i] = Math.random() * xMax/4 + xMax/2.5;
  yPoints[i] = Math.random() * yMax/3 + yMax/4;
}
myPlotter1.plotPoints(numPoints, xPoints, yPoints, "black", 2);
myPlotter2.plotPoints(numPoints, xPoints, yPoints, "green");

for (let i = 0; i < numPoints; i++) {
  xPoints[i] = Math.random() * xMax/4 + xMax/1.4;
  yPoints[i] = Math.random() * yMax/4 + yMax/2;
}
myPlotter1.plotPoints(numPoints, xPoints, yPoints, "black", 2);
myPlotter2.plotPoints(numPoints, xPoints, yPoints, "blue");
