...
<html lang="en-US">
<head>
<title>Cyber Security In-Depth Network Layer</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Cyber Security <span class="color_h1">In-Depth Network Layer</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="cybersecurity_networking.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="cybersecurity_network_transport.php">Next &#10095;</a>
</div>
<hr>
<h2>IP - The Internet Protocol</h2>
<p>IP is used to communicate across networks, not just across physical links, but between networks of routers. The addressing scheme in use is either IPv4 ("IP Version 4") or IPv6 ("IP Version 6").</p>
<p>IP networks can be broken into different sections, often called subnets. This is accomplished by adding an extra piece of information, together with the IP address, called a <em>netmask</em>. The netmask dictates how large a network is and which packet is routed within the network and which should be routed outside of the network.</p>
<p>Netmasks can be represented via decimal numbers or with a slash notation.&nbsp; When using slash notation, the slash follows the systems IP address. Here are some examples:</p>
<table class="ws-table-all notranslate" style="border-collapse: collapse;">
  <tr>
      <td style="width: 33%;"><strong>IP Address</strong></td>
      <td style="width: 33%;"><strong>Slash Notation</strong></td>
      <td><strong>Netmask</strong></td>
  </tr>
  <tr>
      <td style="width: 33%;">10.0.0.1</td>
      <td style="width: 33%;">/8&nbsp; &nbsp;- Example: 10.0.0.1/8</td>
      <td>255.0.0.0</td>
  </tr>
  <tr>
      <td style="width: 33%;">172.16.1.1</td>
      <td style="width: 33%;">/12 - Example: 172.16.1.1/12</td>
      <td>255.240.0.0</td>
  </tr>
  <tr>
      <td style="width: 33%;">192.168.0.1</td>
      <td style="width: 33%;">/16 - Example: 192.168.0.1/16</td>
      <td>255.255.0.0</td>
  </tr>
  <tr>
      <td style="width: 33%;">192.168.0.1</td>
      <td style="width: 33%;">/24 - Example: 192.168.0.1/24</td>
      <td>255.255.255.0</td>
  </tr>
</table>
<p>Some IP networks are reserved for only a certain kind of traffic. The IP addresses in the table above are reserved for only internal organizational use, meaning they should not be routed on the Internet. These kinds of IP addresses are commonly referred to as RFC1918 addresses.</p>
<hr>
<h2>Different Networks</h2>
<p>Let us take a look at different networks within RFC1918 and how large the networks are:</p>
<ul>
    <li>10.0.0.0/8 - More than 16 million IP addresses</li>
    <li>172.16.0.0/12 - About 1 million IP addresses</li>
    <li>192.168.0.0/16 - 65534 IP addresses</li>
</ul>
<p>IP segments can be further broken up into smaller and more granular networks.</p>
...
<p>For each network segment the broadcast is always last IP address in the network. For example in the network 192.168.0.0/24 network, the broadcast address is 192.168.0.255.</p>
<p>The smallest netmask possible is 255.255.255.255, represented as /32. This network only has one IP address.</p>
<p>If traffic needs to be sent back to the host, e.g. for communications between applications, it is sent to the localhost address. This address is always 127.0.0.1 and is a /8 network.</p>
<p>In IP networks the traffic is routed by a router. A router is a networking device which understands the IP format and can forward packets between networks. This is different than a switch as the switch forwards data within a network, while the router forwards between networks.</p>
<p><img class="invert-on-dark" src="img_router_and_switch.png" alt="Router and Switch" style="width:100%;max-width:1280px"></p>
...
<p><img class="invert-on-dark" src="img_ip-header.png" alt="IP Header" style="width:100%;max-width:1280px"></p>
<p><em>Image Credits: By Michel Bakni - Postel, J. (September 1981) RFC 791, Internet Protocol, DARPA Internet Program Protocol Specification, The Internet Society, p. 11 DOI: 10.17487/RFC0791., CC BY-SA 4.0, <a href="https://commons.wikimedia.org/w/index.php?curid=79949694" target="_blank" rel="noopener">https://commons.wikimedia.org/w/index.php?curid=79949694</a></em></p>
<p>The Source Address is the IP address of the system who is sending the packet, and destination is to whom the packet is intended. There are also other fields in the header which is used by the many features of the IP protocol, but which are outside the scope of this introduction class.</p>
<p>You can check your IP address on Windows by running the command <code>ipconfig</code> within a Command Line Window. On Linux this is done with the <code>ip addr show</code> or <code>ifconfig</code> command.</p>
<p>When a computer needs to communicate to something which can not be found on the LAN, it sends traffic to the default gateway as per how the system is configured. The default gateway being a router which is capable of forwarding the traffic to the destination IP address.</p>
<hr>
...
<p>NAT allows a system accepting connections on a public IP address to map those requests to an internal RFC 1918 IP address or vice versa. Systems which do NAT'ing are typically firewalls and routers.</p>
<p>A typical implementation of NAT is where the external IP address is used as a front for multiple internal IP addresses, and the destination port number is used to decide which server the data should be sent to. This allows internal IP addresses to receive traffic from external systems.</p>
<p><img class="invert-on-dark" src="img_nat-port.svg" alt="NAT Port" style="width:100%;max-width:1280px"></p>
<p>Another very common implementation is allowing internal IP addresses to access the internet with an external IP address. The NAT keeps track of connections from internal to destination addresses and forwards traffic across the connections.</p>
<p>NAT can be configured in many ways, but in this class you we do not go into more details of the method.</p>
<div class="w3-panel ws-note"><strong>Note</strong>: NAT allows network engineers to be more flexible with their deployments, allowing many different use-cases to unfold.</div>

<hr>
<h2>IPv6 - IP Version 6</h2>
<p>IP version 6 is the latest standard for IP and was made to support more IP addresses. Instead of using 32 bits of addressing for IP addresses, 128 bits is used. This allows for enough IP addresses for the foreseeable future while IPv4 has already run out.</p>
<p>IPv6 addresses uses 8 groups of of 4 hexadecimal numbers. An IPv6 address look like this: 2a00:1450:400f:80a::200e:. Notice it does not have have the 8 groups of 4 hexadecimal numbers. This is because IPv6 addresses can be shortened via simple rules:</p>
<ul>
    <li>Leading 0's can be shortened</li>
    <li>Double colon (::) can be used to represent a continuous string of 0's.</li>
</ul>
<p>The expanded IPv6 address is: 2a00:1450:400f:080a:0000:0000:0000:200e.</p>
<p>The localhost can be reduced into ::1 and ::.</p>
<p>IPv6 has networks, i.e. subnets, just like IPv4 has.</p>
<p>The IPv6 header looks like this:</p>
<p><img class="invert-on-dark" src="img_ipv6_header-en.svg" alt="IPv6 Header" style="width:100%;max-width:1280px"></p>
<p>We can see a much simpler header with a lot more room for IP addressing.</p>
<p>IPv6 is used more and more, and there is built in support for this protocol in many tools. For example with <code>ping</code> we can switch between IPv4 and IPv6 with the -4 and -6 flag respectively.</p>
<p>Run <code>ipconfig</code> and see if you see any IPv6 addresses. If you have IPv6 enabled, try <code>ping -6 google.com</code> and <code>ping -4 google.com</code>. See how the command allows us to use either IPv4 or IPv6?</p>
<div class="w3-panel ws-note">Note: If you do not have IPv6 today, there are many public cloud services which will grant you a public IPv6 address today which you can use to experiment and explore with.</div>
<hr>

<h2>ICMP</h2>
<p>ICMP is often associated with Ping and Traceroute. ICMP can be used for other things, such as ask a node for its time referred to as an ICMP Timestamp request. An ICMP Timestamp request simply allows e.g. a Router to ask another Router to synchronize their time, an important attribute in network communications.</p>
<p>A common tactic for attackers to check if systems are available on a network, is to conduct a Ping Sweep. The goal of such activity is to make the target device in a network range reply to ping requests so that the attacker knows it is available. This approach is naive as many systems by default block incoming pings.</p>
<hr>

<h2>Traceroute</h2>
<p>Tracerouting is a way to determine which routers are involved in sending a packet from system A to B. Knowing which routers our packets take can be useful both better understanding our networks and also in understanding the attack surface. A router is responsible for routing the package in the right direction. Imagine this as driving on a road, where road signs at intersections guide you to the destination. These signs at intersections&nbsp; represent routers. Traceroute identify these signs and intersections and tells you how far away they are, measured in milliseconds (ms).</p>
<p>The IPv4 TTL and IPv6 Hop Limit headers have the same function. Every router who routes a packet will decrement this value by 1, and if the value reaches 0 the router will discard the packet and return an ICMP Time Exceeded packet to the sender.</p>
<p>To perform a traceroute on Windows:</p>
<div class="w3-code w3-border notranslate">
tracert google.com</div>
<p>To perform a traceroute on Linux (not installed by default):</p>
<div class="w3-code w3-border notranslate">
traceroute google.com</div>
<p>The process of tracerouting via these tools is simple:</p>
<ol>
    <li>The operating system sends a packet google.com, the TTL value is set to 1.</li>
    <li>The packet is routed on the network, and the first router decrements the TTL by 1, leaving it at 0. This causes the router to drop the packet and send "ICMP Time Exceeded" back to the source.</li>
    <li>The client increases the TTL 1, allowing the packet to be routed through one additional hop.</li>
</ol>
<p>This process is repeated, increasing the TTL with 1 until the destination has been reached.</p>
<hr>

<h2>DNS ("Domain Name System")</h2>
<p>DNS is used to map applications, via names, to IP addresses. For example if you want to use your browser to visit http://google.com, the browser must first make a request to a DNS server to resolve the IP address behind google.com</p>
<p>Systems are typically configured with a primary and secondary domain name server. These settings can be configured manually or be provided by a DHCP server. This allows our computer systems to reach a DNS server for it to resolve for us.</p>
<p>The DNS server is then responsible for resolving the request. It will then proceed to check its own cache to see if it already knows the answer. Each DNS answer can be cached, that is stored temporarily to speed up future requests, for a certain TTL ("Time To Live"). The TTL is typically set to a couple of minutes, for example 10 minutes.</p>
<p>If a DNS server does not have a answer in its cache it will then proceed to check who is responsible for giving the answer. This is done via a recursive process which involves asking a hierarchical system of name servers which inevitably will make the DNS request to end up at the Authoritative Name Server.</p>
<p>You can try to do a DNS lookup with Windows or Linux now. From a command line terminal on Windows type <code>nslookup w3schools.com</code> , or on Linux type <code>dig w3schools.com</code>. You should see output like this:</p>
<p><img src="img_dns-dig.png" alt="DNS Dig" style="width:100%;max-width:1280px"></p>

<p>The IP address of w3schools.com can be seen in the <em>;; ANSWER SECTION.</em> When this screenshot was captured, the IP address behind the w3schools.com name was <em>66.29.212.110</em>.</p>
<p>The Authoritative Name Server is the DNS server which is responsible for giving the definitive answer to a question. For example the IP address of google.com will be answered by their authoritative name server, and we can see this server by querying for it:</p>
<p>Finding authoritative name server on Windows:</p>
<div class="w3-code w3-border notranslate">
nslookup -type=SOA google.com</div>
<p>Finding authoritative name server on Linux:</p>
<div class="w3-code w3-border notranslate">
dig -t SOA google.com</div>
<hr>

<h2>DHCP ("Dynamic Host Configuration Protocol")</h2>
<p>As the name implies, the DHCP protocol allows any system on a network to reach out to a server and receive a configuration. Such configuration typically implies receiving IP address and network range, default gateway and DNS servers.</p>
<p>DHCP allows for easy management of clients joining and leaving a network.</p>
<p>If you are curious if you are using DHCP right now you can type <code>ipconfig /all</code> on a Windows system and look for "DHCP Enabled: Yes" in the output. Your computer might have multiple network interfaces</p>
<hr>

<h2>VPN ("Virtual Private Network")</h2>
<p>A VPN is a system which enables two system to establish encrypted forms for communication, enabling network traffic to be encrypted in transit. Many VPN's is a client to server architecture, allowing the client to access multiple services behind the VPN. VPN's hosted by your work place is also likely to provide access to resources otherwise only accessible from the inside.</p>
<p><img class="invert-on-dark" src="img_vpn2.png" alt="VPN" style="width:100%;max-width:1280px"></p>
<p>Some VPN services are designed for user privacy and encryption for data in transit. These services enables users to send network data via the VPN, effectively masquerading the users IP address when navigating the Internet.</p>
<p>It is generally good practice to use VPN's to secure your network communications, however we should not use any kind of VPN service. Free VPN services can sometimes be malicious, inspecting, reading and storing your sensitive data.</p>
<p><img class="invert-on-dark" src="img_vpn-malicious.png" alt="Malicious VPN" style="width:100%;max-width:1280px"></p>
<hr>
