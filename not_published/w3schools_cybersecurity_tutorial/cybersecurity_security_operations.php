...
<html lang="en-US">
<head>
<title>Cyber Security Security Operations</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Cyber Security <span class="color_h1">Security Operations</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="cybersecurity_penetration_testing.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="cybersecurity_incident_response.php">Next &#10095;</a>
</div>
<hr>

<p class="intro">Security Operations is often contained within a SOC ("Security Operations Center"). Terms are used interchangeably.</p>
<p class="intro">Typically the SOC's responsibility is to detect threats in the environment and stop them from developing into expensive problems.</p>
<hr>

<h2>SIEM ("Security Information Event Management")</h2>
<p>Most systems produces logs often containing important security information.</p>
<p>An event is simply observations we can determine from logs and information from the network, for example:</p>
<ul>
  <li>Users logging in</li>
  <li>Attacks observed in the network</li>
  <li>Transactions within applications</li>
</ul>
<p>An incident is something negative we believe will impact our organization. It might be a definitive threat or the potential of such a threat happening. The SOC should do their best to determine which events can be concluded to actual incidents, which should be responded to.</p>
<p>The SIEM processes alerts based on logs from different sensors and monitors in the network, each which might produce alerts that are important for the SOC to respond to. The SIEM can also try to correlate multiple events to determine an alerts.</p>
<p>SIEM's typically allow events from the following areas to be analyzed:</p>
<ul>
  <li>Network</li>
  <li>Host</li>
  <li>Applications</li>
</ul>
<p>Events from the network is the most typical, but least valuable as they don't hold the entire context of what has happened. The network typically reveals who is communicating where, over which protocols, and when, but not the intricate details about what happened, to whom and why.</p>
<p>Host events give more information in regards to what actually happened and to whom. Challenges such as encryption is no longer blurred and more visibility is gained into what is taking place. Many SIEM's are enriched with great details about what happens on the hosts themselves, instead of only from the network.</p>
<p>Events from application is where the SOC typically can best understand what is going on. These events give information about the Triple A, AAA ("Authentication, Authorization and Account"), including detailed information about how the application is performing and what the users are doing.</p>
<p>For a SIEM to understand events from applications it typically requires work from the SOC Team to make the SIEM understand these events, as support is often not included "out-of-the-box". Many applications are proprietary to an organization and the SIEM does not already have an understanding of the data the applications forward.</p>
<hr>

<h2>SOC Staffing</h2>
<p>How a SOC is staffed greatly varies based on the requirements and structure of an organization. In this section we take a quick look at typical roles involved in operating a SOC. An overview of potential roles:</p>
<p><img class="invert-on-dark" src="img_soc-organization.svg" alt="SOC Organization" style="width:100%;max-width:1280px"></p>
<p>As in most organized teams, a role is appointed to lead the department. The SOC Chief determines the strategy and tactics involved to counter threats against the organization.</p>
...
<p>Analyst Lead is responsible that processes, or playbooks, are developed and maintained to ensure analysts are capable to find the information necessary to conclude alerts and potential incidents.</p>
<p>Level 1 Analysts serve as the first responders to alerts. Their duty is, within their capabilities, to conclude alerts and forward any troubles to a higher level analyst.</p>
<p>Level 2 Analysts are distinguished by having more experience and technical knowledge. They should also ensure any troubles in resolving alerts are forwarded to the Analyst Lead to aid the continuous improvement of the SOC. The Level 2, together with the Analyst Lead, escalates incidents to the Incident Response Team.</p>
<p>The IRT ("Incident Response Team") is a natural extension to the SOC Team. The IRT team is deployed to remediate and solve the issues impacting the organization.</p>
<p>Penetration Testers ideally also support the defense. Penetration Testers have intricate knowledge of how attackers operate and can help in root cause analysis and understanding how break-ins occur. Merging attack and defense teams is often referred to as Purple Teaming and is considered a best-practice operation.</p>
<hr>

<h2>Escalation Chains</h2>
<p>Some alerts require immediate actions. It is important for the SOC to have defined a process of whom to contact when different incidents occur. Incidents can occur across many different business units, the SOC should know who to contact, when and on which communication mediums.</p>
<p>Example of an escalation chain for incidents impacting one part of a organization:</p>
<ol>
  <li>Create an Incident in the appointed Incident Tracking System, assigning it to correct department or person(s)</li>
  <li>If no direct action happens from department/person(s): send SMS and Email to primary contact</li>
  <li>If still no direct action: phone call primary contact</li>
  <li>If still no direct action: call secondary contact</li>
</ol>
<hr>

<h2>Classification of Incidents</h2>
<p>Incidents should be classified according to their:</p>
<ul>
  <li>Category</li>
  <li>Criticality</li>
  <li>Sensitivity</li>
</ul>
<p>Depending on the incidents classification and how it is attributed, the SOC might take different measures to solve the issue at hand.</p>
<p>The category of incident will determine how to respond. There exists many kinds of incident and it is important for the SOC to understand what each incident type means for the organization. Example incidents are listed below:</p>
<ul>
  <li>Inside Hacking</li>
  <li>Malware on Client workstation</li>
  <li>Worm spreading across the network</li>
  <li>Distributed Denial of Service Attack</li>
  <li>Leaked Credentials</li>
</ul>
<p>The criticality of an incident is determined based on how many systems is impacted, the potential impact of not stopping the incident, the systems involved and many other things. It is important for the SOC to be able to accurately determine the criticality so the incident can be closed accordingly. Criticality is what determines how fast an incident should be responded to.<br>
Should the incident be responded to immediately or can the team wait until tomorrow?</p>
<p>Sensitivity determines who should be notified about the incident. Some incidents require extreme discretion.</p>
...
<h2>SOAR ("Security Orchestration, Automation and Response")</h2>
<p>To counter the advancements of threat actors, automation is key for a modern SOC to respond fast enough. To facilitate fast response to incidents, the SOC should have tools available to automatically orchestrate solutions to respond to threats in the environment.</p>
<p>The SOAR strategy means ensuring the SOC can use actionable data to help mitigate and stop threats which are developing more real-time than before. In traditional environments it takes attackers very short time from the time of compromise until they have spread to neighboring systems. Contrary to this it takes organizations typically a very long time to detect threats that have entered their environment. SOAR tries to help solve this.</p>
<p>SOAR includes concepts such as IAC "Infrastructure as Code" to help rebuild and remediate threats. SDN ("Software Defined Networking") to control accesses more fluently and easily, and much more.</p>
<hr>

<h2>What to monitor?</h2>
<p>Events can be collected across many different devices, but how do we determine what to collect and monitor? We want the logs to have the highest quality. High fidelity logs that are relevant and identifying to quickly stop the threat actors in our networks. We also want to make it hard for attackers to circumvent the alerts we configure.</p>
<p>If we look at different ways to catch attackers, it becomes evident where we should focus. Here is a list of possible indicators we can use to detect attackers, and how hard it is considered for attackers to change.</p>
<table class="ws-table-all notranslate">
  <tr>
    <th>Indicator</th>
    <th>Difficulty to change</th>
  </tr>
  <tr>
    <td>File checksums and hashes</td>
    <td>Very Easy</td>
  </tr>
  <tr>
    <td>IP Addresses</td>
  ...
  <tr>
    <td>Network and Host Artifacts</td>
    <td>Annoying</td>
  </tr>
  <tr>
    <td>Tools</td>
...
<p>File checksums and hashes can be used to identify known pieces of malware or tools used by attackers. Changing these signatures are considered to be trivial for attackers as their code can be encoded and changed in multiple different ways, making the checksums and hashes change.</p>
<p>IP Addresses are also easy to change. Attackers can use IP addresses from other compromised hosts or simply use IP addresses within the jungle of different cloud and VPS ("Virtual Private Server") providers.</p>
<p>Domain Names can also be reconfigured quite easily by attackers. An attacker can configure a compromised system to use a DGA ("Domain Generation Algorithm") to continuously use a new DNS name as time passes. One week the compromised system uses one name, but the next week the name has changed automatically.</p>
<p>Network and Host Artifacts are more annoying to change, as this involves more changes for the attackers. Their utilities will have signatures, like a user-agent or the lack of thereof, that can be picked up by the SOC.</p>
<p>Tools become increasingly harder to change for attackers. Not the hashes of the tools, but how the tools behave and operate when attacking. Tools will be leaving traces in logs, loading libraries and other things which we can monitor to detect these anomalies.</p>
<p>If the defenders are capable of identifying Tactics, Techniques and Procedures threat actors use, it becomes even harder for attackers to get to their objectives. For example, if we know the threat actor likes to use Spear-Phishing and then Pivoting peer-to-peer via to other victim systems, defenders can use this to their advantage. Defenders can focus training to staff at risk for spear-phishing and start implementing barriers to deny peer-to-peer networking.</p>
<hr>
