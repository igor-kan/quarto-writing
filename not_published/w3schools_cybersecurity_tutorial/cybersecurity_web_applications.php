...
<html lang="en-US">
<head>
<title>Cyber Security Web Applications</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Cyber Security <span class="color_h1">Web Applications</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="cybersecurity_firewalls.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="cybersecurity_mapping_port_scanning.php">Next &#10095;</a>
</div>
<hr>
<p class="intro">Web Applications are integral to almost everything we do, whether it is to access the Internet or to remotely control your lawnmower. In this introduction class we will cover the basics of web application security.</p>

<hr>
<h2>The HTTP protocol</h2>
<p>HTTP is the carrier protocol which allows our browsers and applications to receive content such as HTML ("Hyper Text Markup Language"), CSS ("Cascading Style Sheets"), images and videos.</p>

<hr>
<h2>URLs, Query Parameters and Scheme</h2>
<p>To access a web application we use a URL ("Uniform Resource Locator"), for example: https://www.google.com/search?q=w3schools+cyber+security&amp;ie=UTF-8</p>
<p>The URL to google.com contains a domain, a script being accessed and Query Parameters.</p>
<p>The script we are accessing is called /search. The / indicates it is contained in the top directory on the server where files are being served. The ? indicates the input parameters to the script and the &amp; delimits different input parameters. In our URL the input parameters are:</p>
<ul>
  <li>q with a value of w3schools cyber security</li>
  <li>ie with a value of UTF-8</li>
</ul>
<p>The meaning of these inputs is up to the webservers application to determine.</p>
<p>Sometimes you will see just / or /? indicating that a script has been setup to serve to respond to this address. Typically this script is something like an index file which catches all requests unless a specific script is specified.</p>
<p>The Scheme is what defined the protocol to use. In our case it is the first part of the URL: https. When the scheme is not defined in the URL it allows the application to decide what to use. Schemes can include an entire array of protocols such as:</p>
<ul>
  <li>HTTP</li>
  <li>HTTPS</li>
  <li>FTP</li>
...
<hr>
<h2>HTTP Headers</h2>
<p>The HTTP protocol uses many headers, some custom to the application and others well defined and accepted by the technology.</p>
<p>Example request to http://google.com</p>
...
<p>The request header specifies what the client wants to perform on the target webserver. It also has information regarding if it accepts compression, what kind of client is accessing and any cookies the server has told the client to present. The HTTP request headers are explained here:</p>
<table class="ws-table-all notranslate">
  ...
  <tr>
    ...
    <td>GET is the verb we are using to access the application. Explained in detail in the section HTTP Verbs. We also see the path and query parameters and HTTP version</td>
  </tr>
  <tr>
    <td>Host: google.com</td>
    <td>This header indicates the target service we want to use. A server can have multiple services as explained in the section on VHOSTS.</td>
  </tr>
  <tr>
    <td>User-Agent</td>
    <td>A client application, that is the browser in most cases, can identify itself with the version, engine and operating system</td>
  </tr>
  <tr>
    <td>Accept</td>
    <td>Defines which content the client can accept</td>
  </tr>
  <tr>
    <td>Referer: https://w3schools.com/</td>
    <td>If the client clicked a link from a different website the Referer header is used to say from where the client came from</td>
  </tr>
  <tr>
    <td>Accept-Encoding: gzip, deflate</td>
    <td>Can the content be compressed or encoded? This defines what we can accept</td>
  </tr>
  <tr>
    <td>Cookie</td>
    <td>Cookies are values sent by the server in previous requests which the client sends back in every subsequent request. Explained in detail in the section State</td>
  </tr>
</table>
<p>With this request, the server will reply with headers and content. Example headers are seen below:</p>
<div class="w3-example">
<div class="w3-code notranslate">
HTTP/1.1 200 OK<br>
...
<p>The response header and content is what determines what we will see in our browser. The HTTP response headers are explained as following:</p>
<table class="ws-table-all notranslate">
  <tr>
    <th>Header</th>
    <th>Explanation</th>
  ...
  <tr>
    ...
    <td>The HTTP Response code. Explained in detail in the HTTP Response Codes section</td>
  </tr>
  <tr>
    <td>Content-Type: text/html</td>
    <td>Specifies the type of content being returned, e.g. HTML, JSON or XML</td>
  </tr>
  <tr>
    <td>Set-Cookie:</td>
    <td>Any special values the client should remember and return in the next request</td>
  </tr>
</table>  
...
<h2>HTTP Verbs</h2><p>When accessing a web application the client is instructed on how to send data to the web application. There are many verbs which can be accepted by the application.</p>
...
<table class="ws-table-all notranslate">
  ...
  <tr>
    <td>GET</td>
    <td>Typically used to retrieve values via Query Parameters</td>
  </tr>
  <tr>
    <td>POST</td>
    <td>Used to send data to a script via values in the body of the Request sent to the webserver. Typically it involves creating, uploading or sending large quantities of data</td>
  </tr>
  <tr>
    <td>PUT</td>
    <td>Often use to upload or write data to the webserver</td>
  </tr>
  <tr>
    <td>DELETE</td>
    <td>Indicate a resource which should be deleted</td>
  </tr>
  <tr>
    <td>PATCH</td>
    <td>Can be used to update a resource with a new value</td>
  </tr>
</table>  
<p>These are used as the web application requires. Restful (REST) web services are especially good at using the full array of HTTP Verbs to define what should be done on the backend.</p>

<hr>
<h2>HTTP Response Codes</h2>
<p>The application running on the webserver can respond with different codes based on what occurred on the server side. Listed are common response codes the webserver will issue to the client which security professionals should know about:</p>
<table class="ws-table-all notranslate">
  <tr>
    <th>Code</th>
    <th>Explanation</th>
  ...
  <tr>
    <td>200</td>
    <td>Application returned normally</td>
  </tr>
  <tr>
    <td>301</td>
    <td>Server asks client to permanently remember a redirect to a new location where the client should access</td>
  </tr>
  <tr>
    <td>302</td>
    <td>Redirect temporarily. Client doesn't need to save this reply</td>
  </tr>
  <tr>
    <td>400</td>
    <td>The client made an invalid request</td>
  </tr>
  <tr>
    <td>403</td>
    <td>The client is not allowed to access this resource. Authorization is required</td>
  </tr>
  <tr>
    <td>404</td>
    <td>The client tried to access a resource which does not exist</td>
  </tr>
  <tr>
    <td>500</td>
    <td>The server errored in trying to fulfill the request</td>
  </tr>
</table>

<hr>
<h2>REST</h2>
<p>Rest services, sometimes called RESTful services, employ the full force of HTTP Verbs and HTTP Response Codes to facilitate the use of the web application. RESTful services often uses parts of the URL as a query parameter to determine what happens on the web application. REST is typically used by API's ("Application Programming Interfaces").</p>
<p>REST URLs will invoke functionality based on the different elements of the URL.</p>
<p>An example REST URL: http://example.com/users/search/w3schools</p>
<p>This URL will invoke functionality as part of the URL instead of Query Parameters. We can decipher the URL as:</p>
<table class="ws-table-all notranslate">
  <tr>
    <th>Parameter</th>
    <th>Comment</th>
  ...
  <tr>
    <td>users</td>
    <td>Accessing the users part of the functionality</td>
  </tr>
  <tr>
    <td>search</td>
    <td>Accessing the search feature</td>
  </tr>
  <tr>
    <td>w3schools</td>
    <td>The user to search for</td>
  </tr>
</table>
<hr>

<h2>Sessions &amp; State</h2>
<p>There is no built in way for a server to identify a returning visitor in HTTP. For a webserver to identify the user, a secret value must be communicated to and from the Client in each request. This is typically done via Cookies in headers, however other ways are also common such as via GET and POST parameters or other headers. Passing state via GET parameters is not recommended as such parameters are often logged on the server or in intermediaries such as a proxy.</p>
<p><img class="invert-on-dark" src="img_http-sessions.svg" alt="HTTP Sessions" style="width:100%;max-width:1280px"></p>
<p>Here are some common Cookie examples which allows the application on the webserver to control sessions and state:</p>
<ul>
  <li>PHPSESSID</li>
  <li>JSESSIONID</li>
  <li>ASP.NET_SessionID</li>
</ul>
<p>These values represent a certain state, often called a session, on the server. This state represents things like:</p>
<ul>
  <li>What user you have logged in as</li>
  <li>Privileges and authorizations</li>
</ul>
<p>It is important that session value, sent to the Client, can not be easily guessed or otherwise identified by others. If they could, an attacker could then present themselves as other users on the web application.</p>
<p>State can also be saved on the client. This involves the server sending all the states to the client and relies on the client sending back all the items. Such implementations relies on encryption to check the integrity of the state the client is claiming. Examples of implementations using this is listed below:</p>
<ul>
  <li>JWT ("JSON Web Tokens")</li>
  <li>ASP.Net ViewState</li>
</ul>
<p>You are using cookies to take this class! You can inspect these cookies in your web browser by opening up the developer tools. This is done by hitting <code>F12</code> within the browser, opening up the developer tools window. Within this window you should be able to find the correct place where your cookies are stored.&nbsp;</p>
<p><img src="img_web-developerconsole-cookies.png" alt="Developer Console" style="width:100%;max-width:1280px"></p>
<p>In Google Chrome, the cookies were identified in the Application tab above.&nbsp;</p>
<div class="w3-panel ws-note"><strong>Note</strong>: Can you think of why the cookies have been masked away in the screenshot so you cannot read them?</div>
<hr>

<h2>Virtual Hosts</h2>
<p>One webserver can process many applications via Virtual Hosts, often abbreviated as Vhosts. To facilitate access to other Virtual Hosts the web server typically reads off the Host header of the client request, and based on this value sends the request to the correct application.</p>
<p><img class="invert-on-dark" src="img_vhosts.svg" alt="Virtual Hosts" style="width:100%;max-width:1280px"></p>
<hr>

<h2>URL Encoding</h2>
<p>For an application to safely transfer content between the server and client, some characters must be encoded to ensure they do not impact the protocol. To preserve the integrity of the communications, URL encoding is used.</p>
<p>URL Encoding replaces unsafe characters with a % and two hexadecimal digits. For example:</p>
<ul>
  <li>Percentage is replaced with %25</li>
  <li>Space is replaced with %20</li>
  <li>Quote is replaced with %22</li>
</ul>
<p>An excellent tool to perform text analysis and run operations such as URL Decoding is CyberChef. You can try it out in your browser here: <a href="https://gchq.github.io/CyberChef/" target="_blank" rel="noopener">https://gchq.github.io/CyberChef/</a></p>

<div class="w3-panel ws-note"><strong>Note</strong>: Play around with Cyber Chef and see if you can reveal what the following message in URL encoded characters hold: %48 %65 %6c %6c %6f %20 %64 %65 %61 %72 %20 %77 %33 %73 %63 %68 %6f %6f %6c %73 %20 %73 %74 %75 %64 %65 %6e %74 %2e %20 %48 %6f %70 %65 %20 %79 %6f %75 %20 %61 %72 %65 %20 %6c %65 %61 %72 %6e %69 %6e %67 %20 %73 %6f %6d %65 %74 %68 %69 %6e %67 %20 %74 %6f %64 %61 %79 %21</div>
<hr>

<h2>JavaScript</h2>
<p>To support dynamic content, browsers use the scripting language JavaScript. This enables developers to program solutions which will run on the client, enabling more interactive and "alive" web-content.</p>
<p>JavaScript is also involved in many attacks against web-applications and client applications such as browsers.</p>
<hr>

<h2>Encryption with TLS</h2>
<p>The HTTP protocol does not support encryption for data-in-transit, hence a wrapper around HTTP is added for encryption support. This is indicated with a S following HTTP, i.e. HTTPS.</p>
<p>The encryption used to be SSL ("Secure Sockets Layer"), but has since been deprecated. Instead TLS ("Transport Layer Security") is typically used to enforce encryption.</p>
<hr>
