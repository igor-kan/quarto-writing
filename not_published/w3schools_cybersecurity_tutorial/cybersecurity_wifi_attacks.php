...
<html lang="en-US">
<head>
<title>Cyber Security WiFi Attacks</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>Cyber Security <span class="color_h1">Wi-Fi Attacks</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="cybersecurity_web_applications_attacks.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="cybersecurity_passwords.php">Next &#10095;</a>
</div>
<hr>

<p class="intro">A potent and important area to computer security is WIFI. Devices and systems are no longer required to be interconnected via physical cables, but can instead be reached by anyone within signal radius. WIFI enables many new devices to be capable of networking.</p>
<hr>

<h2>WIFI Basics</h2>
<p>WIFI as most people know it stems from the IEEE 802.11 protocol. There are other protocols which uses radio for signaling too, for example:</p>
<ul>
  <li>Bluetooth, for communicating with devices we carry, typically smartphones, headphones etc.</li>
  <li>NFC ("Near Field Communications"), implemented in access badges and credit cards for wireless transmission of data.</li>
  <li>RFID ("Radio Frequency Identification"), used for access cards and other devices, for example a car which can wirelessly transmit its identifier to a toll-road system.</li>
  <li>ZigBee and Z-Wave, used for enterprise and home automation.</li>
</ul>
<p>Wireless communication is typically done via an AP ("Access Point"), a wireless base station which acts as a switch and router between clients that wish to communicate. Peer-to-peer communications are also possible, but less typical.</p>
<p>The name of a wireless network is known as the SSID ("Service Set Identifier").</p>
<p>Because WIFI signals reach everyone in the vicinity it enables attackers to easily use an antenna to "sniff" communications for anyone transmitting. Sniffing simply means to listen for packets which the network interface can see.</p>
<p>WIFI sometimes allow users to reach internal applications, increasing attack potential. Furthermore, WIFI devices have management interfaces and firmware which can hold vulnerabilities, sometimes not always patched as timely as other assets in the enterprise.</p>
<hr>

<h2>WIFI Security</h2>
<p>WIFI have the option of</p>
<ul>
  <li>No security</li>
  <li>Access list based on MAC addresses</li>
  <li>PSK ("Pre-Shared Key")</li>
  <li>Enterprise authentication</li>
</ul>
<p>Many WIFI attacks rely on network cards with two primary features, namely:</p>
<ul>
  <li>Monitor Mode: Makes the network card forward packets destined to all MAC addresses to the Operating System, not just its own.</li>
  <li>Packet Injection: The network card supports crafting packets with a different source MAC address than its own.</li>
</ul>
<hr>

<h2>Open WIFI Networks</h2>
<p>An open WIFI network is a network with no password on it. Communication between AP and Clients is not encrypted and everyone has to rely on their own sources of encryption to protect their traffic. These kinds of networks are very convenient and accessible for users, but makes security compromises.</p>
<p>An attacker on these kinds of networks can easily see what everyone else is doing by simply sniffing packets. Such packets can contain sensitive details or simply details about what the users are doing on the network.</p>
<p><img class="invert-on-dark" src="img_wifi-open.svg" alt="WIFI Open" style="width:100%;max-width:1280px"></p>
<hr>

<h2>Hidden SSID</h2>
<p>AP's can often turn off broadcasting the name of the wireless network. This requires clients to demonstrate knowledge of the SSID to join the network. It is not considered best-practice to enable hidden SSID, because the name of the network is exposed anytime a client joins. Furthermore, the clients now need to ask and broadcast information about the network they want to join, everywhere they travel. An attacker could then sniff the WIFI traffic of clients and potentially learn more information about whom the clients are and where they have joined networks before.</p>
<p><img class="invert-on-dark" src="img_wifi-hidden-ssid.svg" alt="WIFI Hidden SSID" style="width:100%;max-width:1280px"></p>
<hr>

<h2>MAC Address Filtering</h2>
<p>Some AP's support access control based on MAC Addresses. The AP can create an allow-list of which MAC addresses should be allowed to join and communicate on the network.</p>
<p>This approach is in-secure. An attacker can sniff and observe other systems communicating on the network already. Then record their MAC addresses and update the attackers own MAC address to be one which is already allowed. This effectively bypasses the MAC Address Filtering requirement.</p>
<hr>

<h2>PSK ("Pre-Shared Key")</h2>
<p>PSK simply means the network is configured with a password. PSK protection is typically implemented via a protocol called WPA ("WIFI Protected Access"). Older protocols for authentication can also be used, for example WEP ("Wired Equivalent Privacy") but has for a long time been considered obsolete as it is highly in-secure and easy for attackers to crack.</p>
<p>WPA comes in different forms with WPA3 being the latest standard as of the year 2021. WPA is not entirely safe against attackers either, but offers much more protection than WEP. To break into a WPA enabled network the attacker must try to crack the password with a password cracker. This is considered an expensive process in terms of time if the password is reasonably strong.</p>
<p>If an attacker can observe (sniff) anyone whom authenticates to the network, they have enough to engage in password cracking activities. Tools like aircrack-ng ("https://www.aircrack-ng.org/") supports cracking WIFI passwords.</p>
<p><img class="invert-on-dark" src="img_wifi-psk.svg" alt="WIFI PSK" style="width:100%;max-width:1280px"></p>
...
<p>Enterprise Access Points can also support authenticating clients based on certificates, which requires PKI ("Public Key Infrastructure") or enterprise credentials by integrating to a centralized authentication service.</p>
<p>This has some benefits, especially the concept of key management. With a PSK network, the inherent challenge is how passwords are distributed, rotated and revoked.</p>
<p>While Enterprise Authentication provides better security management regarding keys, it also involves a more complex infrastructure and offers other opportunities for attackers.</p>
<hr>

<h2>Fake WIFI Access Points</h2>
<p>Attackers can easily start broadcasting networks pretending to be other networks. Often clients will automatically connect to networks in range if they present themselves with the appropriate SSID. This allows attackers to make clients connect to the attackers network, allowing them to sniff and change traffic as the attacker wishes.</p>
<p><img class="invert-on-dark" src="img_wifi-fake-ap.svg" alt="WIFI Fake AP" style="width:100%;max-width:1280px"></p>
<hr>
