<!DOCTYPE html>
<html lang="en-US">
<title>C Real-Life Memory Management Example</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>C <span class="color_h1">Memory Management Example</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_memory_struct.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_errors.php">Next &#10095;</a>
</div>
<hr>

<!--start-->
<h2>Real-Life Memory Management Example</h2>
<p>To demonstrate a practical example of <strong>dynamic memory</strong>, we created a program that can make a list of any length.</p>
<p>Regular arrays in C have a fixed length and cannot be changed, but with 
...
<div class="w3-code notranslate javaHigh">
  ...
  &nbsp;
  struct list myList;<br>&nbsp; int amount;<br>&nbsp; int i, j;<br><br>&nbsp; // Create a list and start with 
  ...
  &nbsp;
  for (i = 0; i &lt; amount; i++) {<br>&nbsp;&nbsp;&nbsp; addToList(&amp;myList, i + 1);<br>&nbsp; }<br><br>&nbsp; // 
  Display the contents of the list<br>&nbsp; for (j = 0; j &lt; myList.numItems; j++) {<br>
  &nbsp;&nbsp;&nbsp;
  ...
  *myList, int item) {<br><br>&nbsp; // If the list is full then resize the 
  memory to fit 10 more items<br>&nbsp; if (myList-&gt;numItems == myList-&gt;size) {<br>&nbsp;&nbsp;&nbsp; 
  int newSize = myList-&gt;size + 10;<br><br>&nbsp;&nbsp;&nbsp; // Use a temporary 
  pointer so we don't lose the original on failure<br>&nbsp;&nbsp;&nbsp; int 
  *tmp = realloc(myList-&gt;data, newSize * sizeof(int));<br>&nbsp;&nbsp;&nbsp; if 
  (tmp == NULL) {<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; printf(&quot;Memory resize 
  failed\n&quot;);<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; return; // Leave the list 
  unchanged<br>&nbsp;&nbsp;&nbsp; }<br><br>&nbsp;&nbsp;&nbsp; // Only update 
  fields after a successful reallocation<br>&nbsp;&nbsp;&nbsp; myList-&gt;data = 
  tmp;<br>&nbsp;&nbsp;&nbsp; myList-&gt;size = newSize;<br>&nbsp; }<br><br>&nbsp; 
  // Add the item to the end of the list<br>&nbsp; 
...
<p><strong><a href="c_structs_pointers.php">Pointers to structures</a>:</strong> This example has a pointer to the structure <code class="w3-codespan">myList</code>. Because we are using a <strong>pointer</strong> to the structure instead of the structure itself, we use the arrow syntax (<code class="w3-codespan">-></code>) to access the structure's members.</p>
</div>


<h3>Example explained</h3>
<p>This example has three parts:</p>
<ul>
  <li>A structure <code class="w3-codespan">myList</code> that contains a list's data</li>
  <li>The <code class="w3-codespan">main()</code> function with the program in it.</li>
  <li>A function <code class="w3-codespan">addToList()</code> which adds an item to the list</li>
</ul>

<p><strong>The <code class="w3-codespan">myList</code> structure</strong></p>
<p>The <code class="w3-codespan">myList</code> structure contains all of the information about the list, including its contents. It has three members:</p>
<ul>
  <li><code class="w3-codespan">data</code> - A pointer to the dynamic memory which contains the contents of the list</li>
  <li><code class="w3-codespan">numItems</code> - Indicates the number of items that list has</li>
  <li><code class="w3-codespan">size</code> - Indicates how many items can fit in the allocated memory</li>
</ul>
<p>We use a structure so that we can easily pass all of this information into a function.</p>

<p><strong>The <code class="w3-codespan">main()</code> function</strong></p>

<p>The <code class="w3-codespan">main()</code> function starts by initializing the list with space for 10 items:</p>

<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  // Create a list and start with enough space for 10 items<br>myList.numItems = 
  0;<br>myList.size = 10;<br>myList.data = malloc(myList.size * sizeof(int));</div>
</div>

<p><code class="w3-codespan">myList.numItems</code> is set to 0 because the list starts off empty.</p>
<p><code class="w3-codespan">myList.size</code> keeps track of how much memory is reserved. We set it to 10 because we will reserve enough memory for 10 items.</p>
<p>We then allocate the memory and store a pointer to it in <code class="w3-codespan">myList.data</code>.</p>

<p>Then we include error checking to find out if memory allocation was successful:</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  // Find out if memory allocation was successful<br>if (myList.data == NULL) {<br>&nbsp; printf(&quot;Memory allocation failed&quot;);<br>&nbsp; 
  return 1; // Exit the program with an error code<br>}</div>
</div>


<p>If everything is fine, a loop adds 44 items to the list using the <code class="w3-codespan">addToList()</code> function:</p>

<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  // Add any number of items to the list specified by the amount variable<br>
  amount = 44;<br>for (i = 0; i &lt; amount; i++) {<br>&nbsp; 
  addToList(&amp;myList, i + 1);<br>}</div>
</div>

<p>In the code above, <code class="w3-codespan">&myList</code> is a pointer to the list and <code class="w3-codespan">i + 1</code> is a number that we want to add to the list. We chose <code class="w3-codespan">i + 1</code> so that the list would start at 1 instead of 0. You can choose any number to add to the list.</p>

<p>After all of the items have been added to the list, the next loop prints the contents of the list.</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  // Display the contents of the list<br>for (j = 0; j &lt; myList.numItems; 
  j++) {<br>&nbsp; printf(&quot;%d &quot;, myList.data[j]);<br>}</div>
</div>

<p>When we finish printing the list we free the memory to prevent memory leaks.</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  // Free the memory when it is no longer needed<br>free(myList.data);<br>
  myList.data = NULL;</div>
</div>

<p><strong>The <code class='w3-codespan'>addToList()</code> function</strong></p>
<p>Our <code class='w3-codespan'>addToList()</code> function adds an item to the list. It takes two parameters:</p>

<div class="w3-example">
  <pre class="language-c w3-white"><code>void addToList(struct list *myList, int item)</code></pre>
</div>

<ol>
  <li>A pointer to the list.</li>
  <li>The value to be added to the list.</li>
</ol>

<p>
  The function first checks if the list is full by comparing the number of items in the list to the size (capacity).
  If the list is full, it tries to grow the memory to fit 10 more items.
  We use a <em>temporary pointer</em> with <code class='w3-codespan'>realloc</code> so we don't lose the original block if the resize fails.
  We only update <code class='w3-codespan'>data</code> and <code class='w3-codespan'>size</code> after a successful resize:
</p>

<div class="w3-example">
  <pre class="language-c w3-white"><code>// If the list is full then resize the memory to fit 10 more items
...
<p>
  Finally, the function adds the item to the end of the list.
  The index at <code class='w3-codespan'>myList-&gt;numItems</code> is always at the end of the list because it increases by 1 each time a new item is added.:
</p>

<div class="w3-example">
  <pre class="language-c w3-white"><code>// Add the item to the end of the list
myList-&gt;data[myList-&gt;numItems] = item;
myList-&gt;numItems++;</code></pre>
</div>

<p><strong>Why do we reserve 10 items at a time?</strong></p>
<p>Optimizing is a balancing act between memory and performance. Even though we may be allocating some memory that we are not using, reallocating memory too frequently can be inefficient. There is a balance between allocating too much memory and allocating memory too frequently.</p>
<p>We chose the number 10 for this example, but it depends on how much data you expect and how often it changes. For example, if we know beforehand that we are going to have exactly 44 items then we can allocate memory for exactly 44 items and only allocate it once.</p>
<!--stop-->
...
<div>
<h2>Complete stdlib Reference</h2>
<p>For a complete reference of memory management functions and other functions found in the standard library, go to our <a href="c_ref_stdlib.php">C 
