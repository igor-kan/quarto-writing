...
ny:-->
    ...
    <h1>C <span class="color_h1">Pointer Arithmetic</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_pointers_arrays.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_pointer_to_pointer.php">Next &#10095;</a>
</div>
<hr>

<h2>Pointer Arithmetic</h2>
<p><strong>Pointer arithmetic</strong> means changing the value of a pointer to make it point to a different element in memory.</p>
<p>Like we saw on the <a href="c_pointers_arrays.php">previous page</a>, array elements are stored next to each other in memory.  
So if a pointer points to one element, adding <code class="w3-codespan">1</code> 
moves it to the next one:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate javaHigh">
    int myNumbers[4] = {25, 50, 75, 100};<br>
    int *p = myNumbers; &nbsp;// points to myNumbers[0]<br><br>
    printf("%d\n", *p); &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;// 25<br>
    printf("%d\n", *(p + 1)); // 50<br>
    printf("%d\n", *(p + 2)); // 75<br>
    printf("%d\n", *(p + 3)); // 100</div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_pointer_arith_basic">Try it Yourself &raquo;</a></p>
</div>
...
<p>You can move a pointer with <code class="w3-codespan">++</code> and <code class="w3-codespan">--</code> (and with <code class="w3-codespan">+=</code> / <code class="w3-codespan">-=</code>):</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate javaHigh">
    int myNumbers[3] = {10, 20, 30};<br>
    int *p = myNumbers;&nbsp; // myNumbers[0]<br><br>
    printf("%d\n", *p); // 10<br>
    p++; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// move to myNumbers[1]<br>
    printf("%d\n", *p); // 20<br>
    p--; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// back to myNumbers[0]<br>
    printf("%d\n", *p); // 10<br><br>
    p += 2; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;// jump to myNumbers[2]<br>
    printf("%d\n", *p); // 30</div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_pointer_arith_inc">Try it Yourself &raquo;</a></p>
</div>

<p>This shows that you can move a pointer just like a counter:  
each time you use <code class="w3-codespan">p++</code> or <code class="w3-codespan">p--</code>,  
it moves to the next or previous element in the array.</p>

<hr>

<h2>Pointer Subtraction (Distance)</h2>

<p>You can subtract two pointers that point to elements in the <strong>same array</strong>  
to find out how many elements are between them:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate javaHigh">
    int myNumbers[5] = {10, 20, 30, 40, 50};<br>
    int *start = &myNumbers[1]; // points to 20<br>
    int *end = &myNumbers[4]; &nbsp;&nbsp;// points to 50<br><br>
    printf("%ld\n", end - start); // 3 elements apart</div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_pointer_arith_sub">Try it Yourself &raquo;</a></p>
</div>

<p>Here:</p>
<ul>
  <li><code class="w3-codespan">start</code> points to the second element (<code class="w3-codespan">20</code>).</li>
  <li><code class="w3-codespan">end</code> points to the fifth element (<code class="w3-codespan">50</code>).</li>
  <li><code class="w3-codespan">end - start</code> gives the number of elements between them: <strong>3</strong>.</li>
</ul>

<div class="ws-note ws-panel">
  <p><strong>Note:</strong> Pointer subtraction only works when both pointers point into the <em>same array</em>.  
  The result is measured in <strong>elements</strong>, not bytes.</p>
</div>
...
<h2>Pointer Arithmetic Depends on Type</h2>

<p>Not all pointers move the same way.</p>

<p>When you add <code class="w3-codespan">1</code> to a pointer, it moves forward by the size of the thing it points to - not just by 1 byte.</p>

<p>For example:</p>

<ul>
  <li>An <code class="w3-codespan">int*</code> pointer moves by the size of an integer (usually 4 bytes).</li>
  <li>A <code class="w3-codespan">char*</code> pointer moves by the size of a character (1 byte).</li>
</ul>

<p>So if both pointers start at memory address <code class="w3-codespan">1000</code>:</p>

<ul>
  <li><code class="w3-codespan">int*</code> &#8594; <code class="w3-codespan">p + 1</code> would move to address <code class="w3-codespan">1004</code></li>
  <li><code class="w3-codespan">char*</code> &#8594; <code class="w3-codespan">p + 1</code> would move to address <code class="w3-codespan">1001</code></li>
</ul>

<p>This shows that pointer movement depends on the data type it points to - not 
on the number you add:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate javaHigh">
    int myNumbers[2] = {1, 2};<br>char letters[] = &quot;Hi&quot;; // 'H', 'i', '\0'<br>
    <br>int *pi = myNumbers; // int pointer<br>char *pc = letters; // char 
    pointer<br><br>printf(&quot;%p\n&quot;, (void*)pi);<br>printf(&quot;%p\n&quot;, (void*)(pi + 
    1)); // moves by sizeof(int) (4 bytes)<br>printf(&quot;%p\n&quot;, (void*)(pi + 2)); 
    // moves by sizeof(int) (4 bytes)<br><br>printf(&quot;%p\n&quot;, (void*)pc);<br>
    printf(&quot;%p\n&quot;, (void*)(pc + 1)); // moves by 1 byte</div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_pointer_arith_types">Try it Yourself &raquo;</a></p>
</div>

<hr>

<h2>Looping with Pointers</h2>

<p>In the <a href="c_pointers_arrays.php">previous chapter</a>, you learned how to loop through an array using <code class="w3-codespan">*(ptr + i)</code>.</p>

<p>Now let's look at another way - by <strong>moving the pointer itself</strong> inside the loop.  
Each time the pointer is increased (<code class="w3-codespan">p++</code>), it 
moves to the next element in memory:</p>

<div class="w3-example">
  <h3>Example</h3>
  <div class="w3-code notranslate javaHigh">
    int myNumbers[4] = {25, 50, 75, 100};<br>
    int *p = myNumbers; &nbsp;&nbsp;&nbsp;// start of array<br><br>
    for (int i = 0; i &lt; 4; i++) {<br>
    &nbsp;&nbsp;printf("%d\n", *p);<br>
    &nbsp;&nbsp;p++; // move to next element<br>
    }</div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_pointer_arith_loop_move">Try it Yourself &raquo;</a></p>
</div>

<p>Here's what happens in each loop:</p>
<ul>
  <li><code class="w3-codespan">*p</code> gives the current element value.</li>
  <li><code class="w3-codespan">p++</code> moves the pointer to the next element in the array.</li>
  <li>No array index (<code class="w3-codespan">i</code>) is needed - the pointer keeps track of the position.</li>
</ul>

<div class="ws-note ws-panel">
  <p><strong>Tip:</strong> This way of looping is common when working directly with memory,  
  since the pointer itself moves through the array instead of using an index number.</p>
</div>
<hr>
...
<ul>
  ...
  <li><strong>Using the wrong type:</strong>  
  Remember that pointer movement depends on its type.  
  An <code class="w3-codespan">int*</code> moves in 4-byte steps,  
  but a <code class="w3-codespan">char*</code> moves 1 byte at a time.  
  Mixing them up will point to the wrong memory locations.</li>

  <li><strong>Uninitialized pointers:</strong>  
  Always make sure a pointer is pointing to something real before you use it.  
  Using a pointer that doesn't point anywhere can crash your program.</li>
  <li><strong>Going out of bounds:</strong>  
  Never move a pointer past the end of an array or before it starts.  
  The only safe "outside" position is <em>one step past the end</em>,  
  and that's only for comparing pointers - not for accessing values.</li>
</ul>

<p>And be careful; pointers must be handled with care, since it is possible to damage data stored in other memory addresses.</p>
<hr>
