<!DOCTYPE html>
<html lang="en-US">
<title>C Deallocate Memory / Free Memory</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>C <span class="color_h1">Deallocate Memory</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_memory_reallocate.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_memory_struct.php">Next &#10095;</a>
</div>
<hr>

<!--start-->
<h2>Deallocate (free) Memory</h2>

<p>When you no longer need a block of memory you should deallocate it. Deallocation is also referred to as "freeing" the memory.</p>
<p>Dynamic memory stays reserved until it is deallocated or until the program ends.</p>
<p>Once the memory is deallocated it can be used by other programs or it may even be allocated to another part of your program.</p>
<!--stop-->
...
<p>To deallocate memory, use the <code class="w3-codespan">free()</code> function:</p>
...
<p>The <em>pointer</em> parameter is a pointer to the address of the memory to 
be deallocated:</p>

<div class="w3-example">
<div class="w3-code notranslate javaHigh">
  int *ptr;<br>ptr = malloc(sizeof(*ptr));<br><br><strong>free(ptr);</strong><br>ptr = NULL;</div>
</div>

<div class="ws-note w3-panel">
<p>It is considered a good practice to set a pointer to <code class="w3-codespan">NULL</code> after freeing memory so that you cannot accidentally continue using it.</p>
<p>If you continue using memory after it has been freed you may corrupt data from other programs or even another part of your own program.</p>
</div>

<div class="w3-example">
<h3>Example</h3>
<p>A working example including error checking and freeing:</p>
<div class="w3-code notranslate javaHigh">
  int *ptr;<br>ptr = malloc(sizeof(*ptr)); // Allocate memory for one integer<br>
  <br>// If memory cannot be allocated, print a message and end the main() 
  function<br>if (ptr == NULL) {<br>
  &nbsp;
  printf(&quot;Unable to allocate memory&quot;);<br>&nbsp; return 1;&nbsp; // 
  Exit the program with an error code<br>}<br><br>
  // Set the value of the integer<br>*ptr = 20;<br><br>// Print the 
  integer value<br>printf(&quot;Integer value: %d\n&quot;, *ptr);<br>
  <br>// Free allocated memory<br><strong>free(ptr);</strong><br><br>// Set the pointer to 
  NULL to prevent it from being accidentally used<br><strong>ptr = NULL;</strong></div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_free">Try it Yourself &raquo;</a>
</div>
<!--stop-->
...
<p>A <strong>memory leak</strong> happens when dynamic memory is allocated but never freed.</p>
<p>If a memory leak happens in a loop or in a function that is called frequently it could take up too much memory and cause the computer to slow down.</p>
<p>There is a risk of a memory leak if a pointer to dynamic memory is lost before the memory can be freed. This can happen accidentally, so it is important to be careful and keep track of pointers to dynamic memory. </p>
<p>Here are some examples of how a pointer to dynamic memory may be lost.</p>

<div class="w3-example">
<h3>Example 1</h3>
<p>The pointer is overwritten:</p>
<div class="w3-code notranslate javaHigh">
  int x = 5;<br>int *ptr;<br>ptr = calloc(2, sizeof(*ptr));<br>ptr = &amp;x;</div>
  <p>In this example, after the pointer is changed to point at <code class="w3-codespan">x</code>, the memory allocated by <code class="w3-codespan">calloc()</code> can no longer be accessed.</p>
</div>

<div class="w3-example">
<h3>Example 2</h3>
<p>The pointer exists only inside a function:</p>
<div class="w3-code notranslate javaHigh">
  void myFunction() {<br>&nbsp; int *ptr;<br>&nbsp; ptr = malloc(sizeof(*ptr));<br>
  }<br><br>
  int main() {<br>&nbsp; myFunction();<br>&nbsp; printf(&quot;The function has 
  ended&quot;);<br>&nbsp; return 0;<br>}</div>
  <p>In this example, the memory that was allocated inside of the function remains allocated after the function ends but it cannot be accessed anymore. One way to prevent this problem is to free the memory before the function ends.</p>
</div>

<div class="w3-example">
<h3>&nbsp;Example 3</h3>
<p>The pointer gets lost when reallocation fails:</p>
<div class="w3-code notranslate javaHigh">
  int* ptr;<br>ptr = malloc(sizeof(*ptr));<br>ptr = realloc(ptr, 
  2*sizeof(*ptr));</div>
  <p>If <code class="w3-codespan">realloc()</code> is unable to reallocate memory it will return a pointer to <code class="w3-codespan">NULL</code> 
and the original memory will remain reserved.</p>
<p>In this example, if <code class="w3-codespan">realloc()</code> fails then the <code class="w3-codespan">NULL</code> pointer is assigned to the <code class="w3-codespan">ptr</code> variable, overwriting the original memory address so that it cannot be accessed anymore.</p>
</div>
<!--stop-->
...
<!--start-->
<h2>Summary</h2>
<p>In summary, when managing memory in C, use 
best practices:
<ol>
<li>Remember to check for errors (<code class="w3-codespan">NULL</code> return 
values) to find out if memory allocation was sucessful or not</li>
<li>Prevent memory leaks - always remember to free memory that is no longer used, 
or else the program might underperform or even worse, crash if it runs out of 
memory</li>
<li>Set the pointer to <code class="w3-codespan">NULL</code> after freeing memory so that you cannot accidentally 
continue using it</li>
</ol></p>

<p><strong>Tip:</strong> You will learn more about <code class="w3-codespan">NULL</code> in our <a href="c_null.php">NULL chapter</a>.</p>

<!--stop-->

<hr>
<div id="exercisecontainer" src="xrcise_memory_deallocate.js" data-extensions="php"></div>
<hr>
