...
ny:-->
    ...
    <h1>C <span class="color_h1">Allocate Memory</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_memory_management.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_memory_access.php">Next &#10095;</a>
</div>
<hr>

<!--start-->
<p class="intro">The process of reserving memory is called allocation. The way to allocate memory depends on the type of memory.</p>
<p class="intro">C has two types of memory: Static memory and dynamic memory.</p>
<!--stop-->
...
<!--start-->
<h2>Static Memory</h2>
<p>Static memory is memory that is reserved for variables <strong>before</strong> the program runs. Allocation of static memory is also known as <em>compile time</em> memory allocation.</p>
...
<div class="w3-code notranslate javaHigh">
  int students[20];<br>printf(&quot;%zu&quot;, sizeof(students)); // 80 bytes</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_static">Try it Yourself &raquo;</a>
</div>
...
<p>Dynamic memory is memory that is allocated <strong>after</strong> the program starts running. Allocation of dynamic memory can also be referred to as <em>runtime</em> memory allocation.</p>
...
<p>To allocate dynamic memory, you can use the <code class="w3-codespan">malloc()</code> or <code class="w3-codespan">calloc()</code> functions. It is necessary to include the <code class="w3-codespan">&lt;stdlib.h&gt;</code> header to use them. The <code class="w3-codespan">malloc()</code> and <code class="w3-codespan">calloc()</code> functions allocate some memory and return a pointer to its address.</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
int *ptr1 = malloc(<em>size</em>);<br>
int *ptr2 = calloc(<em>amount</em>, <em>size</em>);
</div>
</div>
<p>The <code class="w3-codespan">malloc()</code> function has one parameter, <em>size</em>, which specifies how much memory to allocate, measured in bytes.</p>
<p>The <code class="w3-codespan">calloc()</code> function has two parameters:</p>
<ul>
 <li><em>amount</em> - Specifies the amount of items to allocate</li>
 <li><em>size</em> - Specifies the size of each item measured in bytes</li>
</ul>

<div class="ws-note w3-panel">
<p><strong>Note:</strong> The data in the memory allocated by <code class="w3-codespan">malloc()</code> is unpredictable. To avoid unexpected values, make sure to write something into the memory before reading it.</p>
<p>Unlike <code class="w3-codespan">malloc()</code>, the <code class="w3-codespan">calloc()</code> function writes zeroes into all of the allocated memory. However, this makes <code class="w3-codespan">calloc()</code> slightly less efficient.</p>
</div>

<p>The best way to allocate the right amount of memory for a data type is to use the <code class="w3-codespan">sizeof</code> operator:</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
int *ptr1, *ptr2;<br>
...
<div class="ws-note w3-panel">
<p><strong>Be careful:</strong> <code class="w3-codespan">sizeof(*ptr1)</code> tells C to measure the size of the data at the address. If you forget the <code class="w3-codespan">*</code> and write <code class="w3-codespan">sizeof(ptr1)</code> instead, it will measure the size of the pointer itself, which is the (usually) 8 bytes that are needed to store a memory address.</p>
<p><strong>Note:</strong> The <code class="w3-codespan">sizeof</code> operator cannot measure how much dynamic memory is allocated. When measuring dynamic memory, it only tells you the size of the <em>data type</em> of the memory. For example, if you reserve space for 5 <code class="w3-codespan">float</code> values, the <code class="w3-codespan">sizeof</code> operator will return 4, which is the number of bytes needed for a single <code class="w3-codespan">float</code> value.</p>
</div>

<p>Let's use dynamic memory to improve the students example above.</p>
<p>As noted previously, we cannot use <code class="w3-codespan">sizeof</code> to 
...
<h3>Example</h3>
<div class="w3-code notranslate javaHigh">
  int *students;<br>int numStudents = 12;<br>students = calloc(numStudents, 
  sizeof(*students));<br>printf(&quot;%d&quot;, numStudents * sizeof(*students)); // 48 
  bytes</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_allocate">Try it Yourself &raquo;</a>
</div>
<!--stop-->
...
<!--start-->
<h2>Notes</h2>
<p>When working with dynamic memory allocation, you should also <strong>check for errors</strong> and 
...
<p>It is good to be aware of stack memory to be able to handle the memory usage of nested function calls and recursion. Recursion that repeats itself too many times may take up too much stack memory. When that happens it is called a <strong>stack overflow</strong>.</p>
<!--stop-->
<hr>
<div id="exercisecontainer" src="xrcise_memory_allocate.js" data-extensions="php"></div>
<hr>
