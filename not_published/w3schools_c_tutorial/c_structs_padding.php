<!DOCTYPE html>
<html lang="en-US">
<title>C Struct Alignment and Padding</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>C <span class="color_h1">Struct Alignment and Padding</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_typedef.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_enums.php">Next &#10095;</a>
</div>
<hr>

<h2>Struct Padding</h2>

<p>When you create a <a href="c_structs.php">struct</a> in C, the compiler may add some extra <strong>bytes of padding</strong> between members.</p>

<p>This is done to make the program run faster on your computer, because most CPUs read data more efficiently when it's <strong>properly aligned</strong> in memory.</p>

<p>You rarely need to worry about this unless you work with low-level memory or file formats.</p>
<div class="ws-note ws-panel">
  <p><strong>In short:</strong> Padding means the compiler sometimes adds empty spaces inside a struct to keep things fast and properly aligned in memory.</p>
</div>

<p>Let's look at a simple struct:</p>
...
<div class="w3-example">
  ...
  <div class="w3-code notranslate javaHigh">
    ...
    int main() {<br>
    &nbsp;&nbsp;printf("Size of struct: %zu bytes\n", sizeof(struct Example));<br>
    &nbsp;&nbsp;return 0;<br>
    }
  </div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_struct_padding">Try it Yourself &raquo;</a></p>
</div>

<p>You might expect the size to be <code class="w3-codespan">1 + 4 + 1 = 6 bytes</code> - but it will usually print <strong>12 bytes</strong>!</p>

<h3>Why?</h3>

<p>The compiler adds <strong>padding bytes</strong> so that the <code class="w3-codespan">int</code> member (<code class="w3-codespan">b</code>) starts at a memory address that's a multiple of 4.  
This helps the CPU read it faster.</p>

<p>Here's how memory is actually arranged:</p>
...
<table class="ws-table-all notranslate">
  ...
  <tr>
    <td><em>padding</em></td>
    <td>3</td>
    <td>Added so <code>b</code> starts at a multiple of 4</td>
  </tr>
  <tr>
    <td>b</td>
    <td>4</td>
    <td>Aligned to 4-byte boundary</td>
  </tr>
  <tr>
    <td>c</td>
    <td>1</td>
  ...
  <tr>
    <td><em>padding</em></td>
    <td>3</td>
    <td>Added to make total size a multiple of 4</td>
  </tr>
</table>

<p>Total = 1 + 3 + 4 + 1 + 3 = <strong>12 bytes</strong>.</p>
...
<h2>How to Reduce Padding</h2>

<p>Padding depends on the <strong>order</strong> of members in the struct.  
If you group larger types first, you can make the struct smaller:</p>
...
<div class="w3-example">
  ...
  <div class="w3-code notranslate javaHigh">
    ...
    int main() {<br>&nbsp; printf(&quot;Size of struct: %zu bytes\n&quot;, sizeof(struct 
    Example));&nbsp; // Usually 8 bytes<br>&nbsp; return 0;<br>}<br>
  </div>
  <p><a target="_blank" class="w3-btn" href="tryc.php?filename=demo_struct_padding_reorder">Try it Yourself &raquo;</a></p>
</div>

<p>Now the struct is only <strong>8 bytes</strong> instead of 12, because there's less padding needed.</p>
...
<ul>
  <li>Padding helps programs run faster by keeping data aligned in memory.</li>
  <li>It can make structs bigger than expected.</li>
  <li>Reordering members can reduce the total size.</li>
  <li><strong>Note:</strong> You rarely need to worry about this unless you work with low-level memory or file formats.</li>
</ul>

<hr>

<h2>Structs vs Unions</h2>

<p>So far, we've talked about padding inside <strong>structs</strong>.  
But what about <a href="c_unions.php">unions</a>?</p>

<p>Unions store all members in the <strong>same memory location</strong>, so there is <strong>no padding between members</strong>.  
However, both structs and unions still follow <strong>alignment rules</strong> - the data must start at memory addresses that match their type size.</p>
...
<div class="w3-black w3-padding">
  <code>Struct size: 12<br>Union size: 4</code>
</div>
  <p>
    <a target="_blank" class="w3-btn" href="tryc.php?filename=demo_struct_padding_union">Try it Yourself &raquo;</a>
  </p>
</div>

<p>Explanation:</p>
<ul>
  <li><strong>Struct</strong> - members are stored one after another, so padding is added between them.</li>
  <li><strong>Union</strong> - all members share the same memory, so only the largest member decides its total size.</li>
</ul>
...
<table class="ws-table-all notranslate">
  ...
  <tr>
    <td>Members stored</td>
    <td>One after another</td>
    <td>All share the same memory space</td>
  </tr>
  <tr>
    <td>Padding between members</td>
    <td>Yes</td>
    <td>No</td>
  </tr>
  <tr>
    <td>Aligned to</td>
    <td>Each member's type</td>
    <td>Largest member's type</td>
  </tr>
  <tr>
    <td>Total size</td>
    <td>Sum of members + padding</td>
    <td>Size of largest member</td>
  </tr>
</table>

<div class="ws-note ws-panel">
  <p><strong>Tip:</strong> You usually don't need to worry about this, but it helps to know why <a href="c_structs.php">structs</a> sometimes take up more memory than expected, while <a href="c_unions.php">unions</a> do not.</p>
</div>
...
<ul>
  <li>Struct members are aligned based on their data type size.</li>
  <li>The compiler may add padding bytes to align data properly.</li>
  <li>The total struct size is often larger than the sum of its members.</li>
  <li>Reordering members can reduce padding and save memory.</li>
</ul>
