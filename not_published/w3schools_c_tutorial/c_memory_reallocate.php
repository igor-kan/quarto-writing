<!DOCTYPE html>
<html lang="en-US">
<title>C Reallocate Memory</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>C <span class="color_h1">Reallocate Memory</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="c_memory_access.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="c_memory_deallocate.php">Next &#10095;</a>
</div>
<hr>
<!--start-->
<h2>Reallocate Memory</h2>

<p>If the amount of memory you reserved is not enough, you can <em>reallocate
</em>it to make it larger.</p>
<p>Reallocating reserves a different (usually larger) amount of memory while keeping the data that was stored in it.</p>
<p>You can change the size of allocated memory with the <code class="w3-codespan">realloc()</code> function.</p>

<p>The <code class="w3-codespan">realloc()</code> function takes two parameters:</p>
<div class="w3-example">
<div class="w3-code notranslate javaHigh">
int *ptr2 = realloc(<em>ptr1</em>, <em>size</em>);
</div>
</div>
<ul>
  <li>The first parameter is a pointer to the memory that is being resized.</li>
  <li>The second parameter specifies the new size of allocated memory, measured in bytes.</li>
</ul>

<p>The <code class="w3-codespan">realloc()</code> function tries to resize the memory at <em>ptr1</em> and return the same memory address. If it cannot resize the memory at the current address then it will allocate memory at a different address and return the new address instead.</p>

<div class="ws-note w3-panel">
<p><strong>Note:</strong> When <code class="w3-codespan">realloc()</code> returns a different memory address, the memory at the original address is no longer reserved and it is not safe to use. When the reallocation is done it is good to assign the new pointer to the previous variable so that the old pointer cannot be used accidentally.</p>
</div>

<div class="w3-example">
<h3>Example</h3>
<p>Increase the size of allocated memory:</p>
<div class="w3-code notranslate javaHigh">
  int *ptr1, *ptr2, size;<br><br>// Allocate memory for four integers<br>size = 
  4 * sizeof(*ptr1);<br>ptr1 = malloc(size);<br><br>printf(&quot;%d bytes allocated 
  at address %p \n&quot;, size, ptr1);<br><br>// Resize the memory to hold six 
  integers<br>size = 6 * sizeof(*ptr1);<br>ptr2 = realloc(ptr1, size);<br><br>
  printf(&quot;%d bytes reallocated at address %p \n&quot;, size, ptr2);</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_reallocate">Try it Yourself &raquo;</a>
</div>
<!--stop-->
<hr>

<!--start-->
<h2>NULL Pointer &amp; Error Checking</h2>
<p>The <code class="w3-codespan">realloc()</code> function returns a <code class="w3-codespan">NULL</code> 
...
<p>The following example checks whether <code class="w3-codespan">realloc()</code> 
is able to resize the memory or not, by checking for a <code class="w3-codespan">NULL</code> pointer:</p>

<div class="w3-example">
<h3>Example</h3>
<p>Check for a NULL pointer:</p>
<div class="w3-code notranslate javaHigh">
  int *ptr1, *ptr2;<br><br>// Allocate memory<br>ptr1 = malloc(4);<br><br>// 
  Attempt to resize the memory<br>ptr2 = realloc(ptr1, 8);<br><br>// Check 
  whether realloc is able to resize the memory or not<br>if (<strong>ptr2 == 
  NULL</strong>) {<br>&nbsp; // If reallocation fails<br>&nbsp; printf(&quot;Failed. Unable to resize memory&quot;);<br>} else {<br>&nbsp; 
  // If reallocation is successful<br>&nbsp; 
  printf(&quot;Success. 8 bytes reallocated at address %p \n&quot;, ptr2);<br>&nbsp; ptr1 = ptr2;&nbsp; 
  // Update ptr1 to point to the newly allocated memory<br>
  }</div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_reallocate_null">Try it Yourself &raquo;</a>
</div>

<p><strong>Note:</strong> You should always include error checking (if <em>pointer</em> == NULL) when 
allocating memory.</p>

<p><strong>Note:</strong> You should also always <strong>free</strong>, or release, 
...
efficient.</p>
<p>To free memory, simply use the <code class="w3-codespan">free()</code> function:</p>
...
<div class="w3-code notranslate javaHigh">
  ...
  <strong>free(ptr1);</strong></div>
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryc.php?filename=demo_memory_reallocate_null2">Try it Yourself &raquo;</a>
</div>
<p>You will learn more about how to free allocated memory and why this is important in the next chapter.</p>
<!--stop-->
...
<div id="exercisecontainer" src="xrcise_memory_reallocate.js" data-extensions="php"></div>
<hr>
