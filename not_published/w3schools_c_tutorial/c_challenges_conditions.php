...
<html lang="en-US">
<head>
<title>C If...Else Challenge</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
<h1>C <span class="color_h1">If...Else Code Challenge</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="c_conditions_reallife.php">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="c_switch.php">Next &#10095;</a>
</div>

<hr>

<h3>Challenge: Make a Decision with If and Else</h3>
<p>Test your understanding of <strong>if</strong> and <strong>else</strong> statements, and run different code based on a condition.</p>

<div class="ch-wrap" id="cChallenges"></div>
<br>
<hr>
...
<![endif]-->
<script>
  window.CHALLENGE_DATA_URL = "c_challenge_conditions.js";
  window.CHALLENGE_HOST_ID = "cChallenges";
  window.CHALLENGE_LAYOUT_BREAKPOINT = 1450;
  window.CHALLENGE_PRISM_LANGUAGE = "c";
</script>
<script src="c_challenge_checks.js"></script>
<script src="/lib/challenge-engine/main.v1.0.7.js"></script>
<!-- C Challenge Runner (self-contained) -->
<style>
...
.ch-runner-tab-btn.active{ color:#fff; }
.ch-runner-output{
  min-height:120px;
  background:#0f111a;
  position:relative;
...
.ch-runner-loader-dots::after{
  content:'';
  animation:chDots 1.2s steps(4,end) infinite;
}
@keyframes chDots{
  0%{ content:''; }
  25%{ content:'.'; }
...
.ch-runner-error-badge.is-visible{ display:inline-block; }
.ch-runner-error-msg{
  display:none;
  padding:8px 14px;
  ...
  font-size:13px;
  font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Courier New",monospace;
  border-bottom:1px solid rgba(240,113,120,0.2);
}
.ch-runner-error-msg.is-visible{ display:block; }

/* Force-hide summary/checklist while waiting for Sphere Engine response */
.ch-waiting-result .ch-summary,
.ch-waiting-result .ch-checkwrap{ display:none !important; }
...
<div class="ch-runner-wrap" id="chRunnerWrap">
  <div class="ch-runner-preview">
    <div class="ch-runner-tab">
      <span class="ch-runner-tab-btn active">Result<span class="ch-runner-error-badge" id="chErrorBadge">Error</span></span>
    </div>
    <div class="ch-runner-output" id="chRunnerOutput">
      <div class="ch-runner-error-msg" id="chErrorMsg">Your code has an error. Check the output below for details.</div>
      ...
      <div class="ch-runner-loader" id="chRunnerLoader">Compiling &amp; running<span class="ch-runner-loader-dots"></span></div>
      <iframe id="chRunnerIframe" name="chRunnerIframe" style="display:none" sandbox="allow-scripts"></iframe>
    </div>
  </div>
</div>

<form id="chRunnerForm" style="display:none" method="post" accept-charset="utf-8" target="chRunnerIframe">
  <input type="hidden" name="code" id="chRunnerCode" />
</form>
...
(function(){
  ...
  var hostId = window.CHALLENGE_HOST_ID || "cChallenges";
  var DATA_URL = window.CHALLENGE_DATA_URL || "";
  var PRISM_LANG = window.CHALLENGE_PRISM_LANGUAGE || "c";
  var ACTION_URL = "https://try.w3schools.com/try_c.php";
  var hooked = false;

  // Tell the challenge engine to keep the checklist visible between Check Code clicks
  window.ChallengeEngine = window.ChallengeEngine || {};
  window.ChallengeEngine.persistChecklist = true;
  window.ChallengeEngine.deferHighlight = true;
  ...
  function showErrorUI(){
    var badge = document.getElementById("chErrorBadge");
    var msg = document.getElementById("chErrorMsg");
  ...
  // Finalize the checklist and apply deferred green-line highlighting
  function finalizeCheck(hasError, errorLines){
    var host = document.getElementById(hostId);
    if (!host) return;
    var card = host.querySelector(".ch-card");
    var sum = host.querySelector(".ch-summary");
    var checkWrap = host.querySelector(".ch-checkwrap");
    var list = host.querySelector(".ch-checklist");
    if (!sum || !checkWrap || !list) return;

    // Only show "No runtime errors" when there IS an error (red)
    if (hasError){
      var li = document.createElement("li");
      li.innerHTML = "<div class='chk-badge no' role='img' aria-label='Failed'>&#10007;</div><div>Runtime Error</div>";
      list.appendChild(li);
      sum.className = "w3-panel w3-pale-red ch-summary";
      sum.innerHTML = "Your code has a <strong>runtime error</strong>. Fix the error and check again.";
    }
    ...
    if (e.source && codeIframe && codeIframe.contentWindow === e.source){
      ...
      if (codeHasError){
        ...
        if (Array.isArray(e.data.errorLine)){
          for (var i = 0; i < e.data.errorLine.length; i++){
            if (e.data.errorLine[i] > 0) errorLines.push(e.data.errorLine[i] - 1);
          }
        }
        ...
        if (!errorLines.length && typeof e.data.errorText === "string"){
          ...
          if (m){
            for (var j = 0; j < m.length; j++){
              var num = parseInt(m[j].replace(/line\s+/i, ""), 10);
              if (num > 0) errorLines.push(num - 1);
            }
          }
        }
      }
  ...
  window.chRunCode = function(){
    ...
    if (!code.trim() || code === starter){
      ...
      placeholder.innerHTML = "<span style='color:#F78C6C'>No changes have been made. Edit the code and try again.</span>";
