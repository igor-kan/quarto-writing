...
<html lang="en-US">
<head>
<title>Java Advanced Sorting (Comparator and Comparable)</title>
<meta charset="utf-8">
...
<h1>Java <span class="color_h1">Advanced Sorting (Comparator and Comparable)</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="java_lambda.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="java_projects.asp">Next &#10095;</a>
</div>
<hr>

<h2>Java Advanced Sorting</h2>
<p>In the <a href="java_sort_list.asp">List Sorting Chapter</a>, you learned how to sort lists alphabetically and numerically, but what if the list has objects in it?</p>
<p>To sort objects you need to specify a rule that decides how objects should be sorted. For example, if you have a list of cars you might want to sort them by year, the rule could be that cars with an earlier year go first.</p>
<p>The <code class="w3-codespan">Comparator</code> and <code class="w3-codespan">Comparable</code> interfaces allow you to specify what rule is used to sort objects.</p>
<p>Being able to specify a sorting rule also allows you to change how strings and numbers are sorted.</p>

<hr>

<h2>Comparators</h2>
<p>An object that implements the <code class="w3-codespan">Comparator</code> interface is called a comparator.</p>
<p>The <code class="w3-codespan">Comparator</code> interface allows you to create a class with a <code class="w3-codespan">compare()</code> method that compares two objects to decide which one should go first in a list.</p>
<p>The <code class="w3-codespan">compare()</code> method should return a number which is:</p>
<ul>
  <li>Negative if the first object should go first in a list.</li>
  <li>Positive if the second object should go first in a list.</li>
  <li>Zero if the order does not matter.</li>
</ul>
<p>A class that implements the <code class="w3-codespan">Comparator</code> interface might look something like this:</p>
<div class="w3-example">
<pre class="w3-white language-java"><code>// Sort Car objects by year
...
<p>To use the comparator, pass it as an argument into a sorting method:</p>
<div class="w3-example">
<pre class="w3-white language-java"><code>// Use a comparator to sort the cars
Comparator myComparator = new SortByYear();
Collections.sort(myCars, myComparator);
</code></pre>
</div>

<p>Here is a complete example using a comparator to sort a list of cars by year:</p>

<div class="w3-example">
<h3>Example</h3>
<pre class="w3-white language-java"><code>import java.util.ArrayList;
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryjava.asp?filename=demo_comparator">Try it Yourself &raquo;</a>
</div>

<h3>Using a Lambda Expression</h3>
<p>To make the code shorter, the comparator can be replaced with a lambda expression which has the same arguments and return value as the <code class="w3-codespan">compare()</code> method:</p>
<div class="w3-example">
<h3>Example</h3>
<p>Use a lambda expression as a comparator:</p>
<pre class="w3-white language-java"><code>Collections.sort(myCars, (obj1, obj2) -> {
  Car a = (Car) obj1;
  Car b = (Car) obj2;
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryjava.asp?filename=demo_comparator_lambda">Try it Yourself &raquo;</a>
</div>

<h3>Special Sorting Rules</h3>
<p>Comparators can also be used to make special sorting rules for strings and numbers. In this example we use a comparator to list all of the even numbers before the odd ones:</p>
<div class="w3-example">
<h3>Example</h3>
<pre class="w3-white language-java"><code>import java.util.ArrayList;
import java.util.Collections;
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryjava.asp?filename=demo_comparator_numbers">Try it Yourself &raquo;</a>
</div>
...
<h2>The Comparable Interface</h2>

<p>The <code class="w3-codespan">Comparable</code> interface allows an object to specify its own sorting rule with a <code class="w3-codespan">compareTo()</code> method.</p>
<p>The <code class="w3-codespan">compareTo()</code> method takes an object as an argument and compares the comparable with the argument to decide which one should go first in a list.</p>
<p>Like the comparator, the <code class="w3-codespan">compareTo()</code> method returns a number which is:</p>

<ul>
  <li>Negative if the comparable should go first in a list.</li>
  <li>Positive if the other object should go first in a list.</li>
  <li>Zero if the order does not matter.</li>
</ul>

<div class="w3-panel ws-note">
  <p>Many native Java classes implement the <code class="w3-codespan">Comparable</code> interface, such as <code class="w3-codespan">String</code> and <code class="w3-codespan">Integer</code>.</p>
  <p>This is why strings and numbers do not need a comparator to be sorted.</p>
</div>

<p>An object that implements the <code class="w3-codespan">Comparable</code> interface might look something like this:</p>

<div class="w3-example">
<pre class="w3-white language-java"><code>class Car implements Comparable {
  public String brand;
  public String model;
  public int year;
...
<p>Here is the same example as before but using the <code class="w3-codespan">Comparable</code> interface instead of a comparator:</p>
<div class="w3-example">
<h3>Example</h3>
<pre class="w3-white language-java"><code>import java.util.ArrayList;
import java.util.Collections;
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="tryjava.asp?filename=demo_comparable">Try it Yourself &raquo;</a>
</div>

<hr>

<div class="w3-panel ws-note">
<h3>A Common Sorting Trick</h3>
<p>The most obvious way to sort two numbers naturally is to write something like this:</p>
<div class="w3-example">
<pre class="w3-white language-java"><code>if(a.year &lt; b.year) return -1; // a is less than b
if(a.year > b.year) return 1; // a is greater than b
return 0; // a is equal to b
</code></pre>
</div>
<p>But it can actually be done with just a single line:</p>
<div class="w3-example">
<pre class="language-java"><code>return a.year - b.year;</code></pre>
</div>
<p>This trick can also be used to easily sort things in reverse:</p>
<div class="w3-example">
<pre class="language-java"><code>return b.year - a.year;</code></pre>
</div>
</div>
...
<p>A comparator is an object with one method that is used to compare two different objects.</p>
<p>A comparable is an object which can compare itself with other objects.</p>
<p>It is easier to use the <code class="w3-codespan">Comparable</code> interface when possible, but the <code class="w3-codespan">Comparator</code> interface is more powerful because it allows you to sort any kind of object even if you cannot change its code.<p>

<hr>
<div id="exercisecontainer" src="xrcise_advanced_sorting.js"></div>
<hr>
