...
<h1>HTML <span class="color_h1">Web Workers API</span></h1>
...
<hr>
<p class="intro">A web worker is an external JavaScript file that runs in the background, without affecting the performance of the page.</p>
<hr>
<!--start-->
...
<p>A web worker is an external JavaScript file that runs in the background, independently 
...
<!--stop-->
<p>Web workers are useful for heavy code that can't be run on the main thread, 
without causing long tasks that make the page unresponsive.</p>
<hr>

<h2>Browser Support</h2>
<p>The numbers in the table specify the first browser version that fully support 
the Web Workers API.</p>
...
<p><strong>Note:</strong> Normally web workers are not used for such simple scripts, but for more CPU intensive tasks!</p>
</div>
<!--stop-->
<hr>
<!--start-->
...
<div class="w3-code notranslate jsHigh">
  ...
  if(typeof(Worker) !== &quot;undefined&quot;) {<br>&nbsp; x.innerHTML = &quot;Your browser 
  support Web Workers!&quot;;<br>} else {<br>&nbsp; x.innerHTML = &quot;Sorry, your 
  browser does not support Web Workers.&quot;;<br>}<br>
  &lt;/script&gt; </div>
<a target="_blank" href="tryit.asp?filename=tryhtml5_webworker_browsertest" class="w3-btn w3-margin-bottom">Try it Yourself &raquo;</a>
</div>
<!--stop-->
<hr>
...
<hr>
<!--start-->
<h2>Create a .js Web Worker File</h2>
...
<p><strong>Note:</strong> The important part of the code above is the <code class="w3-codespan">postMessage()</code> method - which is used to post messages back to the HTML page.</p>
<!--stop-->
<hr>
<!--start-->
...
<p>The following lines checks if a worker (w) already exists, if not - it creates a new web worker object and 
...
<p>Data is sent between web workers and the main thread via a system of messages 
- both sides send their messages using the <code class="w3-codespan">postMessage()</code> 
method, and respond to messages via the <code class="w3-codespan">onmessage</code> 
event handler.</p>
<p>Add an <code class="w3-codespan">onmessage</code> event listener to the web worker 
object.</p>
<div class="w3-code w3-border notranslate jsHigh"><div>
...
<p>When the web worker in the .js posts a message, the code within the event listener is executed. The data 
from the web worker is stored in <code class="w3-codespan">event.data</code>.</p>
<!--stop-->
<hr>
<!--start-->

<h2>Terminate a Web Worker</h2>
<p>When a web worker object is created, it will continue to listen for messages until it is terminated.</p>
...
<p>If you set the web worker variable to undefined, after it has been terminated, 
you can reuse the worker/code:</p>
<div class="w3-code w3-border notranslate"><div>
 w = undefined;</div></div>
<!--stop-->
<hr>
...
<br>
function startWorker()
{<br>&nbsp; const x = document.getElementById(&quot;result&quot;);<br>
  &nbsp;
if (typeof(Worker) !== &quot;undefined&quot;)
{<br>
&nbsp;&nbsp;&nbsp; if (typeof(w) == &quot;undefined&quot;) {<br>
...
{<br>
  &nbsp;&nbsp;&nbsp;
  x.innerHTML = &quot;Sorry! No Web Worker support.&quot;;<br>
  &nbsp;&nbsp;}<br>
}<br>
<br>
function stopWorker()
