...
<html lang="en-US">
<head>
<title>Python Class Properties Code Challenge</title>
<meta charset="utf-8">
...
<h1>Python <span class="color_h1">Class Properties Code Challenge</span></h1>
<div class="w3-clear nextprev">
  <a class="w3-left w3-btn" href="python_class_properties.asp">&#10094; Previous</a>
  <a class="w3-right w3-btn" href="python_class_methods.asp">Next &#10095;</a>
</div>
<hr>

<h2>Challenge: Class Properties</h2>
<p>Test your understanding of class properties by completing a small coding challenge.</p>

<div class="ch-wrap" id="pythonChallenges"></div>
...
<script>
  window.CHALLENGE_DATA_URL = "python_challenge_class_properties.js";
  window.CHALLENGE_HOST_ID = "pythonChallenges";
  window.CHALLENGE_PRISM_LANGUAGE = "python";
</script>
<script src="/lib/challenge-engine/main.v1.0.7.js"></script>
<!-- Python Challenge Runner (self-contained) -->
<style>
/* --- Layout: Instructions full-width, Editor+Preview split --- */

/* Override the original ch-split to always be single-column (instructions on top) */
.ch-split{ grid-template-columns: 1fr !important; }
.ch-layoutbtn{ display:none !important; }

/* New grid: Editor + Preview side-by-side */
...
/* Hide the original toolbar title - replaced by tabs */
.ch-toolbar-title{ display:none !important; }

/* Hide the original "Show solution" button - replaced by tab */
.ch-toolbar [data-action='toggleSolution']{ display:none !important; }

/* Hide Reset button entirely */
.ch-toolbar [data-action='reset']{ display:none !important; }

/* Reorder toolbar: tabs left, then reset + check right */
...
/* Solution view - replaces editor content */
.ch-solution-view{
  display:none;
  position:relative;
  background:#0f111a;
...
.ch-runner-tab-btn.active{
  color:#fff;
}
.ch-runner-tab-btn .ch-eye{
  font-size:12px;
  opacity:0.7;
}
.ch-runner-output{
...
/* Hide the original solution box - we use the tab view instead */
.ch-solutionbox{ display:none !important; }

/* Make c-editor-wrap relative so solution view can overlay it */
...
<div class="ch-runner-wrap" id="chRunnerWrap">
  <div class="ch-runner-preview">
    ...
    <div class="ch-runner-output" id="chRunnerOutput">
      ...
      <div class="ch-runner-loader" id="chRunnerLoader">Running<span class="ch-runner-loader-dots"></span></div>
...
(function(){
  ...
  var hostId = window.CHALLENGE_HOST_ID || "pythonChallenges";
  var DATA_URL = window.CHALLENGE_DATA_URL || "";
  var PRISM_LANG = window.CHALLENGE_PRISM_LANGUAGE || "python";
  var hooked = false;
  ...
  var waitingForResult = false;  // true while we wait for Sphere Engine response
  ...
  function finalizeCheck(hasError, errorLines){
    ...
    if (hasError){
      ...
      list.appendChild(li);
      // Override summary to error regardless of regex results
    ...
    // Remove the hiding class - summary and checklist become visible
  ...
  window.addEventListener("message", function(e){
    ...
    if (e.source && codeIframe && codeIframe.contentWindow === e.source){
      ...
      // If we were waiting for this response, finalize the check
      if (waitingForResult){
  ...
  // Restructure DOM: instructions full-width on top, editor+preview in a new grid
  function restructureLayout(){
    ...
    if (toolbar && toolbarLeft){
      ...
      // Reorder actions: move reset before check
      var actions = card.querySelector(".ch-toolbar-actions");
      var resetBtn = card.querySelector("[data-action='reset']");
      var checkBtn = card.querySelector("[data-action='check']");
      if (actions && resetBtn && checkBtn){
        actions.insertBefore(resetBtn, checkBtn);
      }
      // Set initial label to "Check Code"
      var chkLong = checkBtn ? checkBtn.querySelector(".ch-check-label-long") : null;
      if (chkLong) chkLong.textContent = "Check Code";
    }

    // --- Create solution view inside the editor wrap ---
    ...
    // Create new grid for editor + preview
    var grid = document.createElement("div");
    grid.className = "ch-editor-preview-grid";

    // Move workarea (contains editorpanel) into the grid
    grid.appendChild(workarea);
    // Move preview wrap into the grid
    wrap.style.display = "block";
    grid.appendChild(wrap);

    // Insert grid after the ch-split (which now only has instructions)
  ...
  // Hook "Check code" to run code AND defer the summary until Sphere Engine responds
  function hookCheckButton(){
    ...
    checkBtn.addEventListener("click", function(){
      ...
      // Add class that force-hides summary/checklist via CSS !important
      // This prevents any flash - the engine's setSummary runs but stays invisible
      ...
      // After the engine's sync handler finishes, run the code
      setTimeout(function(){
        chRunCode();
        // If chRunCode early-returned (blank/unchanged), remove the hiding class
        if (!waitingForResult){
          if (card) card.classList.remove("ch-waiting-result");
          if (window.ChallengeEngine && ChallengeEngine.setEditorDisabled) ChallengeEngine.setEditorDisabled(false);
        }
  ...
  // Watch for challenge engine to render
  ...
  // Public: run code (always targets the code iframe)
  window.chRunCode = function(){
    ...
    document.getElementById("chRunnerCode").value = code;
    var form = document.getElementById("chRunnerForm");
    form.action = "https://try.w3schools.com/try_python.php?x=" + Math.random();
    form.submit();
