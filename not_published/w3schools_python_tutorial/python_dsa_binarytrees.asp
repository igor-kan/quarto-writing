...
<html lang="en-US">
<head>
<title>Python Binary Trees</title>
<meta charset="utf-8">
...
<h1>Python <span class="color_h1">Binary Trees</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="python_dsa_trees.asp">&#10094; Previous</a>
<a class="w3-right w3-btn" href="python_dsa_binarysearchtrees.asp">Next &#10095;</a>
</div>
<hr>
...
<p>The Binary Tree above can be implemented much like a <a href="python_dsa_linkedlists.asp">Linked List</a>, except that instead of linking each node to one next node,
  we create a structure where each node can be linked to both its left and right child nodes.</p>

<div class="w3-example">
  <h3>Example</h3>
  <p>Create a Binary Tree in Python:</p>
...
<br>
nodeA.left = nodeC<br>
nodeA.right = nodeD<br>
<br>
nodeB.left = nodeE<br>
nodeB.right = nodeF<br>
<br>
nodeF.left = nodeG<br>
<br>
# Test<br>
print("root.right.left.data:", root.right.left.data)</div>
  <a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_binarytree" target="_blank">Run Example »</a>
</div>
...
<hr>
<h2>Pre-order Traversal of Binary Trees</h2>
<p>Pre-order Traversal is a type of Depth First Search, where each node is visited in a certain order..</p>
<p>Pre-order Traversal is done by visiting the root node first, then recursively do a pre-order traversal of the left subtree, followed by a recursive pre-order traversal of the right subtree. It's used for creating a copy of the tree, prefix notation of an expression tree, etc.</p>
<p>This traversal is "pre" order because the node is visited "before" the recursive pre-order traversal of the left and right subtrees.</p>
<p>This is how the code for pre-order traversal looks like:</p>
...
&nbsp;&nbsp;preOrderTraversal(node.right)
  ...
  <a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_binarytree_traversal" target="_blank">Run Example »</a>
</div>
<p>The first node to be printed is node R, as the Pre-order Traversal works by first visiting, or printing, the current node (line 4), before calling the left and right child nodes recursively (line 5 and 6).</p>
<p>The <code class="w3-codespan">preOrderTraversal()</code> function keeps traversing the left subtree recursively (line 5), before going on to traversing the right subtree (line 6). So the next nodes that are printed are 'A' and then 'C'.</p>
<p>The first time the argument <code class="w3-codespan">node</code> is <code class="w3-codespan">None</code> is when the left child of node C is given as an argument (C has no left child).</p>
<p>After <code class="w3-codespan">None</code> is returned the first time when calling C's left child, C's right child also returns <code class="w3-codespan">None</code>, and then the recursive calls continue to propagate back so that A's right child D is the next to be printed.</p>
<p>The code continues to propagate back so that the rest of the nodes in R's right subtree gets printed.</p>
<hr>
<h2>In-order Traversal of Binary Trees</h2>
<p>In-order Traversal is a type of Depth First Search, where each node is visited in a certain order.</p>
<p>In-order Traversal does a recursive In-order Traversal of the left subtree, visits the root node, and finally, does a recursive In-order Traversal of the right subtree. This traversal is mainly used for Binary Search Trees where it returns values in ascending order.</p>
<p>What makes this traversal "in" order, is that the node is visited in between the recursive function calls. The node is visited after the In-order Traversal of the left subtree, and before the In-order Traversal of the right subtree.</p>
<p>This is how the code for In-order Traversal looks like:</p>
<div class="w3-example">
<h3>Example</h3>
<p>Create an In-order Traversal:</p>
<div class="w3-code notranslate pythonHigh">
...
&nbsp;&nbsp;print(node.data, end=", ")<br>
&nbsp;&nbsp;inOrderTraversal(node.right)</div>
  <a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_binarytree_inorder_traversal" target="_blank">Run Example »</a>
</div>
<p>The <code class="w3-codespan">inOrderTraversal()</code> function keeps calling itself with the current left child node as an argument (line 4) until that argument is <code class="w3-codespan">None</code> and the function returns (line 2-3).</p>
<p>The first time the argument <code class="w3-codespan">node</code> is <code class="w3-codespan">None</code> is when the left child of node C is given as an argument (C has no left child).</p>
<p>After that, the <code class="w3-codespan">data</code> part of node C is printed (line 5), which means that 'C' is the first thing that gets printed.</p>
<p>Then, node C's right child is given as an argument (line 6), which is <code class="w3-codespan">None</code>, so the function call returns without doing anything else.</p>
<p>After 'C' is printed, the previous <code class="w3-codespan">inOrderTraversal()</code> function calls continue to run, so that 'A' gets printed, then 'D', then 'R', and so on.</p>
<hr>

<h2>Post-order Traversal of Binary Trees</h2>
<p>Post-order Traversal is a type of Depth First Search, where each node is visited in a certain order..</p>
<p>Post-order Traversal works by recursively doing a Post-order Traversal of the left subtree and the right subtree, followed by a visit to the root node. It is used for deleting a tree, post-fix notation of an expression tree, etc.</p>
<p>What makes this traversal "post" is that visiting a node is done "after" the left and right child nodes are called recursively.</p>
<p>This is how the code for Post-order Traversal looks like:</p>
<div class="w3-example">
<h3>Example</h3>
<p>Post-order Traversal:</p>
<div class="w3-code notranslate pythonHigh">
def postOrderTraversal(node):<br>
&nbsp;&nbsp;if node is None:<br>
&nbsp;&nbsp;&nbsp;&nbsp;return<br>
&nbsp;&nbsp;postOrderTraversal(node.left)<br>
&nbsp;&nbsp;postOrderTraversal(node.right)<br>
&nbsp;&nbsp;print(node.data, end=", ")</div>
  <a class="w3-btn w3-margin-bottom" href="trypython.asp?filename=demo_dsa_binarytree_postorder_traversal" target="_blank">Run Example »</a>
</div>
<p>The <code class="w3-codespan">postOrderTraversal()</code> function keeps traversing the left subtree recursively (line 4), until <code class="w3-codespan">None</code> is returned when C's left child node is called as the <code class="w3-codespan">node</code> argument.</p>
<p>After C's left child node returns <code class="w3-codespan">None</code>, line 5 runs and C's right child node returns <code class="w3-codespan">None</code>, and then the letter 'C' is printed (line 6).</p>
<p>This means that C is visited, or printed, "after" its left and right child nodes are traversed, that is why it is called "post" order traversal.</p>
<p>The <code class="w3-codespan">postOrderTraversal()</code> function continues to propagate back to previous recursive function calls, so the next node to be printed is 'D', then 'A'.</p>
<p>The function continues to propagate back and printing nodes until all nodes are printed, or visited.</p>
