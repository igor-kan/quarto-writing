...
<html lang="en-US">
<head>
<title>SciPy Interpolation</title>
<!-- [START] inc_meta.inc -->
<meta charset="utf-8">
...
ny:-->
    ...
    <h1>SciPy <span class="color_h1">Interpolation</span></h1>
<div class="w3-clear nextprev">
<a class="w3-left w3-btn" href="scipy_matlab_arrays.php">&#10094; Previous</a>
<a class="w3-right w3-btn" href="scipy_statistical_significance_tests.php">Next &#10095;</a>
</div>
<hr>
<h2>What is Interpolation?</h2>
<p>Interpolation is a method for generating points between given points.</p>
<p>For example: for points 1 and 2, we may interpolate and find points 1.33 and 1.66.</p>

<p>Interpolation has many usage, in Machine Learning we often deal with missing data in a dataset,
interpolation is often used to substitute those values.</p>
<p>This method of filling values is called <em>imputation</em>.</p>

<p>Apart from imputation, interpolation is often used where we need to smooth the discrete points in 
a dataset.</p>
<hr>
<h2>How to Implement it in SciPy?</h2>
<p>SciPy provides us with a module called <code class="w3-codespan">scipy.interpolate</code>
which has many functions to deal with interpolation:</p>
<hr>

<h2>1D Interpolation</h2>

<p>The function <code class="w3-codespan">interp1d()</code> is used to interpolate a distribution with 1 variable.</p>

<p>It takes <code class="w3-codespan">x</code> and <code class="w3-codespan">y</code> points and returns
a callable function that can be called with new <code class="w3-codespan">x</code> and returns corresponding <code class="w3-codespan">y</code>.</p>

<div class="w3-example">
<h3>Example</h3>
<p>For given xs and ys interpolate values from 2.1, 2.2... to 2.9:</p>
...
ys = 2*xs + 1<br>
<br>
interp_func = interp1d(xs, ys)<br>
<br>
newarr = interp_func(np.arange(2.1, 3, 0.1))<br>
<br>
print(newarr)
</div>
<h3>Result:</h3>
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_scipy_interpolation_interp1d">Try it Yourself &raquo;</a>
</div>

<div class="w3-panel ws-note">
  <p><strong>Note:</strong> that new xs should be in same range as of the old xs, meaning that we can't call <code class="w3-codespan">interp_func()</code> with values higher than 10, or less than 0.</p>
</div>
...
<p>In 1D interpolation the points are fitted for a <em>single curve</em> whereas in Spline interpolation
the points are fitted against a <em>piecewise</em> function defined with polynomials called splines.</p>

<p>The <code class="w3-codespan">UnivariateSpline()</code> function takes
<code class="w3-codespan">xs</code> and <code class="w3-codespan">ys</code>
and produce a callable funciton that can be called with new <code class="w3-codespan">xs</code>.
</p>

<div class="w3-panel ws-note">
  <p><strong>Piecewise function:</strong> A function that has different definition for different ranges.</p>
</div>
<div class="w3-example">
<h3>Example</h3>
<p>Find univariate spline interpolation for 2.1, 2.2... 2.9 for the following non linear points:</p>
<div class="w3-code notranslate pythonHigh">
from scipy.interpolate import UnivariateSpline<br>
import numpy as np<br>
<br>
xs = np.arange(10)<br>
ys = xs**2 + np.sin(xs) + 1<br>
<br>
interp_func = UnivariateSpline(xs, ys)<br>
<br>
  newarr =
interp_func(np.arange(2.1, 3, 0.1)) <br><br>print(newarr)</div>
<h3>Result:</h3>
<p>
<pre style="color:#fff;background-color:rgb(40,44,52)!important">
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_scipy_interpolation_univariatespline">Try it Yourself &raquo;</a>
</div>

<hr>

<h2>Interpolation with Radial Basis Function</h2>

<p>Radial basis function is a function that is defined corresponding to a fixed reference point.</p>

<p>The <code class="w3-codespan">Rbf()</code> function also takes <code class="w3-codespan">xs</code> and <code class="w3-codespan">ys</code>
as arguments and produces a callable function that can be called with new <code class="w3-codespan">xs</code>.</p>

<div class="w3-example">
<h3>Example</h3>
<p>Interpolate following xs and ys using rbf and find values for 2.1, 2.2 ... 2.9:</p>
<div class="w3-code notranslate pythonHigh">
from scipy.interpolate import Rbf<br>
import numpy as np<br>
<br>
xs = np.arange(10)<br>
ys = xs**2 + np.sin(xs) + 1<br>
<br>
interp_func = Rbf(xs, ys)<br>
<br>
newarr = interp_func(np.arange(2.1, 3, 0.1))<br>
<br>
print(newarr)
...
<a target="_blank" class="w3-btn w3-margin-bottom" href="trypython.php?filename=demo_scipy_interpolation_rbf">Try it Yourself &raquo;</a>
</div>

<hr>
<div id="exercisecontainer" src="xrcise_interpolation.js" data-extensions="php"></div>
<hr>
